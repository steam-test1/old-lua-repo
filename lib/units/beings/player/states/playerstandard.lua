...
function PlayerStandard:enter( state_data, enter_data )
	...
	-- Force ducking if we can't stand here. Fixes bug where it looked like we where standing, but with crouch mover from bleed out state.
	if ( enter_data and enter_data.wants_crouch ) or not self:_can_stand() then
	...
	if self._ext_movement:nav_tracker() then
		self._pos_reservation = { position = self._ext_movement:m_pos(), radius = 100, filter = self._ext_movement:pos_rsrv_id() }
		self._pos_reservation_slow = { position = mvector3.copy( self._ext_movement:m_pos() ), radius = 100, filter = self._ext_movement:pos_rsrv_id() }
	...
	-- self._equipped_unit:base():on_equip()
	managers.hud:set_ammo_amount( self._equipped_unit:base():selection_index(), self._equipped_unit:base():ammo_info() )
	managers.hud:set_weapon_name( tweak_data.weapon[ self._equipped_unit:base():get_name_id() ].name_id )
	
	-- Set from bleed out and fatal to reequip weapon
	if enter_data and enter_data.equip_weapon then
		self:_start_action_unequip_weapon( managers.player:player_timer():time(), { selection_wanted = enter_data.equip_weapon } )
	end
...
function PlayerStandard:_enter( enter_data )
	...
	local skip_equip = enter_data and enter_data.skip_equip	-- Basicly we don't want to reequip a weapon if it allready equipped (as from bleed out)
	if not self:_changing_weapon() and not skip_equip then -- or enter_data and enter_data.equip_weapon )then -- check if we are playing the right animation
		if not self._state_data.mask_equipped then
			...
			self:_start_action_equip( self.IDS_MASK_EQUIP, 48/30 )
		else
	...
	if not self._state_data.ducking then
		local stand_attention = nil
		if managers.groupai:state():whisper_mode() then
			stand_attention = "pl_enemy_stand_mask_on"
		else
			stand_attention = "pl_enemy_cbt"
		end
		self._ext_movement:set_attention_settings( { stand_attention, "pl_team_idle_std", "pl_civ_cbt" } )
	end
	
	self._ext_network:send( "set_stance", 2 ) -- hos
end
...
function PlayerStandard:exit( state_data, new_state_name )
	...
	-- state_data.ducking = self._ducking
...
function PlayerStandard:update( t, dt )
	-- local p_id = Profiler:start( "PlayerStandard:update" )
	PlayerMovementState.update( self, t, dt )
	
	-- local id = Profiler:start( "_calculate_standard_variables" )
	self:_calculate_standard_variables( t, dt )
	-- Profiler:stop( id )
	
	-- local id = Profiler:start( "_update_ground_ray" )
	self:_update_ground_ray()
	-- Profiler:stop( id )
	
	-- local id = Profiler:start( "_update_fwd_ray" )
	self:_update_fwd_ray()
	-- Profiler:stop( id )
	-- local id = Profiler:start( "_update_check_actions" )
	self:_update_check_actions( t, dt )
	-- Profiler:stop( id )
	
	-- local id = Profiler:start( "_update_movement" )
...
function PlayerStandard:_upd_nav_data()
	if mvec3_dis_sq( self._m_pos, self._pos ) > 1 then
		...
		if self._pos_reservation then
			...
			if mvec3_norm( temp_vec1 ) > slow_dist then
...
function PlayerStandard:_calculate_standard_variables( t, dt )
	self._gnd_ray = nil		--	The ground ray gets reset_here and created by individual actions if needed
	self._gnd_ray_chk = nil		--	Boolean flag that we have updated the ground ray
	...
	-- self._cam_fwd_flat = self._cam_fwd:with_z( 0 )
	...
	-- local xy_vel_len = last_vel_xy:length()
	local sampled_vel_dir = self._unit:sampled_velocity() -- :with_z( 0 )
	...
	if sampled_vel_len == 0 then
		...
		-- local fwd_dot = sampled_vel_dir:dot( last_vel_xy )
		local fwd_dot = mvector3.dot( sampled_vel_dir, last_vel_xy )
		mvector3.set( self._last_velocity_xy, sampled_vel_dir )
		if fwd_dot > sampled_vel_len then -- clamp our last velocity to the sampled velocity
			mvector3.multiply( self._last_velocity_xy, sampled_vel_len )
			-- self._last_velocity_xy = sampled_vel_dir * sampled_vel_len
		else
			mvector3.multiply( self._last_velocity_xy, math.max( 0, fwd_dot ) )
			-- self._last_velocity_xy = sampled_vel_dir * math.max( 0, fwd_dot )
		end
	end
...
function PlayerStandard:_update_ground_ray()
	...
	--[[
	Application:draw_sphere( hips_pos, 30, 0.5,0.5,0.5 )
	if self._gnd_ray then
		Application:draw_cylinder( hips_pos, self._gnd_ray.position, 30, 0,1,0 )
		Application:draw_sphere( down_pos, 30, 1,1,1 )
	else
		Application:draw_sphere( down_pos, 30, 0,0,0 )
	end
	]]
...
function PlayerStandard:_get_input( t, dt )
	-- Only set input values when that type of input has happend
	
	-- local i_id = Profiler:start( "_get_input" )
	...
	-- btn_next_item_press = pressed and self._controller:get_input_pressed( "next_item" ),
	-- btn_previous_item_press = pressed and self._controller:get_input_pressed( "previous_item" ),
	...
	--[[i = 1
	while i < 3 do
		if self._controller:get_input_pressed( "secondary_choice"..i ) then
			input.btn_secondary_choice = i
			break
		end
		i = i + 1
	end]]
...
-- Called from menu manager in singleplayer when menu is opened. It will clear the controller
...
function PlayerStandard:_update_check_actions( t, dt )
	...
	--[[if self._reload_enter_expire_t then
		if self._reload_enter_expire_t <= t then
	...
	if self._reload_expire_t then
		...
		if self._reload_expire_t <= t or interupt then
			self._reload_expire_t = nil
			if self._equipped_unit:base():reload_exit_expire_t() then -- If there is a reload exit timer, the reload will not be doen until it has expired
				...
				if self._equipped_unit then
					...
					managers.hud:set_ammo_amount( self._equipped_unit:base():selection_index(), self._equipped_unit:base():ammo_info() )
					if input.btn_steelsight_state then -- This is only if hold is required to steelsight
	...
	if self._reload_exit_expire_t then
		if self._reload_exit_expire_t <= t then
	...
	--[[if self._melee_expire_t then
		if self._melee_expire_t <= t then
	...
	--[[if self._unequip_weapon_expire_t then
		if self._unequip_weapon_expire_t <= t then
			self._unequip_weapon_expire_t = nil
			self:_start_action_equip_weapon( t )
		end
	end
	
	if self._equip_weapon_expire_t then
		if self._equip_weapon_expire_t <= t then			
	...
	if self._change_item_expire_t then
		 if self._change_item_expire_t <= t then
		 	self._change_item_expire_t = nil
		 end
	end
	
	if self._change_weapon_pressed_expire_t then
		if self._change_weapon_pressed_expire_t <= t then
	...
	if not new_action then
		new_action = self:_check_action_weapon_gadget( t, input )
	end
	...
	if not new_action then
		new_action = self:_check_change_weapon( t, input )
	end
	...
	--[[if not new_action then
		new_action = self:_check_action_secondary_attack( t, input )
	end]]
	
	if not new_action then
		new_action = self:_check_action_equip( t, input )
	end
	...
	--[[if not new_action then
		new_action = self:_check_change_item( t, input )
	end]]
	...
	--[[if input.btn_steelsight_release then
		self._steelsight_wanted = false
		if self._state_data.in_steelsight then
			self:_end_action_steelsight( t )
		end
	...
	-----------------------------
	--[[
	if not new_action and self._current_action then
		local action_type = self._current_action.type
		if action_type == "ranged" then
			new_action = self:_update_action_ranged( t, dt, btn_primary_attack_state )
		elseif action_type == "block" then
			new_action = self:_update_action_block( t, dt )
		elseif action_type == "parry" then
			new_action = self:_update_action_parry( t, dt )
		elseif action_type == "dash" then
			self:_update_action_dash( t, dt )
		elseif action_type == "turn" then
			self:_update_action_turn( t, dt )
		elseif action_type == "hurt" then
			self:_update_action_hurt( t, dt )
		elseif action_type == "use" then
			self:_update_action_use( t, dt )
		end
	end]]
...
-- 	Update the player's position, rotation and movement animations
...
function PlayerStandard:_update_movement( t, dt )
	...
	--	Stick controlled movement
	local pos_new
	-- local self._target_headbob
	self._target_headbob = self._target_headbob or 0
	self._headbob = self._headbob or 0 

	if self._move_dir then -- and not (self._jump_t and t - self._jump_t < 1 ) then	--	Player wants to move
		local enter_moving = not self._moving
		self._moving = true
		
		if enter_moving then 				-- We will enter moving
			self._last_sent_pos_t = t
			self:_update_crosshair_offset() -- This required self._moving to be set
		end
		
		local WALK_SPEED_MAX = self:_get_max_walk_speed( t )		--	The walk speed at full stick input in the current stance ( defensive/standard )
		...
		if self._jump_vel_xy and self._state_data.in_air and mvector3.dot( self._jump_vel_xy, self._last_velocity_xy ) > 0 then
			...
			if fwd_dot < jump_vel then
				...
				--print( "sustain_dot - fwd_dot", sustain_dot - fwd_dot )
				-- achieved_walk_vel = math.step( self._last_velocity_xy, new_move_vec, 700 * dt )
				...
				-- achieved_walk_vel = math.step( self._last_velocity_xy, wanted_walk_speed * self._move_dir:normalized(), acceleration * dt )
			end
			local fwd_component
		else
			mvector3.multiply( mvec_move_dir_normalized, wanted_walk_speed )
		...
		-- pos_new = self._pos + achieved_walk_vel * dt
		
		self._target_headbob = self:_get_walk_headbob()
		self._target_headbob = self._target_headbob * self._move_dir:length()
	elseif not mvector3.is_zero( self._last_velocity_xy ) then -- no input. we have xy velocity
	...
	elseif self._moving then
		self._target_headbob = 0
		-- self._ext_camera:set_shaker_parameter( "headbob", "amplitude", 0 )
		-- managers.hud:set_crosshair_offset( tweak_data.weapon[ self._equipped_unit:base():get_name_id() ].crosshair[ self._state_data.ducking and "crouching" or "standing" ].offset )
		self._moving = false
		self:_update_crosshair_offset()
	end
	
	-- if new_head_bob then 
		if self._headbob ~= self._target_headbob then
			self._headbob = math.step( self._headbob, self._target_headbob, dt/4 )
			-- print( "self._headbob", self._headbob )
	...
	if pos_new then
		...
		-- self._last_velocity_xy = pos_new - self._pos
		...
		--print( self._state_data.in_air, self._unit:sampled_velocity():with_z( 0 ):length(), self._unit:sampled_velocity():with_x( 0 ):with_y( 0 ):length() )
	else
	...
	if move_dis > (150*150) or move_dis > (20*20) and ( t - self._last_sent_pos_t > 1.5 or not pos_new ) then
		--print( "sending new position", move_dis, t - self._last_sent_pos_t )
		self._ext_network:send( "action_walk_nav_point", cur_pos )
		mvector3.set( self._last_sent_pos, cur_pos )
		self._last_sent_pos_t = t
		
		if self._move_dir and self._running and not ( self._state_data.ducking or managers.groupai:state():enemy_weapons_hot() ) then -- produce a footstep alert
			...
			local footstep_alert = { "footstep", alert_epicenter, alert_rad, managers.groupai:state():get_unit_type_filter( "civilians_enemies" ), self._unit }
...
function PlayerStandard:_update_foley( t, input )
	-- local id = Profiler:start( "_update_foley" )
	-- if not self._gnd_ray then
	-- 	return
	-- end
		
	if not self._gnd_ray and not self._state_data.on_ladder then -- or self._gnd_ray.distance > 10 then
		...
		if self._state_data.in_air then -- Land
			...
			-- Draw:pen( "one_second" ):line( self._pos + Vector3( 0, 0, 10 ), self._pos - Vector3( 0, 0, 30 ) )
			
			if self._unit:character_damage():damage_fall( { height = self._state_data.enter_air_pos_z - self._pos.z } ) then
				self._running_wanted = false
				managers.rumble:play( "hard_land" )
				self._ext_camera:play_shaker( "player_fall_damage" ) 
				self:_start_action_ducking( t )
			elseif input.btn_run_state then -- This is only if hold is required to run
		...
		elseif self._jump_vel_xy and t - self._jump_t > 0.3 then
			self._jump_vel_xy = nil
			if input.btn_run_state then -- This is only if hold is required to run
				self._running_wanted = true
			end
		end
	end
...
function PlayerStandard:_check_step( t )
	...
	-- if (self._last_step_pos - self._pos):length() > step_length then
...
function PlayerStandard:_update_crosshair_offset( t )
		do return end -- no crosshair for pd2, no need to update crosshair, restore if this changes
	...
	local spread_multiplier = self._equipped_unit:base():spread_multiplier() -- managers.player:upgrade_value( name_id, "spread_multiplier", 1 )
...
function PlayerStandard:_stance_entered( unequipped )
	...
	local misc_attribs = 	(self._state_data.in_steelsight and stances.steelsight) or 
							(self._state_data.ducking and stances.crouched or stances.standard)
		
	-- local duration_multiplier = self._state_data.in_steelsight and 1/managers.player:upgrade_value( weapon_id, "enter_steelsight_speed_multiplier", 1 ) or 1
	...
	local new_fov = self:get_zoom_fov( misc_attribs ) + 0 -- (self._state_data.in_steelsight and stances.steelsight.zoom_fov) and managers.user:get_setting( "fov_zoom" ) or managers.user:get_setting( "fov_standard" )
...
--------------------------------------------------------------------------------------
--	Check how fast the player can go in his current stance ( defensive/standard )
...
function PlayerStandard:_get_max_walk_speed( t )
	...
	if self._state_data.in_steelsight then
		local has_normal_speed_upgrade = managers.player:has_category_upgrade( "player", "steelsight_normal_movement_speed" )
		movement_speed = has_normal_speed_upgrade and speed_tweak.STANDARD_MAX or speed_tweak.STEELSIGHT_MAX
...
function PlayerStandard:_start_action_steelsight( t )
	if self:_changing_weapon() or self:_is_reloading() or self:_interacting() 
		or self._melee_expire_t or self._use_item_expire_t or self:_is_throwing_grenade() then
	...
	--[[if self:_is_reloading() or self:_interacting() then
		self._steelsight_wanted = true
		return
	end
	if self._melee_expire_t or self._use_item_expire_t then
		self._steelsight_wanted = true
		return
	end]]
	...
	self._ext_network:send( "set_stance", 3 ) -- cbt
end
...
function PlayerStandard:_start_action_running( t )
	if not self._move_dir then
		self._running_wanted = true -- self._setting_hold_to_run and true or false
		return
	end
	...
	if( ( self._shooting and not self.RUN_AND_SHOOT ) or self:_changing_weapon() or self._melee_expire_t or self._use_item_expire_t or self._state_data.in_air
			or self:_is_throwing_grenade() ) then
		self._running_wanted = true
		return
	end
	...
	self:set_running( true ) -- self._running = true
...
function PlayerStandard:_start_action_ducking( t )
	...
	self._ext_network:send( "set_pose", 2 )	--crouch
	...
	self._ext_movement:set_attention_settings( { crh_attention, "pl_team_idle_std", "pl_civ_sneak" } )
end
...
function PlayerStandard:_can_stand( ignored_bodies )
	...
	local ray_table = { "ray", hips_pos, up_pos, "slot_mask", self._slotmask_gnd_ray, "ray_type", "body mover", "sphere_cast_radius", radius, "bundle", 20 }
...
-- checks if you are allowed to run with current input direction ( Forward / Strafing / backward / in between / etc )
function PlayerStandard:_can_run_directional()
	--[[ We are masters of parkour, let us run as we wish ]]
	
	if( managers.player:has_category_upgrade( "player", "can_free_run" ) ) then
		return true
	end
	
	local running_angle = ( managers.player:has_category_upgrade( "player", "can_strafe_run" ) and 92 ) or 50			-- adding a few degrees for safety ( 90 ,45 )
	return ( mvector3.angle( self._stick_move, math.Y ) <= running_angle )		-- while using dot by itself is superior to angle in performance, requiring self._stick_move to be normalized causes angle to win
end
...
function PlayerStandard:_check_action_throw_grenade( t, input )
	...
	local action_forbidden = not PlayerBase.USE_GRENADES or self:chk_action_forbidden( "interact" ) or self._unit:base():stats_screen_visible()
...
function PlayerStandard:_update_throw_grenade_timers( t, input )
	if self._state_data.throw_grenade_expire_t then
		if self._state_data.throw_grenade_expire_t <= t then
...
function PlayerStandard:_interupt_action_interact( t, input, complete )
	if self._interact_expire_t then
		...
		-- This equip weapon is a bit dirty to do, only a copy from action equip weapon
		-- self._equip_weapon_expire_t = managers.player:player_timer():time() + 0.7
		local tweak_data = self._equipped_unit:base():weapon_tweak_data()
...
function PlayerStandard:_check_action_melee( t, input )
	...
	if col_ray then -- Deal damage
		...
		if not (hit_unit:character_damage() and hit_unit:character_damage()._no_blood) then	
		...
		if can_shield_knock and hit_unit:in_slot( 8 ) and alive( hit_unit:parent() ) then -- shield
			shield_knock = true
			character_unit = hit_unit:parent()
		end
		character_unit = character_unit or hit_unit
		
		--print( "can_shield_knock", can_shield_knock, "is_shield", hit_unit:in_slot( 8 ), "character_unit", character_unit )
		if character_unit:character_damage() and character_unit:character_damage().damage_melee then
			...
			action_data.shield_knock = can_shield_knock -- this flag means that if the enemy is a shield he should play a special "shield_knock" hurt
...
function PlayerStandard:_update_reload_timers( t, dt, input )
	if self._state_data.reload_enter_expire_t then
		if self._state_data.reload_enter_expire_t <= t then
	...
	if self._state_data.reload_exit_expire_t then
		if self._state_data.reload_exit_expire_t <= t then
...
--[[function PlayerStandard:_check_change_item( t, input )
	local new_action
	local action_wanted = input.btn_next_item_press or input.btn_previous_item_press
	
	if action_wanted then
		local action_forbidden = self._change_weapon_pressed_expire_t -- Try to prevent accidentaly changing item
		if not action_forbidden then
			if input.btn_next_item_press then
				managers.player:select_next_item()
				managers.hud:pressed_d_pad( "up" )
			elseif input.btn_previous_item_press then
				managers.player:select_previous_item()
				managers.hud:pressed_d_pad( "down" )
			end
			self._change_item_expire_t = t + 0.33
			new_action = true
		end	
	end
...
function PlayerStandard:_check_use_item( t, input )
	...
	if action_wanted then
		local action_forbidden = self._use_item_expire_t or self:_interacting() or self:_changing_weapon() -- Changing weapon here blocks all items now, not only grenades
			or self:_is_throwing_grenade()
...
function PlayerStandard:_update_use_item_timers( t, input )
	-- print( "_update_use_item_timers", self._use_item_expire_t )
	if self._use_item_expire_t then
		local valid = managers.player:check_selected_equipment_placement_valid( self._unit )
		-- print( "managers.player:check_selected_equipment_placement_valid()", valid )
...
function PlayerStandard:_end_action_use_item( valid )
	...
	self:_interupt_action_use_item( nil, nil, true and valid )
end
...
function PlayerStandard:_check_change_weapon( t, input ) -- Wil change weapon by calling next or previous weapon
	local new_action
	local action_wanted = input.btn_switch_weapon_press -- input.btn_next_weapon_press or input.btn_previous_weapon_press
	
	if action_wanted then
		local action_forbidden = self:_changing_weapon()
		action_forbidden = action_forbidden or self._melee_expire_t or self._use_item_expire_t or self._change_item_expire_t -- Try to prevent accidentaly changing weapon
		...
		if not action_forbidden then
			...
			--[[if input.btn_next_weapon_press then
				data.next = true
				-- new_action = self._ext_inventory:equip_next( false )
				-- if new_action then
				--	managers.hud:pressed_d_pad( "right" )
				-- end
			elseif input.btn_previous_weapon_press then
				data.previous = true
				-- new_action = self._ext_inventory:equip_previous( false )
				-- if new_action then
	...
	if dis < max_dis then
		...
		if angle < max_angle then
			...
			if interaction_through_walls then
				table.insert(char_table, { unit = unit, inv_wgt = ing_wgt, unit_type = unit_type })
			else
				...
				if not ray or mvector3.distance( ray.position, u_head_pos ) < 30 then -- the distance check is a workaround for civilians with their heads behind statics
					table.insert(char_table, { unit = unit, inv_wgt = ing_wgt, unit_type = unit_type })
				end
			end
...
function PlayerStandard:_get_interaction_target( char_table, my_head_pos, cam_fwd )
	...
	-- Raycasting to look for a primary target here
	...
	-- If the raycast found no target, find the closest target by weight
	...
	if prime_target then
		if prime_target.unit_type == unit_type_teammate then
			...
			if rally_skill_data and rally_skill_data.range_sq > mvector3.distance_sq( self._pos, record.m_pos ) then
				...
				if prime_target.unit:base().is_husk_player then -- other player
			...
			elseif voice_type == "boost" then -- Team AI, boost
			...
			plural = false
			-- TODO(nikmyr): Add other interaction types here
		else
			...
			elseif prime_target.unit_type == unit_type_camera then
				plural = false
			-- TODO(nikmyr): Add conversation criteria for enemies here?
				voice_type = "mark_camera"
			else
				if tweak_data.character[ prime_target.unit:base()._tweak_table ].is_escort then
					plural = false
					...
					-- TODO(nikmyr): Add conversation criteria for civilians here?
				end
			end
			
			local max_inv_wgt = 0
			for _, char in pairs (char_table) do
				if char.inv_wgt > max_inv_wgt then
...
function PlayerStandard:_get_unit_intimidation_action( intimidate_enemies, intimidate_civilians, intimidate_teammates, only_special_enemies, intimidate_escorts, intimidation_amount, primary_only, detect_only )
	...
	if intimidate_enemies then
		...
		for u_key, u_data in pairs( enemies ) do
			if not ( u_data.is_converted or u_data.unit:anim_data().hands_tied ) then
				if u_data.char_tweak.priority_shout or not only_special_enemies then
					if managers.groupai:state():whisper_mode() then
						if u_data.char_tweak.silent_priority_shout and u_data.unit:movement():cool() then
							self:_add_unit_to_char_table(char_table, u_data.unit, unit_type_enemy, highlight_range, false, false, 0.01, my_head_pos, cam_fwd ) -- highlight
						elseif not u_data.unit:movement():cool() then
							self:_add_unit_to_char_table(char_table, u_data.unit, unit_type_enemy, intimidate_range_ene, false, false, 10000, my_head_pos, cam_fwd ) -- intimidate
						end
					elseif u_data.char_tweak.priority_shout then -- special enemy in combat
						self:_add_unit_to_char_table(char_table, u_data.unit, unit_type_enemy, highlight_range, false, false, 0.01, my_head_pos, cam_fwd ) -- highlight special
					else
						self:_add_unit_to_char_table(char_table, u_data.unit, unit_type_enemy, intimidate_range_ene, false, false, 10000, my_head_pos, cam_fwd ) -- intimidate
					end
				end
		...
		for u_key, u_data in pairs( criminals ) do
			...
			if u_key ~= self._unit:key() then
				...
				if rally_skill_data and rally_skill_data.long_dis_revive and rally_skill_data.range_sq > mvector3.distance_sq( self._pos, u_data.m_pos ) then
					local needs_revive
					if u_data.unit:base().is_husk_player then -- other player
			...
			if not ( added or u_data.is_deployable or u_data.unit:movement():downed() or u_data.unit:base().is_local_player ) then
				self:_add_unit_to_char_table(char_table, u_data.unit, unit_type_teammate, 100000, true, true, 0.01, my_head_pos, cam_fwd)
			end
		end
	end
...
function PlayerStandard:_start_action_intimidate( t )
	if not self._intimidate_t or t - self._intimidate_t > tweak_data.player.movement_state.interaction_delay then
		...
		elseif voice_type == "down_stay" then
			...
			if self._shout_down_t and self._shout_down_t + 2 > t then
				sound_name = "f03b_any"
			else
		...
		elseif voice_type == "come" then
			...
			if not static_data then	-- safety check possibly related to other client getting the "sudden black screen" bug
				return
			end
		...
		elseif voice_type == "escort_go" then
			...
			local e_guy = prime_target.unit
--			if not CivilianLogicEscort.too_scared_to_move( e_guy:brain()._logic_data ) then
--				local stopped_t = e_guy:brain()._logic_data.internal_data.stopped_t or 0
				local stopped_t = 0
				-- TODO(nikmyr): This wont work on clients?
				if stopped_t + 2 > t then
					sound_name = "e02x_" .. sound_suffix
				else
					sound_name = "e03x_" .. sound_suffix
				end
--			else
--				sound_name = "e04x_" .. sound_suffix
--			end
...
-- This function is called from other locations, which means self is not always this class.
function PlayerStandard:say_line( sound_name, skip_alert )
	...
	if not skip_alert then
		local alert_rad = 500 --managers.groupai:state():enemy_weapons_hot() and 700 or 400
		local new_alert = { "vo_cbt", self._unit:movement():m_head_pos(), alert_rad, self._unit:movement():SO_access(), self._unit }
...
function PlayerStandard:_break_intimidate_redirect( t )
	...
	-- Mayby need a initimadate timer to know that we actually are playing the animation
	self._ext_camera:play_redirect( self.IDS_IDLE )
end
...
function PlayerStandard:_break_interact_redirect( t )
	-- Mayby need a interact timer to know that we actually are playing the animation
	self._ext_camera:play_redirect( self.IDS_IDLE )
end

--------------------------------------------------------------------------------------

--	Check if we want to equip a different weapon

function PlayerStandard:_check_action_equip( t, input )
	local new_action
	local selection_wanted = input.btn_primary_choice -- or input.btn_secondary_choice and self._ext_inventory._secondary_selection_start_index + input.btn_secondary_choice - 1
		
	if selection_wanted then
		local action_forbidden = self:chk_action_forbidden( "equip" )
		action_forbidden = action_forbidden or not self._ext_inventory:is_selection_available( selection_wanted ) or self._melee_expire_t or self._use_item_expire_t
			or self:_changing_weapon()
			or self:_interacting()
			or self:_is_throwing_grenade()
		
		if not action_forbidden then
			-- new_action = self._ext_inventory:equip_selection( selection_wanted, false )
...
function PlayerStandard:_check_action_jump( t, input )
	...
	if action_wanted then
		...
		if not action_forbidden then
			if self._state_data.ducking then -- Break ducking
				self:_interupt_action_ducking( t )
			else
				if self._state_data.on_ladder then
...
function PlayerStandard:_start_action_jump( t, action_start_data )
	...
	--print( "action_start_data.jump_vel_xy", action_start_data.jump_vel_xy, "action_start_data.jump_vel_z", action_start_data.jump_vel_z )
	if self._move_dir then
		...
		-- fix for set_velocity setting double value
		--local jump_vel_xy = action_start_data.jump_vel_xy * 0.5
		--print( "jump_vel_xy", jump_vel_xy )
		--jump_vec = jump_vec + move_dir_clamp * jump_vel_xy
...
function PlayerStandard:_check_action_run( t, input )
	if (self._setting_hold_to_run and input.btn_run_release) or (self._running and not self._move_dir) then -- Either run button released or running but move dir is released
		self._running_wanted = false
		if self._running then
			self:_end_action_running( t )	
			if input.btn_steelsight_state and not self._state_data.in_steelsight then -- This is only if hold is required to steelsight
				self._steelsight_wanted = true
			end
		end
...
function PlayerStandard:_update_running_timers( t )
	if self._end_running_expire_t then
		if self._end_running_expire_t <= t then
			self._end_running_expire_t = nil
			self:set_running( false ) -- self._running = false
		end
...
function PlayerStandard:get_zoom_fov( stance_data )
	local fov = stance_data and stance_data.FOV or 75 -- managers.user:get_setting( "fov_standard" )
	
	local fov_multiplier = managers.user:get_setting( "fov_multiplier" )
	if( self._state_data.in_steelsight ) then
		fov = self._equipped_unit:base():zoom()
		fov_multiplier = 1 + (fov_multiplier - 1)/2 -- math.max( 1, fov_multiplier/2 ) 
	end
	
	-- print( "fov_multiplier", fov_multiplier )
	return fov * fov_multiplier
end

function PlayerStandard:_check_action_primary_attack( t, input )
	-- local id = Profiler:start( "_check_action_primary_attack" )
	local new_action
		
	local action_wanted = input.btn_primary_attack_state
	
	if action_wanted then
		local action_forbidden = self:_is_reloading() or self:_changing_weapon() or self._melee_expire_t or self._use_item_expire_t
			or self:_interacting()
			or self:_is_throwing_grenade()
		if not action_forbidden then
			self._queue_reload_interupt = nil
			...
			if self._equipped_unit then
				...
				if weap_base:out_of_ammo() then
					if input.btn_primary_attack_press then
						-- Call to play a click sound here
						weap_base:dryfire()
					end
				elseif weap_base.clip_empty and weap_base:clip_empty() then 	-- Reload
					if fire_mode == "single" then 																-- Only reload on press when single
						if input.btn_primary_attack_press then
							self:_start_action_reload_enter( t )
						end
					else
				...
				elseif self._running and not self.RUN_AND_SHOOT then																		-- Break running
					self:_interupt_action_running( t )
				else
					if not self._shooting then
						if weap_base:start_shooting_allowed() then
							local start = fire_mode == "single" and input.btn_primary_attack_press					-- When single fire mode, start should only happen when we press
							...
							-- Return here, otherwise shooting would be possible but without sound and recoiling
							return false
						end
					end
					...
					if fired then
						...
						-- self._ext_camera:play_shaker( "fire_weapon", weap_tweak_data.fire_weapon_shaker_multiplier )
						...
						self._ext_camera:play_shaker( "fire_weapon_kick", 1 * shake_multiplier, 1, 0.15 ) -- amplitude, frequency, offset
						
						if not( self._state_data.in_steelsight and weap_base:tweak_data_anim_play( "fire_steelsight", weap_base:fire_rate_multiplier() ) ) then
							weap_base:tweak_data_anim_play( "fire", weap_base:fire_rate_multiplier())
						end
						
						if fire_mode == "single" and weap_base:get_name_id() ~= "saw" then
							if not self._state_data.in_steelsight then
								self._ext_camera:play_redirect( self.IDS_RECOIL, 1 ) -- weap_base:fire_rate_multiplier() )
							elseif weap_tweak_data.animations.recoil_steelsight then
								self._ext_camera:play_redirect( self.IDS_RECOIL_STEELSIGHT, 1 ) -- weap_base:fire_rate_multiplier() )
							end
						end
						-- local kick_v = self._equipped_unit:base():recoil() * weap_tweak_data.kick.v[ self._state_data.in_steelsight and "steelsight" or self._state_data.ducking and "crouching" or "standing" ]
						-- local kick_h = weap_tweak_data.kick.h and ( self._equipped_unit:base():recoil() * (weap_tweak_data.kick.h[ self._state_data.in_steelsight and "steelsight" or self._state_data.ducking and "crouching" or "standing" ]) ) or 0
						local recoil_multiplier = weap_base:recoil() * weap_base:recoil_multiplier() -- Stats recoil * Upgrade recoil
						...
						--[[  -- pd2 got no crosshair, restore this if that changes, or for pd3
						...
						local impact = not fired.hit_enemy	-- this means that the shot unit has synced the impact already
						self._ext_network:send( "shot_blank", impact )
					elseif fire_mode == "single" then -- Make sure that stop shooting happens for single weapons even if trigger is pressed (state)
						new_action = false
					end
			...
			if self:_is_reloading() and self._equipped_unit:base():reload_interuptable() then
				if input.btn_primary_attack_press then
					-- self:_interupt_action_reload( t )
...
--	Check if we want to initiate the available secondary action

--[[function PlayerStandard:_check_action_secondary_attack( t, input )
	
	local new_action
	
	local action_forbidden = self:chk_action_forbidden( "secondary_attack" )
	local action_wanted = input.btn_secondary_attack_press
	if action_wanted and not action_forbidden then
		self._ext_inventory:equip_selected_secondary( false )
		if self._equipped_unit then
			local action_start_data = {}
			action_start_data.has_direction_input = self._move_dir and true
			action_start_data.direction_vec = self._ext_camera:forward()
			action_start_data.weapon_unit = self._equipped_unit
				
			new_action = self:_start_action_secondary_attack( t, action_start_data )
		end
	end
...
--[[function PlayerStandard:_start_action_secondary_attack( t, action_start_data )
	
	action_start_data.weapon_unit:base():trigger_pressed( self._ext_camera:position(), action_start_data.direction_vec, { self._unit, action_start_data.weapon_unit }, self._unit )
...
function PlayerStandard:_find_pickups( t )
	-- local id = Profiler:start( "_find_pickups" )
...
--	Called from the damage extension. Determines if we are going to counter the attack with a defensive action. Returns the action decided
...
--	Accepts as arguements the reference forward vector and a direction vector and returns a string 
--	describing the relative direction of the second with respect to the fwd one
...
--	Called after tweak_data has been reloaded
