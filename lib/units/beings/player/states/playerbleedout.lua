...
function PlayerBleedOut:enter( state_data, enter_data )
	...
	if not managers.player:has_category_upgrade( "player", "primary_weapon_when_downed" ) then
	...
	--<shake name="player_bleedout_land" spin="sin(t)/50" tilt="cos(t)/50" frequency="100.0" amplitude="1.0" attack="0.0" sustain="2.0" decay="0.5" />
...
function PlayerBleedOut:exit( state_data, new_state_name )
	...
	if Network:is_server() then
		if new_state_name == "fatal" then -- do not re-register the SO
...
function PlayerBleedOut:update( t, dt )
	...
	if self._tilt_wait_t then
		...
		-- print( tilt, (t - self._tilt_wait_t)/self._tilt_wait_t )
...
function PlayerBleedOut:_update_check_actions( t, dt )
	...
	if not new_action then
		new_action = self:_check_action_steelsight( t, input )
	end
...
-- HIJACKED TO DROP CARRY
--[[function PlayerBleedOut:_check_use_item( t, input )
	local new_action
	local action_wanted = input.btn_use_item_press
	if action_wanted then
		local action_forbidden = self._use_item_expire_t or self:_changing_weapon() or self:_interacting() -- Changing weapon here blocks all items now, not only grenades
...
--[[ function PlayerBleedOut:_check_action_primary_attack( t, input )
	...
	if prime_target then
		if detect_only then
			if not prime_target.unit:sound():speaking( t ) then
				prime_target.unit:sound():say( "_a01x_any", true ) -- TEMP
			end
		else
			if not prime_target.unit:sound():speaking( t ) then
...
function PlayerBleedOut:_unregister_revive_SO()
	...
	elseif self._revive_SO_data.rescuer then
		...
		if alive( rescuer ) then
			rescuer:brain():set_objective( nil ) -- this will trigger the fail callback of the objective. thats what the above copy is for
		end
	end
...
--[[
	-- Register enemy follow objective
...
function PlayerBleedOut:_update_movement( t, dt )
	if self._ext_network then
		...
		if move_dis > (150*150) or move_dis > (20*20) and ( t - self._last_sent_pos_t > 1.5 ) then
...
function PlayerBleedOut:on_rescue_SO_failed( revive_SO_data, rescuer )
	if revive_SO_data.rescuer then
		revive_SO_data.rescuer = nil	-- this teammate failed to rescue us. re-register the SO immediately
...
-----------------------------------------------------------------------------
-- the teammate that was coming to our rescue has started reviving us
...
function PlayerBleedOut:on_civ_revive_failed( revive_SO_data, sympathy_civ )
	if revive_SO_data.sympathy_civ then
		...
		revive_SO_data.sympathy_civ = nil	-- this teammate failed to rescue us. re-register the SO immediately
	end
end
