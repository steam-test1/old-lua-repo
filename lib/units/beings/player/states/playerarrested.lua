...
function PlayerArrested:_enter( enter_data )
	self._ext_movement:set_attention_settings( { "pl_enemy_cur_peaceful", "pl_team_cur_peaceful", "pl_civ_cbt" } )
...
function PlayerArrested:_update_check_actions( t, dt )
	...
	if self._unit:character_damage()._arrested_timer <= 0 then
		if not self._timer_finished then
	...
	if self._unequip_weapon_expire_t then
		if self._unequip_weapon_expire_t + 0.5 <= t then
