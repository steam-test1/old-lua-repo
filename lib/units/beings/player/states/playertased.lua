...
function PlayerTased:_update_check_actions( t, dt )
	...
	--[[if self._reload_expire_t then
		if self._reload_expire_t <= t then
	...
	if self._next_shock < t then
		self._next_shock = t + 0.25 + math.rand( 1 )					-- Time until next shock
		self._unit:camera():play_shaker( "player_taser_shock", 1, 10 )	-- Camera shake
		
		self._unit:camera():camera_unit():base():set_target_tilt( (math.random( 2 ) == 1 and -1 or 1) * math.random( 10 ) ) -- Random camera tilt
		self._taser_value = math.max( self._taser_value - 0.25, 0 )		-- Taser value (blue tint)
		
		self._unit:sound():play( "tasered_shock" )
		managers.rumble:play( "electric_shock" )
		
		if not alive( self._counter_taser_unit ) then
			self._camera_unit:base():start_shooting()						-- Enable recoil kick (for when no ammo)
			self._recoil_t = t + 0.5					
			
			if not managers.player:has_category_upgrade( "player", "resist_firing_tased" ) then
				input.btn_primary_attack_state = true							-- Trigger shooting
				input.btn_primary_attack_press = true
			end
			
			self._camera_unit:base():recoil_kick( -5, 5, -5, 5 ) -- Kick when no ammo
	...
	if self._shooting then
		self._camera_unit:base():recoil_kick( -5, 5, -5, 5 ) -- Extra kick
	end
	...
	if action_wanted then
		if not action_forbidden then
			...
			if self._equipped_unit then
				...
				elseif self._running then																		-- Break running
					...
					if fired then
						...
						if not self._state_data.in_steelsight then
							-- self._unit:camera():play_redirect( "recoil", weap_base:fire_rate_multiplier() )
						elseif weap_tweak_data.animations.recoil_steelsight then
							-- self._unit:camera():play_redirect( "recoil_steelsight", weap_base:fire_rate_multiplier() )
						end
						-- local kick_v = weap_base:recoil() * weap_tweak_data.kick.v[ self._state_data.in_steelsight and "steelsight" or self._state_data.ducking and "crouching" or "standing" ]
						-- local kick_h = weap_tweak_data.kick.h and weap_base:recoil() * weap_tweak_data.kick.h[ self._state_data.in_steelsight and "steelsight" or self._state_data.ducking and "crouching" or "standing" ] or 0
...
function PlayerTased:_start_action_counter_tase( t, prime_target )		-- start counter tase animation
...
function PlayerTased:clbk_exit_to_fatal()
	...
	-- self._unit:movement():change_state( "fatal" )
end
...
function PlayerTased:clbk_exit_to_std()
	...
	-- self._unit:movement():change_state( "standard" )
end
...
function PlayerTased:give_shock_to_taser()		-- taser gets counter tased here, playing tased anim and so
	if( not alive( self._counter_taser_unit ) ) then
		return
	end
	-- self._tase_ended = true
...
function PlayerTased:_give_shock_to_taser( taser_unit )	
			--   breaking tase here for now, so that taser still works after counter tased
	-- taser_unit:movement():play_redirect( "counter_tased" )	-- played on taser
	local action_data = {
		variant = "counter_tased",
		damage = taser_unit:character_damage()._HEALTH_INIT * ( tweak_data.upgrades.counter_taser_damage or 0.2 ),
		damage_effect = taser_unit:character_damage()._HEALTH_INIT * 2, -- * 0
