...
function PlayerCarry:enter( state_data, enter_data )
	PlayerCarry.super.enter( self, state_data, enter_data )
				...
				--[[
	if not managers.player:has_category_upgrade( "player", "primary_weapon_when_carrying" ) then
...
function PlayerCarry:_check_dye_explode()
	...
	if chance < 0.25 then
		...
		-- World:effect_manager():spawn( { effect = Idstring( "effects/payday2/particles/dye_pack/dye_pack_smoke" ), parent = self._unit:camera():camera_unit():orientation_object()} )
...
function PlayerCarry:_check_change_weapon( ... )
	-- if managers.player:has_category_upgrade( "player", "primary_weapon_when_carrying" ) then
