...
-- Player equipment has been moved to playermanager.
...
-- Used to validate placement of trip_mine and ecm_jammer
...
-- Used to validate placement ammo_bag and doctor_bag
...
function PlayerEquipment:use_ecm_jammer()
	if self._ecm_jammer_placement_requested then-- I am client and have requested from the server to place my sentrygun. Need to wait for a reply
		return
	end
...
function PlayerEquipment:valid_shape_placement( equipment_id, equipment_data )
	...
	if ray then
		...
		local rot = self._unit:movement():m_head_rot()
		rot = Rotation( rot:yaw(), 0, 0 )
		-- self._dummy_unit = self:_spawn_dummy( "units/equipment/sentry_gun/sentry_gun_dummy", pos, rot )
		if not alive( self._dummy_unit ) then 
			self._dummy_unit = World:spawn_unit( Idstring( equipment_data.dummy_unit ), pos, rot )
			self:_disable_contour( self._dummy_unit )
		end
		...
		-- local main_body = self._dummy_unit:body( "main" )
		...
		-- print( "bodies", inspect( bodies ) )
		for _,body in ipairs( bodies ) do			
			if body:unit() ~= self._dummy_unit and body:has_ray_type( Idstring( "body" ) ) then
				-- print( body:unit(), body:name(), body:has_ray_type( Idstring( "body" ) ) )
				valid = false
				break
			end
		end
...
function PlayerEquipment:use_sentry_gun( selected_index )
	if self._sentrygun_placement_requested then-- I am client and have requested from the server to place my sentrygun. Need to wait for a reply
		return
	end
...
-- 		managers.challenges:count_up( "deploy_ammobag" )
		...
		-- local ammo_multiplier = managers.player:upgrade_level( "sentry_gun", "ammo_increase" )
		...
		if Network:is_client() then
			...
			return false -- remove when server has confirmed
		else
...
function PlayerEquipment:use_flash_grenade()
	-- self._unit:camera():play_redirect( "throw_grenade" )
...
-- Callback from animation (via FPCameraPlayerBase)
