...
function PlayerDamage:init( unit )
	...
	self._focus_delay_mul = 1		-- multiplier applied to enemies' focus delay when aiming at the player
	
	self._listener_holder = EventListenerHolder:new()
	
	self._dmg_interval = tweak_data.player.damage.MIN_DAMAGE_INTERVAL
	self._next_allowed_dmg_t = Application:digest_value( -100, true ) -- Next time we are allowed to take damage
	self._last_received_dmg = 0			-- The value of the last received damage
...
function PlayerDamage:update( unit, t, dt )
	-- local id = Profiler:start( "PlayerDamage:update" )
	
	if self._regenerate_timer and not ( self._dead or self._bleed_out ) then
		self._regenerate_timer = self._regenerate_timer - dt
		
		local top_fade = (math.clamp(self._hurt_value - 0.8, 0, 1) / 0.2)
		local hurt = self._hurt_value - (1-top_fade)*((1+math.sin( t* 500  ))/2)/10 -- Pulsating
		managers.environment_controller:set_hurt_value( hurt )
		
		if self._regenerate_timer < 0 then
			-- self:_regenerated()
			self:_regenerate_armor()
			-- managers.hud:set_player_armor( { current = self._armor, total = self:_max_armor() } )
		end
	elseif self._hurt_value then
		if not self._bleed_out then
			-- Fade away hurt
			self._hurt_value = math.min( 1, self._hurt_value  + dt )
			-- self._armor_value = math.min( 1, self._armor_value  + dt )
			local top_fade = (math.clamp(self._hurt_value - 0.8, 0, 1) / 0.2)
			local hurt = self._hurt_value - (1-top_fade)*((1+math.sin( t* 500  ))/2)/10 -- Pulsating
			managers.environment_controller:set_hurt_value( hurt )
			local armor_value = math.max( self._armor_value or 0, self._hurt_value ) -- Hurt value is not trusted here 
			managers.hud:set_player_armor( { current = self:get_real_armor() * armor_value, total = self:_total_armor(), max = self:_max_armor() } )
			SoundDevice:set_rtpc( "shield_status" , self._hurt_value*100 )
			if self._hurt_value >= 1 then -- Fade done
				self._hurt_value = nil
				managers.environment_controller:set_hurt_value( 1.0 )
			end
		else
			-- Pulsating in bleed out / fatal
			local hurt = self._hurt_value - ((1+math.sin( t* 500  ))/2)/10 -- Pulsating
			managers.environment_controller:set_hurt_value( hurt )
		end
	end
	...
	if( self._auto_revive_timer ) then
		if( not managers.platform:presence() == "Playing" or not self._bleed_out or self._dead or self:incapacitated() or self:arrested() ) then
			...
			if( self._auto_revive_timer <= 0 ) then
				self:revive( true )
				-- managers.hud:set_player_armor( { current = self._armor, total = self:_total_armor(), max = self:_max_armor() } )
				self._auto_revive_timer = nil
			end
		end
	end
	...
	--[[
	if self._revive_health_boost then
		if not managers.player:has_activate_temporary_upgrade( "temporary", "revive_health_multiplier" ) then
			self._revive_health_boost = nil
			self._revive_health_multiplier = nil
			self:set_health( math.min( self._health, self:_max_health() ) )
		end
	elseif managers.player:has_activate_temporary_upgrade( "temporary", "revive_health_multiplier" ) then
		self._revive_health_boost = true
	end
	]]
	...
	if( not ( self._dead or self._bleed_out ) ) then
...
function PlayerDamage:replenish()
	...
	self:_regenerate_armor()
	managers.hud:set_player_health( { current = self:get_real_health(), total = self:_max_health(), revives = Application:digest_value( self._revives, false ) } )
	managers.hud:set_player_armor( { current = self:get_real_armor(), total = self:_total_armor(), max = self:_max_armor() } )
...
function PlayerDamage:_regenerate_armor()
	...
	self:set_armor( self:_max_armor() )  -- self._armor = self:_max_armor( total_armor )
...
function PlayerDamage:_regenerated( no_messiah )
	...
	self._revives = Application:digest_value( tweak_data.player.damage.LIVES_INIT + managers.player:upgrade_value( "player", "additional_lives", 0), true )	-- when these are gone, you're out!
...
function PlayerDamage:set_health( health )
	...
	self._health = Application:digest_value( math.clamp( health, 0, max_health ), true)  -- math.max( 0, health )  --
	self:_send_set_health()
	self:_set_health_effect()

	managers.hud:set_player_health( { current = self:get_real_health(), total = max_health, revives = Application:digest_value( self._revives, false ) } )
end
...
function PlayerDamage:set_armor( armor )
	...
	for i=1, armor_steps do
		...
		if( step_ratio >= armor_ratio ) then
...
function PlayerDamage:_total_armor()	-- total armor
	-- return ( self._ARMOR_INIT + managers.player:body_armor_value() ) * managers.player:upgrade_value( "player", "passive_armor_multiplier", 1 ) -- math.round()?
	...
	return ( self._ARMOR_INIT + managers.player:body_armor_value() ) * managers.player:upgrade_value( "player", "passive_armor_multiplier", 1 ) * managers.player:upgrade_value( "player", "armor_multiplier", 1 )
		-- NOTE : ULF : trying to find the armor not recharging bug
end

function PlayerDamage:_max_armor()		-- max non damaged armor
	return (self:_total_armor()) --  * (self._armor_step or 1)
end
...
function PlayerDamage:damage_tase( attack_data )
	...
	if cur_state ~= "tased" and cur_state ~= "fatal" then
		...
		-- self._unit:movement():change_state( "tased" )
		local damage_info = {
			result = {
				type = "hurt",
				variant = "tase"
...
function PlayerDamage:damage_bullet( attack_data )
	...
	local armor_reduction_multiplier = 0 --1 - self:_armor_damage_reduction()
	if self:get_real_armor() <= 0 then
	...
	elseif self._bleed_out then -- We received enought bullet damage to go to bleed out
...
function PlayerDamage:_calc_armor_damage( attack_data )
	...
	if self:get_real_armor() > 0 then
		...
		self:set_armor( self:get_real_armor() - attack_data.damage ) -- self._armor = math.max( 0, self._armor - attack_data.damage )
...
function PlayerDamage:_calc_health_damage( attack_data )
	...
	if( self:get_real_health() == 0 and attack_data.variant and attack_data.variant == "bullet" and Application:digest_value( self._revives, false ) > 1 ) then		-- a bullet killed me, or did it?
		--[[
		local last_man_standing = true
		for _, player in pairs( managers.groupai:state():all_player_criminals() ) do
			if( player.unit:key() ~= self._unit:key() ) then
		...
		if( --[[last_man_standing and ]] managers.player:has_category_upgrade( "player", "cheat_death_chance" ) ) then
			local r = math.rand(1)
			...
			if( r <= managers.player:upgrade_value( "player", "cheat_death_chance" ) ) then
				...
				-- self._health = math.clamp( self:_max_health() * managers.player:upgrade_value( "player", "cheat_death_health_regen", 0 ), 0, self:_max_health() )
			end
		end
	end
...
function PlayerDamage:_send_damage_drama( attack_data, health_subtracted )
	...
	local attacker
	if not attacker or attack_data.attacker_unit:id() == -1 then -- dead units become detatched from the network and cannot be sent
		attacker = self._unit	-- this means we cannot send the attacker
	end
	...
	if Network:is_server() then
		...
		if attacker and not attack_data.attacker_unit:movement() then -- trip mine
			attacker = nil
		end
...
function PlayerDamage:damage_fall( data )
	...
	local die = data.height > death_limit -- Can go to bleed out if height is to much 
		
		
	self._unit:sound():play( "player_hit" )
	
	-- managers.overlay_effect:play_effect( tweak_data.overlay_effects.damage_down )
	...
	if die then
		self:set_armor( 0 ) -- self._armor = math.clamp( self._armor - max_armor * managers.player:upgrade_value( "player", "fall_damage_multiplier", 1 ), 0, max_armor )
	else
		self:set_armor( self:get_real_armor() - max_armor * managers.player:upgrade_value( "player", "fall_damage_multiplier", 1 ) ) -- self._armor = math.clamp( self._armor - max_armor * managers.player:upgrade_value( "player", "fall_damage_multiplier", 1 ), 0, max_armor )
	end
	managers.hud:set_player_armor( { current = self:get_real_armor(), total = self:_total_armor(), max = max_armor, no_hint = true } )
...
-- Updated from ingame states bleedout and fatal
function PlayerDamage:update_downed( t, dt )
	if self._downed_timer and self._downed_paused_counter == 0 then -- if self._downed_timer is nil then we have been recently revived
...
function PlayerDamage:_check_bleed_out()
	if self:get_real_health() == 0 then
		...
		--self._unit:network():send( "set_need_revive", true )
		...
		SoundDevice:set_rtpc( "downed_state_progression", 0 )
		--self._unit:sound():play( "stinger_down" )
...
function PlayerDamage:_drop_blood_sample()
	...
	if managers.player:has_special_equipment( "blood_sample" ) then
		removed = true
		managers.player:remove_special( "blood_sample" ) 	-- Remove equipment
		managers.hint:show_hint( "dropped_blood_sample" ) 	-- Hint
	end
		
	if managers.player:has_special_equipment( "blood_sample_verified" ) then
		removed = true
		managers.player:remove_special( "blood_sample_verified" ) 	-- Remove equipment
		managers.hint:show_hint( "dropped_blood_sample" ) 			-- Hint
	end
	...
	if removed then
		...
		-- Draw:brush( Color(0.1, 0, 1, 0), 6 ):cone( splatter_from, splatter_to, 50 )
...
-- Updated from ingame state arrested
...
-- Updated from ingame state incapacitated
function PlayerDamage:update_incapacitated( t, dt )
	return self:update_downed( t, dt )
	--[[if self._incapacitated_paused_counter == 0 then
		self._incapacitated_timer = self._incapacitated_timer - dt
		return self._incapacitated_timer <= 0
	end
	return false]]
end
...
function PlayerDamage:on_incapacitated()
	...
	--[[self._incapacitated_timer = tweak_data.player.damage.INCAPACITATED_TIME
	self._incapacitated_paused_counter = 0
	managers.hud:pd_start_timer( { time = self._incapacitated_timer } )]]
end

--[[function PlayerDamage:pause_incapacitated_timer()
	self._incapacitated_paused_counter = self._incapacitated_paused_counter + 1
	managers.hud:pd_pause_timer()
end

function PlayerDamage:unpause_incapacitated_timer()
	self._incapacitated_paused_counter = self._incapacitated_paused_counter - 1
	if self._incapacitated_paused_counter == 0 then
		managers.hud:pd_unpause_timer()
	end
...
function PlayerDamage:_hit_direction( col_ray )
	if col_ray then
		...
		if infront < -0.90 then
			-- managers.overlay_effect:play_effect( tweak_data.overlay_effects.damage )
			managers.environment_controller:hit_feedback_front()
		elseif infront > 0.90 then
			...
			if math.abs( direction.x ) > math.abs( direction.y ) then
				...
				-- managers.overlay_effect:play_effect( direction.x < 0 and tweak_data.overlay_effects.damage_left or tweak_data.overlay_effects.damage_right )
				...
				-- managers.overlay_effect:play_effect( direction.y < 0 and tweak_data.overlay_effects.damage_up or tweak_data.overlay_effects.damage_down )
...
function PlayerDamage:_damage_screen()
	...
	-- self._regenerate_timer = tweak_data.player.damage.REGENERATE_TIME * managers.player:upgrade_value( "player", "armor_regen_timer_multiplier", 1 )
	-- self._hurt_value = 1 - math.clamp( 0.8 - math.pow( self._health/self._HEALTH_INIT, 2 ), 0, 1 )
...
function PlayerDamage:revive( helped_self )	-- This is called both for freeing and reviving. Separate the 2 in the future maybe
	--[[if not self._bleed_out then
		return
	end]]
	...
	-- self._unit:sound():play( "stinger_revive" )
	-- self._unit:network():send( "set_need_revive", false )
...
function PlayerDamage:set_danger_level( danger_level )
	self._danger_level = self._danger_level ~= danger_level and danger_level or nil -- If the same danger level is recieved, consider it to be toggled off
...
------------------------------------------------------------------
-- ... is the rpc params including the function name
function PlayerDamage.clbk_msg_overwrite_criminal_hurt( overwrite_data, msg_queue, msg_name, crim_unit, attacker_unit, dmg )
	if msg_queue then
		...
		if overwrite_data.indexes[ crim_key ] and overwrite_data.indexes[ crim_key ][ attacker_key ] then
			...
			old_msg[4] = math.clamp( dmg + old_msg[4], 1, 100 ) -- increase the damage of the old msg
		else
			table.insert( msg_queue, { msg_name, crim_unit, attacker_unit, dmg } )
			overwrite_data.indexes[ crim_key ] = { [attacker_key] = #msg_queue } -- mark the position on the queue of the overwiteable message of this type
		end
	else
...
function PlayerDamage:_upd_suppression( t, dt )
	...
	if data.value then
		...
		elseif data.value == tweak_data.player.suppression.max_value then
...
function PlayerDamage:_upd_health_regen( t, dt )	-- in percent of max health
	...
	if( not self._health_regen_update_timer ) then
		local regen_rate = 0 + managers.player:temporary_upgrade_value( "temporary", "wolverine_health_regen", 0 )	-- base regen of 0% per second
		local max_health = self:_max_health()
		
		if( regen_rate > 0 and self:get_real_health() < max_health ) then
			self:change_health( max_health * regen_rate )
			self._health_regen_update_timer = 1						-- update once every second, to reduce load on network syncing and such
		end
	end
end
