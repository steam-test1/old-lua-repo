...
function PlayerMovement:post_init()
	...
	-- Create references to the camera orientation
...
function PlayerMovement:set_character_anim_variables()
	...
	if camera_unit:damage() then
		...
		-- local sequence = tweak_data.blackmarket.characters[ "locked" ][ character ].sequence -- Wrong to hardcode locked?
		...
		if camera_unit:damage():has_sequence( sequence ) then
			camera_unit:damage():run_sequence_simple( sequence ) -- tweak_data.blackmarket.characters[ self._peer:character_id() ].sequence )
		end
	end
end
...
function PlayerMovement:change_state( name )
	...
	if self._current_state then
		--cat_debug( "george", tostring( self._current_state_name ) .. " (EXIT)" )
		exit_data = self._current_state:exit( self._state_data, name )
	end

	local new_state = self._states[ name ]
	--self._machine:set_callback_object( new_state )

	--cat_debug( "george", tostring( name ) .. " (ENTER)" )
...
function PlayerMovement:update( unit, t, dt )
	...
	if self._kill_overlay_t then
		if self._kill_overlay_t < t then
	...
	-- if self._suspicion_ratio then
		-- Application:draw_sphere( unit:position(), ( 1-self._suspicion_ratio ) * 500, 1,0,0 )
	-- end
end
...
--	self._unit:body( "body" ):set_position( pos + math.UP*80 )
end
...
-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function PlayerMovement:m_pos()	-- unit root pos
	return self._m_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function PlayerMovement:m_stand_pos()	-- head position if unit was standing
	return self._m_stand_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function PlayerMovement:m_com()	-- current centre of mass ( mid height ) of the unit
	return self._m_com
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function PlayerMovement:m_head_pos() -- current position of unit's head/look point
	return self._m_head_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function PlayerMovement:m_head_rot()	-- look rotation of unit
	return self._m_head_rot
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function PlayerMovement:m_detect_pos()	-- unit root pos
	return self._m_head_pos
end
...
function PlayerMovement:downed()
	return  self._current_state_name == "bleed_out" or 
			self._current_state_name == "fatal" or 
			self._current_state_name == "arrested" or 
			self._current_state_name == "incapacitated"
end
...
function PlayerMovement:on_uncovered( enemy_unit )
	if self._current_state_name ~= "mask_off" and self._current_state_name ~= "clean" then
		return -- this can happen on client
	end
...
function PlayerMovement:set_attention_settings( setting_names )
	--print( "[PlayerMovement:set_attention_settings]", self._unit, inspect( setting_names ) )
	...
	local _add_attentions_to_all = function ( names )
		for _, setting_name in ipairs( names ) do
			...
			if setting_desc then
				...
				--print( "adding attention", inspect( setting ) )
			else
	...
	if Network:is_client() then
		...
		if changes.removed then
			for _, id in ipairs( changes.removed ) do
				local index = tweak_data.attention:get_attention_index( id )
				--print( "sending removed", id, index )
...
function PlayerMovement:set_attention_setting_enabled( setting_name, state, sync )
	--print( "[PlayerMovement:set_attention_setting_enabled]", setting_name, state, sync )
	--Application:stack_dump()
	if state then
		local setting_desc = tweak_data.attention.settings[ setting_name ]
		if setting_desc then
	...
	if sync then
		...
		--print( "sending", setting_name, index )
...
function PlayerMovement:clbk_attention_notice_sneak( observer_unit, status )
	--print( "[PlayerMovement:attention_notice_clbk]", observer_unit, status, type(status) )
...
-- print( "[PlayerMovement:on_suspicion]", observer_unit, status, type(status) )

	if Network:is_server() then
		self._suspicion_debug = self._suspicion_debug or {}
		self._suspicion_debug[ observer_unit:key() ] = { unit = observer_unit, name = observer_unit:name(), status = status }
		...
		else	-- observer_unit or police already discovered you, no need to update/spam  hud/sound
			return
		end
...
--------------------------------------------------------------------------------------
-- status=false : lost, status=true : noticed, status=[number] : notice progress 0-1
function PlayerMovement:_calc_suspicion_ratio_and_sync( observer_unit, status ) -- SERVER ONLY
	--print( "[PlayerMovement:_calc_suspicion_ratio_and_sync]", observer_unit, status, type(status) )
...
function PlayerMovement.clbk_msg_overwrite_suspicion( overwrite_data, msg_queue, msg_name, suspect_peer_id, suspicion )
	--print( "[PlayerMovement.clbk_msg_overwrite_suspicion] msg_name", msg_name, "suspect_peer_id", suspect_peer_id, "suspicion", suspicion )
	if msg_queue then
		if overwrite_data.indexes[ suspect_peer_id ] then
			...
			old_msg[3] = suspicion -- overwrite with the new suspicion_value
		else
			table.insert( msg_queue, { msg_name, suspect_peer_id, suspicion } )
			overwrite_data.indexes[ suspect_peer_id ] = #msg_queue -- mark the position on the queue of the overwiteable message of this type
		end
	else -- reset
...
-------------------------------------------------------------------------------
-- also called for HuskPlayerMovement
function PlayerMovement:clbk_enemy_weapons_hot()
	...
	if Network:is_server() then
		if self._synced_suspicion ~= 0 then
...
function PlayerMovement:chk_play_mask_on_slow_mo( state_data )
	if not state_data.uncovered then
		-- Do I see anyone?
		if managers.enemy:chk_any_unit_in_slotmask_visible( managers.slot:get_mask( "enemies" ), self._unit:camera():position(), self._nav_trakcer ) then
...
function PlayerMovement:_upd_underdog_skill( t )
	...
	if not self._attackers or not ( data.has_dmg_dampener or data.has_dmg_mul ) or t < self._underdog_skill_data.chk_t then
		return
	end
...
function PlayerMovement:object_interaction_blocked()
	-- return self._current_state:is_deploying() or 
...
function PlayerMovement:save( data )
	...
	if self._current_state_name == "clean" or self._current_state_name == "mask_off" then
		-- default stance for husk is ntl so we dont need to sync anything
	elseif self._state_data.in_steelsight then
		data.movement.stance = 3 -- cbt
	else
		data.movement.stance = 2 -- hos
	end
...
function PlayerMovement:_change_stamina( value )
	...
	elseif( not stamina_maxed and self._stamina >= max_stamina ) then
	...
	local stamina_breath = math.clamp( (self._stamina - tweak_data.player.movement_state.stamina.MIN_STAMINA_THRESHOLD) / stamina_to_threshold, 0, 1 ) * 100 -- (self._stamina / math.max( 1, max_stamina )) * 100 -- (self:is_above_stamina_threshold() and (self._stamina / math.max( 1, max_stamina )) * 100) or 0
	SoundDevice:set_rtpc( "stamina", stamina_breath )
end

function PlayerMovement:subtract_stamina( value )		-- for jumping or other sudden out of breath moments
	self:_change_stamina( -math.abs(value) ) -- add bonus to stamina reduction here
end

function PlayerMovement:add_stamina( value )
	self:_change_stamina( math.abs(value) * managers.player:upgrade_value( "player", "stamina_regen_multiplier", 1 )  )  -- add bonus to stamina regen here
end
