...
PlayerInventory._all_event_types = { "add", "equip", "unequip" }
...
function PlayerInventory:init( unit )
	self._unit = unit
	self._available_selections = {}		--	Indexed table: selection index, item data
	self._equipped_selection = nil	--	The index in self._available_selections of the weapon we have equipped
	self._latest_addition = nil
	
	self._selected_primary = nil	--	The weapon we are going to use if the player triggers the primary weapon
	
	self._use_data_alias = "player"
	
	self._align_places = {}
	self._align_places.right_hand = { obj3d_name = Idstring("a_weapon_right"), on_body = false }
	self._align_places.left_hand = { obj3d_name = Idstring("a_weapon_left"), on_body = false }
...
function PlayerInventory:add_unit( new_unit, is_equip, equip_is_instant )
	...
	-- If there is another unit occupying that selection index, remove it
	if self._available_selections[ selection_index ] then
		local old_weapon_unit = self._available_selections[ selection_index ].unit
		is_equip = is_equip or old_weapon_unit == self:equipped_unit() -- Equip new weapon if the old one was equipped
	...
	--print( "PlayerInventory:add_unit", new_unit, selection_index, is_equip, equip_is_instant )
...
function PlayerInventory:get_latest_addition_hud_data()
	...
	return { is_equip = self._latest_addition == self._selected_primary, amount = amount, inventory_index = self._latest_addition, unit = unit }
end

-----------------------------------------------------------------------------------

function PlayerInventory:add_unit_by_name( new_unit_name, equip, instant )
	--[[if new_unit_name == Idstring( "units/weapons/glock/glock" ) then
		self:add_unit_by_factory_name( "wpn_fps_pis_g18c", equip, instant )
		return
	end
	if new_unit_name == Idstring( "units/weapons/m4_rifle/m4_rifle" ) then
		self:add_unit_by_factory_name( "wpn_fps_ass_m4", equip, instant )
		return
	end]]
...
function PlayerInventory:remove_selection( selection_index, instant )
	...
	--print( "[PlayerInventory:remove_selection]", self._unit, selection_index, weap_unit, "self._listener_id", self._listener_id )
...
function PlayerInventory:equip_selection( selection_index, instant )
	if selection_index and selection_index ~= self._equipped_selection and self._available_selections[ selection_index ] then
		...
		--print( "PlayerInventory:equip_selection", selection_index, instant )
		...
		--print( "PlayerInventory:equip_selection FAILED", selection_index, instant )
	end
	return false
end
...
function PlayerInventory:unequip_selection( selection_index, instant )
	if not selection_index or selection_index == self._equipped_selection then	--	Is it the currently equipped item that we are unequipping?
...
function PlayerInventory:_place_selection( selection_index, is_equip )
	...
	--print( "PlayerInventory:_place_selection", is_equip, is_equip and "equip" or "unequip" )
	
	local align_place = self._align_places[ weap_align_data.align_place ]
	if align_place then	-- Do we have the align place required to link the weapon? If not, disable the unit.
		if is_equip then
		...
		--print( "linking", res, parent_unit, align_place.obj3d_name, unit, unit:orientation_object():name() )
	else
		unit:unlink()
		unit:set_enabled( false )
		unit:base():on_disabled()
		-- If has gadget, make sure to turn off permanent cbt stance
		if unit:base().gadget_on then
			if self._unit:movement().set_cbt_permanent then
...
-----------------------------------------------------------------------------------
--	example:	unit:inventory():add_listener( "PlayerDamage", { "unequip", "add" }, callback( self, self, "on_inventory_event" ) )
-- events can be nil or false which means you want to listen to all event types
...
function PlayerInventory:_get_weapon_sync_index_from_name( wanted_weap_name )
	for i, weap_name in pairs( self._index_to_weapon_list ) do
		if (type( weap_name ) == "string" and type( wanted_weap_name ) == "string") or 
			(type( weap_name ) ~= "string" and type( wanted_weap_name ) ~= "string")  then
...
function PlayerInventory:load( data )
	if data.equipped_weapon_index then
		...
		if type( eq_weap_name ) == "string" then -- New weapon
...
function PlayerInventory:set_mask_visibility( state )
	...
	-- Prep and spawn new mask
	local mask_unit_name = managers.criminals:character_data_by_name( character_name ).mask_obj -- Notie how this is no longer a 3Dobj ref, but rather a unit ref.
	-- mask_unit_name tweak_data.blackmarket.masks[ managers.criminals:character_data_by_name( character_name ).mask_id ]
	...
	-- managers.blackmarket:apply_mask_craft_on_unit( mask_unit, managers.criminals:character_data_by_name( character_name ).mask_blueprint )

	-- Link mask to player characters head bone(mask_align)
...
function PlayerInventory:set_ammo( ammo )
	...
	-- managers.hud:set_ammo_amount( self:equipped_unit():base():selection_index(), self:equipped_unit():base():ammo_info() )
end
...
-- Returns true if all weapons are out of ammo
