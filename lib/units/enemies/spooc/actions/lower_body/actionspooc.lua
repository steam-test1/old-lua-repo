...
function ActionSpooc:init( action_desc, common_data )
	...
	-- Determine if we are leading the action or following
	local is_local
	...
	if not is_server then -- the client sometimes needs a nav_point inserted to compensate from hurt action movement
		...
		if host_stop_pos ~= common_data.pos then
			...
			--print( "inserted host_stop_pos", inspect( self._nav_path ) )
		end
	end
	...
	elseif is_local then
		if not is_server and self:_chk_target_invalid() then	-- server cannot have an invalid target
			self:_wait()
		else
	...
	--print( "end of init", inspect( self ), inspect( self._nav_path ) )
...
function ActionSpooc:_upd_chk_walkway( t )
	-- wait until i have walkability to the attention unit
	...
	if self._common_data.nav_tracker:lost() then
		self:_expire()	-- cannot start walking if i am outside the nav field
	end
...
function ActionSpooc:_start_sprint()
	--print( "[ActionSpooc:_start_sprint]" )
	...
	--print( "self._nav_index", self._nav_index, "self._nav_path", inspect( self._nav_path ) )
...
function ActionSpooc:_upd_strike_first_frame( t )
	...
	self._stroke = true
	if self._is_local then
		--print( "sending strike", self._common_data.pos )
		...
		--print( "nav_path", inspect( self._nav_path ) )
...
function ActionSpooc:_upd_chase_path()
	...
	-- check if we need to add a nav_point in the path ahead of us in order to avoid collision
	local walk_ray
	if not simplified then	-- we dont need to check again if the shortcut check has succeeded
	...
	--managers.navigation:draw_path( self._nav_path, {0.2, 0.1, 0.6, 0.6 }, { 0.2, 0.2, 0.8 } )
end
...
function ActionSpooc:_upd_sprint( t )
	...
	if self._move_dir then	--	Player wants to move
		...
		if math.abs( fwd_dot ) > math.abs( right_dot ) then		--	We want to walk fwd/bwd
		...
		--print( "real velocity1", real_velocity )
		local variant = "run"
		...
		--print( "wanted_walk_anim_speed", wanted_walk_anim_speed, "anim_walk_speed", anim_walk_speed )
		self:_adjust_walk_anim_speed( dt, wanted_walk_anim_speed )
	end
	--print( "real velocity2", mvector3.distance( self._common_data.pos, self._last_pos ) / dt )
...
function ActionSpooc:get_husk_interrupt_desc()
	...
	--print( "[ActionSpooc:get_husk_interrupt_desc]", inspect( old_action_desc ) )
...
function ActionSpooc:_nav_chk( t, dt )
	...
	if not self._stroke and self._strike_nav_index and ( self._strike_nav_index <= new_nav_index or complete and self._strike_nav_index == new_nav_index + 1 ) then
		--print( "strike now! self._strike_nav_index", self._strike_nav_index, "new_nav_index", new_nav_index, inspect( path ) )
	...
	if complete then
		--print( "advanced nav index", old_nav_index, new_nav_index )
		self._end_of_path = true
	end
	...
	if old_nav_index ~= new_nav_index then
		if self._is_local and not self._was_interrupted then
			--print( "sending due to advancement" )
...
function ActionSpooc:_upd_striking( t )
	if self._ext_anim.act then
		if self._is_local then
			if not ( self._ext_anim.spooc_exit or self._ext_anim.spooc_enter or alive( self._strike_unit ) and self._strike_unit:character_damage().incapacitated and self._strike_unit:character_damage():incapacitated() ) then
				self._ext_movement:play_redirect( "spooc_exit" )
			end
		elseif not ( self._ext_anim.spooc_exit or self._ext_anim.spooc_enter ) and self._end_of_path then
			self._ext_movement:drop_held_items()
			self:_start_sprint()
		end
	else
		--print( "[ActionSpooc:_upd_striking] ended in", self._machine:segment_state( Idstring( "base" ) ) )
		self._ext_movement:drop_held_items()
		self:_start_sprint()
	end
end
...
function ActionSpooc:sync_stop( pos, stop_nav_index )
	--print( "[ActionSpooc:stop]", self._unit, "self._nav_index", self._nav_index, "my pos", self._common_data.pos, "stop_nav_index", stop_nav_index, "pos", pos, "self._host_stop_pos_inserted", self._host_stop_pos_inserted )
	...
	while #nav_path > stop_nav_index do
		--print( "popping", #nav_path, nav_path[ #nav_path ] )
		table.remove( nav_path )
	end
	
	self._stop_pos = pos
	if #nav_path < stop_nav_index - 1 then	-- this means that it is not the host who has control over the unit, and the host has received more nav_points than us. wait to receive the rest of the nav points and then stop
		self._nr_expected_nav_points = stop_nav_index - #nav_path + 1
		--print( "self._nr_expected_nav_points", self._nr_expected_nav_points )
	else
...
-----------------------------------------------------------------------------------
-- we are a husk and the controlling client/host has given us the next position to go to
function ActionSpooc:sync_append_nav_point( nav_point )
	if self._stop_pos and not self._nr_expected_nav_points then
		--print( "[ActionSpooc:sync_append_nav_point] too late", self._unit )
		return
	end
	--print( "[ActionSpooc:sync_append_nav_point]", self._unit, "appended", nav_point, "my pos", self._unit:position(), "self._nav_index", self._nav_index, "nav_path b4", inspect( self._nav_path ) )
	table.insert( self._nav_path, nav_point )
	if self._end_of_path then -- we have a new nav_point and can start moving
		self._end_of_path = nil
		local nav_index = math.min( #self._nav_path - 1, self._nav_index + 1 )
		self._nav_index = nav_index
		--print( "ActionSpooc:_upd_wait end self._nav_index", self._nav_index )
		self._cur_vel = 0
...
-----------------------------------------------------------------------------------
-- The owner peer of the spooc wants us to strike at this position
function ActionSpooc:sync_strike( pos )
	--print( "[ActionSpooc:sync_strike]", self._unit, "self._nav_index", self._nav_index, "my pos", self._common_data.pos, "pos", pos )
	if self._stop_pos and not self._nr_expected_nav_points then
		--print( "[ActionSpooc:sync_strike] too late", self._unit )
		return
	end
	table.insert( self._nav_path, pos )
	self._strike_nav_index = #self._nav_path
	--print( "self._strike_nav_index", self._strike_nav_index, "self._nav_path", inspect( self._nav_path ) )
	if self._nr_expected_nav_points then
		--print( "self._nr_expected_nav_points", self._nr_expected_nav_points )
		if self._nr_expected_nav_points == 1 then
			self._nr_expected_nav_points = nil
...
function ActionSpooc:_send_nav_point( nav_point )
	--print( "sending nav_point", nav_point )
...
function ActionSpooc:_set_updator( name )
	--print( "[ActionSpooc:_set_updator]", name )
...
function ActionSpooc:on_attention( attention )
	--print( "[ActionSpooc:on_attention]", attention and inspect( attention ) )
