...
function SpoocLogicAttack.queued_update( data )
	...
	if my_data.spooc_attack then
		if my_data.spooc_attack.action:complete()
				and data.attention_obj
				and ( not data.attention_obj.criminal_record or not data.attention_obj.criminal_record.status )
				and ( data.attention_obj.verified or data.attention_obj.nearly_visible )
				and data.attention_obj.dis < my_data.weapon_range.close then
...
function SpoocLogicAttack._upd_spooc_attack( data, my_data )
	...
	if focus_enemy.nav_tracker and focus_enemy.is_person and focus_enemy.criminal_record and not focus_enemy.criminal_record.status and
		not my_data.spooc_attack and ( focus_enemy.reaction >= AIAttentionObject.REACT_SHOOT )
		and ( data.t > data.spooc_attack_timeout_t )
		and focus_enemy.verified_dis < ( my_data.want_to_take_cover and 1500 or 2500 )
