...
--CopActionWalk.nr_moving_ppl = 0

CopActionWalk._walk_anim_velocities = {
	stand = {
		ntl = {
			walk = { fwd = 110, bwd = 111.4, l = 124.5, r = 124.5 },
	...
	crouch = {
		cbt = {
			walk = { fwd = 174.45, bwd = 163.74, l = 152.14, r = 162.85 },
			run = { fwd = 312.25, bwd = 268.68, l = 282.93, r = 282.93 }
		}
	},
	wounded = {
		cbt = {
			walk = { fwd = 148, bwd = 114, l = 122, r = 135 },
			run = { fwd = 280, bwd = 230, l = 246, r = 224 }
		}
	}
}
...
CopActionWalk._walk_anim_lengths = {
	...
	wounded = {
		cbt = {
			walk = { fwd = 28, bwd = 29, l = 29, r = 29 }, 
			run = { fwd = 19, bwd = 18, l = 19, r = 19 }
		}
	},
	panic = { 
		ntl = {
			run = { fwd = 15, bwd = 15, l = 15, r = 16 }
		}
	}
}
...
CopActionWalk._matching_walk_anims = { fwd = { bwd = true }, bwd = { fwd = true }, l = { r = true }, r = { l = true } }

CopActionWalk._walk_side_rot = { fwd = Rotation(), bwd = Rotation(180), l = Rotation(-90), r = Rotation(90) }
...
function CopActionWalk:init( action_desc, common_data )
	...
	--CopActionWalk.nr_moving_ppl = CopActionWalk.nr_moving_ppl + 1
	
	if self._ext_anim.needs_idle then
		--print( "[CopActionAct:init] self._waiting_full_blend", self._ext_anim.needs_idle, self._ext_anim.idle_full_blend )
		self._waiting_full_blend = true
		self:_set_updator( "_upd_wait_for_full_blend" ) -- wait until the idle animation has blended in
	elseif not self:_init() then	-- start walking
		debug_pause_unit( self._unit, "[CopActionWalk:init] failed _init", self._unit ) 
		return	-- something went wrong. report failure
	end
	...
	if not self._ext_anim.pose then
		...
		if not self._ext_anim.pose then
			...
			if not self._ext_movement:play_state( "std/stand/still/idle/look" ) then
				debug_pause()
				return	-- we are in a state where we cannot start walking. somethings wrong on the AI/net_synch side
			end
		end
	end
...
function CopActionWalk:_chk_start_anim( next_pos )
	...
	--print( "path_angle", path_angle, "path_len", path_len )
	if math_abs( path_angle ) > 135 then	-- turn bwd
		--print( "bwd" )
		if can_turn_and_fire then
			...
			if ds:length() < path_len - 100 then
				--Application:draw_line( pos1, pos2, 0,1,0 )
				if path_angle > 0 then
					path_angle = path_angle - 360
					--print( "converting" )
				end
				--print( "path_angle", path_angle )
				self._start_run_turn = { self._common_data.rot:yaw(), path_angle, "bwd" }
			end
		end
	elseif path_angle < -65 then -- turn to the right
		--print( "right" )
		if can_turn_and_fire then
			local pose = self._ext_anim.pose
			...
			if ds:length() < path_len - 100 then
				--Application:draw_line( pos1, pos2, 0,1,0 )
				--print( "path_angle", path_angle )
				self._start_run_turn = { self._common_data.rot:yaw(), path_angle, "r" }
			end
		end
	elseif path_angle > 65 then	-- turn to the left
		--print( "left" )
		if can_turn_and_fire then
			local pose = self._ext_anim.pose
			...
			if ds:length() < path_len - 100 then
				--Application:draw_line( pos1, pos2, 0,1,0 )
				--print( "path_angle", path_angle )
				self._start_run_turn = { self._common_data.rot:yaw(), path_angle, "l" }
			end
		end
	end
	...
	--print( "self._start_run_turn", self._start_run_turn )
	if not self._start_run_turn then
		...
		--	Get the wanted_walk_dir
		if math_abs( fwd_dot ) > math_abs( right_dot ) then		--	We want to walk fwd/bwd
			self._start_run_straight = fwd_dot > 0 and "fwd" or "bwd"
		else
			self._start_run_straight = right_dot > 0 and "r" or "l"
		end
		--print( "self._start_run_straight", self._start_run_straight )
	end
end
...
function CopActionWalk:on_exit()
	...
	if self._expired and self._common_data.ext_anim.move then		--	We want to stop and we are still walking
		self:_stop_walk()
	end
	...
	if self._sync then
		...
		--print( "[CopActionWalk:on_exit] sending stop", mvec3_cpy( self._ext_movement:m_pos() ), math.min( 256, self._simplified_path_index ) )
	else
...
function CopActionWalk:update( t )
	...
	elseif vis_state > self._skipped_frames then
		...
		--print( "skipping", self._skipped_frames, vis_state )
		return
	else
		self._skipped_frames = 1
		--print( "updating", vis_state )
		dt = t - self._last_upd_t
		self._last_upd_t = TimerManager:game():time()
	end
	...
	if self._end_of_path then
		if self._next_is_nav_link then	-- we have reached a nav_link. change updator
	...
	local move_dir = tmp_vec3	-- the direction we want to move into
	...
	if move_dir and not self._expired then
		local face_fwd = tmp_vec1	-- the direction we want to face at
		local wanted_walk_dir
		local move_dir_norm = move_dir:normalized()
		--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + move_dir_norm * 200, 3, 1, 1, 1 )
		if self._no_strafe or self._walk_turn then
			wanted_walk_dir = "fwd"
		else
			if self._curve_path_end_rot and mvector3.distance_sq( self._last_pos, self._footstep_pos ) < 140*140 then
			...
			--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + face_fwd * 200, 4, 1, 1, 0 )
			local face_right = tmp_vec2	-- right of the direction we want to face at
		...
		if self._chk_stop_dis and not self._common_data.char_tweak.no_run_stop then -- We are on a straight line to the end of the path
			local end_dis = mvec3_dis( self._nav_point_pos( self._simplified_path[ #self._simplified_path ] ), self._last_pos )
			if self._chk_stop_dis > end_dis then
				...
				if stop_dis and stop_dis > end_dis then
		...
		if not self._walk_anim_velocities[ pose ] or
		not self._walk_anim_velocities[ pose ][ self._stance.name ] or
		not self._walk_anim_velocities[ pose ][ self._stance.name ][ variant ] or
		not self._walk_anim_velocities[ pose ][ self._stance.name ][ variant ][ wanted_walk_dir ] then
...
function CopActionWalk:_upd_start_anim( t )
	if not self._ext_anim.run_start then
		...
		while self._curve_path[3] do -- If the walk action has overshot, remove intermediate points
			...
			if mvec3_dot( tmp_vec1, tmp_vec2 ) < 0 and not CopActionWalk._chk_shortcut_pos_to_pos( self._curve_path[1], self._curve_path[3], nil ) then
				--[[print( "shortcutting ", mvec3_dot( tmp_vec1, tmp_vec2 ) )
				Application:draw_cylinder( self._curve_path[3], self._curve_path[2], 20, 1,1,1 )
				Application:draw_cylinder( self._curve_path[2], self._curve_path[1], 20, 0,0,0 )
...
function CopActionWalk:get_husk_interrupt_desc()
	...
	--print( "[CopActionWalk:get_husk_interrupt_desc]", inspect( old_action_desc ), self._unit )
...
function CopActionWalk:_nav_chk_walk( t, dt, vis_state )
	...
	if not ( self._sync or self._start_run ) and self:_husk_needs_speedup() then -- husk only
		vel = 1.25 * vel
	end
	...
	local upd_footstep, reservation_failed
	--	Walk the unit's chase position from the initial spline pos
	while not self._end_of_curved_path do
		...
		if complete then
			...
			if #s_path == 2 then	--	We have completed our journey
				...
				nav_advanced = true
				break
			elseif self._next_is_nav_link then	-- the nav point we reached is a nav_link
				self._end_of_curved_path = true
	...
	if not reservation_failed then
		...
		--	Walk the unit's center of mass from the initial spline pos
		new_pos, new_c_index, complete = self._walk_spline( c_path, self._last_pos, c_index, walk_dis )
		if complete then
			--	We completed the entire path
			...
			if new_c_index ~= self._curve_path_index or nav_advanced then
				...
				if future_pos then
					...
					if self._haste ~= "run" and turn_dot > -0.7 and turn_dot < 0.7 and not self._attention_pos and future_dis_flat > 80 and self._common_data.stance.name == "ntl" and mvec3_dot( self._common_data.fwd, cur_vec ) > 0.97 then
						--[[local pos1 = self._last_pos -- my current_pos
						local pos2 = next_pos -- the corner ahead
						local pos3 = future_pos -- the nav_point after the corner
						mvec3_set( tmp_vec3, pos3 )
						mvec3_sub( tmp_vec3, pos2 )
						
						local right_dot = mvec3_dot( self._common_data.right, tmp_vec3 )
						print( ( right_dot > 0 and "right" or "left" ).." turn ahead" )
						Application:draw_cylinder( pos1, pos2, 4, 0.6, 0, 0 )
						Application:draw_cylinder( pos2, pos3, 4, 1, 0, 0 )
						Application:set_pause( true )]]
					...
					if vis_state < 3 and self._end_of_curved_path and not ( self._NO_RUN_STOP or self._no_walk or self._haste == "walk" or mvec3_dis( c_path[ new_c_index + 1 ], new_pos ) < 120 or not self._ext_anim.run ) then
						self._chk_stop_dis = 210
					end
					self._walk_turn = nil
				end
	...
	--managers.navigation:draw_path( c_path, {0.2, 0.9, 0.1, 0.2 }, { 0.2, 0.1, 0.2 }, 0 )
...
function CopActionWalk._walk_spline( path, pos, index, walk_dis )
	while true do
		...
		mvec3_set_z( tmp_vec1, 0 ) -- the segment direction
		local dis = mvec3_norm( tmp_vec1 )
		mvec3_set( tmp_vec2, pos )
		mvec3_sub( tmp_vec2, path[ index ] )
		mvec3_set_z( tmp_vec2, 0 ) -- my_progress vector, flat
		local my_dis = mvec3_dot( tmp_vec2, tmp_vec1 )
		
		if dis == 0 or my_dis + walk_dis >= dis and walk_dis >= 0 then	--	we reached the next nav point
			if index == #path - 1 then
				return path[ index + 1 ], index, true
			else
				index = index + 1
			end
		elseif my_dis + walk_dis < 0 and walk_dis < 0 then	--	we reached the previous nav point
			if index == 1 then
...
function CopActionWalk:_rserve_pos_step_clbk( data, test_pos )
	...
	if not blocked then	-- we must stop testing in this direction
...
function CopActionWalk:_adjust_move_anim( side, speed )
	...
	local enter_t
	local move_side = anim_data.move_side	-- the direction of our current playing animation
	if move_side and ( side == move_side or self._matching_walk_anims[ side ][ move_side ] ) then	
		--Some animation combinations match well enough to allow us to continue with the new animation from where the old one currently is
		local seg_rel_t = self._machine:segment_relative_time( idstr_base )
		
		if not self._walk_anim_lengths[ anim_data.pose ] or
		not self._walk_anim_lengths[ anim_data.pose ][ self._stance.name ] or
		not self._walk_anim_lengths[ anim_data.pose ][ self._stance.name ][ speed ] or
		not self._walk_anim_lengths[ anim_data.pose ][ self._stance.name ][ speed ][ side ] then
	...
	--print( "redirect_name", redirect_name, "enter_t", enter_t )
...
-----------------------------------------------------------------------------------
-- We are are doing an animated stop
function CopActionWalk:_upd_stop_anim_first_frame( t )
	...
	if self._ext_anim.pose ~= "crouch" then	-- non-crouch
		if self._stop_anim_side == "fwd" then
			self._stop_anim_displacement_f = function( p1, p2, t )
		...
		elseif self._stop_anim_side == "bwd" then
			self._stop_anim_displacement_f = function( p1, p2, t )
				local low = 0.97
				local p_1_5 = 0.9
				local t_clamp = t--math.clamp( t, 0, 0.8 ) / 0.8
		...
		elseif self._stop_anim_side == "l" then
			...
			self._stop_anim_displacement_f = function( p1, p2, t )
				--Draw:brush( Color(0.2,0,0,1) ):cylinder( p1, p2, 7 )
				local low = 0.9
				local p_1_5 = 0.85
				local t_clamp = math.clamp( t, 0, 0.75 ) / 0.75
				if t_clamp < p_1_5 then
...
-----------------------------------------------------------------------------------
-- we are a husk and the host wants us to stop at a certain position and nav_index
function CopActionWalk:stop( pos )
	--print( "[CopActionWalk:stop]", self._unit, "self._simplified_path_index", self._simplified_path_index, "my pos", self._common_data.pos, "nav_index", nav_index, "pos", pos )
	...
	if #s_path > 0 and not s_path[ #s_path ].x then -- this nav_link was never performed on the server side
		s_path[ #s_path ] = self._nav_point_pos( s_path[ #s_path ] )
	end
		
	table.insert( s_path, pos )
	--print( "s_path after", inspect( s_path ) )
	--managers.navigation:draw_path( s_path, {0.2, 0,0,1}, {0.3,1,1,0}, 0 )
...
-----------------------------------------------------------------------------------
-- we are a husk and the host has given us the next position to go to
function CopActionWalk:append_nav_point( nav_point )
	--print( "[CopActionWalk:append_nav_point]", self._unit, "appended", nav_point, "my pos", self._unit:position(), "self._simplified_path_index", self._simplified_path_index, "s_path b4", inspect( self._simplified_path ) )
	if not nav_point.x then
		nav_point.element.value = function ( element, name ) return element[ name ] end
		nav_point.element.nav_link_wants_align_pos = function ( element ) return element.from_idle end
	end
...
function CopActionWalk:set_blocks( preset_name, state )
	...
	if self._blocks.action then
		self._ext_movement:action_request( { type = "idle", body_part = 3, client_interrupt = true, non_persistent = true } )
	end
end
...
function CopActionWalk:_upd_nav_link_first_frame( t )
	if self._next_is_nav_link.element:nav_link_wants_align_pos() then --this nav_link needs to be played from idle in order to align correctly
		if self:_stop_walk() then
...
function CopActionWalk:_play_nav_link_anim( t )
	...
	if self._nav_link_invul and not self._nav_link_invul_on then	-- server only. team AI uses it
	...
	if result then
		...
		if self._blocks == self._anim_block_presets.block_all then	-- default blocks. stop upper_body actions
			self._ext_movement:action_request( { type = "idle", body_part = 3, client_interrupt = true, non_persistent = true } )
		end
	else
	...
	elseif self._simplified_path[2] then
		...
		if self._sync then
			...
			if res then
				...
				--Application:draw_cylinder( ray_params.tracker_from:position(), ray_params.pos_to, 5, 1,0,0)
...
function CopActionWalk:_upd_walk_turn_first_frame( t )
	local pos1 = self._last_pos -- my current_pos
	local pos2 = self._curve_path[ self._curve_path_index + 1 ] -- the corner ahead
	local pos3 = self._curve_path[ self._curve_path_index + 2 ] -- the nav_point after the corner
	mvec3_set( tmp_vec2, pos3 )
	mvec3_sub( tmp_vec2, pos2 )
	
	local right_dot = mvec3_dot( self._common_data.right, tmp_vec2 )
	
	--[[Application:draw_cylinder( pos1, pos2, 4, 0, 0.6, 0 )
	Application:draw_cylinder( pos2, pos3, 4, 0, 1, 0 )
	Application:set_pause( true )]]
	
	local seg_rel_t = self._machine:segment_relative_time( idstr_base )
	--print( "seg_rel_t", seg_rel_t )
...
function CopActionWalk:_upd_walk_turn( t )
	--print( "[CopActionWalk:_upd_walk_turn]", t )
	if self._ext_anim.walk_turn then
		--self._last_pos = mvec3_cpy( CopActionHurt._get_pos_clamped_to_graph( self, false ) )
		self._last_pos = self._unit:position()
		self._ext_movement:set_m_pos( self._last_pos )
		local dt = TimerManager:game():delta_time()
		...
		if self._walk_turn_blend_to_middle then
			--print( "jumping to halfway walk loop" )
		...
		local c_index = self._curve_path_index + 2 -- the position two steps ahead of us
		...
		while c_index < #self._curve_path do
			...
			if col then
				...
				table.remove( self._curve_path, c_index - 1 ) -- pop the position one step ahead of us
			end
		end
		
		self._curve_path[ self._curve_path_index ] = mvec3_cpy( self._common_data.pos ) -- overwrite the nav_link with our current position and continue from here
		self._walk_turn = nil
...
function CopActionWalk:on_nav_link_unregistered( element_id )
	--print( "[CopActionWalk:on_nav_link_unregistered]", self._unit, element_id )
...
function CopActionWalk:_husk_needs_speedup()
	...
	elseif #self._simplified_path > 2 then
		...
		if dis_error_total > 90000 then -- 3m
			return true
		end
	end
end
