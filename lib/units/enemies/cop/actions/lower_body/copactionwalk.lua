...
do
	local (for generator), (for state), (for control) = pairs(CopActionWalk._walk_anim_lengths)
	do
		do break end
...
function CopActionWalk:_init()
	...
	if self._sync then
		...
		local nav_path = {}
		do
			local (for generator), (for state), (for control) = ipairs(self._nav_path)
			do
				do break end
				if nav_point.x then
	...
	if action_desc.path_simplified and action_desc.persistent then
		if self._sync then
			...
			do
				local (for generator), (for state), (for control) = ipairs(original_path)
				do
					do break end
...
function CopActionWalk._apply_padding_to_simplified_path(path)
	...
	while index < #path do
		...
		if pos.x then
			do
				local (for generator), (for state), (for control) = ipairs(diagonals)
				do
					do break end
...
function CopActionWalk:get_husk_interrupt_desc()
	...
	if self._blocks or self._old_blocks then
		local blocks = {}
		do
			local (for generator), (for state), (for control) = pairs(self._old_blocks or self._blocks)
			do
				do break end
				blocks[i] = -1
			end
...
function CopActionWalk:on_nav_link_unregistered(element_id)
	...
	local (for generator), (for state), (for control) = ipairs(self._simplified_path or self._nav_path)
	do
		do break end
		if not nav_point.x then
