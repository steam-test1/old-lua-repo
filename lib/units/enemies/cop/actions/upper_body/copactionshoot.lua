...
function CopActionShoot:init( action_desc, common_data )
	--print( "[CopActionShoot:init]", inspect( action_desc ) )
	...
	if Network:is_server() then
		self._ext_movement:set_stance_by_code( 3 )	--cbt
	end
		...
		if attention.unit then
			...
			if shoot_hist then
				...
				shoot_hist = { focus_error_roll = focus_error_roll,
												...
												focus_delay = usage_tweak.focus_delay,	-- maximum focus delay
...
function CopActionShoot:update( t )
	local vis_state = self._ext_base:lod_stage()	-- mul 3
	...
	if self._attention then
		...
		if self._turn_allowed then
			...
			if not ( active_actions[2] and active_actions[2]:type() ~= "idle" or queued_actions and ( queued_actions[1] or queued_actions[2] ) or self._ext_movement:chk_action_forbidden( "walk" ) ) then
	...
	if ext_anim.reload or ext_anim.equip or ext_anim.melee or ext_anim.equip then	-- we are reloading/equipping
	elseif self._weapon_base:clip_empty() then
		...
		if self._autofiring then
			if not ( target_vec and self._common_data.allow_fire ) then -- stop firing
				self._weapon_base:stop_autofire()
				self._shoot_t = t + 0.6 -- delay until next shot
				self._autofiring = nil
				self._autoshots_fired = nil
				if ext_anim.recoil then
					self._ext_movement:play_redirect( "up_idle" )
				...
				if fired then
					...
					if not self._autofiring or self._autoshots_fired == self._autofiring - 1 then
						...
						if vis_state == 1 then
							self._shoot_t = t + ( self._common_data.is_suppressed and 1.5 or 1 ) * math.lerp( falloff.recoil[1], falloff.recoil[2], math.random() ) -- delay until next shot
						else
			...
			if target_vec and self._common_data.allow_fire and t > self._shoot_t and t > self._mod_enable_t then
			
				local shoot
				if autotarget or self._shooting_husk_player and ( t > self._next_vis_ray_t ) then -- this applies only when shooting a player unit
					if self._shooting_husk_player then
...
function CopActionShoot:_get_unit_shoot_pos( t, pos, dis, w_tweak, falloff, i_range, shooting_local_player )
	...
	if math.random() < hit_chance then	-- we hit
...
function CopActionShoot:_get_transition_target_pos( shoot_from_pos, attention, t )
	...
	--Application:draw_sphere( shoot_from_pos, math.lerp( 15, 30, prog ), 0,1,0 )
	if prog > 1 then	-- end of transition
		self._aim_transition = nil
		self._get_target_pos = nil
	...
	--Application:draw_line( shoot_from_pos, shoot_from_pos + target_vec * 5000, 0,1,0 )
...
function CopActionShoot:set_ik_preset( preset_name )
	self[ self._ik_preset.stop ]( self )	-- stop previous IK
...
function CopActionShoot:_upd_ik_spine( target_vec, fwd_dot, t )
	if fwd_dot > 0.5 then
		...
		--Application:draw_cone( target_pos, shoot_from_pos, 15, 0,0,1 )
		return target_vec
	else
		--Application:draw_cone( target_pos, shoot_from_pos, 15, 1,0,0 )
		if self._modifier_on then
			self._modifier_on = nil
			self._machine:allow_modifier( self._modifier_name )
		end
...
function CopActionShoot:anim_clbk_melee_strike()
	--print( "[CopActionShoot:anim_clbk_melee_strike]", inspect( self._attention ) )
	...
	if not ( autotarget and target_dis < max_dix ) then
		return
	end
