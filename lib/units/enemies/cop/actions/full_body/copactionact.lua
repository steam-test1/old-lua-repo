...
function CopActionAct:init( action_desc, common_data )
	--print( "[CopActionAct:init]", common_data.unit, inspect( action_desc ) )
	...
	if action_desc.needs_full_blend and self._ext_anim.idle and ( not self._ext_anim.idle_full_blend or self._ext_anim.to_idle ) then
		--print( "[CopActionAct:init] self._waiting_full_blend", self._ext_anim.idle, self._ext_anim.idle_full_blend )
		self._waiting_full_blend = true
		self:_set_updator( "_upd_wait_for_full_blend" ) -- wait until the idle animation has blended in
	elseif not self:_play_anim() then	-- start animating
...
function CopActionAct:_ik_update_func( t )
	...
	if self._attention and self._ik_type then
		...
		elseif not self._modifier_on then
			--self._machine:set_modifier_blend( self._modifier_name, 0 ) -- this glitches
			...
			self._look_trans = { start_t = TimerManager:game():time(), duration = duration, start_vec = old_look_vec }
		end
		...
		if self._modifier_on then
			--Application:draw_cylinder( look_from_pos, look_from_pos + target_vec * 1000, 2, 0,0,1 )
...
function CopActionAct:update( t )
	...
	if self._freefall then
		...
		else -- the animation does not want freefall any more
			self._freefall = nil
...
function CopActionAct:chk_block( action_type, t )
	...
	return unblock_t and ( unblock_t == -1 or unblock_t > t )
end
...
function CopActionAct:_play_anim()
	...
	if type( self._action_desc.variant ) == "number" then -- an action originating from a save game
		redir_name = self._machine:index_to_state_name( self._action_desc.variant ) -- returns whole state name, not redirect
	...
	if Network:is_client() then
		if self._action_desc.start_rot then
	...
	if self._action_desc.clamp_to_graph then
		...
		self:_set_updator() -- default updator
	end
