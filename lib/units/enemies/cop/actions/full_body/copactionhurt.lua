...
CopActionHurt.death_anim_variants = {
	normal = {
		crouching = { fwd = { high = 14, low = 5 }, bwd = { high = 3, low = 1 }, l = { high = 3, low = 1 }, r = { high = 3, low = 1 } },
		not_crouching = { fwd = { high = 14, low = 5 }, bwd = { high = 3, low = 2 }, l = { high = 3, low = 1 }, r = { high = 3, low = 1 } }
	},
	heavy = {
		crouching = { fwd = { high = 7, low = 2 }, bwd = { high = 3, low = 1 }, l = { high = 3, low = 1 }, r = { high = 3, low = 1 } },
		not_crouching = { fwd = { high = 6, low = 2 }, bwd = { high = 1, low = 1 }, l = { high = 1, low = 1 }, r = { high = 1, low = 1 } }
	}
}

CopActionHurt.death_anim_fe_variants = {
	normal = {
		crouching = { fwd = { high = 5, low = 2 }, bwd = { high = 2, low = 0 }, l = { high = 2, low = 0 }, r = { high = 2, low = 0 } },
		not_crouching = { fwd = { high = 6, low = 2 }, bwd = { high = 3, low = 0 }, l = { high = 2, low = 0 }, r = { high = 2, low = 0 } }
	},
	heavy = {
		crouching = { fwd = { high = 0, low = 0 }, bwd = { high = 0, low = 0 }, l = { high = 0, low = 0 }, r = { high = 0, low = 0 } },
		not_crouching = { fwd = { high = 0, low = 0 }, bwd = { high = 0, low = 0 }, l = { high = 0, low = 0 }, r = { high = 0, low = 0 } }
	}
}
...
function CopActionHurt:init( action_desc, common_data )
	--print( "CopActionHurt:init]", common_data.unit, inspect( action_desc ) )
	...
	local is_civilian = tweak_table == "civilian" or tweak_table == "civilian_female" or tweak_table == "bank_manager"	-- civilians currently allways dies in one shot (=heavy death all the time), so to get more variants I run normal death only on civilians // jansve
	local is_female = (self._machine:get_global( "female" ) or 0) == 1
	
	--self._ext_anim.ragdoll = true	-- force ragdolling. useful for testing
	...
	elseif action_type == "light_hurt" then
		if not self._ext_anim.upper_body_active or self._ext_anim.upper_body_empty or self._ext_anim.recoil then	-- we only play light hurts if we are idle
	...
	elseif action_type == "hurt_sick" then
		...
		if is_civilian then
			is_cop = false
			-- is_female = self._common_data.char_tweak.female or false
		end
	...
	elseif action_type == "death" and ( self._ext_anim.run and self._ext_anim.move_fwd or self._ext_anim.sprint ) and not common_data.char_tweak.no_run_death_anim then
		-- run fwd death. special anim state
	...
	elseif action_type == "death" and ( self._ext_anim.run or self._ext_anim.ragdoll ) and self:_start_ragdoll() then
		...
		-- Still hurt/death. get direction variant etc
		...
		if action_desc.variant == "bleeding" then
			...
			if nr_variants then
				...
				elseif action_type ~= "shield_knock" and action_type ~= "counter_tased" then
					...
					if old_variant then
						-- Check for crouch here as well if adding crouching hurts
						if old_info[ dir_str ] == 1 and
							old_info[ height ] == 1 and
							(old_info.mod == 1 and action_type == "hurt") or (old_info.hvy == 1 and action_type == "heavy_hurt") then
							variant = old_variant
						end
					end
				...
				-- print( "variant: " .. tostring(variant), "dir:" .. tostring(dir_str), "crouching: " .. tostring(crouching), "height: " .. tostring(height) )
			end
			
			variant = variant or 1
			--print( action_start_data.type, "dir", dir_str, "height", height, "variant", variant )
			if variant then
				self._machine:set_parameter( redir_res, "var"..tostring(variant), 1 )
			end
			if height then
	...
	if action_type == "bleedout" then
		...
		if not self._shoot_history then
			self._shoot_history = {
				...
				focus_start_t = t,
				focus_delay = weapon_usage_tweak.focus_delay,	-- maximum focus delay
	...
	if shoot_chance then
		...
		if equipped_weapon and not ( equipped_weapon:base().clip_empty and equipped_weapon:base():clip_empty() ) and math.random() < shoot_chance then
			self._weapon_unit = equipped_weapon
			self._original_bullet_hit_slotmask = equipped_weapon:base()._bullet_slotmask
			equipped_weapon:base():set_bullet_hit_slotmask( managers.slot:get_mask( "bullet_impact_targets_shooting_death" ) ) -- hit only friendly and neutral units
	...
	if not self._unit:base().nick_name then -- Simple way to see if it's a criminal
		if action_type == "death" then
		...
		if Network:is_server() then
			...
			local new_alert = { "vo_distress", common_data.ext_movement:m_head_pos(), radius, managers.groupai:state():get_unit_type_filter( filter_name ), self._unit }
...
function CopActionHurt:_get_floor_normal( at_pos, fwd, right )
	...
	mvec3_add( from_pos, center_pos )
	mvec3_set( to_pos, from_pos ) -- to_pos = from_pos + down_vec
...
function CopActionHurt:_upd_hurt( t )
	...
	if self._ext_anim.hurt or self._ext_anim.death then
		...
		self._last_pos = self:_get_pos_clamped_to_graph( true ) -- tmp_vec1!!!
...
function CopActionHurt:_upd_bleedout( t )
	...
	if not self._ext_anim.bleedout_enter and self._weapon_unit then
		if self._attention and not ( self._ext_anim.reload or self._ext_anim.equip ) then
			...
			--Application:draw_cylinder( shoot_from_pos, target_pos, 4, 0,0,1 )
			...
			if self._look_dir then
				...
				--[[
				local v1 = mvector3.copy(target_vec)
				mvector3.normalize(v1)
				mvector3.multiply(v1, 250)
				local p1 = mvector3.copy(shoot_from_pos)
				mvector3.add(p1, v1)

				local v2 = mvector3.copy(self._look_dir)
				mvector3.normalize(v2)
				mvector3.multiply(v2, 250)
				local p2 = mvector3.copy(shoot_from_pos)
				mvector3.add(p2, v2)
				
				Draw:brush(Color(0.2, 0,1,0), 0):cylinder(shoot_from_pos, p1, 2)
				Draw:brush(Color(0.2, 1,0,0), 0):cylinder(shoot_from_pos, p2, 2)
				]]
			else
			...
			if t > self._shoot_t then
				...
				elseif self._common_data.allow_fire then
					...
					local rand = math.random()	-- determines recover delay and recoil animation strength
					self._shoot_t = t + math.lerp( falloff.recoil[1], falloff.recoil[2], rand ) -- delay until next shot
				end
			end
...
function CopActionHurt:_upd_ragdolled( t )
	...
	if self._ragdoll_active then	-- this means that a body is active
		self._hips_obj:m_position( tmp_vec1 )
		self._ext_movement:set_position( tmp_vec1 )
	end
	
	if not( self._ragdoll_freeze_clbk_id or self._shooting_hurt ) then
		self._died = true
	end
end
...
function CopActionHurt:need_upd()
	if self._died then	-- death animation completed. we do not exit the action to avoid an idle action taking over
		return false
	else
		return true
	end
...
function CopActionHurt:_start_ragdoll()
	...
	if self._unit:damage() and self._unit:damage():has_sequence( "switch_to_ragdoll" ) then
		self:on_death_drop( self._unit, 2 ) -- drop weapon
...
function CopActionHurt:force_ragdoll() -- animation callback
	if self:_start_ragdoll() then
		self.update = self._upd_ragdolled
		self._ext_movement:enable_update()
	end
	...
	if self._root_act_tags[ tag:key() ] then
		if activated then
			...
			if not self._shooting_hurt then
				self._died = true	-- stop the extension update
			end
		end
	end
end
...
function CopActionHurt:_freeze_ragdoll()
	self._root_act_tags = {}	-- this will make body activate callbacks to be ignored
...
function CopActionHurt:clbk_chk_freeze_ragdoll()
	...
	self._hips_obj:m_position( tmp_vec1 )	-- new position
