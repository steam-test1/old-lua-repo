...
CopDamage._all_event_types = { "light_hurt", "hurt", "heavy_hurt", "death", "shield_knock", "stun", "counter_tased" }
...
-- ULF : Heavy FBI Heavy swat plating
	-- if self._unit:body( Idstring( "body_plate" ) ) then
...
function CopDamage:get_damage_type( damage_percent, category )
	...
	if hurt_table.health_reference == "full" then
		-- no change
	elseif hurt_table.health_reference == "current" then -- express dmg as a ration of current health
		dmg = math.min( 1, self._HEALTH_INIT * dmg / self._health )
	else	 -- express dmg as a ration of reference value
...
-- ULF : FBI Heavy swat plating
	...
	local damage_percent = math.ceil( math.clamp( damage / self._HEALTH_INIT_PRECENT, 1, self._HEALTH_GRANULARITY ) ) -- Cannot give less than 1% damage
		...
		if attack_data.attacker_unit == managers.player:player_unit() then
			...
			if( not self:_type_civilian( self._unit:base()._tweak_table ) ) then
				...
				if alive( attack_weapon ) and attack_weapon:base() then
					...
					local weapon_pass, enemy_pass, mask_pass = nil, nil, nil
...
function CopDamage:_show_death_hint( type )
	if self:_type_civilian( type ) and not self._unit:base().enemy then
		-- managers.hint:show_hint( "hint_killing_people" )
	end
end

function CopDamage:_comment_death( unit, type )
	-- Hack for calling PlayerStandard
	if type == "tank" then
...
function CopDamage:damage_explosion( attack_data )
	...
	-- if result.type == "death" and attack_data.owner == managers.player:player_unit() then
	if result.type == "death" then
...
function CopDamage:damage_melee( attack_data )
	...
	local result
	local head = self._head_body_name and attack_data.col_ray.body and attack_data.col_ray.body:name() == self._ids_head_body_name
	-- local damage = head and self._health or attack_data.damage
	local damage = attack_data.damage
	...
	local damage_effect = attack_data.damage_effect	-- this is used to determine the severity of the hurt
	local damage_effect_percent
	
	-- Clamp the damage received within limits
	damage = math.clamp( damage, self._HEALTH_INIT_PRECENT, self._HEALTH_INIT ) -- Cannot give less than 1% damage
	local damage_percent = math.ceil( damage / self._HEALTH_INIT_PRECENT )
	damage = damage_percent * self._HEALTH_INIT_PRECENT
	
	if damage >= self._health then
		damage_effect_percent = 1 -- this doesn't matter in this case
		attack_data.damage = self._health
		result = { type = "death", variant = attack_data.variant }
		self:die( attack_data.variant )
	else
		attack_data.damage = damage
		damage_effect = math.clamp( damage_effect, self._HEALTH_INIT_PRECENT, self._HEALTH_INIT ) -- Cannot give less than 1% damage
	...
	--print( "CopDamage:damage_melee damage", damage, "self._health_ratio", self._health_ratio )
...
------------------------------------------------------------------------------------------
-- This method is unfinished. george
function CopDamage:get_ranged_attack_autotarget_data( shoot_from_pos, aim_vec )
	...
	--print( "dis", dis, "shoot_rel_spin", shoot_rel_spin )
	if dis > 3500 then		--	We are too far to distinguish autotarget objects
		--print( "too_far" )
		autotarget_data = { body = self._unit:body( "b_spine1" ) }
	else
		self._aim_bodies = {}
		...
		--table.insert( self._aim_bodies, self._unit:body( "b_RightLeg" ) )
		--table.insert( self._aim_bodies, self._unit:body( "b_LeftLeg" ) )
		
		local uncovered_body
		local best_angle
		for i, body in ipairs( self._aim_bodies ) do
			...
			if not best_angle or best_angle > body_angle then
				...
				if not aim_ray then
					...
					--Application:draw_cylinder( shoot_from_pos, body_pos, 30, 1, 0, 0 )
					--print( body:name(), "unprotected" )
				else
					--print( body:name(), "protected" )
				end
			end
		end
		
		if uncovered_body then
			--Application:draw_cylinder( shoot_from_pos, uncovered_body:center_of_mass(), 10, 0, 1, 0 )
			--print( uncovered_body:name(), "chosen" )
...
function CopDamage:_spawn_head_gadget( params )
	...
	self._head_gear = false	-- do not spawn any more headgear
end
...
------------------------------------------------------------------------------------------
-- when self._mover_collision_state is nil the collision is enabled. any negative value means disabled
...
function CopDamage:sync_damage_bullet( attacker_unit, damage_percent, i_body, hit_offset_height, death )
	...
	if attacker_unit then
		...
		attack_dir = self._unit:rotation():y() -- the attacker unit was destroyed. it was probably a cop. it was probably from behind
	end
	...
	if death then
		if head then
			if damage > math.random(10) then
				self:_spawn_head_gadget( { position = body:position(), rotation = body:rotation(), dir = attack_dir } )
			end
		end
		...
		local data = { name = self._unit:base()._tweak_table, head_shot = head, weapon_unit = attacker_unit and attacker_unit:inventory() and attacker_unit:inventory():equipped_unit(), variant = attack_data.variant }
...
function CopDamage:sync_damage_explosion( attacker_unit, damage_percent, i_attack_variant, death, direction )
	...
	if death then
		...
		local data = { name = self._unit:base()._tweak_table, head_shot = false, weapon_unit = attacker_unit and attacker_unit:inventory() and attacker_unit:inventory():equipped_unit(), variant = "explosion" }
		managers.statistics:killed_by_anyone( data )
	else
...
function CopDamage:sync_damage_melee( attacker_unit, damage_percent, damage_effect_percent, i_body, hit_offset_height, variant, death )
	...
	if death then
		...
		local data = { name = self._unit:base()._tweak_table, head_shot = false, variant = "melee" }
		managers.statistics:killed_by_anyone( data )
	else
...
function CopDamage:build_suppression( amount )
	if self._dead or not self._char_tweak.suppression then
		return -- this character cannot be suppressed
	end
	...
	elseif Network:is_server() and self._suppression_hardness_t and t < self._suppression_hardness_t then
		amount = amount * 0.5 -- we have recently been suppressed. we are tougher now
	end
	...
	if not self._suppression_data.brown_zone and self._suppression_data.brown_point and self._suppression_data.value >= self._suppression_data.brown_point then -- enter brown zone
		self._suppression_data.brown_zone = true -- once we go in effect then we must wait for decay
		self._unit:brain():on_suppressed( true )
	end
	
	if not self._suppression_data.react_zone and self._suppression_data.react_point and self._suppression_data.value >= self._suppression_data.react_point then -- enter react zone
		self._suppression_data.react_zone = true -- once we go in effect then we must wait for decay
...
function CopDamage:load( data )
	...
	if data.char_dmg.is_converted then
		self._unit:set_slot( 16 )
		managers.enemy:add_delayed_clbk( "converted_contour"..tostring(self._unit:key()), callback( self, self, "clbk_apply_converted_contour" ), TimerManager:game():time() + 1 ) -- applying contour same frame as load doesn't seem to work
