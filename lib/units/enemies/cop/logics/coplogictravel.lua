...
local fun = function( key, ... )
	if tostring( key ) == "userdata: 000002E7" then
...
function CopLogicTravel.enter( data, new_logic_name, enter_params )
	...
	--print( "[CopLogicTravel.enter]", data.unit:key() )
	...
	if objective.pose then
		if data.objective.pose == "crouch" then
			if data.char_tweak.allow_crouch and not ( data.unit:anim_data().crouch or data.unit:anim_data().crouching ) then
				CopLogicAttack._chk_request_action_crouch( data )
			end
		elseif not ( data.char_tweak.no_stand or data.is_suppressed ) then
...
--[[
{ "idle", "spooc", "stand", "hurt" },
{ "idle", "walk", "turn" },
{ "idle", "shoot", "reload", "tase" },
{ "hurt", "stand", "crouch" }
]]
...
function CopLogicTravel.exit( data, new_logic_name, enter_params )
	...
	--print( "[CopLogicTravel.exit]", data.unit )
			...
			if cur_index == total_nav_points then		-- We have reached the final nav point. The area is investigated.
				...
				if start_pathing then
					...
					if cur_index == total_nav_points - 1 then
						...
						if new_occupation then
							...
							elseif new_occupation.type == "defend" then
								...
								elseif new_occupation.pos then
									...
									local reservation = { position = mvector3.copy( to_pos ), radius = 60, filter = data.pos_rsrv_id }
						...
						--print( "starting advance search" )
						--print( inspect(coarse_path) )
		...
		if not ( my_data.turning or unit:movement():chk_action_forbidden( "walk" ) or data.unit:anim_data().reload ) then
			...
			elseif data.attention_obj and data.attention_obj.reaction >= AIAttentionObject.REACT_SCARED then
				if not CopLogicTravel._chk_request_action_turn_to_cover( data, my_data ) then
					if not ( my_data.best_cover and my_data.best_cover[4] or unit:anim_data().crouch ) and data.char_tweak.allow_crouch then
						CopLogicAttack._chk_request_action_crouch( data )
					end
				end
			end
...
function CopLogicTravel._upd_enemy_detection( data )
	...
	--print( "[CopLogicTravel._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
	...
	if allow_trans then
		if obj_failed or not objective or objective.type ~= "follow" then
			...
			if wanted_state and wanted_state ~= data.name then
				...
				if my_data == data.internal_data and not objective.is_default then -- if we haven't changed state already, change now
	...
	if my_data == data.internal_data then	-- we are still in this logic
	...
	if new_attention then
		if data.char_tweak.chatter.entrance and not data.entrance and new_attention.criminal_record and new_attention.verified and AIAttentionObject.REACT_SCARED <= new_reaction and math.abs( data.m_pos.z - new_attention.m_pos.z ) < 4000 then
	...
	if data.pathing_results then
		...
		if path then
			...
			if path ~= "failed" then
				...
				--Application:draw_cylinder( data.m_pos, data.m_pos + math.UP * 5000, 30, 0,1,0 )
				--Application:draw_cylinder( data.m_pos, my_data.path_to_pos, 10, 1,0,0 )
				--Application:set_pause( true )
				managers.groupai:state():on_objective_failed( data.unit, data.objective )
				return
			end
		...
		if path then
			...
			if path ~= "failed" then
				...
				if my_data.coarse_search_failed then	-- we have failed a safe search before. drop the objective
					...
					--[[Application:set_pause( true )
					Application:draw_cylinder( data.m_pos, data.m_pos + math.UP * 50000, 30, 1,0,0 )
					Application:draw_cylinder( data.m_pos, managers.navigation._nav_segments[ data.objective.nav_seg ].pos, 10, 1,1,0 )]]
					managers.groupai:state():on_objective_failed( data.unit, data.objective )
					return
				else
...
function CopLogicTravel._chk_request_action_turn_to_cover( data, my_data )
	...
	if math.abs( error_spin ) > 25 then
		...
		if data.unit:brain():action_request( new_action_data ) then
			--print( "CopLogicTravel turn action requested", new_action_data.angle )
...
function CopLogicTravel.action_complete_clbk( data, action )
	...
	if action_type == "walk" then
		...
		if action:expired() and not my_data.starting_advance_action and my_data.coarse_path_index then	-- Action has terminated normally ( was not interrupted )
...
function CopLogicTravel._get_pos_accross_door( guard_door, nav_seg )
	...
	local rot_angle = 90
	if room_1_seg == nav_seg then -- objective vis group is on the neg side
	...
	--Application:draw_sphere( door_pos, 27, 0, 0, 1 )
	--Application:draw_sphere( door_pos + accross_vec, 27, 0, 1, 0 )
	...
	if accross_positions then
		...
		return best_pos
		--Application:draw_sphere( to_pos, 27, 1, 0, 0 )
	end
	managers.navigation:destroy_nav_tracker( door_tracker )
end
...
-----------------------------------------------------------------------------
-- attempts to reserve the closest pos to wanted_pos between wanted_pos and look_pos
function CopLogicTravel._reserve_pos_along_vec( look_pos, wanted_pos )
	...
	local data = { start_pos = wanted_pos,
								step_vec = step_vec,
...
function CopLogicTravel._chk_request_action_walk_to_advance_pos( data, my_data, speed, end_rot, no_strafe )
	if not data.unit:movement():chk_action_forbidden( "walk" ) or data.unit:anim_data().act_idle then
		--print( "[CopLogicTravel._chk_request_action_walk_to_advance_pos]", data.t )
		...
		if my_data.advancing then
			if my_data.rsrv_pos.path then
				...
				local end_pos = mvector3.copy( path[ #path ] )
				local rsrv_desc = { filter = data.pos_rsrv_id, position = end_pos, radius = 30 }
			...
			if my_data.nearest_cover and not ( my_data.delayed_clbks and my_data.delayed_clbks[ my_data.cover_update_task_key ] ) then
				CopLogicBase.add_delayed_clbk( my_data, my_data.cover_update_task_key, callback( CopLogicTravel, CopLogicTravel, "_update_cover", data ), data.t + 1 )
			end
		end
	end
...
function CopLogicTravel._get_pos_on_wall( from_pos, max_dist, step_offset, is_recurse )
	...
	repeat
		...
		if ray_res then
			...
			if is_free then
				--local brush = Draw:brush( Color( 0.5,1,1,1 ), 1 )
...
function CopLogicTravel.chk_should_turn( data, my_data )
	return not ( my_data.advancing or my_data.turning or data.unit:movement():chk_action_forbidden( "walk" ) )
end

-----------------------------------------------------------------------------
-- if the first nav_seg is not our current_nav_seg then append all nav segments in between so we can path there
function CopLogicTravel.complete_coarse_path( data, my_data, coarse_path )
	--print( "[CopLogicTravel.complete_coarse_path]", data.unit, "coarse_path\n",  inspect( coarse_path ) )
	local first_seg_id = coarse_path[1][1]
	local current_seg_id = data.unit:movement():nav_tracker():nav_segment()
	--fun( data.key, "current_seg_id", current_seg_id )
	
	-- first make sure the path is complete
	...
	while i_nav_point < #coarse_path do
		...
		if not nav_seg.neighbours[ next_nav_seg_id ] then
			--fun( data.key, "coarse path missing links. not neighbours:", nav_seg_id, next_nav_seg_id )
			local search_params = {
				from_seg = nav_seg_id,
			...
			while i_insert > 1 do
				...
				--fun( data.key, "inserting", ins_coarse_path[ i_insert ][1], "att position", i_nav_point + 1, "result:", inspect( coarse_path ) )
				i_insert = i_insert - 1
			end
		end
	...
	-- check if I stand on an intermediate point on the coarse path. In which case, skip the previous nav_points
	local start_index
	...
	if start_index then
		start_index = math.min( start_index, #coarse_path - 1 ) -- cannot go directly to the last nav_point. we need to go to final position first
		return start_index
	end
	
	--fun( data.key, "[CopLogicTravel.complete_coarse_path] completing...", data.unit, "coarse_path\n", inspect( coarse_path ), "current_seg_id", current_seg_id )
	...
	repeat
		...
		for other_seg_id, door_list in pairs( search_seg.neighbours ) do
			...
			if not ( other_seg.disabled or found_segs[ other_seg_id ] ) then
				...
				if other_seg_id == first_seg_id then
					...
					while found_segs[ last_added_seg_id ] ~= "init" do
						...
						table.insert( coarse_path, 1, { last_added_seg_id, all_nav_segs[ last_added_seg_id ].pos } )
					end
					--print( "complete:\n", inspect( coarse_path ), "start_index", start_index )
					return 1
				else
	...
	--debug_pause_unit( data.unit, "[CopLogicTravel.complete_coarse_path] could not complete coarse path", data.unit, "coarse_path", inspect( coarse_path ), "current_seg_id", current_seg_id )
	return 1
end
...
function CopLogicTravel.apply_wall_offset_to_cover( data, my_data, cover, wall_fwd_offset )
	...
	mvector3.set( to_pos_fwd, cover[2] ) -- cover fwd vec
	...
	--print( "[CopLogicTravel.apply_wall_offset_to_cover] col_pos_fwd", col_pos_fwd, "to_pos_fwd", to_pos_fwd )
	local space_needed = mvector3.distance( col_pos_fwd, to_pos_fwd ) + wall_fwd_offset * 1.05 -- +5% extra space
	
	local to_pos_bwd = tmp_vec2
	mvector3.set( to_pos_bwd, cover[2] ) -- cover fwd vec
	...
	--[[Application:set_pause(true)
	if collision then
		Application:draw_cylinder( cover[1], ray_params.trace[1], 20, 0.5,0,0 )
	else
		Application:draw_cylinder( cover[1], to_pos_bwd, 20, 1,0,0 )
	end
	Application:draw_cylinder( data.m_pos, cover[1], 20, 0.5,0.5,0.5 )
	]]
...
function CopLogicTravel._find_cover( data, search_nav_seg, near_pos )
	--local pos = objective.follow_unit and objective.follow_unit:movement():nav_tracker():field_position() or managers.navigation._nav_segments[ objective.nav_seg ].pos
	...
	if data.unit:movement():cool() then
		...
		-- choose a cover aligned towards a criminal
