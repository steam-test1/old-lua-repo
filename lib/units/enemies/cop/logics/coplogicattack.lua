...
function CopLogicAttack._upd_aim( data, my_data )
	...
	if focus_enemy and focus_enemy.reaction >= AIAttentionObject.REACT_AIM then	-- I have an enemy
				...
				if focus_enemy.reaction >= AIAttentionObject.REACT_SHOOT then
					if focus_enemy.verified then
						...
						elseif last_sup_t and data.t - last_sup_t < 7 then
							shoot = true
						elseif focus_enemy.reaction == AIAttentionObject.REACT_SHOOT then -- REACT_COMBAT means we are afraid of the object. if we are not afraid shoot even if "avoid"
							shoot = true
						elseif my_data.attitude == "engage" then
							if focus_enemy.verified_dis < data.internal_data.weapon_range.far then
								shoot = true
							end
						-- elseif focus_enemy.verified_dis < data.internal_data.weapon_range.close or data.unit:anim_data().move and focus_enemy.verified_dis < math.lerp( data.internal_data.weapon_range.close, data.internal_data.weapon_range.optimal, 0.75 ) then
						else
							-- shoot = true
							-- if focus_enemy.verified_dis < data.internal_data.weapon_range.far then
							if focus_enemy.verified_dis < data.internal_data.weapon_range.close then
								shoot = true
							elseif last_sup_t and data.t - last_sup_t < 20 then
								if focus_enemy.verified_dis < data.internal_data.weapon_range.far then
									shoot = true
								end
							end
		...
		elseif focus_enemy.verified_t then
			if data.t - focus_enemy.verified_t < ( data.name == "travel" and 1.5 or 4 ) and focus_enemy.verified_dis < data.internal_data.weapon_range.optimal and math.abs( focus_enemy.verified_pos.z - data.m_pos.z ) < 250 then
				aim = true
				if focus_enemy.reaction >= AIAttentionObject.REACT_SHOOT and my_data.shooting and data.t - focus_enemy.verified_t < 3 then
					shoot = true
				end
			else
				...
				elseif data.t - focus_enemy.verified_t < 20 or focus_enemy.verified_dis < 1000 then
					aim = true
					if my_data.shooting and my_data.firing and last_sup_t and data.t - last_sup_t < 7 and focus_enemy.reaction >= AIAttentionObject.REACT_SHOOT and my_data.shooting and data.t - focus_enemy.verified_t < 3 then
						shoot = true
					end
				end
			end
	...
	if aim or shoot then
		...
		if not ( my_data.shooting or data.unit:anim_data().reload or data.unit:movement():chk_action_forbidden( "action" ) ) then
			local shoot_action = {
				type = "shoot",
				body_part = 3
			}
