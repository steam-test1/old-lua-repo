...
function CopLogicBase._get_logic_state_from_reaction( data, reaction )
	...
	if not reaction or reaction <= AIAttentionObject.REACT_SCARED then
		if not ( police_is_being_called or managers.groupai:state():is_police_called() or data.unit:movement():cool() or data.is_converted) then
	...
	elseif reaction == AIAttentionObject.REACT_ARREST and not data.is_converted then
		...
		if not ( data.char_tweak.no_arrest or police_is_being_called or managers.groupai:state():is_police_called() or data.unit:movement():cool() or data.is_converted ) and not ( data.attention_obj and data.attention_obj.verified and data.attention_obj.dis < 1500 ) then
			return "arrest" -- call the police
		else
			return "attack"
		end
