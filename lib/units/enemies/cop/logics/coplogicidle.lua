...
function CopLogicIdle.enter( data, new_logic_name, enter_params )
	...
	if not my_data.rsrv_pos.stand then
		local pos_rsrv = { position = mvector3.copy( data.m_pos ), radius = 30, filter = data.pos_rsrv_id }
...
function CopLogicIdle._upd_stance_and_pose( data, my_data, objective )
	...
	if not obj_has_stance then
		if data.char_tweak.allowed_stances and not data.char_tweak.allowed_stances[ data.unit:anim_data().stance ] then
...
function CopLogicIdle._start_idle_action_from_act( data )
	data.unit:brain():action_request( { type = "act", body_part = 1, variant = "idle", blocks = { idle = -1, action = -1, walk = -1, light_hurt = -1, hurt = -1, heavy_hurt = -1, expl_hurt = -1 } } )
end
