...
function CopLogicIdle._upd_scan(data, my_data)
	...
	if not beanbag then
		beanbag = {}
		do
			local (for generator), (for state), (for control) = ipairs(my_data.stare_pos)
			do
				do break end
...
function CopLogicIdle._chk_stare_into_wall_1(data)
	...
	while next(areas_to_search) do
		...
		if expand then
			local (for generator), (for state), (for control) = pairs(test_area.neighbours)
			do
				do break end
	...
	local path_tasks = {}
	do
		local (for generator), (for state), (for control) = pairs(dangerous_near_areas)
		do
			do break end
...
function CopLogicIdle._chk_stare_into_wall_2(data)
	...
	do
		local (for generator), (for state), (for control) = ipairs(stare_path)
		do
			do break end
...
function CopLogicIdle._get_priority_attention(data, attention_objects, reaction_func)
	...
	do
		local (for generator), (for state), (for control) = pairs(attention_objects)
		do
			do break end
			local att_unit = attention_data.unit
...
function CopLogicIdle._upd_stance_and_pose(data, my_data, objective)
	...
	if not obj_has_stance and data.char_tweak.allowed_stances and not data.char_tweak.allowed_stances[data.unit:anim_data().stance] then
		local (for generator), (for state), (for control) = pairs(data.char_tweak.allowed_stances)
		do
			do break end
			if state then
