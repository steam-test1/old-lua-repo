...
function CopLogicIdle.enter( data, new_logic_name, enter_params )
	...
	--print( "[CopLogicIdle.enter]", data.unit )
	...
	if old_internal_data then
		--	Stuff inheritted from the previous state
	...
	if not my_data.rsrv_pos.stand then
		local pos_rsrv = { position = mvector3.copy( data.m_pos ), radius = 60, filter = data.pos_rsrv_id }
	...
	if my_data.scan and not ( objective and objective.action ) then
	...
	if not ( entry_action or data.unit:movement():chk_action_forbidden( "walk" ) ) then
...
function CopLogicIdle._upd_enemy_detection( data )
	...
	--print( "[CopLogicIdle._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
...
function CopLogicIdle._upd_pathing( data, my_data )
	...
	if path then
		...
		if path ~= "failed" then
			...
			--print( "CopLogicIdle:_upd_pathing() we have a stare path", data.unit:key() )
...
function CopLogicIdle._upd_scan( data, my_data )
	--print( "[CopLogicIdle._upd_scan]", data.t )
	
	if CopLogicIdle._chk_focus_on_attention_object( data, my_data ) then
		--print( "[CopLogicIdle._upd_scan] focusing on attention instead" )
		return
	end
	...
	if not my_data.stare_pos or not my_data.next_scan_t or data.t < my_data.next_scan_t then
		...
		if not my_data.turning and my_data.fwd_offset then	-- No suspect. turn fwd again
	...
	--print( not my_data.stare_pos, not my_data.next_scan_t, not my_data.next_scan_t or data.t < my_data.next_scan_t )
	...
	if not beanbag then
		...
		--print( "[CopLogicIdle._upd_scan] new beanbag", inspect( beanbag ) )
	end
	
	-- find a random pos from the bean bag to look at
	local nr_pos = #beanbag
	local scan_pos
	--print( "my_data.next_scan_t", my_data.next_scan_t )
	--print( "scan_beanbag", beanbag and unpack( beanbag ) )
	...
	--print( "lucky_i_pos", lucky_i_pos, "scan_pos", scan_pos )
...
function CopLogicIdle.damage_clbk( data, damage_info )
	...
	if enemy and enemy:in_slot( data.enemy_slotmask ) then
		--print( "CopLogicIdle.damage_clbk Enemy identified by hostility" )
		local my_data = data.internal_data
	...
	--[[local objective = data.objective
	local is_free_objective = not objective or objective.type == "free" and not objective.interrupt_dis and not objective.interrupt_health and ( not objective.nav_seg or objective.in_place )
	if is_free_objective and
		damage_info.result.type ~= "death" and
		data.unit:character_damage():health_ratio() < 0.55 and
		not managers.groupai:state():get_assault_mode() and
		not ( data.internal_data.acting or data.objective and data.objective.no_retreat or data.char_tweak.no_retreat ) then
...
function CopLogicIdle.on_alert( data, alert_data )
	...
	elseif was_cool and ( alert_type == "footstep" or alert_type == "bullet" or alert_type == "aggression" or alert_type == "vo_cbt" or alert_type == "vo_intimidate" or alert_type == "vo_distress" ) then -- friendly unit or civilian in action. switch to not cool
...
function CopLogicIdle.on_new_objective( data, old_objective )
	--print( "[CopLogicIdle.on_new_objective]", data.unit, data.objective and inspect( data.objective ), "old_objective", old_objective and inspect( old_objective ) )
	...
	if new_objective then
		...
		elseif new_objective.action or not ( data.attention_obj and data.attention_obj.reaction >= AIAttentionObject.REACT_AIM ) then
			CopLogicBase._exit( data.unit, "idle" )
		else
...
-----------------------------------------------------------------------------
-- Returns if it's a good idea to attempt to arrest or assault this unit
	...
	elseif can_arrest and ( not record.assault_t or data.t - record.assault_t > att_unit:base():arrest_settings().aggression_timeout ) and data.t > record.arrest_timeout and not record.status then
		...
		if attention_data.dis < 2000 then
			for u_key, other_crim_rec in pairs( managers.groupai:state():all_criminals() ) do
				...
				if other_crim_attention_info then
					if other_crim_attention_info.is_deployable
					or other_crim_attention_info.verified and other_crim_rec.assault_t and data.t - other_crim_rec.assault_t < other_crim_rec.unit:base():arrest_settings().aggression_timeout then
						under_threat = true
						break
					end
				end
		...
		if under_threat then -- I am under attack from other criminals cannot risk arresting
		elseif attention_data.dis < 2000 and visible then -- non-hostile criminal, within arrest range
...
function CopLogicIdle.on_criminal_neutralized( data, criminal_key )
	-- check if he was my focus enemy. say something maybe
end
		...
		if is_enemy then
			if next( test_area.criminal.units ) then
				if test_area == my_area then
					break -- my area has enemies, scaning is not a very good idea
				end
		...
		if not nav_seg.disabled then
			...
			if ray_hit then
				...
				if ray_hit then
					...
					--print( "path_task:",seg_pos )
					--Application:draw_line( ray_from_pos, ray_to_pos, 1,0,0 )
				else
					table.insert( stare_pos, ray_to_pos )
					--print( "stare_pos by ray:", seg_pos )
					--Application:draw_line( ray_from_pos, ray_to_pos, 0,1,0 )
				end
			end
			if not ray_hit then
				table.insert( stare_pos, seg_pos + math.UP * 160 )
				--print( "stare_pos by walk:", seg_pos )
				--Application:draw_line( ray_from_pos, ray_to_pos, 0,0,1 )
			end
		end
	end
...
-----------------------------------------------------------------------------
-- we have pathed to a neighbour area. check how far u can see
function CopLogicIdle._chk_stare_into_wall_2( data )
	...
	for i, nav_point in ipairs( stare_path ) do -- convert nav links to positions
		stare_path[ i ] = f_nav_point_pos( nav_point )
	end
	--managers.navigation:_debug_draw_path( stare_path )
	...
	--print( "total_dis", total_dis, "dis_step", dis_step )
	local ray_from_pos = data.unit:movement():m_stand_pos()
	local ray_to_pos = Vector3()
	...
	repeat
		...
		--print( "index", index, "dis_in_seg", dis_in_seg, "seg_dis", seg_dis )
...
function CopLogicIdle.action_complete_clbk( data, action )
	...
	elseif action_type == "act" then
		local my_data = data.internal_data
		my_data.acting = nil
		if my_data.scan and not ( my_data.exiting or my_data.queued_tasks and my_data.queued_tasks[ my_data.wall_stare_task_key ] or my_data.stare_path_pos ) then
			CopLogicBase.queue_task( my_data, my_data.wall_stare_task_key, CopLogicIdle._chk_stare_into_wall_1, data, data.t )
		end
...
function CopLogicIdle.is_available_for_assignment( data )
	return not ( ( data.internal_data.performing_act_objective and not data.unit:anim_data().act_idle ) or data.internal_data.exiting or data.path_fail_t and data.t < data.path_fail_t + 6 )
end
	...
	if data.logic.is_available_for_assignment( data ) and not data.unit:movement():chk_action_forbidden( "walk" ) then
		...
		if turn_angle and current_attention.reaction < AIAttentionObject.REACT_CURIOUS then
			if turn_angle > 70 then
				--print( "ignoring attention object due to turn" )
				return -- don't turn around for idle attention
			else
				turn_angle = nil -- do not turn
			end
		end
...
function CopLogicIdle._chk_turn_needed( data, my_data, my_pos, look_pos )
	...
	local error_polar = target_vec:to_polar_with_reference( fwd, math.UP )	-- positive spin means target_vec is clockwise from fwd
	...
	for u_key, attention_data in pairs( attention_objects ) do
		...
		elseif attention_data.stare_expire_t and data.t > attention_data.stare_expire_t then
			if attention_data.settings.pause then
				...
				attention_data.pause_expire_t = data.t + math.lerp( attention_data.settings.pause[1], attention_data.settings.pause[2], math.random() )
				--print( "[CopLogicIdle._get_priority_attention] pausing for", attention_data.pause_expire_t - data.t, "sec" )
			end
			...
			if not reaction_too_mild then
				...
				if data.attention_obj and data.attention_obj.u_key == u_key and data.t - attention_data.acquire_t < 4 then
					-- alert_dt = alert_dt and alert_dt * 0.3 --0.8
					-- dmg_dt = dmg_dt and dmg_dt * 0.3 --0.8
					-- distance = distance * 0.3 --0.8
					old_enemy = true
				end
				...
				if visible and not reviving then
					if distance < 500 then -- Locked now to test all AI without having to go through all the data.
						target_priority_slot = 2
					else
						if distance < 1500 then -- Locked now to test all AI without having to go through all the data.
					...
					target_priority_slot = math.clamp(target_priority_slot, 1 , 10) -- TODO : replace with math.max(x, 0)
...
function CopLogicIdle._upd_curious_reaction( data )
	...
	local _get_spin_to_att_obj = function()
...
function CopLogicIdle._chk_objective_needs_travel( data, new_objective )
	if not ( new_objective.nav_seg or new_objective.type == "follow" ) then
		return -- objective does not require movement
	end
	...
	if new_objective.pos then
		return true -- this objective needs exact pos
	end
	...
	if new_objective.area and new_objective.area.nav_segs[ data.unit:movement():nav_tracker():nav_segment() ] then
		...
		return -- we are already in this area
	end
