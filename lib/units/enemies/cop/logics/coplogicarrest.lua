...
function CopLogicArrest.enter( data, new_logic_name, enter_params )
	...
	--print( "[CopLogicArrest.enter]", data.unit )
...
function CopLogicArrest.exit( data, new_logic_name, enter_params )
	...
	--print( "[CopLogicArrest.exit]", data.unit, inspect( data.attention_obj ) )
...
function CopLogicArrest.queued_update( data )
	...
	-- Aim at the criminal
	if not ( data.unit:anim_data().reload or data.unit:movement():chk_action_forbidden( "action" ) ) then
		if attention_obj and attention_obj.reaction >= AIAttentionObject.REACT_ARREST then
			if not ( my_data.shooting or data.unit:anim_data().reload ) then
				local shoot_action = {
					type = "shoot",
					body_part = 3
				}
	...
	--[[for enemy_key, arrest_target_data in pairs( arrest_targets ) do
		if arrest_data and arrest_data.intro_pos then
			Application:draw_sphere( arrest_data.intro_pos, 200, 1,1,1 )
			Application:draw_sphere( arrest_data.intro_pos, 20, 1,1,1 )
		elseif arrest_data.intro_t then
			Application:draw_sphere( attention_obj.unit:position(), 200, 0.5,0.5,0.5 )
			Application:draw_sphere( attention_obj.unit:position(), 20, 0.5,0.5,0.5 )
		end
		Application:draw_cylinder( data.m_pos, attention_obj.unit:position(), 10, 1,1,1 )
	end]]
	
	if arrest_data then
		if not arrest_data.intro_t then -- say this without delay
		...
		elseif not arrest_data.intro_pos and data.t - arrest_data.intro_t > 1 then
			-- We are done giving the warning. mark the criminal's position and see to it that he doesn't leave 
	...
	if arrest_data and arrest_data.intro_pos or my_data.should_stand_close then
		--Application:draw_sphere( arrest_data.intro_pos, 30, 0,0,1 )
		CopLogicArrest._upd_advance( data, my_data, attention_obj, arrest_data )
	end
		
	if attention_obj and not ( my_data.turning or my_data.advancing or data.unit:movement():chk_action_forbidden( "walk" ) ) then
...
function CopLogicArrest._upd_advance( data, my_data, attention_obj, arrest_data )
	...
	if arrest_data and my_data.should_arrest then
		--print( "[CopLogicArrest._upd_advance] dis", attention_obj.dis )
		if attention_obj.dis < 180 then -- we are close enough. first stop, then crouch & cuff the suspect
			if not action_taken then
				...
				if data.unit:anim_data().idle_full_blend then
					--print( "cuffing", attention_obj.unit )
	...
	elseif not my_data.in_position and data.t > my_data.next_action_delay_t then
		...
		elseif my_data.should_stand_close then
			...
			elseif close_pos then
				my_data.advance_path = { mvector3.copy( data.m_pos ), close_pos }
			else
...
-----------------------------------------------------------------------------
-- Gets rid of covers reserved in previous logics
function CopLogicArrest._upd_cover( data )
	local my_data = data.internal_data
	if not ( my_data.best_cover or my_data.nearest_cover ) then
		return
	end
	...
	if nearest_cover and mvector3.distance( nearest_cover[1][1], m_pos ) > cover_release_dis	then -- I dont want cover and have one
...
function CopLogicArrest._upd_enemy_detection( data )
	...
	local all_attention_objects = data.detected_attention_objects	-- Enemies whose whereabouts I am personally aware of
	local arrest_targets = my_data.arrest_targets
	
	CopLogicArrest._verify_arrest_targets( data, my_data ) -- remove arrest targets that get downed or disappear
	...
	if should_arrest then
		if not my_data.arrest_targets[ new_attention.u_key ] then
	...
	--print( "should_arrest", should_arrest, "should_stand_close", should_stand_close )
	if not ( my_data.should_arrest or my_data.should_stand_close ) then
		my_data.in_position = true
	end
	...
	--print( "[CopLogicArrest._upd_enemy_detection] new_reaction", new_reaction )
	if new_reaction ~= AIAttentionObject.REACT_ARREST then
		if ( not new_reaction or new_reaction < AIAttentionObject.REACT_SHOOT or not ( new_attention.verified and new_attention.dis < 1500 ) ) and ( my_data.in_position or not ( my_data.should_arrest or my_data.should_stand_close ) ) then
			if data.t > my_data.next_action_delay_t and not ( managers.groupai:state():is_police_called() or my_data.calling_the_police or my_data.turning or data.unit:sound():speaking( data.t ) ) then
				--print( "calling!!!", data.unit, new_reaction, inspect( new_attention ) )
			...
			if ( managers.groupai:state():is_police_called() or managers.groupai:state():chk_enemy_calling_in_area( managers.groupai:state():get_area_from_nav_seg_id( data.unit:movement():nav_tracker():nav_segment() ), data.key ) ) and not my_data.calling_the_police then
				...
				--print( "exiting!!!", data.unit, wanted_state, new_reaction, inspect( new_attention ) )
			...
			--print( "[CopLogicArrest._upd_enemy_detection] wanted_state", wanted_state )
...
-----------------------------------------------------------------------------
-- Returns if its a good idea to attempt to arrest or assault this unit
function CopLogicArrest._chk_reaction_to_attention_object( data, attention_data, stationary )
	...
	if not ( record and attention_data.is_person ) then
		return attention_data.settings.reaction
	end
	
	-- Only person criminals come further than this
	...
	elseif record.being_arrested then
		local my_data = data.internal_data
		if record.being_arrested[ data.key ] then -- It is me who is arresting this guy
	...
	if can_arrest and ( not record.assault_t or data.t - record.assault_t > att_unit:base():arrest_settings().aggression_timeout ) and data.t > record.arrest_timeout and not record.status then
		if attention_data.dis < 2000 and visible then -- non-hostile criminal, within arrest range
			return math.min( attention_data.settings.reaction, AIAttentionObject.REACT_ARREST )
		else -- non-hostile criminal, out of arrest range
			return math.min( attention_data.settings.reaction, AIAttentionObject.REACT_AIM )
		end
	end
...
function CopLogicArrest._verify_arrest_targets( data, my_data )
	...
	for u_key, arrest_data in pairs( arrest_targets ) do
		...
		if not record then
		elseif arrest_data.intro_pos and mvector3.distance_sq( arrest_data.attention_obj.m_pos, arrest_data.intro_pos ) > 170 * 170 then
			drop = true
			penalty = true
		elseif arrest_data.intro_t and record.assault_t and record.assault_t > arrest_data.intro_t + 0.6 then	-- Have we started warning this criminal but he kept being aggressive?
			drop = true
			penalty = true
		elseif record.status or data.t < record.arrest_timeout then	-- someone else marked this criminal as dangerous
			drop = true
		elseif not ( all_attention_objects[ u_key ] == arrest_data.attention_obj and arrest_data.attention_obj.identified ) then
			drop = true
			if arrest_data.intro_pos then	-- Has this unit been warned?
				penalty = true
			end
		end
...
function CopLogicArrest.damage_clbk( data, damage_info )
	...
	if my_data ~= data.internal_data then		-- CopLogicIdle.damage_clbk can change logic state
		return
	end
	
	local enemy = damage_info.attacker_unit
	if enemy then -- if damaged by an enemy, set timeouts to all arrest targets
...
function CopLogicArrest.on_criminal_neutralized( data, criminal_key )
	...
	if record.status == "dead" or record.status == "removed" then
		...
		if my_data.arrest_targets[ criminal_key ] and my_data.arrest_targets[ criminal_key ].intro_pos then
...
function CopLogicArrest._get_priority_attention( data, attention_objects, reaction_func )
	...
	local best_target
	local best_target_priority_slot
	local best_target_priority
	local best_target_reaction
	...
	for u_key, attention_data in pairs( attention_objects ) do
		...
		local crim_record = attention_data.criminal_record -- this exists only for criminal units
		if not attention_data.identified then
		elseif attention_data.pause_expire_t then
			if data.t > attention_data.pause_expire_t then
				attention_data.pause_expire_t = nil
			end
			--print( "ignoring paused reaction", data.unit, att_unit )
			--Application:draw_cylinder( data.m_pos, attention_data.m_pos, 30, 0, 0, 0 )
		elseif attention_data.stare_expire_t and data.t > attention_data.stare_expire_t then
			if attention_data.settings.pause then
				...
				--print( "[CopLogicArrest._get_priority_attention] pausing for", attention_data.pause_expire_t - data.t, "sec" )
			end
			--print( "ignoring expired reaction", data.unit, att_unit )
			--Application:draw_cylinder( data.m_pos, attention_data.m_pos, 30, 0, 0, 0 )
		else
			...
			elseif distance < 150 and reaction == AIAttentionObject.REACT_IDLE then
				reaction_too_mild = true -- too close
			end
			
			if not reaction_too_mild then
				local arrest_targets = data.internal_data.arrest_targets
				if reaction == AIAttentionObject.REACT_ARREST and reaction == best_target_reaction and arrest_targets[ best_target.u_key ] and arrest_targets[ best_target.u_key ].intro_t and not ( arrest_targets[ u_key ] and arrest_targets[ u_key ].intro_t ) then
...
function CopLogicArrest._get_att_obj_close_pos( data, my_data )
	...
	local destroy_att_nav_tracker
	local att_nav_tracker
	...
	if my_dis > optimal_dis * 0.8 and my_dis < optimal_dis * 1.2 and nav_manager:is_pos_free( { position = my_nav_tracker:field_position(), radius = 40, filter = data.pos_rsrv_id } ) then -- are we withing 20% error of optimal distance
		if destroy_att_nav_tracker then
			nav_manager:destroy_nav_tracker( att_nav_tracker )
		end
		--Application:draw_cylinder( my_nav_tracker:field_position(), att_obj_pos, 20, 1,1,1 )
		--Application:set_pause( true )
		return my_nav_tracker:lost() and my_nav_tracker:field_position() or false, false -- we are in good distance already. no adjustment, no pathing
	end
	...
	if not ray_res and nav_manager:is_pos_free( { position = optimal_pos, radius = 40, filter = data.pos_rsrv_id } ) then
		if destroy_att_nav_tracker then
			nav_manager:destroy_nav_tracker( att_nav_tracker )
		end
		--Application:draw_cylinder( optimal_pos, att_obj_pos, 20, 0,1,0 )
		--Application:set_pause( true )
	...
	if my_error > hit_dis_error and nav_manager:is_pos_free( { position = hit_pos, radius = 40, filter = data.pos_rsrv_id } ) then
		if destroy_att_nav_tracker then
			nav_manager:destroy_nav_tracker( att_nav_tracker )
		end
		--Application:draw_cylinder( hit_pos, att_obj_pos, 20, 0,0,1 )
		--Application:set_pause( true )
		return hit_pos, true
	end
	
	local pos_on_wall = CopLogicTravel._get_pos_on_wall( att_obj_pos, optimal_dis, nil, false )
	--Application:draw_cylinder( pos_on_wall, att_obj_pos, 20, 0,0,0 )
	--Application:set_pause( true )
...
function CopLogicArrest._chk_say_discovery( data, my_data, attention_obj )
	...
	if not my_data.discovery_said then
		if attention_obj.reaction == AIAttentionObject.REACT_SCARED then
			my_data.discovery_said = true
			data.unit:sound():say( "a07a", true ) -- say: "huh??"
		end
	end
end
