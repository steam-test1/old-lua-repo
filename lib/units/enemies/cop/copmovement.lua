...
local stance_ctl_pts = {0,0,1,1}	-- control points for bezier curve controlling stance parameter transition rate
...
CopMovement._stance.blend = { 0.8, 0.5, 0.3, 0.4 }	-- blend in for ntl, hos, cbt, wnd
...
function CopMovement:init( unit )
	...
	self._root_blend_ref = 0	-- Reference count of sources disabling root blending
	...
	self._active_actions = { false, false, false, false } -- 1.full_body, 2.lower_body, 3.upper_body, 4.superficial
	self._need_upd = true
	self._cool = true
...
function CopMovement:post_init()
	...
	--self._attention = nil	-- table containing info on what is attracting our attention
	--self._allow_fire = nil	-- are we allowed to shoot or just aim?
	
	self._stance = {
		code = 1,
		name = "ntl",
		values = { 1, 0, 0, 0 } -- ntl, hos, cbt, wnd
	}
...
function CopMovement:set_character_anim_variables()
	...
	if self._tweak_data.allowed_stances then
		if not self._tweak_data.allowed_stances.ntl then
			if self._tweak_data.allowed_stances.hos then
				self:_change_stance( 2 ) -- hos
			else
				self:_change_stance( 3 ) -- cbt
			end
		end
	end
	
	if self._tweak_data.allowed_poses then
		if not self._tweak_data.allowed_poses.stand then
...
function CopMovement:update( unit, t, dt )
	...
	--[[if self._m_host_stop_pos then
		Application:draw_cylinder( self._m_host_stop_pos, self._m_pos, 20, 0,0.2,1 )
		Application:draw_sphere( self._m_host_stop_pos, 30, 0,0,1 )
	end]]
end
...
function CopMovement:_upd_actions( t )
	...
	for i_action, action in ipairs( a_actions ) do -- 1.full_body, 2.lower_body, 3.upper_body
		if action then
			...
			if action.expired and action:expired() then
				...
				for _, action in ipairs( a_actions ) do -- 1.full_body, 2.lower_body, 3.upper_body
					if action then
						has_no_action = nil
						break
					end
	...
	if not ( a_actions[1] or a_actions[3] or self._queued_actions or self:chk_action_forbidden( "action" ) ) then
...
function CopMovement:upd_m_head_pos()
	...
	-- mvec3_lerp( self._m_com, self._m_pos, self._m_head_pos, 0.5 )
end
...
-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function CopMovement:m_pos()	-- unit root position
	return self._m_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function CopMovement:m_stand_pos()	-- head position if unit was standing
	return self._m_stand_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function CopMovement:m_com()	-- current centre of mass ( mid height ) of the unit
	return self._m_com
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function CopMovement:m_head_pos() -- current position of unit's head/look point
	return self._m_head_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function CopMovement:m_head_rot()	-- look rotation of unit
	return self._obj_head:rotation()
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!! (not always mutable. do not count on it being mutated)
function CopMovement:m_fwd()	-- look rotation of unit
	return self._action_common_data.fwd
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function CopMovement:m_rot()	-- unit root rotation
	return self._m_rot
end
...
function CopMovement:chk_action_forbidden( action_type )
	...
	for i_action, action in ipairs( self._active_actions ) do
		...
		--elseif action_type == "walk" and not self._ext_brain:is_important() and CopActionWalk.nr_moving_ppl > 9 then
			--return true
		end
	end
end
...
function CopMovement:action_request( action_desc )
	--print( "[CopMovement:action_request]", self._unit, inspect( action_desc ) )
	...
	local _interrupt_action = function( body_part )
		local old_action = active_actions[ body_part ]
		if old_action then
			--print( "interrupting", old_action:type(), inspect( old_action ) )
	...
	if body_part == 1 then	-- full_body
		_interrupt_action( 2 )
		_interrupt_action( 3 )
	elseif body_part == 2 or body_part == 3 then-- upper or lower body
		_interrupt_action( 1 )
	end
	...
	if success and not ( action.expired and action:expired() ) then
...
-----------------------------------------------------------------------------------
-- This is our general attention data. Can be world position or unit
function CopMovement:set_attention( attention )
	...
	for _, action in ipairs( self._active_actions ) do
		if action and action.on_attention then
			action:on_attention( attention, old_attention )	-- Notify all ongoing actions that our attention data has been altered
		end
	end
end
...
function CopMovement:synch_attention( attention )
	...
	for _, action in ipairs( self._active_actions ) do
		if action and action.on_attention then
			action:on_attention( attention )	-- Notify all ongoing actions that our attention data has been altered
		end
	end
end
...
-----------------------------------------------------------------------------------
-- Set a flag that we want to allow the unit to fire
function CopMovement:set_allow_fire_on_client( state, unit )
--	self:synch_allow_fire( state )
	if Network:is_server() then
		unit:network():send_to_unit( { state and "cop_allow_fire" or "cop_forbid_fire", self._unit } )
	end
--	self:enable_update()
end
...
function CopMovement:synch_allow_fire( state )
	for _, action in pairs( self._active_actions ) do
		if action and action.allow_fire_clbk then
			action:allow_fire_clbk( state )	-- Notify all ongoing actions that our attention data has been altered
		end
	end
...
function CopMovement:upd_ground_ray( from_pos )
	-- Get the ground Z using a spherecast
		-- Optimizied to two raycasts
		-- Could be furhter optimized by using cluster raycast
		
	local ground_z = self._nav_tracker:field_z()
	
	local safe_pos = temp_vec1
	mvec3_set( temp_vec1, from_pos or self._m_pos )
	mvec3_set_z( temp_vec1, ground_z + 100 ) -- 100
	
	local down_pos = temp_vec2
	mvec3_set( temp_vec2, safe_pos )
	mvec3_set_z( temp_vec2, ground_z - 140 ) -- 200
	...
	if old_pos.z == new_pos.z then
		...
		-- move_vec = move_vec * math.min(move_vec_len, 20)
	...
	local fake_ray = { position = ( new_pos ):with_z( ground_z ), ray = math.DOWN, unit = hit_ray and hit_ray.unit }
...
-----------------------------------------------------------------------------------
-- an act animation wants to spawn a prop. save the info and spawn when ready
function CopMovement:anim_clbk_wanted_item( unit, item_type, align_place, droppable )
	self._wanted_items = self._wanted_items or {}
	table.insert( self._wanted_items, { item_type, align_place, droppable } )
end
...
function CopMovement:anim_clbk_police_called( unit )
	if Network:is_server() and not managers.groupai:state():is_ecm_jammer_active( "call" ) then
		...
		if cop_type == "civ" then
			group_state:on_police_called( self:coolness_giveaway() ) -- "civ_911" )
		else
...
function CopMovement:drop_held_items()
	...
	for _, drop_item_unit in ipairs( self._droppable_gadgets ) do
		...
		if alive( drop_item_unit ) then
			...
			drop_item_unit:unlink()	--sequence here instead
			drop_item_unit:set_slot( 0 )
		else	-- dead equipped unit!
			for align_place, item_list in pairs( self._equipped_gadgets ) do
				if wanted_item_key then
					for i_item, item_unit in ipairs( item_list ) do
...
function CopMovement:sync_taser_fire()
	...
	if tase_action and tase_action:type() == "tase" and not tase_action:expired() then
		tase_action:fire_taser()	-- fire the taser on next update
	end
end
...
function CopMovement:save( save_data )
	--print( "[CopMovement:save]", self._unit )
	local my_save_data = {}
	...
	for _, action in ipairs( self._active_actions ) do
		if action and action.save then
			...
			if next( action_save_data ) then
				--print( "saving action", action:type(), inspect( action_save_data ) )
				my_save_data.actions = my_save_data.actions or {}
				table.insert( my_save_data.actions, action_save_data )
			--else
				--print( "no save data", action:type(), inspect( action ) )
			end
		end
	end
	...
	if self._attention then
		...
		else -- self._attention.unit...
			if self._attention.unit:id() == -1 then -- Network-detached unit
				my_save_data.attention = { pos = self._attention.unit:movement():m_com() }
			else -- save the id of the unit. we find him later
	...
	if self._equipped_gadgets then -- Save equipped items ( bags, cigarettes etc )
		local equipped_items = {}
		my_save_data.equipped_gadgets = equipped_items
		
		local _get_item_type_from_unit = function( item_unit )
		...
		local _is_item_droppable = function( item_unit )
			if not self._droppable_gadgets then
				return
			end
		...
		for align_place, item_list in pairs( self._equipped_gadgets ) do
			for i_item, item_unit in ipairs( item_list ) do
				if alive( item_unit ) then
					table.insert( equipped_items, { _get_item_type_from_unit( item_unit ), align_place, _is_item_droppable( item_unit ) } )
				end
			end
		end
	end
...
function CopMovement:load( load_data )
	...
	-- this is done to prevent the problem with loading deleting anim_data flags
	...
	if my_load_data.actions then
		for _, action_load_data in ipairs( my_load_data.actions ) do
			--print( "load action", inspect( action_load_data ) )
			self:action_request( action_load_data )
		end
	end
	
	if my_load_data.equipped_gadgets then -- Load equipped items ( bags, cigarettes etc )
...
function CopMovement:_cancel_latest_action( search_type, explicit )
	...
	-- Case 2. we have an ongoing walk action
	for body_part, action in ipairs( self._active_actions ) do
...
function CopMovement:_get_latest_walk_action()
	-- We need to figure out if the nav pos refers to an ongoing or a queued action. search queued actions first
	for i = #self._queued_actions, 1, -1 do
		if self._queued_actions[i].type == "walk" and self._queued_actions[i].persistent then --If it is not perssitent then this action is complete and cannot receive more data
...
function CopMovement:_get_latest_act_action()
	for i = #self._queued_actions, 1, -1 do
		if self._queued_actions[i].type == "act" and not self._queued_actions[i].host_expired then --If it is not perssitent then this action is complete and cannot receive more data
			return self._queued_actions[i], true
		end
	end
	
	-- Case 2. we have an ongoing action
	if self._active_actions[1] and self._active_actions[1]:type() == "act" then
		return self._active_actions[1]
	end
	
	--debug_pause_unit( self._unit, "[CopMovement:_get_latest_act_action] no queued or ongoing act action", self._unit, inspect( self._queued_actions ), inspect( self._active_actions ) )
end

-----------------------------------------------------------------------------------

function CopMovement:sync_action_walk_nav_point( pos )
	--print( "[CopMovement:sync_action_walk_nav_point]", self._unit, inspect( self._active_actions ), inspect( self._queued_actions ) )
...
function CopMovement:sync_action_walk_nav_link( pos, rot, anim_index, from_idle )
	--print( "[CopMovement:sync_action_walk_nav_link]", self._unit, inspect( self._active_actions ), inspect( self._queued_actions ) )
...
function CopMovement:sync_action_spooc_stop( pos, nav_index )
	...
	if is_queued then
		...
		--print( "nav_path b4", inspect( nav_path ) )
		while #nav_path > nav_index do
			--print( "popping", #nav_path, nav_path[ #nav_path ] )
		...
		if #nav_path < nav_index - 1 then
			...
			--print( "spooc_action.nr_expected_nav_points", spooc_action.nr_expected_nav_points )
		else
			table.insert( nav_path, pos )
			--print( "nav_path", inspect( nav_path ) )
...
-----------------------------------------------------------------------------------
-- Server Only. I receive this callback 1" after I save. I need to send my attention to ppl
-- Also used by SentryGunMovement
