...
function SecurityCamera:update( unit, t, dt )
	if managers.groupai:state():is_ecm_jammer_active( "camera" ) then
		self:_destroy_all_detected_attention_object_data() -- camera is dead atm
...
function SecurityCamera:apply_rotations( yaw, pitch )
	...
	local new_yaw_rot = Rotation( 180 + yaw, original_yaw_rot:pitch(), original_yaw_rot:roll() ) -- +180 is due to the root having -y for fwd
	...
	self._look_fwd = nil -- recalculate next frame
...
function SecurityCamera:_upd_acquire_new_attention_objects( t )
	...
	for u_key, attention_info in pairs( all_attention_objects ) do
		if u_key ~= my_key and not detected_obj[ u_key ] then
			--print( "[SecurityCamera:_upd_acquire_new_attention_objects] examining", inspect( attention_info ) )
			...
			if settings then
				...
				if self:_detection_angle_and_dis_chk( my_pos, my_fwd, attention_info.handler, settings, attention_pos ) then
					...
					if not vis_ray or vis_ray.unit:key() == u_key then
						...
						--print( "acquired", attention_info.unit )
			...
			if not attention_info.identified then
				...
				if noticable then
					...
					if angle == -1 then -- instant detection
						...
						local angle_mul_mod = 0.25 * math.min( angle / self._cone_angle, 1 ) -- angle only plays 25% role
...
function SecurityCamera:generate_cooldown( amount )
	...
	-- managers.money:perform_action( "security_camera" )
	-- managers.experience:perform_action( "security_camera" )
	-- managers.hint:show_hint( "destroyed_security_camera" )
	managers.statistics:camera_destroyed() -- Who shot?
...
function SecurityCamera:on_detected_attention_obj_modified( modified_u_key )
	--print( "[SecurityCamera:on_detected_attention_obj_modified]" )
...
function SecurityCamera:_upd_suspicion( t )
	local _exit_func = function( attention_data )
		--print( "uncovered!!!!!" )
	...
	for u_key, attention_data in pairs( self._detected_attention_objects ) do
		if attention_data.identified and attention_data.reaction == AIAttentionObject.REACT_SUSPICIOUS then
			--print( "[SecurityCamera:_upd_suspicion]", inspect( attention_data ) )
			if not attention_data.verified then
				-- no changes in suspicion
				if attention_data.last_suspicion_t then
					attention_data.last_suspicion_t = t
				end
			else -- suspicious and verified
				...
				local uncover_range = 0--attention_data.settings.uncover_range
				...
				if attention_data.settings.uncover_range and dis < math.min( max_range, uncover_range * susp_settings.range_mul ) then
					--print( "came too close", dis, math.min( max_range, uncover_range * susp_settings.range_mul ) )
				...
				elseif suspicion_range and dis < math.min( max_range, suspicion_range * susp_settings.range_mul ) then
					...
					if attention_data.last_suspicion_t then
						...
						--print( "attention_data.uncover_progress", attention_data.uncover_progress, "progress", progress, "dis", dis, "mul", mul, "dt", dt )
...
function SecurityCamera:_upd_sound( unit, t )
	...
	-- determine the maximum suspicion / notice progress ratio
...
function SecurityCamera:save( data )
	...
	if self._yaw then -- yaw and pitch have been altered. need to sync
		data.yaw = self._yaw
