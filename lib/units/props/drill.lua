...
function Drill:set_jammed( jammed )
	...
	if self._jammed then
		...
		--managers.dialog:queue_dialog( "gen_drill_jammed", {} )
	...
	elseif self._jammed_effect then
		...
		self:_start_drill_effect()
		--managers.dialog:queue_dialog( "gen_drill_fixed", {} )
...
function Drill:set_powered( powered )
	...
	if not self._powered then
		...
		--managers.dialog:queue_dialog( "gen_drill_jammed", {} )
	else --if self._jammed_effect then
		-- self:_kill_jammed_effect()
		self:_start_drill_effect()
		-- managers.dialog:queue_dialog( "gen_drill_fixed", {} )
...
function Drill:_register_sabotage_SO()
	...
	local so_descriptor = { 
		...
		search_dis_sq = 1000*1000, -- 10m radius
		verification_clbk = callback( self, self, "clbk_sabotage_SO_verification" ),
		usage_amount = 1,
		AI_group = "enemies",
		access = managers.navigation:convert_access_filter_to_number( { "gangster", "security", "security_patrol", "cop", "fbi", "swat", "murky", "sniper", "spooc", "tank", "taser" } ), -- all except shield
...
function Drill:_unregister_sabotage_SO()
	...
	elseif self._saboteur then
		...
		if alive( saboteur ) then
			saboteur:brain():set_objective( nil ) -- this will trigger the fail callback of the objective. thats what the above copy is for
		end
	end
end
...
function Drill:on_sabotage_SO_failed( saboteur )
	if self._saboteur then -- it is legal to not have a saboteur
		self._saboteur = nil
		self:_register_sabotage_SO()
	end
end
...
function Drill:on_sabotage_SO_started( saboteur )
	...
	self._saboteur = nil -- this is in order to let him continue his animation
...
function Drill:_register_investigate_SO()
	--print( "[Drill:_register_investigate_SO]", self._unit )
	--Application:set_pause( true )
	if self._investigate_SO_data then
		return
	end
	if not ( Network:is_server() and managers.navigation:is_data_ready() ) then
		--print( "not server or data not ready", Network:is_server(), managers.navigation:is_data_ready() )
		return
	end
	...
	local investigate_pos = Vector3() -- gets filled later
	...
	--Application:draw_cylinder( investigate_pos, self._nav_tracker:field_position(), 3, 0, 0, 1 )
	...
	--Application:draw_cylinder( investigate_pos, investigate_pos + math.UP * 50, 30, 0, 0, 1 )
	...
	--Application:draw_rotation( investigate_pos, investigate_rot )
...
function Drill:_unregister_investigate_SO()
	...
	elseif self._investigate_SO_data.receiver_unit then
		...
		if alive( receiver_unit ) then
			receiver_unit:brain():set_objective( nil ) -- this will trigger the fail callback of the objective. thats what the above copy is for
		end
	end
...
function Drill:clbk_investigate_SO_verification( candidate_unit )
	--print( "[Drill:clbk_investigate_SO_verification]", candidate_unit, candidate_unit:movement():cool() )
	if not ( self._investigate_SO_data and self._investigate_SO_data.SO_id ) then
	...
	if ray then
		...
		if my_dis > self._alert_radius * 0.5 then -- half radius when obstructed
			--Application:draw_line( candidate_listen_pos, sound_source_pos, 1, 0, 0 )
			return
		end
	end
...
function Drill:on_investigate_SO_administered( receiver_unit )
	--print( "[Drill:on_investigate_SO_administered]", receiver_unit, self._investigate_SO_data.receiver_unit )
...
function Drill:on_investigate_SO_completed( receiver_unit )
	--print( "[Drill:on_investigate_SO_completed]", receiver_unit, self._investigate_SO_data.receiver_unit )
...
function Drill:on_investigate_SO_failed( receiver_unit )
	--print( "[Drill:on_investigate_SO_failed]", thief, self._investigate_SO_data.receiver_unit )
