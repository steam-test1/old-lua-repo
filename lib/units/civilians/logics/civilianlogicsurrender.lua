...
function CivilianLogicSurrender.enter( data, new_logic_name, enter_params )
	...
	--print( "[CivilianLogicSurrender.enter]" )
	...
	my_data.scare_meter = 0 -- relative to max
	my_data.submission_meter = 0 -- relative to max
	my_data.last_upd_t = data.t
	my_data.nr_random_screams = 0 -- how many times has the character screamed as a result of gunfire
	...
	data.run_away_next_chk_t = nil -- this resets the delay before the character decides to run away
	...
	if data.objective then	-- we entered the logic due to an event
		if data.objective.aggressor_unit then
			if not data.objective.initial_act then
				CivilianLogicSurrender.on_intimidated( data, data.objective.amount, data.objective.aggressor_unit, true ) -- caution: logic change can take place here
			else
...
function CivilianLogicSurrender.exit( data, new_logic_name, enter_params )
	...
	--print( "[CivilianLogicSurrender.exit]" )
	local my_data = data.internal_data
	
	if data.unit:anim_data().tied and new_logic_name ~= "trade" and new_logic_name ~= "inactive" and not ( data.objective and data.objective.forced ) then
	...
	if new_logic_name ~= "inactive" then
		data.unit:base():set_slot( data.unit, 21 )	-- alive and non-handcuffed civilians
	end
...
function CivilianLogicSurrender.queued_update( rubbish, data )
	...
	if my_data.submission_meter == 0 and not ( data.unit:anim_data().tied or ( data.unit:anim_data().react_enter and not data.unit:anim_data().idle ) ) then
		if data.unit:anim_data().drop then
			local new_action = { type = "act", variant = "stand", body_part = 1 }
			data.unit:brain():action_request( new_action )
		end
		--print("Go to flee")
		my_data.surrender_clbk_registered = false	-- it's not queued atm since it has been called and thus auto-popped from the queue
		data.unit:brain():set_objective( { type = "free", is_default = true } )
		return
	else
		CivilianLogicFlee._chk_add_delayed_rescue_SO( data, my_data )
		
		managers.groupai:state():add_to_surrendered( data.unit, callback( CivilianLogicSurrender, CivilianLogicSurrender, "queued_update", data ) ) -- this task gets removed by default. we need to add it every time
	end
...
function CivilianLogicSurrender._do_initial_act( data, amount, aggressor_unit, initial_act )
	...
	local action_data = { type = "act", body_part = 1, variant = initial_act, clamp_to_graph = true }
...
function CivilianLogicSurrender.on_intimidated( data, amount, aggressor_unit, skip_delay )
	...
	if not ( my_data.delayed_intimidate_id and my_data.delayed_clbks and my_data.delayed_clbks[ my_data.delayed_intimidate_id ] ) then
		...
		if skip_delay then
			...
			CopLogicBase.add_delayed_clbk( my_data, my_data.delayed_intimidate_id, callback( CivilianLogicSurrender, CivilianLogicSurrender, "_delayed_intimidate_clbk", { data, amount, aggressor_unit } ), TimerManager:game():time() + delay)
		end
	end
end
...
function CivilianLogicSurrender._delayed_intimidate_clbk( ignore_this, params )
	...
	if anim_data.drop or ( anim_data.react_enter and not anim_data.idle ) then	-- do nothing
	elseif anim_data.react or anim_data.panic or anim_data.halt then
		local action_data = { type = "act", body_part = 1, clamp_to_graph = true, variant = anim_data.move and "halt" or "drop" }
		local action_res = data.unit:brain():action_request( action_data )
...
function CivilianLogicSurrender.on_alert( data, alert_data )
	...
	if anim_data.halt or anim_data.react then -- they are more scared when not taking cover
		scare_modifier = scare_modifier * 4
	end
	my_data.scare_meter = math.min( my_data.scare_max, my_data.scare_meter + scare_modifier )
	
	if my_data.scare_meter == my_data.scare_max and data.t - my_data.state_enter_t > 5 then -- 5 sec delay before he is allowed to panic
		data.unit:sound():say( "a01x_any", true )
		if not my_data.inside_intimidate_aura then
			data.unit:brain():set_objective( { type = "free", is_default = true, alert_data = clone( alert_data ) } )
		end
	elseif not data.unit:sound():speaking( TimerManager:game():time() ) then
		...
		local max_scare_dis_sq = 2000 * 2000
		if alert_dis_sq < max_scare_dis_sq then
			rand = math.lerp( rand, rand * 2, math.min( alert_dis_sq ) / 4000000 ) -- min chance at 20m
			...
			if data.char_tweak.female then -- females scream more
...
function CivilianLogicSurrender._update_enemy_detection( data, my_data )
	...
	local closest_dis, closest_enemy
	
	my_data.inside_intimidate_aura = nil -- re-evaluate
		...
		if not u_data.is_deployable and chk_vis_func( my_tracker, u_data.tracker ) then
			...
			if u_data.unit:base().is_local_player then
				if managers.player:has_category_upgrade( "player", "intimidate_aura" ) then
					if dis < managers.player:upgrade_value( "player", "intimidate_aura", 0 ) then
						inside_aura = true
					end
				end
			elseif u_data.unit:base().is_husk_player and u_data.unit:base():upgrade_value( "player", "intimidate_aura" ) then -- husk
				if dis < u_data.unit:base():upgrade_value( "player", "intimidate_aura" ) then
					inside_aura = true
				end
			end
			
			if inside_aura or dis < 700 then
	...
	if not attention_unit then	
		...
		if mvector3.distance( my_pos, attention_unit:movement():m_head_pos() ) > 900 or not data.unit:anim_data().ik_type then
			CopLogicBase._reset_attention( data )
		end
	end
	--[[
	local ik_type = data.unit:anim_data().ik_type
	if ik_type then
		if ik_type ~= my_data.active_ik_type then
			my_data.active_ik_type = ik_type
			
			if my_data.attention_unit then
				CopLogicBase._set_attention_on_unit( data, my_data.attention_unit )
			end
		end
	elseif my_data.active_ik_type then
		my_data.active_ik_type = nil
		CopLogicBase._reset_attention( data )
	end
	]]
...
function CivilianLogicSurrender.is_available_for_assignment( data, objective )
	if objective and objective.forced then
		return true -- he can perform an act objective to stand in place
	end
