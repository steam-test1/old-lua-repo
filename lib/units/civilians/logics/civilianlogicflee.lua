...
function CivilianLogicFlee.enter( data, new_logic_name, enter_params )
	...
	--print( "[CivilianLogicFlee.enter]", data.unit )
	...
	if data.objective then
		if data.objective.alert_data then	-- someone is creating panic near me
			...
			if data.unit:anim_data().react_enter and not data.unit:anim_data().idle then
				...
				CopLogicBase.add_delayed_clbk( my_data, my_data.delayed_post_react_alert_id, callback( CivilianLogicFlee, CivilianLogicFlee, "post_react_alert_clbk", { data = data, alert_data = clone( data.objective.alert_data ) } ), TimerManager:game():time() + math.lerp( 4, 8, math.random() ) )
			end
		elseif data.objective.dmg_info then	-- someone shot me
	...
	if data.objective and data.objective.was_rescued then	-- I've been rescued
	...
	if not managers.groupai:state():enemy_weapons_hot() then
		...
		managers.groupai:state():add_listener( my_data.enemy_weapons_hot_listen_id, { "enemy_weapons_hot" }, callback( CivilianLogicIdle, CivilianLogicIdle, "clbk_enemy_weapons_hot", data ) )
	end
	
	CivilianLogicFlee.schedule_run_away_clbk( data )
	
	-- if we haven't paniced due to an alert, just panic
	...
	my_data.next_action_t = 0 -- delay timer between walk actions
end

-----------------------------------------------------------------------------

function CivilianLogicFlee.exit( data, new_logic_name, enter_params )
	--print( "[CivilianLogicFlee.exit]", data.key )
		...
		if not my_data.advancing and data.t > my_data.next_action_t then
			...
			if cur_index == total_nav_points then		-- We have reached the final nav point. The area is investigated.
				...
				if cur_index == total_nav_points - 1 then
					...
					--local cover = managers.navigation:find_cover_from_threat( next_area.nav_segs, 10000, data.m_pos, data.m_pos )
				...
				if to_cover then
					...
					local reservation = managers.navigation:reserve_pos( nil, nil, to_pos, nil, 30, data.pos_rsrv_id ) -- no duration, no step callback
...
function CivilianLogicFlee._upd_detection( data )
	...
	if my_data.advancing or not ( data.unit:anim_data().react or data.unit:anim_data().react_enter ) then
	...
	if data.pathing_results then
		...
		if path then
			if path ~= "failed" then
				...
				--print( "CivilianLogicFlee:_update_pathing() we have an flee path" )
			else
				--print( "[CivilianLogicFlee:_update_pathing] flee_path failed!" )
			end
...
function CivilianLogicFlee.action_complete_clbk( data, action )
	...
	if action:type() == "walk" then
		...
		else -- action was interrupted
			if my_data.rsrv_pos.move_dest then
				if not my_data.rsrv_pos.stand then
					my_data.rsrv_pos.stand = managers.navigation:add_pos_reservation( { position = mvector3.copy( data.m_pos ), radius = 45, filter = data.pos_rsrv_id } )
				end
...
function CivilianLogicFlee.on_alert( data, alert_data )
	--print( "[CivilianLogicFlee.on_alert]", inspect( data.internal_data ), inspect( alert_data ) )
	
	local my_data = data.internal_data
	
	if my_data.coarse_path then	-- I have been rescued. I stop at intimidation but not at shots
		return
	end
	...
	if anim_data.react_enter and not anim_data.idle then
		--print( "react_enter..." )
		if not my_data.delayed_post_react_alert_id then -- he cannot proceed to the next anim yet. he is in transition. repeat the alert a bit later
			my_data.delayed_post_react_alert_id = "postreact_alert"..tostring( data.key )
			CopLogicBase.add_delayed_clbk( my_data, my_data.delayed_post_react_alert_id, callback( CivilianLogicFlee, CivilianLogicFlee, "post_react_alert_clbk", { data = data, alert_data = clone( alert_data ) } ), TimerManager:game():time() + 1 )
		end
		return
	elseif anim_data.peaceful or data.unit:movement():stance_name() == "ntl" then	-- pre-combat
		local action_data = { type = "act", body_part = 1, variant = "panic", clamp_to_graph = true }
		data.unit:brain():action_request( action_data )
...
function CivilianLogicFlee._run_away_from_alert( data, alert_data )
	...
	local avoid_pos
	if alert_data[1] == "bullet" then	-- cylindrical alert
		--Application:draw_cylinder( alert_data[2], alert_data[3], alert_data[4], 1, 0, 0 )
	...
	else -- spherical alert
		--Application:draw_sphere( alert_data[2], alert_data[3], 1, 0, 0 )
		avoid_pos = alert_data[2] or alert_data[5] and alert_data[5]:position() or math.UP:random_orthogonal() * 100 + data.m_pos
	end
	--Application:draw_cylinder( avoid_pos, data.unit:movement():m_com(), 10, 1, 0, 1 )
	--Application:draw_sphere( avoid_pos, 50, 1, 0, 1 )
...
function CivilianLogicFlee.post_react_alert_clbk( shait, params )
	...
	if anim_data.react_enter then
		...
		return -- still playing react_enter
	end
...
function CivilianLogicFlee.on_intimidated( data, amount, aggressor_unit )
	...
	if not my_data.delayed_intimidate_id then
		...
		CopLogicBase.add_delayed_clbk( my_data, my_data.delayed_intimidate_id, callback( CivilianLogicFlee, CivilianLogicFlee, "_delayed_intimidate_clbk", { data, amount, aggressor_unit } ), TimerManager:game():time() + delay)
	end
end

-----------------------------------------------------------------------------

-- params = { data, amount, aggressor_unit }
...
-----------------------------------------------------------------------------
-- This function is also called from CivilianLogicSurrender
function CivilianLogicFlee._add_delayed_rescue_SO( data, my_data )
	--print( "[CivilianLogicFlee._add_delayed_rescue_SO]", data.unit )
...
function CivilianLogicFlee.register_rescue_SO( ignore_this, data )
	--print( "[CivilianLogicFlee.register_rescue_SO]", data.unit )
	local my_data = data.internal_data
	...
	-- trace?
	local ray_params = { tracker_from = data.unit:movement():nav_tracker(), pos_to = test_pos, allow_entry = false, trace = true }
...
function CivilianLogicFlee._unregister_rescue_SO( data, my_data )
	--print( "[CivilianLogicFlee._unregister_rescue_SO]", data.unit, my_data.rescuer, my_data.rescue_SO_id, my_data.delayed_rescue_SO_id )
	if my_data.rescuer then
		local rescuer = my_data.rescuer
		my_data.rescuer = nil
...
function CivilianLogicFlee.on_rescue_SO_administered( ignore_this, data, receiver_unit )
	--print( "[CivilianLogicFlee.on_rescue_SO_administered]", inspect( data ), data.unit, receiver_unit )
...
function CivilianLogicFlee.rescue_SO_verification( ignore_this, areas, unit )
	--print( "[CivilianLogicFlee.rescue_SO_verification] areas", inspect( areas ), "unit", unit )
...
function CivilianLogicFlee.on_rescue_SO_failed( ignore_this, data )
	--print( "[CivilianLogicFlee.on_rescue_SO_failed]", inspect( data ), inspect( data.internal_data ), data.unit, data.internal_data.rescuer )
	local my_data = data.internal_data
	if my_data.rescuer then
		my_data.rescue_active = nil
...
--	Application:stack_dump()
	...
	if data.internal_data.rescuer and good_pig:key() == data.internal_data.rescuer:key() then
		...
		if data.unit:anim_data().tied or data.unit:anim_data().drop then
			...
			data.unit:brain():set_objective( { type = "free", is_default = true, was_rescued = true } )
		elseif not CivilianLogicFlee._get_coarse_flee_path( data ) then
			return
		end
--	else
--		debug_pause( "[CivilianLogicFlee.on_rescue_SO_completed] rescuer mismatch", inspect( data ), data.unit, good_pig, data.internal_data.rescuer )
	end
...
function CivilianLogicFlee.on_rescue_allowed_state( data, state )
	--[[local my_data = data.internal_data
	if state then
		if not ( my_data.advancing or my_data.delayed_rescue_SO_id ) then
...
-----------------------------------------------------------------------------
-- Do I want to be rescued? if yes then return the SO ID
...
function CivilianLogicFlee._chk_add_delayed_rescue_SO( data, my_data )
	if not ( my_data.exiting or data.unit:anim_data().move ) and managers.groupai:state():rescue_state() then
...
function CivilianLogicFlee.clbk_chk_run_away( ignore_this, data )
	...
	if not my_data.coarse_path then	-- I have been rescued. I stop at intimidation but not at shots
		if CivilianLogicFlee._get_coarse_flee_path( data ) then
			--debug_pause_unit( data.unit, "[CivilianLogicFlee.clbk_chk_run_away] flee coarse path!", inspect( my_data.coarse_path ) )
			data.unit:brain():set_update_enabled_state( true )
		end
	end
...
function CivilianLogicFlee.clbk_chk_call_the_police( ignore_this, data )
	...
	if not ( already_calling or my_data.calling_the_police and data.unit:movement():chk_action_forbidden( "walk" ) ) then
		local action = {
			type = "act",
			body_part = 1,
