...
--[[
function TripMineBase:set_sensor_upgrade( sensor_upgrade )
	self._sensor_upgrade = sensor_upgrade
	
	if managers.network:session() then
		managers.network:session():send_to_peers_synched( "sync_trip_mine_set_sensor_upgrade", self._unit, self._sensor_upgrade )
	end
end
]]
...
function TripMineBase:init( unit )
	...
	-- self._g_trip_mine = self._unit:get_object( Idstring( "g_trip_mine" ) )
	...
	if self._use_draw_laser then
		...
		self._laser_brush = Draw:brush( self._laser_color ) -- Color( 0.05, 0, 1, 0 ) )
...
function TripMineBase:sync_trip_mine_set_armed( armed, length )
	-- local brush = Draw:brush( Color.red:with_alpha( 0.5 ) )
	-- brush:set_persistance( 2 ) 
	-- brush:sphere( self._unit:position(), 30 )
	self._length = length
...
--[[
function TripMineBase:sync_trip_mine_set_sensor_upgrade( sensor_upgrade )
	self._sensor_upgrade = sensor_upgrade
end
]]
...
function TripMineBase:_update_draw_laser()
	if self._use_draw_laser and self._first_armed then
		if self._armed or self._sensor_upgrade then
...
function TripMineBase:_sensor( t )
	...
	-- local brush = Draw:brush( Color.green:with_alpha( 0.25 ) )
	-- brush:cylinder( self._ray_from_pos, self._ray_to_pos, 1 )
...
-- If the body where the trip mine is placed is disabled, moved or rotated - boom.
...
-- Sequencemanager callback on bullet damage. Only run it on the client that owns the mine.
...
function TripMineBase:_explode( col_ray )
	...
	if alive( player ) then
		player:character_damage():damage_explosion( { position = self._position, range = damage_size, damage = 6 } )
	else
		player = nil
	end
	...
	local bodies = World:find_bodies( "intersect", "cylinder", self._ray_from_pos, self._ray_to_pos, damage_size, slotmask )
		...
		-- local brush = Draw:brush( Color.green:with_alpha( 0.25 ) )
		-- brush:set_persistance( 2 )
		-- brush:cylinder( self._ray_from_pos, self._ray_to_pos, damage_size )
	...
	for _, hit_body in ipairs( bodies ) do
		...
		if character and not characters_hit[ hit_body:unit():key() ] then
			...
			if ray_hit then
					-- brush:cylinder( ray_from, hit_body:position(), 3 )
	...
	if Network:is_server() then
		local alert_event = { "aggression", self._position, tweak_data.weapon.trip_mines.alert_radius, self._alert_filter, self._unit }
