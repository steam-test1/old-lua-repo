...
function RaycastWeaponBase:init( unit )
	...
	self._muzzle_effect_table = { effect = self._muzzle_effect, parent = self._obj_fire, force_synch = true }
	...
	self._shot_fired_stats_table = { hit = false, weapon_unit = self._unit }
end
...
function RaycastWeaponBase:_create_use_setups()
	...
	npc_setup.unequip = {} -- discard the unit when unequipping
end
...
function RaycastWeaponBase:setup( setup_data )
	...
	if( stats ) then
		...
		--[[
		if( not self._zoom ) then
			self._zoom = 75
			debug_pause( "[RaycastWeaponBase] Weapon \"" .. tostring( self._name_id ) .. "\" is missing stat \"zoom\"!" )
		end
		if( not self._alert_size ) then
			self._alert_size = 5000
			debug_pause( "[RaycastWeaponBase] Weapon \"" .. tostring( self._name_id ) .. "\" is missing stat \"alert_size\"!" )
		end
		if( not self._suppression ) then
			self._suppression = 1
			debug_pause( "[RaycastWeaponBase] Weapon \"" .. tostring( self._name_id ) .. "\" is missing stat \"suppression\"!" ) 
		end
		]]
	else
...
-- Returns the weapon fire mode (currently "auto" or "single")
...
function RaycastWeaponBase:_check_ammo_total( unit )
	if self:get_ammo_total() <= 0 then
		if unit:base().is_local_player then
			if unit:inventory():all_out_of_ammo() then
...
function RaycastWeaponBase:_fire_raycast( user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul )
	...
	-- mvector3.spread(Vector3 v, spread )
	-- mvector3.set( mvec_to, spread_direction )
	...
	if self._autoaim then
		...
		if col_ray and col_ray.unit:in_slot( managers.slot:get_mask( "enemies" ) ) then
			...
			if autoaim then	--	We missed an autoaim unit
		...
		-- print( "self._autohit_current", self._autohit_current )
	elseif col_ray then
		hit_unit = InstantBulletBase:on_collision( col_ray, self._unit, user_unit, damage )
	end
	...
	if col_ray and col_ray.distance > 600 or not col_ray then
		if alive( self._obj_fire ) then
			...
			-- local trail = World:effect_manager():spawn( { effect = self.TRAIL_EFFECT, position = self._obj_fire:position(), normal = spread_direction } )
...
function RaycastWeaponBase:check_autoaim( from_pos, direction, max_dist, use_aim_assist )
	...
	--[[for i = 1, far_dis / 100 do
		...
		local suppression_angle = math_lerp( suppression_near_angle, suppression_far_angle, ( proj_dis / far_dis )^0.25 )
		local suppression_radius = proj_dis * math.tan( suppression_angle * 0.5 )
		Application:draw_cone( from_pos, from_pos + direction * proj_dis, suppression_radius * 2, 0,0,0.7)
		
		local autohit_angle = math_lerp( autohit_near_angle, autohit_far_angle, ( proj_dis / far_dis )^0.25 )
		local autohit_radius = proj_dis * math.tan( autohit_angle * 0.5 )
		Application:draw_cone( from_pos, from_pos + direction * proj_dis, autohit_radius * 2, 0,0.7,0)
	end]]
	...
	for u_key, enemy_data in pairs( enemies ) do
		...
		if enemy:base():lod_stage() == 1 then
			...
			local tar_aim_dot = mvec3_dot( direction, tar_vec ) -- unnormalized vec dot. equal to length of projected tar_vec
			
			if tar_aim_dot > 0 and ( not max_dist or tar_aim_dot < max_dist ) then
				local tar_vec_len = math_clamp( mvec3_norm( tar_vec ), 1, far_dis )
				local error_dot = mvec3_dot( direction, tar_vec ) -- dot of normalized vectors
				...
				if suppression_min_angle > error_angle then
				...
				if autohit_min_angle > error_angle then
					local percent_error = error_angle / autohit_min_angle
					-- Application:draw_cylinder( enemy_data.m_pos, enemy_data.m_pos + math.UP * 500, 20, 1,1,0)
					-- Application:draw_sphere( com, 30, 1,1,0)
					if not closest_error or closest_error > percent_error then
						tar_vec_len = tar_vec_len + 100
						mvec3_mul( tar_vec, tar_vec_len ) -- add 1m to the ray
	...
	--[[if closest_ray then
		Application:draw_sphere( closest_ray.position, 35, 1,1,1)
	end]]
...
function RaycastWeaponBase:_check_alert( rays, fire_pos, direction, user_unit )
	...
	-- local from_pos = direction * -alert_rad
	...
	if #rays > 0 then
		for _, ray in ipairs( rays ) do
			...
			for i = #all_alerts, 1, -1 do
				if mvec3_dis( all_alerts[i][1], event_pos ) < tolerance and mvec3_dis( all_alerts[i][2], from_pos ) < tolerance then -- this alert is fresh and very close to the new one. skip the new alert
					event_pos = nil
					break
				end
			end
			if event_pos then
				...
				table.insert( all_alerts, { event_pos, from_pos, exp_t } ) -- { alert epicenter, alert_expiry_time }
				local new_alert = { "bullet", event_pos, alert_rad, self._setup.alert_filter, user_unit, from_pos }
	...
	for i = #fire_alerts, 1, -1 do
		if fire_alerts[i][2] < t then	-- This alert is too old. remove
			table.remove( fire_alerts, i )
		elseif mvec3_dis( fire_alerts[i][1], fire_pos ) < tolerance then -- this alert is fresh and very close to the new one. skip the new alert
			cached = true
			break
		end
	end
	...
	if not cached then
		table.insert( fire_alerts, { fire_pos, exp_t } ) -- { alert epicenter, alert_expiry_time }
		local new_alert = { "bullet", fire_pos, self._alert_size, self._setup.alert_filter, user_unit, from_pos }
		group_ai:propagate_alert( new_alert )
	end
end
...
function RaycastWeaponBase:damage_player( col_ray, from_pos, direction )
	...
	if cos_f <= 0.1 then	-- we are shooting in the opposite direction
		return
	end
	
	local b = head_dis / cos_f
	if not col_ray or col_ray.distance > b then -- we shot long enough to hit the player. did we shoot close enough?
		...
		if proj_len < 30 then
			if World:raycast( "ray", from_pos, head_pos, "slot_mask", self._bullet_slotmask, "ignore_unit", self._setup.ignore_units, "report" ) then -- do not hit if player camera position is not seen
				return nil, ray_data
			else
...
function RaycastWeaponBase:force_hit( from_pos, direction, user_unit, impact_pos, impact_normal, hit_unit, hit_body )
	...
	local col_ray = { position = impact_pos, ray = direction, normal = impact_normal, unit = hit_unit, body = hit_body or hit_unit:body( 0 ) }
...
function RaycastWeaponBase:replenish()
	-- Replenish happens on level up, which is after upgrades has been revieved. Thats why these values are suitable to update here
	-- local ammo_max_multiplier = managers.player:equipped_upgrade_value( "extra_start_out_ammo", "player", "extra_ammo_multiplier" )
	-- ammo_max_multiplier = ((ammo_max_multiplier == 0 and 1) or ammo_max_multiplier)
	...
	self._ammo_max_per_clip = self:get_ammo_max_per_clip() -- tweak_data.weapon[ self._name_id ].CLIP_AMMO_MAX + managers.player:upgrade_value( self._name_id, "clip_ammo_increase" )
	-- self._ammo_max_per_clip = self._ammo_max_per_clip + managers.player:upgrade_value( "weapon", "clip_ammo_increase", 0 )
...
-- Returns damage with current temporary effects
...
function RaycastWeaponBase:recoil_multiplier()
	...
	-- print( "team suppression_recoil_multiplier", managers.player:team_upgrade_value( category, "suppression_recoil_multiplier", 1 ) )
	-- print( "team recoil_multiplier", managers.player:team_upgrade_value( category, "recoil_multiplier", 1 ) )
...
-- Called during the reload expire timer
...
-- Returns if reload can be interupted, used for shotguns for example
...
-- Called when reload timer has expired
...
-- Only possible to reload if ammo total is greater then ammo remaining in clip
...
-- Template for flashlights state changed
function RaycastWeaponBase:flashlight_state_changed()
end

-- Template for setting enable flag for flashlights equip/unequip on/off
...
function InstantBulletBase:on_collision( col_ray, weapon_unit, user_unit, damage, blank )
	...
	if hit_unit:damage() then
		if col_ray.body:extension() and col_ray.body:extension().damage then
			local local_damage = not blank or hit_unit:id() == -1 -- normal fire or not synced unit 
			local sync_damage = not blank and hit_unit:id() ~= -1 -- normal fire and synced unit
			...
			if sync_damage then -- hit_unit:id() ~= -1 then
