...
function NPCRaycastWeaponBase:init( unit )
	...
	if false and self._multivoice then -- Simon requested that only variation 1a is played. Will be re-applied later. George
...
function NPCRaycastWeaponBase:setup( setup_data )
	...
	self._setup.user_sound_variant = 1 -- Simon requested that only variation 1a is played. Will be re-applied later. George
end
...
function NPCRaycastWeaponBase:_get_spread( user_unit ) -- spread has already been applied by npc character
end
	...
	if col_ray then
		if col_ray.unit:in_slot( self._character_slotmask ) then
			...
			if shoot_player and self._hit_player then
				hit, ray = self:damage_player( col_ray, from_pos, direction ) -- Check if ray hits player
				if hit then -- if the player was not suppressed then we do not deal damage
