...
function SyncUnitData:save(data)
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._unit:get_objects_by_type(Idstring("light")))
		do
			do break end
			local l = {
				name = light:name(),
	...
	(for control) = self._unit:get_objects_by_type(Idstring("light")) and {
		name = light:name(),
		enabled = light:enable(),
		far_range = light:far_range(),
		near_range = light:near_range(),
