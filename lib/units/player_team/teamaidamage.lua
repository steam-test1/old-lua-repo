...
TeamAIDamage._all_event_types = { "bleedout", "death", "hurt", "light_hurt", "heavy_hurt", "fatal", "none" }
TeamAIDamage._RESULT_INDEX_TABLE = { hurt = 1, bleedout = 2, death = 3, light_hurt = 4, heavy_hurt = 5, fatal = 6 }
...
function TeamAIDamage:init( unit )
	...
	self._next_allowed_dmg_t = -100 -- Next time we are allowed to take damage
...
function TeamAIDamage:update( unit, t, dt )
	...
	elseif self._arrested_timer then
		if self._arrested_paused_counter == 0 then
			...
			if self._arrested_timer <= 0 then -- The result of arrested timer running out is stolen from bleed out. Should have something of its own.
...
function TeamAIDamage:force_bleedout( )
	local attack_data = { pos = Vector3(), col_ray = { position= Vector3() }, damage = 100000 }
...
function TeamAIDamage:damage_tase()	-- also called from HuskTeamAIDamage:sync_damage_tase()
	if self:_cannot_take_damage() then
		return
	end
...
function TeamAIDamage:_apply_damage( attack_data, result )
	-- round damage to a % of max health
	local damage = attack_data.damage
	damage = math.clamp( damage, self._HEALTH_TOTAL_PERCENT, self._HEALTH_TOTAL ) -- Cannot give less than 1% damage
...
function TeamAIDamage:pause_bleed_out()
	...
	if self._bleed_out or self._fatal then
		if self._bleed_out_paused_count == 1 then
			self._to_dead_remaining_t = self._to_dead_t - TimerManager:game():time()
			if self._to_dead_remaining_t < 0 then -- the timer has a small delay. doublecheck
				return
			end
			if Network:is_server() then
...
function TeamAIDamage:unpause_bleed_out()
	self._bleed_out_paused_count = self._bleed_out_paused_count - 1
	if self._bleed_out or self._fatal then
		if self._bleed_out_paused_count == 0 then
...
function TeamAIDamage:_on_hurt()
	if self._to_incapacitated_clbk_id then -- being tased. cannot regenerate
		return
	end
	
	local regen_time = self._char_dmg_tweak.REGENERATE_TIME_AWAY
	local dis_limit = 2500 * 2500
	for _, crim in pairs( managers.groupai:state():all_player_criminals() ) do
		if mvector3.distance_sq( self._unit:movement():m_pos(), crim.unit:movement():m_pos() ) < 2500 * 2500 then
...
function TeamAIDamage:_clamp_health_percentage( health_abs )
	health_abs = math.clamp( health_abs, self._HEALTH_TOTAL_PERCENT, self._HEALTH_TOTAL ) -- Cannot give less than 1% damage
...
------------------------------------------------------------------------------------------
-- Unit received damage on a client machine
function TeamAIDamage:sync_damage_bullet( attacker_unit, damage, i_body, hit_offset_height )
	...
	local attack_data = { variant = "bullet"
...
function TeamAIDamage:sync_damage_melee( attacker_unit, damage, damage_effect_percent, i_body, hit_offset_height )
	...
	if attacker_unit then
		...
		attack_dir = self._unit:rotation():y() -- the attacker unit was destroyed
	...
	local attack_data = { variant = "melee"
												,attacker_unit = attacker_unit
												,damage = damage
												,attack_dir = attack_dir
												,pos = hit_pos
...
function TeamAIDamage:clbk_exit_to_incapacitated()
	...
	self._to_incapacitated_clbk_id = nil
	local dmg_info = { variant = "bleeding", result = { type = "fatal" } }
...
function TeamAIDamage:clbk_exit_to_dead()
	...
	local dmg_info = { variant = "bleeding", result = { type = "death" } }
	self:_call_listeners( dmg_info )
	self:_unregister_unit()
end
