...
function TeamAILogicTravel.enter( data, new_logic_name, enter_params )
	...
	--debug_pause_unit( data.unit, "[TeamAILogicTravel.enter]", data.unit )
	...
	-- This is preliminary. I will have to make it possible that AI can switch AI state while performing actions. george
...
function TeamAILogicTravel.exit( data, new_logic_name, enter_params )
	...
	--debug_pause_unit( data.unit, "[TeamAILogicTravel.exit]", data.unit )
		...
		if my_data.processing_advance_path then
			...
			if path then
				...
				if path ~= "failed" then
					...
					--print( "[TeamAILogicTravel:_upd_pathing] we have an advance path" )
				else
					print( "[TeamAILogicTravel:_upd_pathing] advance_path failed!", data.unit, inspect( my_data.coarse_path ), my_data.coarse_path_index )
					--debug_pause_unit( data.unit, "[TeamAILogicTravel:_upd_pathing] advance_path failed!", inspect( my_data.coarse_path ), my_data.coarse_path_index, inspect( my_data._search_nav_segs ), my_data._path_tag )
					--Application:draw_cylinder( my_data._path_trail[1], my_data._path_trail[2], 23, 0, 1, 0 )
		...
		if my_data.processing_coarse_path then
			...
			if path then
				...
				if path ~= "failed" then
					...
					--print( "[TeamAILogicTravel:_upd_pathing] we have a coarse path", inspect( path ) )
				else
					print( "[TeamAILogicTravel:_upd_pathing] coarse_path failed!" )
					data.path_fail_t = data.t
					--debug_pause_unit( data.unit, "[TeamAILogicTravel:_upd_pathing] coarse_path failed!", data.unit )
					managers.groupai:state():on_criminal_objective_failed( data.unit, data.objective )
					return
				end
			end
...
function TeamAILogicTravel._determine_destination_occupation( data, objective )
	...
	elseif objective.type == "revive" then
		...
		-- find a position on the side of the downed person
		...
		if ray_res then
			...
			if opposite_ray_res then
				if mvector3.distance( ray_params.trace[1], revive_u_pos ) > mvector3.distance( revive_pos, revive_u_pos ) then
					...
					--Application:draw_sphere( revive_pos, 15, 0,0,0.5 )
				else
					revive_pos = old_trace
					--Application:draw_sphere( revive_pos, 15, 0,0.5,0 )
				end
			else
				revive_pos = ray_params.trace[1]
				--Application:draw_sphere( revive_pos, 15, 0,0,1 )
			end
		else
			revive_pos = ray_params.trace[1]
			--Application:draw_sphere( revive_pos, 15, 0,1,0 )
		end
...
function TeamAILogicTravel._upd_enemy_detection( data )
	...
	--print( "[TeamAILogicTravel._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
	...
	if new_attention then
		...
		if data.unit:movement():chk_action_forbidden( "walk" ) and not data.unit:anim_data().act_idle then
			...
			--[[if objective.type == "follow" then
				...
				if my_data.called or
					(target_prio_slot > 3 and (dist2 > max_dist * max_dist or zdist > 300 )) or 
					(target_prio_slot <= 3 and (dist2 > max_dist * max_dist * 2 or zdist > 600 )) then
					dont_exit = true
				end
			end]]
		end
...
function TeamAILogicTravel._get_exact_move_pos( data, nav_index )
	...
	if not reservation and wants_reservation then
		reservation = { position = mvector3.copy( to_pos ), radius = 60, filter = data.pos_rsrv_id }
...
function TeamAILogicTravel._check_start_path_ahead( data )
	...
	local nav_segs = CopLogicTravel._get_allowed_travel_nav_segs( data, my_data, to_pos )
	--[[my_data._search_nav_segs = nav_segs
	my_data._path_trail = { mvector3.copy(from_pos), mvector3.copy(to_pos) }
	my_data._path_tag = "ahead"]]
