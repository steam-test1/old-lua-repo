...
function TeamAILogicIdle.enter( data, new_logic_name, enter_params )
	...
	--print( "[TeamAILogicIdle.enter]", data.key )
...
function TeamAILogicIdle.exit( data, new_logic_name, enter_params )
	...
	--print( "[TeamAILogicIdle.exit]", data.unit, new_logic_name )
	local my_data = data.internal_data
	
	if my_data.delayed_clbks and my_data.delayed_clbks[ my_data.revive_complete_clbk_id ] then	-- we are reviving / untieing someone
	...
	if objective then
		if not my_data.acting then
			if objective.type == "follow" then
				if TeamAILogicIdle._check_should_relocate( data, my_data, objective ) and not data.unit:movement():chk_action_forbidden( "walk" ) then-- checks pathing results and decides if we should relocate
...
function TeamAILogicIdle.on_objective_unit_damaged( data, unit, attacker_unit )
	if attacker_unit ~= nil then
		TeamAILogicIdle.on_alert( data, { "aggression", attacker_unit:movement():m_pos(), nil, nil, attacker_unit } )
	end
end
...
function TeamAILogicIdle.on_alert( data, alert_data )
	--print( "[CopLogicIdle.on_alert]", data.unit, inspect( alert_data ) )
...
function TeamAILogicIdle._upd_enemy_detection( data )
	...
	--print( "[TeamAILogicIdle._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
	
	if ( not my_data._intimidate_t or my_data._intimidate_t + 2 < data.t ) and not ( data.cool or my_data._turning_to_intimidate or my_data.acting or new_attention and new_reaction >= AIAttentionObject.REACT_SCARED ) then
		...
		if civ then
			--[[
			local brush = Draw:brush(Color(0.3, 0,1,0), 2)
			brush:cylinder(data.unit:movement():m_head_pos(), civ:movement():m_head_pos(), 5)
			]]
...
function TeamAILogicIdle.intimidate_civilians( data, criminal, play_sound, play_action, primary_target )
	-- Return false if global cooldown
	...
	--print("Act:", act_name, "Sound:", sound_name)
	...
	if not managers.groupai:state():enemy_weapons_hot() then
		local alert = { "vo_intimidate", data.m_pos, 800, data.SO_access, data.unit }
...
function TeamAILogicIdle.action_complete_clbk( data, action )
	--print( "TeamAILogicIdle.action_complete_clbk", action:type() )
	local my_data = data.internal_data
...
function TeamAILogicIdle.is_available_for_assignment( data, new_objective )
	...
	elseif data.objective then
		...
		elseif old_objective_type == "revive" then
			return
		elseif old_objective_type == "follow" then
			if data.objective.called then
				return
			end
		end
	end
...
function TeamAILogicIdle.clbk_revive_complete( ignore_this, data )
	...
	if alive( revive_unit ) then
		...
		elseif revive_unit:character_damage() then
			if revive_unit:character_damage():need_revive() or revive_unit:character_damage():arrested() then
				-- Show a "hint" for the client 
				local hint = revive_unit:character_damage():need_revive() and 2 or 3 -- "teammate_helpedup" or "teammate_rescued"
...
function TeamAILogicIdle._check_should_relocate( data, my_data, objective )
	...
	for _, area in ipairs( my_areas ) do
		if area.nav_segs[ follow_unit_nav_seg_id ] then
			return -- I stand on the same area as the follow_unit
		end
	end
	...
	if is_my_area_dangerous and not is_follow_unit_area_dangerous then
		--debug_pause_unit( data.unit, "is_my_area_dangerous and not is_follow_unit_area_dangerous" )
		return true
	end
	...
	local too_far
	if math.abs( mvector3.z( tmp_vec1 ) ) > max_allowed_dis_z then
		too_far = true
	else
		mvector3.set_z( tmp_vec1, 0 )
		if mvector3.length( tmp_vec1 ) > max_allowed_dis_xy then
			too_far = true
		end
	end
	
	if too_far then
		--debug_pause_unit( data.unit, "too far" )
		return true
	end
end
	...
	for u_key, attention_data in pairs( attention_objects ) do
		...
		elseif attention_data.stare_expire_t and data.t > attention_data.stare_expire_t then
			if attention_data.settings.pause then
				...
				--print( "[TeamAILogicIdle._get_priority_attention] pausing for", attention_data.pause_expire_t - data.t, "sec" )
			end
			--print( "ignoring expired reaction", attention_data.pause_expire_t and attention_data.pause_expire_t - data.t, data.unit, att_unit )
			--Application:draw_cylinder( data.m_pos, attention_data.m_pos, 30, 0, 0, 0 )
		else
