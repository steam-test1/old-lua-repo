...
function TeamAILogicAssault.enter( data, new_logic_name, enter_params )
	...
	-- my_data.want_cover = nil	-- This is a boolean flag telling _update_cover() that we want to keep verifying my_data.best_cover
...
function TeamAILogicAssault._upd_enemy_detection( data, is_synchronous )
	...
	--print( "[TeamAILogicAssault._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
	...
	if data.objective and data.objective.type == "follow" and TeamAILogicIdle._check_should_relocate( data, my_data, data.objective ) and not data.unit:movement():chk_action_forbidden( "walk" ) then-- checks pathing results and decides if we should relocate
	...
	if not my_data._intimidate_t or my_data._intimidate_t + 2 < data.t and not my_data._turning_to_intimidate and data.unit:character_damage():health_ratio() > 0.5 then
		...
		if civ then
			if not is_assault or (civ:anim_data().run or civ:anim_data().stand) then
--[[				
				local brush = Draw:brush(Color(0.3, 1,0,0), 2)
	...
	if ( not TeamAILogicAssault._mark_special_chk_t or TeamAILogicAssault._mark_special_chk_t + 0.75 < data.t ) and
		( not TeamAILogicAssault._mark_special_t or TeamAILogicAssault._mark_special_t + 6 < data.t ) and not ( my_data.acting or data.unit:sound():speaking() ) then
...
--	print( "TeamAILogicAssault.action_complete_clbk", action:type() )
...
function TeamAILogicAssault._chk_request_combat_chatter( data, my_data )
	local focus_enemy = data.attention_obj
	if focus_enemy and focus_enemy.verified and focus_enemy.is_person and
		focus_enemy.reaction >= AIAttentionObject.REACT_COMBAT and
		( my_data.firing or data.unit:character_damage():health_ratio() < 1 ) and
		not data.unit:movement():chk_action_forbidden( "walk" ) and 
		not data.unit:sound():speaking() then
