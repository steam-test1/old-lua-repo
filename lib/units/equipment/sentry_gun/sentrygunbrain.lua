...
function SentryGunBrain:update( unit, t, dt )
	if Network:is_server() then
		...
		-- check current focus_enemy is valid
		self:_chk_focus_enemy_valid()
		-- check if we need to change focus enemy
		self:_choose_focus_enemy()
	end
	-- check if we need to start, stop or continue firing
	self:_check_fire( t )
end
...
function SentryGunBrain:set_active( state )
	state = state and true or false -- convert to bool
...
function SentryGunBrain:_choose_focus_enemy( t )
	...
	for e_key, enemy_data in pairs( enemies ) do
		...
		if enemy_data.is_converted or enemy_unit:brain()._current_logic_name == "trade" then
			...
			if self._AI_data.detected_enemies[ e_key ] then	-- We already know about this enemy. Verify him
				local enemy_data = self._AI_data.detected_enemies[ e_key ]
				
				local visible
				
				local enemy_pos = enemy_data.m_com		-- MUTABLE!!!
				...
				if visible then
					...
					-- if the enemy has moved 7m from his last verified pos, then we no longer know his whereabouts
				...
				if chk_vis_func( my_tracker, enemy_data.tracker ) then
					local my_pos = self._m_head_object_pos
					local enemy_pos = enemy_unit:movement():m_head_pos()		-- MUTABLE!!!
					...
					if dis_multiplier < 1 then	-- Enemy is not too far
	...
	for e_key, enemy_data in pairs( self._AI_data.detected_enemies ) do
		if not enemy_data.death_verify_t then	-- we cannot start focusing on dead enemies because they are disconnected from network
	...
	if self._AI_data.focus_enemy ~= focus_enemy then
		if focus_enemy then
			local attention = { unit = focus_enemy.unit--[[, focus_delay = math.lerp( 3.5, 0.5, focus_enemy_weight / max_dis ), focus_overshot = 5 + 15 * math.random()]] }
...
function SentryGunBrain:_check_fire( t )
	...
	elseif focus_enemy and not self._ext_movement:warming_up( t ) then
		if self._firing then
			self._unit:weapon():trigger_held( false, true ) -- no blanks, expend_ammo
		else
			mvec3_dir( tmp_vec1, self._eye_object_pos, focus_enemy.m_com )
			local max_dot = tweak_data.weapon.sentry_gun.KEEP_FIRE_ANGLE
			max_dot = math.min( 0.99, 1 - ( 1 - max_dot ) * self._shaprness_mul ) -- some minimum tolerance is necessary, otherwise it may never fire
			if mvec3_dot( tmp_vec1, self._ext_movement:m_head_fwd() ) > max_dot then
...
function SentryGunBrain:synch_allow_fire( state, instant )
	...
	elseif not state then
		if self._unit:weapon():out_of_ammo() then	-- stop all activity
			self:switch_off()
		elseif self._firing then
			self._unit:weapon():stop_autofire() -- simply stop firing
		end
	end
	self._firing = state
end
...
function SentryGunBrain:switch_off()
	...
	self._ext_movement:switch_off() -- take "deactivated" stance and stay still
	self._unit:set_slot( 26 )	-- dead
	managers.groupai:state():on_criminal_neutralized( self._unit ) -- this will tell enemies to stop shooting at me
...
function SentryGunBrain:load( save_data )
	if not ( save_data and save_data.brain ) then
		return
	end
