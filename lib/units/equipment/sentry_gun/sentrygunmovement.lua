...
function SentryGunMovement:init( unit )
	...
	if managers.navigation:is_data_ready() then
		...
		self._pos_reservation = { position = self._unit:position(), radius = 30 }
	...
	self._warmup_t = 0 -- The sentry gun needs some time to warm up
	self._rot_speed_mul = 1 -- default value
end
...
function SentryGunMovement:chk_play_alert( attention, old_attention )
	if not attention and old_attention then
		self._last_attention_t = TimerManager:game():time() -- we do not have attention any more
	end
	
	if attention and not old_attention then
		if TimerManager:game():time() - self._last_attention_t > 3 then
...
function SentryGunMovement:set_look_vec3( look_vec3 )
	--Application:draw_line( self._m_head_pos, self._m_head_pos + look_vec3 * 150, 0,1,0)
...
function SentryGunMovement:_upd_movement( dt )
	--print( " " )
	--print( "[SentryGunMovement:_update_movement]", inspect( self._attention ) )
	local target_dir = self:_get_target_dir( self._attention )
	--Application:draw_line( self._m_head_pos, self._m_head_pos + target_dir * 5000, 0,0.4,0)
	
	local unit_fwd_polar = self._unit_fwd:to_polar()
	--print( "unit_fwd_polar", unit_fwd_polar.pitch, unit_fwd_polar.spin )
	local fwd_polar = self._m_head_fwd:to_polar_with_reference( self._unit_fwd, self._unit_up )
	--print( "fwd_polar", fwd_polar.pitch, fwd_polar.spin )
	...
	--print( "error_polar", error_polar.pitch, error_polar.spin )
	
	local _ramp_value = function( value, err, vel, slowdown_at, max_vel, min_vel, acc )
		--print( "_ramp_value", value, err, vel, slowdown_at, max_vel, min_vel, acc )
		...
		--print( "new_vel", new_vel, "correction", correction )
	...
	if self._motor_sound then
		self._sound_source:set_rtpc( "spin_vel", math.clamp( ( new_vel - self._tweak.MAX_VEL_SPIN * self._rot_speed_mul * 0.25 ) / ( self._tweak.MAX_VEL_SPIN * self._rot_speed_mul ), 0, 1 ) )--todo
	end
...
--[[function SentryGunMovement:test()
	local attention_vec = Rotation(math.random()*360, math.random()*360, math.random()*360 ):y() * 5000
	if self._test then
		attention_vec = -attention_vec
	end
	self:set_attention( { pos = self._unit:position() + attention_vec } )
...
function SentryGunMovement:_get_target_dir( attention )
	if not attention then
		if self._switched_off then
			...
			mvector3.rotate_with( tmp_vec2, self._switch_off_rot ) -- pitch 35 degrees downwards
			return tmp_vec2
		else
	...
	if self._attention then
		if self._attention.pos then
			...
			if self._attention.unit:id() == -1 then -- Network-detached unit
				...
				managers.enemy:add_delayed_clbk( "clbk_sync_attention"..tostring( self._unit:key() ), callback( self, CopMovement, "clbk_sync_attention", { self._unit, self._attention.unit } ), TimerManager:game():time() + 0.1 )
			end
		end
	end
