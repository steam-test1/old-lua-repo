...
function SpecialObjectiveGroupElement:update_selected( t, dt, selected_unit, all_units )	
	...
	if self._hed.spawn_instigator_ids then
		for _,id in ipairs( self._hed.spawn_instigator_ids ) do
			...
			if draw then
				self:_draw_link( { from_unit = unit, to_unit = self._unit,  r = 0, g = 0.0, b = 0.75 } )
			end
		end
	end
end
...
function SpecialObjectiveGroupElement:_lmb()
	-- First check if we click another special objective element
...
function SpecialObjectiveGroupElement:_build_panel( panel, panel_sizer )
	...
	local mode_params = {
		...
		panel							= panel,
		sizer 						= panel_sizer,
		options						= { "randomizer", "forced_spawn", "recurring_cloaker_spawn", "recurring_spawn_1" },
	...
	base_chance:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = base_chance, value = "base_chance" } )
	base_chance:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = base_chance, value = "base_chance" } )
end
