...
function AchievmentManager:init()
	self.exp_awards = { none = 0, a = 500, b = 1500, c = 5000 }
	self.script_data = {}
	...
	if SystemInfo:platform() == Idstring( "WIN32" ) then
		...
		if not Global.achievment_manager then
			...
			Global.achievment_manager = { handler = self.handler, achievments = self.achievments }
		else
...
function AchievmentManager:award_x360( id )
	...
	--[[if true then
		Application:error( "SKIP XBOX Achievment SINCE IT CRASHES WHEN THEY DON'T EXIST." )
		return 
	end]]
