...
function TimeSpeedManager:_update_playing_effects()
	...
	if playing_effects then
		...
		do
			local (for generator), (for state), (for control) = pairs(all_affected_timers)
			do
				do break end
				timer_info.mul = 1
			end
		...
		do
			local (for generator), (for state), (for control) = pairs(playing_effects)
			do
				do break end
				...
				if effect_speed then
					do
						local (for generator), (for state), (for control) = pairs(effect.affect_timers)
						do
							do break end
...
function TimeSpeedManager:play_effect(id, effect_desc)
	...
	if effect_desc.affect_timer then
		if type(effect_desc.affect_timer) == "table" then
			...
			local (for generator), (for state), (for control) = ipairs(effect_desc.affect_timer)
			do
				do break end
