...
function BlackMarketManager:concealment_modifiers()
	...
	--[[
	skill_bonuses = skill_bonuses - ( managers.player:has_category_upgrade( "player", "suspicion_multiplier" ) and 1 or 0 )
	skill_bonuses = skill_bonuses - ( managers.player:has_category_upgrade( "player", "passive_suspicion_multiplier" ) and 1 or 0 )
]]

	return skill_bonuses	-- add skill upgrades here
end
...
function BlackMarketManager:get_dropable_mods_by_weapon_id( weapon_id )
	...
	for id, data in pairs( weapon_mods ) do
		...
		for _, mod in ipairs( data ) do
			...
			if my_mod then
				if my_mod.pcs or my_mod.pc then
					local is_dlc = my_mod.dlcs or my_mod.dlc
					if not is_dlc or my_mod.infamous then
						table.insert( dropable_mods[id], { mod, all_mods.infamous and "infamous" or "normal" } )
					end
				end
			end
		end
