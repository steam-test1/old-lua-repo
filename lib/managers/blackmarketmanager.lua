...
function BlackMarketManager:_setup()
	...
	-- self._defaults.character = "npc_criminal_casual_1_var01"
	-- self._defaults.character = "npc_criminal_suit_1_var02"
	self._defaults.character = "locked"
	self._defaults.armor = "level_1"
	self._defaults.preferred_character = "russian"	-- dallas
	self._defaults.grenade = "frag"
	self._defaults.melee_weapon = "gun"
	-- self._defaults.deployable = nil
...
function BlackMarketManager:_setup_grenades()
	...
	for grenade, _ in pairs( tweak_data.blackmarket.grenades ) do
		grenades[ grenade ] = { unlocked = true, equipped = false, amount = 0 }
	end
...
--[[
function BlackMarketManager:_setup_deployables()
	local deployables = {}
	Global.blackmarket_manager.deployables = deployables
	
	for deployable,_ in pairs( tweak_data.blackmarket.deployables ) do
		deployables[ deployable ] = { unlocked = false, owned = false, equipped = false }
	end
end
]]
...
function BlackMarketManager:_setup_weapon_upgrades()
	...
	-- Upgrade properties are unlocked and owned per weapon
	...
	-- weapon_upgrades.m4.barrel1.owned = false
...
-- Currently called from PlayerManager load since it contains the information in PD1
-- function BlackMarketManager:setup_weapons() self:_setup_weapons() end
function BlackMarketManager:_setup_weapons()
	...
	for weapon,data in pairs( tweak_data.weapon ) do
		if data.autohit then
			...
			-- local unlocked = managers.player:has_weapon( weapon ) and true or false
			-- local owned = not BlackMarketManager.weapons_to_buy[ weapon ]
			local equipped = weapon == managers.player:weapon_in_slot( selection_index )
			-- print( weapon, "unlocked, equipped", unlocked, equipped )
			local factory_id = managers.weapon_factory:get_factory_id_by_weapon_id( weapon )
					
			-- local condition	= math.random( 17 ) - 1
			-- weapons[ weapon ] = { unlocked = unlocked, owned = owned, equipped = equipped, selection_index = selection_index, condition = condition }
...
function BlackMarketManager:equipped_item( category )
	...
	-- 	return self:equipped_deployable()
	end
end
									-- if you are missing one of these, give default; primary has no default before level 2 (amcar)
...
function BlackMarketManager:equipped_deployable()
	for deployable_id, data in pairs( tweak_data.blackmarket.deployables ) do
		if Global.blackmarket_manager.deployables[ deployable_id ].equipped then
			return deployable_id
		end
	end
end]]
...
function BlackMarketManager:equip_deployable( deployable_id )
	-- for s, data in pairs( Global.blackmarket_manager.deployables ) do
	-- 	data.equipped = s == deployable_id
	-- end
...
function BlackMarketManager:equip_mask( slot )
	...
	if managers.menu_scene then
		-- managers.menu_scene:set_character_equipped_weapon( nil, new_mask_data.factory_id, new_mask_data.blueprint, (category == "primaries" and "primary") or "secondary" )
...
-- string.split( managers.blackmarket:outfit_string(), " " )
...
function BlackMarketManager:_outfit_string_mask()
	...
	if type( equipped ) == "string" then	-- Fallback if old mask system
		s = s.." "..equipped
		s = s.." ".."nothing"
...
function BlackMarketManager:outfit_string()
	...
	--[[for mask_id,data in pairs( tweak_data.blackmarket.masks ) do
		if self._global.masks[ mask_id ].equipped then
			s = s.." "..mask_id
		end
	end]]
...
function BlackMarketManager:load_all_crafted_weapons()	-- debug
	print( "--PRIMARIES-------------------------" )
...
function BlackMarketManager:preload_equipped_weapons()	-- queue loading
...
function BlackMarketManager:preload_done_callback( category, preload_table, parts )
	...
	table.insert( self._preloading_list, { category, preload_table, parts } )
end
...
function BlackMarketManager:create_preload_ws()
	...
	new_script.set_progress = function( progress )
	...
	for i = 1, num_squares do
		...
		last_rect = square_panel:rect( { x=x, y=y, w=14, h=14, color=Color( 0.3, 0.3, 0.3 ), blend_mode="add" } )
		x = x + 24
	...
	panel:rect( { name="progress", w=20, h=20, color=Color( 0.3, 0.3, 0.3 ), layer=2, blend_mode="add" } )
	
	
	local bg = panel:rect( { color = Color.black, alpha = 0.8 } )
			...
			if self._preloading_index > #self._preloading_list then
				...
				if is_load then
					if next_in_line.part_id then
						-- print( "[BlackMarketManager] Loading part_id: " .. tostring(next_in_line.part_id), "next_in_line", inspect(next_in_line) )
					end
...
function BlackMarketManager:alter_global_value_item( global_value, category, slot, id, ... )
	...
	--[[
	INV_TO_CRAFT = Idstring( "inventory_to_crafted" )
	CRAFT_TO_INV = Idstring( "crafted_to_inventroy" )
	INV_ADD = Idstring( "add_to_inventory" )
	INV_REMOVE = Idstring( "remove_from_inventory" )
	CRAFT_ADD = Idstring( "add_to_crafted" )
	CRAFT_REMOVE = Idstring( "remove_from_crafted" )
	]]
...
function BlackMarketManager:got_any_new_drop()
	...
	if amount_new_loot > 0 then
		...
		-- local normal_texture = self:check_new_drop_category( "normal", "textures" )
		-- local infamous_texture = self:check_new_drop_category( "infamous", "textures" )
		
		-- local normal_color = self:check_new_drop_category( "normal", "colors" )
		-- local infamous_color = self:check_new_drop_category( "infamous", "colors" )
...
function BlackMarketManager:got_new_drop( global_value, category, id )
	...
	-- print( global_value, category, id )
	...
	for global_value,categories in pairs( self._global.inventory ) do
		if categories[ category ] then
			for id,amount in pairs( categories[ category ] ) do
				-- print( global_value, id, amount )
				table.insert( t, { id = id, global_value = global_value, amount = amount } )
				-- masks[ global_value ] = masks[ global_value ] or {}
				-- table.insert( masks[ global_value ], mask_id )
			end
		end
	end
	return t
...
function BlackMarketManager:merge_inventory_masks()
	...
	for global_value, categories in pairs( self._global.inventory ) do
		if( global_value ~= "normal" and global_value ~= "infamous" ) then
...
function BlackMarketManager:get_inventory_masks()
	...
	for global_value,categories in pairs( self._global.inventory ) do
		if categories[ "masks" ] then
			for mask_id,amount in pairs( categories[ "masks" ] ) do
				-- print( global_value, mask_id, amount )
...
--[[
function BlackMarketManager:get_concealment_from_local_player()
	local stats_tweak_data = tweak_data.weapon.stats
	
	local primary_concealment = self:calculate_weapon_concealment( self:equipped_primary() )
	local secondary_concealment = self:calculate_weapon_concealment( self:equipped_secondary() )
	local armor_concealment = math.max( #stats_tweak_data.concealment - tweak_data.blackmarket.armors[ self:equipped_armor() ].concealment, 0 )
	
	local total_concealment = math.clamp( primary_concealment + secondary_concealment + armor_concealment+ 1, 1, #stats_tweak_data.concealment )
	return stats_tweak_data.concealment[ total_concealment ], total_concealment
end

function BlackMarketManager:get_concealment_from_peer( peer )
	local outfit = peer:blackmarket_outfit()
	local stats_tweak_data = tweak_data.weapon.stats
	
	local primary_concealment = self:calculate_weapon_concealment( outfit.primary )
	local secondary_concealment = self:calculate_weapon_concealment( outfit.secondary )
	local armor_concealment = math.max( #stats_tweak_data.concealment - tweak_data.blackmarket.armors[ outfit.armor ].concealment, 0 )
...
function BlackMarketManager:get_dropable_mods_by_weapon_id( weapon_id, weapon_data )
	...
	local blueprint, blueprint_gv = nil, nil
...
function BlackMarketManager:_sell_item( item_data )
	...
	local pc = item_def.pc or item_def.pcs[ 1 ] -- HOW TO SPECIFY REAL VALUE
...
function BlackMarketManager:apply_mask_craft_on_unit( unit, blueprint )
	...
	local tint_color_a = Vector3( 0, 0 ,0 ) -- Vector3() -- Vector3( 0.0, 0.2, 0.7 ) -- Vector3( 0.7, 0.2, 0.0 ) -- Vector3() -- Vector3( 0.7, 0.2, 0.0 )
	local tint_color_b = Vector3( 0, 0, 0 ) -- Vector3( 0.7, 0.2, 0.0 ) -- Vector3() -- Vector3( 0.7, 0.2, 0.0 ) -- Vector3() -- Vector3( 0.0, 0.0, 0.5 )
	...
	-- material:set_texture( Idstring( "material_texture" ), Object texture)
	
	local pattern = tweak_data.blackmarket.textures[ pattern_id ].texture
	-- local pattern = Idstring( "units/payday2/masks/shared_textures/patterns/pattern_zebra_df" )
	print( "pattern", pattern )
	local material_texture = TextureCache:retrieve( pattern, "normal" ) 
	material:set_texture( "material_texture", material_texture ) 
	
	-- Application:set_material_texture( material, Idstring( "material_texture" ), pattern, Idstring( "normal" ) )
	
	local reflection = tweak_data.blackmarket.materials[ material_id ].texture
	local material_amount = tweak_data.blackmarket.materials[ material_id ].material_amount or 1
	-- local reflection = Idstring( "units/payday2/matcaps/matcap_gold_df" )
	-- Application:set_material_texture( material, Idstring( "reflection_texture" ), reflection, Idstring( "normal" ) )
...
function BlackMarketManager:test_craft_mask( slot )
	...
	--[[blueprint.masks = { id = "hilary_clinton", global_value = "exceptional" }
	blueprint.materials = { id = "wood", global_value = "normal" }
	blueprint.colors = { id = "black_grey", global_value = "normal" }
	blueprint.textures = { id = "bullseye", global_value = "normal" }]]
...
function BlackMarketManager:has_inventory_item( default_global_value, category, id )
	...
	for global_value, data in pairs( self._global.inventory ) do
		if default_global_value ~= global_value then
			if self:get_item_amount( global_value, category, id, true ) > 0 then
				return true
			end
		end
...
function BlackMarketManager:craft_item( category, slot, blueprint )
	...
	for category,data in pairs( blueprint ) do
		-- print( category, data.id, data.global_value )
...
function BlackMarketManager:sell_crafted_item( category, slot )
	...
	-- print( inspect( blueprint ) )
	for category,data in pairs( blueprint ) do
		-- print( category, data.id, data.global_value )
		self:_sell_item( { global_value = data.global_value, category = category, id = data.id } )
...
function BlackMarketManager:on_aquired_weapon_platform( upgrade, id, loading )
	-- print( "[BlackMarketManager:on_aquired_weapon_platform]", inspect( upgrade ), id )
	
	-- Save something to an unlocked list
...
function BlackMarketManager:on_unaquired_weapon_platform( upgrade, id )
	...
	if equipped_primariy then
		if equipped_primariy.weapon_id == id then
	...
	if equipped_secondary then
		if equipped_secondary.weapon_id == id then
...
function BlackMarketManager:aquire_default_weapons( only_enable )
	...
	if glock_17 and ( not self._global.crafted_items[ "secondaries" ] or not glock_17.unlocked ) then
		if not managers.upgrades:aquired( "glock_17" ) then
			if only_enable then
	...
	if amcar and ( not self._global.crafted_items[ "primaries" ] or not amcar.unlocked ) then
		if not managers.upgrades:aquired( "amcar" ) then
			if only_enable then
...
-- Category should be primaries or secondaries
function BlackMarketManager:on_buy_weapon_platform( category, weapon_id, slot, free )
	...
	self._global.crafted_items[ category ][ slot ] = { weapon_id = weapon_id, factory_id = factory_id, blueprint = blueprint, global_values = {} }
	...
	if not free then
		...
		-- Achievement, first bought weapon
...
function BlackMarketManager:on_sell_weapon( category, slot )
	...
	-- Calculate value, if not done before
	...
	for _, part_id in pairs( blueprint ) do
		...
		-- self:alter_global_value_item( global_value, "weapon_mods", slot, part_id, INV_ADD )
	end
...
function BlackMarketManager:remove_weapon_part( category, slot, global_value, part_id )
	...
	-- self:alter_global_value_item( global_value, "weapon_mods", slot, part_id, CRAFT_TO_INV )
	self:add_to_inventory( global_value, "weapon_mods", part_id, true )
	return true
end
...
function BlackMarketManager:modify_weapon( category, slot, global_value, part_id )
	...
	for _, removed_part_id in pairs( removed_parts ) do
		...
		self:alter_global_value_item( global_value, category, slot, removed_part_id, CRAFT_REMOVE )	-- add_to_inventory adds gvi aswell, so no CRAFT_TO_INV
	end
		-- 
	self:_on_modified_weapon( category, slot )
end
...
function BlackMarketManager:buy_and_modify_weapon( category, slot, global_value, part_id, free_of_charge )
	...
	if not free_of_charge then
		...
		self:remove_item( global_value, "weapon_mods", part_id )
		-- self:alter_global_value_item( global_value, "weapon_mods", slot, part_id, INV_TO_CRAFT )
		...
		-- Achievment, first weapon modification
...
function BlackMarketManager:view_weapon_platform( weapon_id, open_node_cb )
	...
	table.insert( self._preloading_list, { done_cb=function() managers.menu_scene:spawn_item_weapon( factory_id, blueprint ) end} )
...
function BlackMarketManager:view_weapon( category, slot, open_node_cb )
	...
	table.insert( self._preloading_list, { done_cb=function() managers.menu_scene:spawn_item_weapon( weapon.factory_id, weapon.blueprint ) end } )
	table.insert( self._preloading_list, { done_cb=open_node_cb } )
end
...
function BlackMarketManager:view_weapon_with_mod( category, slot, part_id, open_node_cb  )
	...
	self:preload_weapon_blueprint( "preview", weapon.factory_id, weapon.blueprint )
	table.insert( self._preloading_list, { done_cb=function() managers.menu_scene:spawn_item_weapon( weapon.factory_id, blueprint ) end } )
	table.insert( self._preloading_list, { done_cb=open_node_cb } )
end
...
--[[
function BlackMarketManager:on_aquired_deployable( equipment, id )
	if not self._global.deployables then
		self:_setup_deployables()
	end
	
	self._global.deployables[ id ].unlocked = true
	self._global.deployables[ id ].owned = true
end

function BlackMarketManager:on_unaquired_deployable( equipment, id )
	if not self._global.deployables then
		self:_setup_deployables()
	end
	self._global.deployables[ id ].unlocked = false
	self._global.deployables[ id ].owned = false
	
	if self._global.deployables[ id ].equipped then
		self._global.deployables[ id ].equipped = false
...
function BlackMarketManager:can_modify_mask( slot )
	...
	return true -- ( #textures == 0 and #colors == 0 and #materials > 0 ) or ( #textures > 0 and #colors > 0 ) or false
end
...
function BlackMarketManager:start_customize_mask( slot )
	...
	-- self._customize_mask.colors = { id = "nothing", global_value = "normal"}
	self._customize_mask.textures = { id = "no_color_full_material", global_value = "normal"}
	-- self._customize_mask.materials = { id = "plastic", global_value = "normal"}
...
function BlackMarketManager:select_customize_mask( category, id, global_value )
	...
	self._customize_mask[category] = { id = id, global_value = global_value or "normal"}
...
function BlackMarketManager:customize_mask_category_id( category )
	-- print( "customize_mask_category_id", category )
...
function BlackMarketManager:get_customize_mask_blueprint()
	return { material = self._customize_mask.materials, pattern = self._customize_mask.textures, color = self._customize_mask.colors }
end
...
function BlackMarketManager:finish_customize_mask()
	...
	self._customize_mask.textures = self._customize_mask.textures or { id = "no_color_full_material", global_value = "normal" }
	self._customize_mask.materials = self._customize_mask.materials or { id = "plastic", global_value = "normal" }
	
	--[[
	if( not blueprint.color ) then
		self:abort_customize_mask()
		return
	end
	]]
...
function BlackMarketManager:on_buy_mask( mask_id, global_value, slot )
	...
	self._global.crafted_items[ category ][ slot ] = { mask_id = mask_id, global_value = global_value, blueprint = blueprint, modded = false }
	self:_verfify_equipped_category( category )
end
...
function BlackMarketManager:mask_unit_name_by_mask_id( mask_id, peer_id )
	...
	local character = self:get_preferred_character() -- "russian"
	if managers.network and managers.network:session() and peer_id then
		print( "HERE", managers.network:session(), peer_id )
		-- local peer_id = managers.network:session():local_peer():id()
		character = managers.network:session():peer( peer_id ):character() -- CriminalsManager:character_workname_by_peer_id( peer_id )
	end
	...
	if data.blackmarket then
		...
		for armor,_ in pairs( tweak_data.blackmarket.armors ) do
			if not self._global.armors[ armor ] then
				self._global.armors[ armor ] = { unlocked = false, owned = false, equipped = false }
			else
		...
		if self._global.grenades[ self._defaults.grenade ] then self._global.grenades[ self._defaults.grenade ].equipped = false end
		...
		if self._global.melee_weapons[ self._defaults.melee_weapon ] then self._global.melee_weapons[ self._defaults.melee_weapon ].equipped = false end
		...
		for weapon,data in pairs( tweak_data.weapon ) do
			if not self._global.weapons[ weapon ] then
				if data.autohit then -- Is player weapon
					local selection_index = data.use_data.selection_index
					local factory_id = managers.weapon_factory:get_factory_id_by_weapon_id( weapon )
					self._global.weapons[ weapon ] = { owned = false, equipped = false, unlocked = false, factory_id = factory_id, selection_index = selection_index }
				end
			end
		end
		...
		for character,_ in pairs( tweak_data.blackmarket.characters ) do
			if not self._global.characters[ character ] then
				self._global.characters[ character ] = { unlocked = true, owned = true, equipped = false }
			end
		end
				...
				for i, part_id in ipairs( blueprint ) do
					if not index_table[ part_id ] then
						...
						if table.contains( default_blueprint, part_id ) then
							...
							if default_mod then
								table.insert( invalid_parts, { slot = slot, global_value = "normal", default_mod = default_mod, part_id = part_id } )
							else
								table.insert( invalid_parts, { slot = slot, global_value = item.global_values[ part_id ] or "normal", part_id = part_id } )
							end
						end
					end
				end
...
function BlackMarketManager:_verfify_equipped_category( category )
	...
	for slot, craft in pairs( self._global.crafted_items[ category ] ) do
		...
		--[[
		print( "DIDN'T HAVE EQUIPPED IN CATEGORY", category, "IS NOW", slot, inspect( craft ) )
		if category == "primaries" and self._global.weapons[ craft.weapon_id ].unlocked then
			print( "  Equip", category, slot )
			craft.equipped = true
			return
		...
		--[[if craft.unlocked == nil and true or craft.unlocked then
		-- if table.has( craft, "unlocked" ) and craft.unlocked then
			print( "  Equip", category, slot )
			craft.equipped = true
			return
		end ]]
	...
	if category == "secondaries" or category == "primaries" then
		...
		self._global.crafted_items[ category ][ free_slot ] = { weapon_id = weapon_id, factory_id = factory_id, blueprint = blueprint, equipped = true }
...
function BlackMarketManager:debug_inventory()
	local t = {}
	for gv,cat in pairs( self._global.inventory ) do -- .normal.weapon_mods
