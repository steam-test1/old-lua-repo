...
function BlackMarketManager:_setup_grenades()
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.blackmarket.grenades)
		do
			do break end
			grenades[grenade] = {
				unlocked = true,
...
function BlackMarketManager:_setup_melee_weapons()
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.blackmarket.melee_weapons)
		do
			do break end
...
function BlackMarketManager:_setup_track_global_values()
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.lootdrop.global_values)
		do
			do break end
			if td.track then
...
function BlackMarketManager:_setup_weapon_upgrades()
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.weapon_upgrades.weapon)
		do
			do break end
			weapon_upgrades[weapon] = {}
			local (for generator), (for state), (for control) = pairs(tweak_data.weapon_upgrades.weapon[weapon])
			do
				do break end
...
function BlackMarketManager:weapon_level(weapon_id)
	do
		...
		do
			do break end
			local (for generator), (for state), (for control) = ipairs(level_data.upgrades)
			do
				do break end
...
function BlackMarketManager:equipped_mask()
	...
	local (for generator), (for state), (for control) = pairs(Global.blackmarket_manager.crafted_items.masks)
	do
		do break end
...
function BlackMarketManager:equipped_secondary()
	...
	do
		local (for generator), (for state), (for control) = pairs(Global.blackmarket_manager.crafted_items.secondaries)
		do
			do break end
			if data.equipped then
				return data
...
function BlackMarketManager:equipped_primary()
	...
	do
		local (for generator), (for state), (for control) = pairs(Global.blackmarket_manager.crafted_items.primaries)
		do
			do break end
			if data.equipped then
				return data
...
function BlackMarketManager:equipped_weapon_slot(category)
	...
	do
		local (for generator), (for state), (for control) = pairs(Global.blackmarket_manager.crafted_items[category])
		do
			do break end
			if data.equipped then
				return slot
...
function BlackMarketManager:load_all_crafted_weapons()
	print("--PRIMARIES-------------------------")
	do
		local (for generator), (for state), (for control) = pairs(self._global.crafted_items.primaries)
		do
			do break end
...
function BlackMarketManager:resource_loaded_callback(category, loaded_table, parts)
	...
	if loaded_category then
		local (for generator), (for state), (for control) = pairs(loaded_category)
		do
			do break end
...
function BlackMarketManager:release_preloaded_blueprints()
	do
		local (for generator), (for state), (for control) = pairs(self._category_resource_loaded)
		do
			do break end
...
function BlackMarketManager:create_preload_ws()
	...
	function new_script.set_progress(progress)
		...
		local (for generator), (for state), (for control) = ipairs(square_panel:children())
		do
			do break end
	...
	local num_squares = 0
	do
		local (for generator), (for state), (for control) = ipairs(self._preloading_list)
		do
			do break end
...
function BlackMarketManager:get_weapon_new_part_drops(id)
	...
	local new_parts = {}
	do
		local (for generator), (for state), (for control) = pairs(uses_parts)
		do
			do break end
			local (for generator), (for state), (for control) = ipairs(parts)
			do
				do break end
	...
	do
		...
		do
			...
			if categories[category] then
				local (for generator), (for state), (for control) = pairs(categories[category])
				do
					do break end
					table.insert(t, {
						id = id,
	...
	do
		...
		do
			...
			if global_value ~= "normal" and global_value ~= "infamous" and categories.masks then
				local (for generator), (for state), (for control) = pairs(categories.masks)
				do
					do break end
...
function BlackMarketManager:get_weapon_stats(category, slot)
	...
	do
		local (for generator), (for state), (for control) = pairs(weapon_tweak_data.stats)
		do
			do break end
...
function BlackMarketManager:get_dropable_mods_by_weapon_id(weapon_id, weapon_data)
	...
	do
		local (for generator), (for state), (for control) = pairs(weapon_mods)
		do
			do break end
			...
			do
				...
				if part and (part.pcs or part.pc) then
					...
					if not is_infamous then
						local dlcs = {}
						if part.dlcs then
							local (for generator), (for state), (for control) = ipairs(part.dlcs)
							do
								do break end
...
function BlackMarketManager:on_buy_weapon_platform(category, weapon_id, slot, free)
	...
	if self._global.crafted_items.primaries and self._global.crafted_items.secondaries then
		...
		local (for generator), (for state), (for control) = pairs(tweak_data.achievement.weapons_owned)
		do
			do break end
...
function BlackMarketManager:modify_weapon(category, slot, global_value, part_id, remove_part)
	...
	local removed_parts = {}
	do
		local (for generator), (for state), (for control) = pairs(replaces)
		do
			do break end
...
function BlackMarketManager:_on_modified_weapon(category, slot)
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.achievement.weapon_blueprints)
		do
			do break end
...
function BlackMarketManager:load(data)
	if data.blackmarket then
		...
		do
			local (for generator), (for state), (for control) = pairs(self._global.melee_weapons)
			do
				do break end
				...
				if not is_default and melee_weapon_level == 0 and not tweak_data.blackmarket.melee_weapons[melee_weapon].dlc then
					-- unhandled boolean indicator
				else
				end

				self._global.melee_weapons[melee_weapon].skill_based = true
			end
		...
		local old_drops = {}
		do
			local (for generator), (for state), (for control) = pairs(self._global.new_drops)
			do
				do break end
				local (for generator), (for state), (for control) = pairs(categories)
				do
					do break end
...
function BlackMarketManager:refill_track_global_values()
	...
	do
		local (for generator), (for state), (for control) = pairs(global_value_items)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(crafted_weapons)
		do
			do break end
			local (for generator), (for state), (for control) = pairs(category_data)
			do
				do break end
				global_values = data.global_values or {}
				local (for generator), (for state), (for control) = pairs(global_values)
				do
					do break end
	...
	do
		...
		do
			...
			local (for generator), (for state), (for control) = pairs(data.blueprint or {})
			do
				do break end
				local converted_category = category == "color" and "colors" or category == "material" and "materials" or category == "pattern" and "textures" or category
...
function BlackMarketManager:_cleanup_blackmarket()
	...
	do
		local (for generator), (for state), (for control) = pairs(crafted_items)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(crafted_masks)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(self._global.inventory or {})
		do
			do break end
						...
						do
							...
							if not item_tweak_data then
								...
								if item_tweak_data.dlcs then
									local (for generator), (for state), (for control) = ipairs(item_tweak_data.dlcs)
									do
										do break end
				...
				elseif self._global.global_value_items[package_id] then
					...
					if mask then
						...
						if not is_locked then
							local (for generator), (for state), (for control) = pairs(mask.blueprint)
							do
								do break end
...
function BlackMarketManager:debug_inventory()
	...
	do
		...
		do
			do break end
			local (for generator), (for state), (for control) = pairs(cat)
			do
				do break end
				t[type] = t[type] or {amount = 0}
				local (for generator), (for state), (for control) = pairs(entry)
				do
					do break end
