...
function ContractBoxGui:create_contract_box()
	...
	if contact_data then
		...
		local total_xp, base_xp, risk_xp = managers.experience:get_contract_xp_by_stars( job_stars, difficulty_stars, job_data.professional, (#job_data.chain) )
		local job_xp = self._contract_panel:text( { font=font, font_size=font_size, text=tostring( math.round( base_xp ) ), color=tweak_data.screen_colors.text } )
		do
		...
		local risk_xp = self._contract_panel:text( { font=font, font_size=font_size, text=" +" .. tostring( math.round( risk_xp ) ), color=risk_color } )
		do
		...
		local job_money = self._contract_panel:text( { font=font, font_size=font_size, text=managers.experience:cash_string( math.round( total_stage_value + total_job_value ) ), color=tweak_data.screen_colors.text } )
		do
		...
		local risk_money = self._contract_panel:text( { font=font, font_size=font_size, text=" +"..managers.experience:cash_string( math.round( total_stage_risk_value + total_job_risk_value ) ), color=risk_color } )
		do
