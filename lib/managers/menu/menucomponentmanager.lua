...
function MenuComponentManager:mouse_pressed(o, button, x, y)
	...
	(for control) = button and data.panel
...
function MenuComponentManager:_request_done_callback(texture_ids)
	...
	do
		local (for generator), (for state), (for control) = pairs(requests)
		do
			do break end
			count = count + 1
...
function MenuComponentManager:close()
	...
	do
		local (for generator), (for state), (for control) = pairs(self._texture_cache)
		do
			do break end
			TextureCache:unretrieve(texture_ids)
		end
