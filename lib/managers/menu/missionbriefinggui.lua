	...
	for i=1, 2 do
		...
		if table.size(perks) > 0 then
			...
			for perk in pairs( perks ) do
				local perk_object = self._panel:bitmap( { texture="guis/textures/pd2/blackmarket/inv_mod_"..perk, w=16, h=16, rotation=math.random(2)-1.5, alpha=0.8, layer=2 } )
...
function TeamLoadoutItem:set_slot_outfit( slot, criminal_name, outfit )
	...
	if outfit.primary.factory_id then
		...
		local primary_bitmap = player_slot.panel:bitmap( { texture="guis/textures/pd2/blackmarket/icons/weapons/"..primary_id, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
		...
		if table.size(perks) > 0 then
			local perk_index = 0
			for perk in pairs( perks ) do
				local perk_object = player_slot.panel:bitmap( { texture="guis/textures/pd2/blackmarket/inv_mod_"..perk, w=16, h=16, rotation=math.random(2)-1.5, alpha=0.8 } )
	...
	if outfit.secondary.factory_id then
		...
		local secondary_bitmap = player_slot.panel:bitmap( { texture="guis/textures/pd2/blackmarket/icons/weapons/"..secondary_id, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
	...
	if outfit.armor then
		local armor_bitmap = player_slot.panel:bitmap( { texture="guis/textures/pd2/blackmarket/icons/armors/"..outfit.armor, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
	...
	if outfit.deployable and outfit.deployable ~= "nil" then
		local deployable_bitmap = player_slot.panel:bitmap( { texture="guis/textures/pd2/blackmarket/icons/deployables/"..outfit.deployable, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
