...
function MissionBriefingTabItem:init( panel, text, i )
	...
	self._tab_text = self._main_panel:text( { name="tab_text_"..tostring(self._index), text=text, h=32, x=offset, align="center", vertical="center", font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, color=tweak_data.screen_colors.button_stage_3, layer=1, blend_mode="add" } )
	
	local _, _, tw, th = self._tab_text:text_rect()
	self._tab_text:set_size( tw + 15, th + 10 )
	self._tab_select_rect = self._main_panel:bitmap( { name = "tab_select_rect_"..tostring(self._index), texture="guis/textures/pd2/shared_tab_box", layer=0, color=tweak_data.screen_colors.text, visible=false } )
...
function MissionBriefingTabItem:select( no_sound )
	...
	if( self._tab_text and alive( self._tab_text ) ) then
		self._tab_text:set_color( tweak_data.screen_colors.button_stage_1 ) -- set_alpha( 1 )
...
function MissionBriefingTabItem:deselect()
	...
	self:hide()
	if( self._tab_text and alive( self._tab_text ) ) then
		self._tab_text:set_color( tweak_data.screen_colors.button_stage_3 ) -- set_alpha( 0.5 )
...
function MissionBriefingTabItem:mouse_moved( x, y )
	...
	--[[
	if( self._tab_text:inside( x, y ) ) then
		self._tab_text:set_color(  and tweak_data.screen_colors.button_stage_1 or tweak_data.screen_colors.button_stage_2 ) -- set_alpha( math.max( self._tab_text:alpha(), 0.75 ) )
	else
		self._tab_text:set_color( self._selected and tweak_data.screen_colors.button_stage_1 or tweak_data.screen_colors.button_stage_3 ) -- set_alpha( self._selected and 1 or 0.5 )
	end
	]]
...
function DescriptionItem:init( panel, text, i, saved_descriptions )
	...
	local title_text = self._panel:text( { name="title_text", text=managers.localization:to_upper_text(name_id), font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, y=10, x=10, color=tweak_data.screen_colors.text } )
	...
	if managers.job:is_current_job_professional() then
		local pro_text = self._panel:text( { name="pro_text", text=managers.localization:to_upper_text( "cn_menu_pro_job" ), font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, color=tweak_data.screen_colors.pro_color, blend_mode="add" } )
		local x, y, w, h = pro_text:text_rect()
		pro_text:set_size( w, h )
		pro_text:set_position( title_text:right() + 10, title_text:y() )
	end
	
	self._scroll_panel = self._panel:panel( { x=10, y=title_text:bottom() } )
	self._scroll_panel:grow( -self._scroll_panel:x()-10, -self._scroll_panel:y() )
	local desc_text = self._scroll_panel:text( { name="description_text", text=managers.localization:text(briefing_id), font_size=tweak_data.menu.pd2_small_font_size, font=tweak_data.menu.pd2_small_font, wrap=true, word_wrap=true, color=tweak_data.screen_colors.text } )
	local _, _, _, h = desc_text:text_rect()
	desc_text:set_h( h )
	
	if desc_text:h() > self._scroll_panel:h() then
		self._scrolling = true
		self._scroll_box = BoxGuiObject:new( self._scroll_panel, { sides = { 0, 0, 0, 0 } } )
		self._show_scroll_line_top = false
		self._show_scroll_line_bottom = false
		
		local show_scroll_line_top = desc_text:top() < 0 -- self._show_scroll_line_top
		local show_scroll_line_bottom = desc_text:bottom() > self._scroll_panel:h() -- self._show_scroll_line_bottom
		
		if show_scroll_line_top~=self._show_scroll_line_top or show_scroll_line_bottom~=self._show_scroll_line_bottom then
			self._scroll_box:create_sides( self._scroll_panel, { sides = { 0, 0, show_scroll_line_top and 2 or 0, show_scroll_line_bottom and 2 or 0 } } )
		...
		if not managers.menu:is_pc_controller() then
			...
			local legend_text = self._panel:text( { font=tweak_data.menu.pd2_small_font, font_size=tweak_data.menu.pd2_small_font_size, text=t_text, halign="right", valign="top" } )
			local _, _, lw, lh = legend_text:text_rect()
			legend_text:set_size( lw, lh )
			legend_text:set_righttop( self._panel:w() - 5, 10 )
		end
	end
	-- self._scroll_panel:set_debug(true)
...
function DescriptionItem:update( t, dt )
	...
	if desc_text:h() > self._scroll_panel:h() and self._scroll_speed then
		...
		--[[
		if desc_text:bottom() > self._scroll_panel:h() then
			show_scroll_line_top = true
		end
		if desc_text:top() < 0 then
			show_scroll_line_bottom = true
		end]]
...
function DescriptionItem:mouse_pressed( button, x, y )
	...
	if( inside == false ) then		-- incorrect button pressed
		return false
	end
...
function AssetsItem:mouse_pressed( button, x, y )
	...
	--[[
	for i, asset in ipairs( self._assets_list ) do
		if( asset:inside( x, y ) and asset:visible() ) then
			return i
		end
	end
	]]
...
function AssetsItem:move_left()
	...
	if( self._asset_selected and self._assets_list[ self._asset_selected ] ) then
		...
		self._asset_selected = self._last_asset_selected or 0
	end
	...
	while( not found_asset ) do
		asset_selected = math.max( asset_selected - 1, 1 )
		-- if( self._asset_selected <= 0 ) then
		-- 	self._asset_selected = #self._assets_list
		-- end
		found_asset = (self._assets_list[ asset_selected ] and self._assets_list[ asset_selected ]:visible()) and true or false
		...
		if( set_gone > #self._assets_list ) then
			print( "ALL ASSETS HIDDEN" )
			break
		end
	end
	
	if( found_asset ) then
		self:select_asset( self._asset_selected )
		
		-- self._assets_list[ self._asset_selected ]:stop()
		-- self._assets_list[ self._asset_selected ]:animate( self.animate_select )
		-- self._asset_text:set_text( managers.localization:text( self._assets_names[self._asset_selected][2] ) )
	end]]
end
...
function AssetsItem:move_right()
	...
	local asset_selected = self._asset_selected
	while( not found_asset ) do
		asset_selected = math.min( asset_selected + 1, #self._assets_list )
		-- if( self._asset_selected > #self._assets_list ) then
		-- 	self._asset_selected = 1
		-- end
		found_asset = (self._assets_list[ asset_selected ] and self._assets_list[ asset_selected ]:visible()) and true or false
	...
	for i=1, 2 do
		...
		if table.size(perks) > 0 then
			...
			for perk in pairs( perks ) do
				...
				if DB:has( Idstring( "texture" ), texture ) then
					local perk_object = self._panel:bitmap( { texture=texture, w=16, h=16, rotation=math.random(2)-1.5, alpha=0.8, layer=2 } )
...
function LoadoutItem:mouse_moved( x, y )
	...
	-- self._panel:child("wip"):set_color( Color.white:with_alpha( self._panel:child("wip"):inside( x, y ) and 0.6 or 1 ) )
end
...
function LoadoutItem:open_node( node )
	...
	if( node == 1 ) then
		...
		managers.menu_component:post_event( "menu_enter" )
		managers.menu:open_node( "loadout", { self:create_primaries_loadout() } )
	elseif( node == 2 ) then
		...
		managers.menu_component:post_event( "menu_enter" )
		managers.menu:open_node( "loadout", { self:create_secondaries_loadout() } )
	elseif( node == 3 ) then
		managers.menu_component:post_event( "menu_enter" )
		managers.menu:open_node( "loadout", { self:create_melee_weapon_loadout() } )
	elseif( node == 4 ) then
		managers.menu_component:post_event( "menu_enter" )
		managers.menu:open_node( "loadout", { self:create_armor_loadout() } )
	elseif( node == 5 ) then
		managers.menu_component:post_event( "menu_enter" )
		managers.menu:open_node( "loadout", { self:create_deployable_loadout() } )
	end
...
function LoadoutItem:populate_armors( data )
	...
	for armor_id, armor_data in pairs( tweak_data.blackmarket.armors ) do
		...
		if bm_data.owned then
			...
			if( not new_data.equipped ) then table.insert( new_data, "a_equip" ) end
			data[index] = new_data
		end
	end
...
function LoadoutItem:populate_deployables( data )
	...
	for i, deployable in ipairs( deployables ) do
		...
		new_data.stream = false
		-- table.insert( new_data, "btn_mod" )
		if( not new_data.equipped ) then table.insert( new_data, "lo_d_equip" ) end
...
function LoadoutItem:create_deployable_loadout()
	local data = {}
	-- table.insert( data, { name="bm_menu_deployables", category="deployables", on_create_func=callback(self, self, "populate_deployables"), identifier=Idstring( "deployable" ) } )
...
function LoadoutItem:create_melee_weapon_loadout()
	local data = {}
	table.insert( data, { name="bm_menu_melee_weapons", category="melee_weapons", on_create_func_name="populate_melee_weapons", override_slots={ 3, 2 }, identifier=Idstring("melee_weapon") } )
	-- table.insert( data, { name="bm_menu_melee_weapons", category="melee_weapons", on_create_func=callback(self, self, "populate_melee_weapons") } )
	data.topic_id = "menu_loadout_blackmarket"
	data.topic_params = { category=managers.localization:text( "bm_menu_melee_weapons" ) }
...
function LoadoutItem:create_armor_loadout()
	...
	-- table.insert( data, { name="bm_menu_armors", category="armors", on_create_func=callback(self, self, "populate_armors") } )
	data.topic_id = "menu_loadout_blackmarket"
...
function LoadoutItem.animate_select(o, center_helper, instant)
	-- o:set_color( tweak_data.screen_colors.button_stage_2 )
...
function LoadoutItem.animate_deselect(o, center_helper, instant)
	-- o:set_color( tweak_data.screen_colors.button_stage_3 )
	-- o:set_blend_mode("add")
...
function TeamLoadoutItem:init( panel, text, i )
	...
	for i=1, 4 do
		...
		slot_panel = self._panel:panel( { x=old_right, y=0, w=quarter_width, h=self._panel:h(), valign="grow" } )
		-- slot_panel:set_debug(true)
...
function TeamLoadoutItem:reduce_to_small_font()
	...
	for i=1, 4 do
		if self._player_slots[i].box then
			self._player_slots[i].box:create_sides( self._player_slots[i].panel, { sides = { 1, 1, 1, 1 } } )
		end
	end
end
...
function TeamLoadoutItem:set_slot_outfit( slot, criminal_name, outfit )
	...
	-- Application:debug( slot, inspect(outfit) )
	...
	local slot_color = tweak_data.chat_colors[ slot ] -- managers.criminals:character_color_id_by_name( managers.criminals:character_workname_by_peer_id( slot ) ) ]
	local criminal_text = player_slot.panel:text( { font_size=tweak_data.menu.pd2_small_font_size, font=tweak_data.menu.pd2_small_font, color=slot_color, x=10, y=10, text=utf8.to_upper(CriminalsManager.convert_old_to_new_character_workname(criminal_name) or criminal_name) } )
	...
	if outfit.primary.factory_id then
		...
		local primary_bitmap = player_slot.panel:bitmap( { texture=texture, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
		...
		if table.size(perks) > 0 then
			...
			for perk in pairs( perks ) do
				local texture = "guis/textures/pd2/blackmarket/inv_mod_"..perk
				if DB:has( Idstring( "texture" ), texture ) then
					local perk_object = player_slot.panel:bitmap( { texture=texture, w=16, h=16, rotation=math.random(2)-1.5, alpha=0.8 } )
	...
	if outfit.secondary.factory_id then
		...
		local texture = guis_catalog .. "textures/pd2/blackmarket/icons/weapons/" .. texture_name
		local secondary_bitmap = player_slot.panel:bitmap( { texture=texture, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
	...
	if outfit.melee_weapon then
		...
		if outfit.melee_weapon == "weapon" then
			if primary_texture and secondary_texture then
				local primary = player_slot.panel:bitmap( { texture = primary_texture, w = w * 0.75, h = h * 0.75, rotation = math.random( 2 ) - 21.5, alpha = 0.8 } )
				aspect = primary:texture_width() / math.max( 1, primary:texture_height() )
				primary:set_w( primary:h() * aspect )
				
				local secondary = player_slot.panel:bitmap( { texture = secondary_texture, w = w * 0.75, h = h * 0.75, rotation = math.random( 2 ) - 21.5, alpha = 0.8 } )
			...
			local melee_weapon_bitmap = player_slot.panel:bitmap( { texture = guis_catalog .. "textures/pd2/blackmarket/icons/melee_weapons/" .. outfit.melee_weapon, w = w, h = h, rotation = math.random( 2 ) - 1.5, alpha = 0.8 } )
	...
	if outfit.armor then
		...
		local armor_bitmap = player_slot.panel:bitmap( { texture=guis_catalog.."textures/pd2/blackmarket/icons/armors/"..outfit.armor, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
	...
	if outfit.deployable and outfit.deployable ~= "nil" then
		...
		local deployable_bitmap = player_slot.panel:bitmap( { texture=guis_catalog.."textures/pd2/blackmarket/icons/deployables/"..outfit.deployable, w=w, h=h, rotation=math.random(2)-1.5, alpha=0.8 } )
		...
		if deployable_amount > 1 then
			local deployable_text = player_slot.panel:text( { text = "x"..tostring( deployable_amount ), font_size = tweak_data.menu.pd2_small_font_size, font = tweak_data.menu.pd2_small_font, rotation = deployable_bitmap:rotation(), color = tweak_data.screen_colors.text } )
...
function NewLoadoutItem:init( panel, columns, rows, x, y, params )
	...
	self._panel = panel:panel( { w = w - 2, h = h - 2, x = ( x - 1 ) * w + 1, y = ( y - 1 ) * h + 1 } )
	self._info_panel = self._panel:panel( { h = font_size } )
	self._item_panel = self._panel:panel( { h = math.max( 0, self._panel:h() - self._info_panel:h() ) } )
	...
	self._info_text = self._info_panel:text( { font = font, font_size = font_size, visible = false, align = "center", rotation = 360 } )
	...
	if params then
		...
		if params.item_texture and DB:has( Idstring( "texture" ), params.item_texture ) then
			self._item_image = self._item_panel:bitmap( { texture = params.item_texture, layer = 1 } )
		...
		elseif params.dual_texture_1 and params.dual_texture_2 then
			if DB:has( Idstring( "texture" ), params.dual_texture_1 ) then
				self._item_image1 = self._item_panel:bitmap( { texture = params.dual_texture_1, layer = 1 } )
			...
			if DB:has( Idstring( "texture" ), params.dual_texture_2 ) then
				self._item_image2 = self._item_panel:bitmap( { texture = params.dual_texture_2, layer = 1 } )
		...
		if params.info_icons then
			...
			for index, icon in ipairs( params.info_icons ) do
				local texture = icon.texture
				if DB:has( Idstring( "texture" ), texture ) then
					local object = self._info_icon_panel:bitmap( { texture = texture, w = 16, h = 16, alpha = icon.equipped and 1 or 0.25, layer = 1 } )
...
function NewLoadoutTab:init( panel, text, i, menu_component_data )
	...
	local primary = { item_texture = primary_texture, info_text = primary_string, info_icons = primary_perks }
	local secondary = { item_texture = secondary_texture, info_text = secondary_string, info_icons = secondary_perks }
	local melee_weapon = { item_texture = melee_weapon_texture, info_text = melee_weapon_string, dual_texture_1 = primary_texture, dual_texture_2 = secondary_texture }
	...
	local items = { primary, secondary, melee_weapon, armor, deployable }
...
function MissionBriefingGui:init( saferect_ws, fullrect_ws, node )
	...
	self._panel = self._safe_workspace:panel():panel( { w=self._safe_workspace:panel():w()/2, layer=6 } )
	self._panel:set_right( self._safe_workspace:panel():w() )
	self._panel:set_top( 160 + 25 + tweak_data.menu.pd2_medium_font_size ) -- self._safe_workspace:panel():h()/2 )
	...
	-- <option icon="guis/textures/menu_tickbox" value="off" x="0" y="0" w="24" h="24" s_icon="guis/textures/menu_tickbox" s_x="0" s_y="24" s_w="24" s_h="24" />
	-- <option icon="guis/textures/menu_tickbox" value="on" x="24" y="0" w="24" h="24" s_icon="guis/textures/menu_tickbox" s_x="24" s_y="24" s_w="24" s_h="24" />
		
	self._ready_button = self._panel:text( { name="ready_button", text=ready_text, align="right", vertical="center", font_size=tweak_data.menu.pd2_large_font_size, font=tweak_data.menu.pd2_large_font, color=tweak_data.screen_colors.button_stage_3, layer=1, blend_mode="add" } )
	local _, _, w, h = self._ready_button:text_rect()
	self._ready_button:set_size( w, h )
	
	if not managers.menu:is_pc_controller() then
		-- self._ready_button:set_color( tweak_data.screen_colors.text )
	end
	
	self._ready_tick_box = self._panel:bitmap( { name="ready_tickbox", texture="guis/textures/pd2/mission_briefing/gui_tickbox", layer=1 } )
	...
	local big_text = self._fullscreen_panel:text( { name="ready_big_text", text=ready_text, h=90, align="right", vertical="bottom", font_size=tweak_data.menu.pd2_massive_font_size, font=tweak_data.menu.pd2_massive_font, color=tweak_data.screen_colors.button_stage_3, alpha=0.4 } )
	...
	-- self._wallet = WalletGuiObject:new( self._safe_workspace:panel() )
	...
	local loadout = 
	{
		{ primary_texture, primary_string, true, primary_perks },
		{ secondary_texture, secondary_string, true, secondary_perks },
		{ armor_texture, armor_string, true },
		{ deployable_texture, deployable_string, true },
	}
	]]
	...
	-- new_data.bitmap_texture = "guis/textures/pd2/blackmarket/icons/weapons/" .. (category=="primaries" and "primary" or "secondary") .. "_" .. tostring(crafted.weapon_id)
	...
	if not managers.menu:is_pc_controller() then
		local prev_page = self._panel:text( { name="tab_text_0", y=0, w=0, h=tweak_data.menu.pd2_medium_font_size, font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, layer=2, text=managers.localization:get_default_macro("BTN_BOTTOM_L"), vertical="top" } )
	...
	if not managers.menu:is_pc_controller() then
		local next_page = self._panel:text( { name="tab_text_"..tostring(#self._items+1), y=0, w=0, h=tweak_data.menu.pd2_medium_font_size, font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, layer=2, text=managers.localization:get_default_macro("BTN_BOTTOM_R"), vertical="top" } )
	...
	if self._items[ #self._items ]._tab_text:right() > max_x then
		for i, tab in ipairs( self._items ) do
			tab:reduce_to_small_font()
		end
	end
	-- local tab_h = self._items[self._selected_item]._tab_select_rect:h()
...
function MissionBriefingGui:ready_text()
	...
	return legend .. utf8.to_upper(managers.localization:text( "menu_waiting_is_ready" )) -- (self._ready and utf8.to_upper(managers.localization:text( "menu_waiting_is_ready" )) or utf8.to_upper(managers.localization:text( "menu_click_when_ready" )))
end
...
function MissionBriefingGui:create_asset_tab()
	...
	local assets_names = {}
	if( #asset_ids > 0 ) then
		self._fullscreen_asset_background_h = self._fullscreen_panel:gradient( { name="asset_background_h", layer=99, orientation="horizontal", color=Color.black:with_alpha(0.1) } )
		...
		self._fullscreen_asset_background_v = self._fullscreen_panel:gradient( { name="asset_background_v", layer=99, orientation="vertical", color=Color.black:with_alpha(0.1) } )
		...
		for i, asset_id in ipairs( asset_ids ) do
			-- if( managers.assets:get_asset_visible_by_id(asset_id) ) then
				local asset_tweak_data = managers.assets:get_asset_tweak_data_by_id( asset_id )
				assets_names[i] = { managers.assets:get_asset_texture( asset_id ), asset_tweak_data.name_id, managers.assets:get_asset_unlocked_by_id(asset_id), asset_id, managers.assets:get_asset_can_unlock_by_id( asset_id ), managers.assets:get_asset_no_mystery_by_id(asset_id) }
				
				local asset = self._fullscreen_panel:bitmap( { name="asset_"..tostring(i), texture=assets_names[i][1], w=512, h=512, layer = 100 } )
...
function MissionBriefingGui:open_asset( asset_index )
	...
	if( fullscreen_asset and alive(fullscreen_asset) ) then
		local animate_show = function(o)
			...
			over(0.1, function(p) o:set_alpha( math.lerp(start_alpha,1,p) ) end )
		end
		...
		-- fullscreen_asset:set_size( fullscreen_asset:texture_width() * self._fullscreen_asset_zoom, fullscreen_asset:texture_height() * self._fullscreen_asset_zoom )
		-- fullscreen_asset:set_center( cx, cy )
...
function MissionBriefingGui:close_asset()
	...
	if( fullscreen_asset and alive(fullscreen_asset) ) then
		local animate_hide = function(o)
			local start_alpha = o:alpha()
			over(0.05, function(p) o:set_alpha( math.lerp(start_alpha,0,p) ) end )
			o:hide()
		end
...
function MissionBriefingGui:next_tab( no_sound )
	...
	-- if( new_selected_item > #self._items ) then
	-- 	new_selected_item = 1
	-- end
...
function MissionBriefingGui:prev_tab( no_sound )
	local new_selected_item = self._selected_item - 1
	-- if( new_selected_item <= 0 ) then
	-- 	new_selected_item = #self._items
	-- end
...
function MissionBriefingGui:mouse_pressed( button, x, y )
	...
	if button == Idstring( "mouse wheel down" ) then
		-- if self._items[self._selected_item]:panel():inside( x, y ) then
	...
	for index, tab in ipairs(self._items) do
		...
		if( pressed == true ) then		-- inside tab selection item
			self:set_tab( index )
...
--[[
function MissionBriefingGui:_setup_controller_input()
...
function MissionBriefingGui:_destroy_controller_unput()
...
function MissionBriefingGui:_axis_move( o, axis_name, axis_vector, controller )
