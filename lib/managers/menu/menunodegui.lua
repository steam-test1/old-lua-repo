...
function MenuNodeGui:init( node, layer, parameters )
	...
	lang_mods[ Idstring("german"):key()] = 
											name == "kit" and 0.95
											or name == "challenges_active" and 0.875
											or name == "challenges_awarded" and 0.875
											or 0.9
	lang_mods[ Idstring("french"):key()] = 
											name == "kit" and 0.975
											or name == "challenges_active" and 0.874
											or name == "challenges_awarded" and 0.874	
											or name == "upgrades_support" and 0.875
											or 0.9
	...
	lang_mods[ Idstring("spanish"):key()] = 
											name == "kit" and 1
											or name == "challenges_active" and 0.84
											or name == "challenges_awarded" and 0.84
											or (name == "upgrades_assault"
											or name == "upgrades_sharpshooter"
											or name == "upgrades_support") and 0.975
											or 0.925
	lang_mods[ Idstring("english"):key()] = 
											name == "challenges_active" and 0.925
											or name == "challenges_awarded" and 0.925
											or 1
	...
	self._no_menu_wrapper = true -- node:parameters().no_menu_wrapper
...
function MenuNodeGui:_setup_panels( node )
	...
	self._topic_panel2 = self.ws:panel():panel( { visible = not self._no_menu_wrapper, x = safe_rect_pixels.x, y = safe_rect_pixels.y, w = safe_rect_pixels.width, h = tweak_data.load_level.upper_saferect_border } )
		
	self._topic_line = self._topic_panel2:bitmap( { texture = "guis/textures/headershadow", layer = self.layers.items, color = Color.white, w = safe_rect_pixels.width - self:_mid_align(), y = 0 } )
	self._topic_text2 = self._topic_panel2:text( { text = node:parameters().topic_id and managers.localization:text( node:parameters().topic_id ) or "Topic",
												x = 0, y = 0, w = safe_rect_pixels.width,
												-- font_size = self.font_size, -- self.topic_font_size,
												font_size = tweak_data.menu.topic_font_size,
												align="right", halign="right", vertical="bottom", valign="bottom",
												font = self.font,
	...
	self._items_bottom_line = self._item_panel_parent:bitmap( { visible = not self._no_menu_wrapper, texture = "guis/textures/headershadowdown", layer = self.layers.items, color = Color.white, w = safe_rect_pixels.width - self:_mid_align(), y = 0 } )
	...
	local base = 20
	local height = 15
	self._list_arrows = {} -- Positioned further down
	
	local aw = safe_rect_pixels.width - self:_mid_align() -- self._align_data.panel:world_center()
	self._list_arrows.up = self.ws:panel():bitmap( { texture="guis/textures/scroll_up", visible = false, x = self:_mid_align(), y = 0, w = aw, layer = self.layers.items + 2, color = self.row_item_color:with_alpha( 1 ) } )
	self._list_arrows.down = self.ws:panel():bitmap( { texture="guis/textures/scroll_down", visible = false, x = self:_mid_align(), y = 0, w = aw, layer = self.layers.items + 2, color = self.row_item_color:with_alpha( 1 ) } )
			
	self._info_bg_rect = self.safe_rect_panel:rect( { visible = false, x = 0, y = tweak_data.load_level.upper_saferect_border, w = safe_rect_pixels.width*0.41, h = safe_rect_pixels.height - tweak_data.load_level.upper_saferect_border*2, layer = self.layers.first, color = Color( 0.5, 0,0,0 ) } )
...
function MenuNodeGui:_create_legends( node )
	...
	self._legends_panel = self.ws:panel():panel( { visible = visible, x = safe_rect_pixels.x, y = safe_rect_pixels.y, w = safe_rect_pixels.width, h = safe_rect_pixels.height } )
	...
	local text = self._legends_panel:text( { text = t_text,
									font = self.font,
...
function MenuNodeGui:_create_align( node )
	self._align_data = {}
	self._align_data.panel = self._item_panel_parent:panel( { x = self:_left_align(), y = 0, w = self._align_line_padding*2, h = self._item_panel_parent:h(), layer = self.layers.marker } )
	-- self._align_data.rect = self._align_data.panel:rect( { x = self._align_data.panel:w()/2, y = 0, w = 2, h = self._align_data.panel:h(), layer = 0, color = self.row_item_color:with_alpha( 1 ) } )
end

function MenuNodeGui:_create_marker( node )
	self._marker_data = {}
	self._marker_data.marker = self.item_panel:panel( { x = 0, y = 0, w = 1280, h = 10, layer = self.layers.marker } )
	-- self._marker_data.marker:set_debug( true )
	-- self._marker_data.gradient = self._marker_data.marker:gradient ( { x = self._marker_data.marker:w()/2, y = 0, w = 100, h = 10, layer = 0, gradient_points = { 0, self.row_item_color, 1 , self.row_item_color:with_alpha( 0 ) } } )	
	-- self._marker_data.gradient = self._marker_data.marker:gradient ( { x = 0, y = 0, h = 10, layer = 0, gradient_points = { 0, tweak_data.menu.highlight_background_color_left, 1 ,  tweak_data.menu.highlight_background_color_right } } )
	self._marker_data.gradient = self._marker_data.marker:bitmap ( { texture = "guis/textures/menu_selected", x = 0, y = 0, layer = 0, blend_mode="add", alpha=self.marker_alpha or 0.6 } )
...
function MenuNodeGui:_setup_item_panel_parent( safe_rect, shape )
	...
	-- self._item_panel_parent:set_shape( x, y, w, h/2.2 )
	self._item_panel_parent:set_shape( x, y, w, h )
	self._align_data.panel:set_h( self._item_panel_parent:h() )
	-- self._align_data.rect:set_h( self._align_data.panel:h() )
 	-- self._list_arrows.up:set_h( 20 * tweak_data.scale.menu_arrow_padding_multiplier )
 	self._list_arrows.up:set_h( 64 *(24/32) * tweak_data.scale.menu_arrow_padding_multiplier )
	...
	self._list_arrows.down:set_h( 64 *(24/32) * tweak_data.scale.menu_arrow_padding_multiplier )
	...
	self._item_panel_parent:set_h( self._item_panel_parent:h() - 0*2 * tweak_data.scale.menu_arrow_padding_multiplier )
end
...
function MenuNodeGui:_setup_item_panel( safe_rect, res )
	if self._align == "mid" and false then
		...
		if self:_item_panel_height() < self._item_panel_parent:h() then
			...
			-- self._item_panel_y.target = self.item_panel:y() + self:_item_panel_height()/2
		end
		-- end
		-- else
			-- self._item_panel_y = nil
		-- end
...
function MenuNodeGui:_set_topic_position()
	-- self._topic_panel2:set_debug( true )
	self._topic_panel2:set_right( self.item_panel:right() )
	-- self._topic_text2:set_right( self.item_panel:right() )
	...
	self._items_bottom_line:set_y( self.item_panel:bottom() ) --  + self._items_bottom_line:h() )
	self._items_bottom_line:set_right( self.item_panel:right() )
	-- self._topic_text2:set_bottom( bottom )
end

function MenuNodeGui:_create_menu_item( row_item )
	local safe_rect = self:_scaled_size() -- managers.viewport:get_safe_rect_pixels()
	local align_x = safe_rect.width * self._align_line_proportions -- + self._align_line_padding
	...
	if row_item.item:parameters().back then
		...
		row_item.selected = row_item.gui_panel:bitmap( { visible = false, texture = "guis/textures/menu_selected", x = 0, y = 0, valign = nil, halign = nil, h = row_item.gui_panel:h(), w = row_item.gui_panel:w(), layer = 0 } )
		row_item.shadow = row_item.gui_panel:bitmap( { visible = false, texture = "guis/textures/headershadowdown", layer = self.layers.items } )
		row_item.shadow_bottom = row_item.gui_panel:bitmap( { visible = false, texture = "guis/textures/headershadowdown", rotation = 180, layer = self.layers.items } )
		row_item.arrow_selected = row_item.gui_panel:bitmap( { blend_mode = "add", visible = false, texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, x = 0, y = 0, valign = nil, halign = nil, h = row_item.gui_panel:w(), w = row_item.gui_panel:w(), layer = self.layers.items } )
		row_item.arrow_unselected = row_item.gui_panel:bitmap( { blend_mode = "add", visible = true, texture = "guis/textures/menu_arrows", texture_rect = {24, 0, 24, 24}, x = 0, y = 0, valign = nil, halign = nil, h = row_item.gui_panel:w(), w = row_item.gui_panel:w(), layer = self.layers.items } )
		-- row_item.shadow:set_debug( true )
		-- row_item.gui_panel:set_debug( true )
	elseif row_item.item:parameters().pd2_corner then
		row_item.gui_panel = self._item_panel_parent:panel( { layer = self.layers.items, w = 3, h = 3 } )	-- let the core have its own fun with this one
		-- row_item.gui_panel = self.ws:panel():panel( { layer = self.layers.items } )
		...
		if( MenuBackdropGUI and managers.menu:is_pc_controller() ) then
			local bg_text = row_item.gui_pd2_panel:text( { text=utf8.to_upper( row_item.text ), h=90, align="right", vertical="bottom", font_size=tweak_data.menu.pd2_massive_font_size, font=tweak_data.menu.pd2_massive_font, color=tweak_data.screen_colors.button_stage_3, alpha=0.4, layer=-1, rotation=360 } )	-- rotation=360 equals no clipping, hax
...
--	elseif row_item.type == "toggle" then	-- Moved to CoreMenuItemToggle
	elseif row_item.type == "level" then -- Not really used in PD2 right now
		...
		row_item.gui_level_panel = self._item_panel_parent:panel( { visible = false, layer = self.layers.items, x = 0, y = 0, w = self:_left_align(), h = self._item_panel_parent:h() } )
		...
		--[[if level_data.movie and SystemInfo:platform() == Idstring( "WIN32" ) then
			row_item.level_movie = row_item.gui_level_panel:video( { visible = true, video = level_data.movie, loop = true } )
...
--	elseif row_item.type == "multi_choice" then -- Moved to MenuItemMultiChoice
	elseif row_item.type == "chat" then -- Only WIN32, but not used right now in PD2
		...
		-- row_item.gui_text:set_text( "TEST ME" )
		row_item.chat_output = row_item.gui_panel:gui( Idstring( "guis/chat/textscroll" ), { layer = self.layers.items, h = 120, w = 500, valign="grow", halign="grow" } )
		row_item.chat_input = row_item.gui_panel:gui( Idstring( "guis/chat/chat_input" ), { h = 25, w = 500, layer = self.layers.items, valign = "bottom", halign = "grow", y = 125 } )
		...
		--[[ row_item.chat_text = row_item.gui_panel:text( { text = "THIS IS A TEST\nIT IS A TEST", font = row_item.font, font_size = self.font_size, h = self.font_size, layer = self.layers.items, 
														...
														word_wrap = true, wrap = true } )]]
		self:_align_chat( row_item )
	elseif row_item.type == "friend" then -- Only PS3
		local cot_align = ( row_item.align == "right" and "left" ) or ( row_item.align == "left" and "right" ) or row_item.align
		row_item.gui_panel = self.item_panel:panel( { w = self.item_panel:w() } )
...
--	elseif row_item.type == "customize_controller" then -- Moved to MenuItemCustomizeController
	elseif row_item.type == "weapon_expand" or row_item.type == "weapon_upgrade_expand" then
		...
		if not row_item.item:setup_gui( self, row_item ) then
			-- MenuNodeGui.super._create_menu_item( self, row_item )
			local cot_align = ( row_item.align == "right" and "left" ) or ( row_item.align == "left" and "right" ) or row_item.align
			...
			if row_item.item:parameters().current then
				...
				row_item.current_of_total:set_color( row_item.color ) -- Color( 0.5, 1, 1, 1 ) )
			...
			if row_item.item:parameters().icon then
				row_item.icon = self.item_panel:bitmap( { texture=row_item.item:parameters().icon, rotation=row_item.item:parameters().icon_rotation or 360, visible=row_item.item:icon_visible(), layer=self.layers.items+1 } )
			end
			self:_align_normal( row_item )
		end
	end
	
	-- print( "row_item.item:parameters().is_expanded", row_item.item:name(), row_item.item:parameters().is_expanded )
	local visible = row_item.item:menu_unselected_visible( self, row_item ) and not row_item.item:parameters().back
	row_item.menu_unselected = self.item_panel:bitmap( { visible = false, texture = "guis/textures/menu_unselected", x = 0, y = 0, layer = self.layers.items - 2 } )
	row_item.menu_unselected:set_color( row_item.item:parameters().is_expanded and Color( 0.5, 0.5, 0.5) or Color.white )
	-- print( row_item.menu_unselected:color() )
end

function MenuNodeGui:_setup_trial_buy( row_item )
	local font_size = SystemInfo:language() == Idstring("italian") and 25 or 28
	row_item.row_item_color = Color( 1, 255/255, 168/255, 0 )
...
function MenuNodeGui:_create_info_panel( row_item )
	row_item.gui_info_panel = self.safe_rect_panel:panel( { visible = false, layer = self.layers.first, x = 0, y = 0, w = self:_left_align(), h = self._item_panel_parent:h() } )
	
	row_item.help_title = row_item.gui_info_panel:text( {
		x = 0, y = 0, align="left", halign="top", vertical="top",
		font_size = self.font_size,	font = row_item.font, color = Color.white,
		layer = self.layers.items, text = utf8.to_upper( row_item.text ),
	} )
	
	row_item.help_text = row_item.gui_info_panel:text( {
		x = 0, y = 0, align="left", halign="top", vertical="top",
		font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = Color.white,
		layer = self.layers.items, text = utf8.to_upper( row_item.help_text ), wrap = true, word_wrap = true,
	} )
end
...
--	row_item.gui_info_panel:set_debug( true )
	...
	-- row_item.level_movie = row_item.gui_info_panel:video( { visible = false, video = movie_name, loop = true } )
	...
	row_item.level_title = row_item.gui_info_panel:text( {
					x = 0, y = 0, align="left", halign="top", vertical="top",
					font_size = self.font_size,	font = row_item.font, color = Color.white,
					layer = self.layers.items, text = string.upper( managers.localization:text( tweak_data.levels[ row_item.level_id ].name_id ) ),
				} )
	...
	MenuNodeGui.lobby_campaign = { level_id = Global.game_settings.level_id, gui_info_panel = row_item.gui_info_panel, level_movie = row_item.level_movie, level_title = row_item.level_title, level_briefing = row_item.level_briefing }
	
	--[[
	row_item.win_ratio_panel = row_item.gui_info_panel:panel(
		{
			visible = true, 
			layer = self.layers.items, 
			x = 0, y = 0,
	...
	local d = { "Easy", "Normal", "Hard", "Overkill", "Overkill 145+" }
	
	local h = tweak_data.menu.small_font_size
	local w = 200
	local p = 3
	for i, diff in ipairs( d ) do
		local ratio = math.random()

		row_item.win_ratio_panel:rect( { x = 0, y = (h + p) * (i - 1), w = w, h = h, layer = self.layers.items, color = Color.black } )
		row_item.win_ratio_panel:rect( { x = 0, y = (h + p) * (i - 1), w = w * ratio, h = h, layer = self.layers.items, color = tweak_data.menu.default_changeable_text_color } )


		row_item.level_title = row_item.win_ratio_panel:text( {
					x = p, y = (h + p) * (i - 1) - 2,
					w = w, h = h + 2, 
					align="left", halign="top", vertical="top",
					font_size = tweak_data.menu.small_font_size, font = tweak_data.menu.small_font, color = Color.white,
					layer = self.layers.items, text = string.upper( diff .. ": " .. string.format( "%.0f", ratio * 100) .. "%" ),
				} )
	end
	]]
	
	
--	row_item.win_ratio_panel:set_debug( true )
end
...
function MenuNodeGui:_align_lobby_campaign( row_item )
	...
	--[[
	if row_item.win_ratio_panel then
		row_item.win_ratio_panel:set_w( row_item.gui_info_panel:w() )
		row_item.win_ratio_panel:set_h( row_item.gui_info_panel:h() - row_item.level_briefing:bottom() )

		row_item.win_ratio_panel:set_x( 0 )
		row_item.win_ratio_panel:set_bottom( row_item.gui_info_panel:h() )
	end
	]]
	
	--[[
	row_item.win_ratio_panel:set_x( 0 )
	row_item.win_ratio_panel:set_bottom( row_item.gui_info_panel:bottom() )
	]]
end

function MenuNodeGui:_highlight_lobby_campaign( row_item )
	-- Detect change of level and set correct movie
...
function MenuNodeGui:_create_lobby_difficulty( row_item )
	...
	row_item.help_text = row_item.gui_info_panel:text( {
					x = 0, y = 0, align="left", halign="top", vertical="top",
					font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = Color.white,
					layer = self.layers.items, text = utf8.to_upper( managers.localization:text( "menu_difficulty_help" ) ), wrap = true, word_wrap = true,
				} )
	...
	row_item.difficulty_help_text = row_item.gui_info_panel:text( {
					...
					layer = self.layers.items, text = ut8f.to_upper( "sdsd" ), wrap = true, word_wrap = true,
				} )
				
	--[[ row_item.help_text = row_item.gui_info_panel:text( {
					x = 0, y = 0, align="left", halign="top", vertical="top",
					font = tweak_data.menu.small_font, font_size = tweak_data.menu.small_font_size, color = Color.white,
					layer = self.layers.items, text = string.upper( row_item.help_text ), wrap = true, word_wrap = true,
				} )
...
function MenuNodeGui:_align_lobby_difficulty( row_item )
	...
	row_item.difficulty_help_text:set_shape( row_item.difficulty_help_text:text_rect() )
	row_item.difficulty_help_text:set_x( 0 )
	row_item.difficulty_help_text:set_top( row_item.help_text:bottom() + tweak_data.menu.info_padding )]]
	-- row_item.difficulty_title
end
...
function MenuNodeGui:_align_lobby_difficulty_help_text( row_item )
	...
	-- row_item.difficulty_help_text:set_size( tw, th )
...
--[[
	print( "[MenuNodeGui:activate_customize_controller]" )
	...
	local f = function( o, key ) self:_key_press( o, key, "keyboard", item ) end
	row_item.controller_binding:set_text( "_" )
	row_item.controller_binding:key_press( f )
	local f = function( o, key ) self:_key_press( o, key, "mouse", item ) end
	row_item.controller_binding:mouse_click( f ) 

	local f = function( index, key ) self:_key_press( row_item.controller_binding, key, "mouse", item, true ) end
...
function MenuNodeGui:_key_press( o, key, input_id, item, no_add )
	...
	-- Special case handling for mouse 0, since it slips through
	...
	-- local forbidden_btns = {"esc", "enter", "tab","num abnt c1", "num abnt c2" ,"@","ax","convert","kana","kanji","no convert","oem 102","stop","unlabeled","yen", "mouse 8", "mouse 9", "f1", "f2", ""}
	...
	if item:parameters().axis then
		...
		managers.controller:set_user_mod( item:parameters().connection_name, { axis = item:parameters().axis, button = item:parameters().button, connection = key_name } ) 
		...
		managers.controller:set_user_mod( item:parameters().connection_name, { button = item:parameters().button, connection = key_name, controller_id = input_id } )
		item:parameters().binding = key_name
	end
...
function MenuNodeGui:_end_customize_controller( o, item )
	...
	setup:add_end_frame_clbk( function() self._listening_to_input = false end )
...
function MenuNodeGui:_cb_chat( row_item )
	...
	row_item.chat_input:child("text"):set_text("")	-- Clear input line.
	row_item.chat_input:child("text"):set_selection(0,0)	
end

function MenuNodeGui:sync_say( message, row_item, id )
	-- local row_item = self:row_item_by_name( "chat" )
...
function MenuNodeGui:_text_item_part( row_item, panel, align_x, text_align )
	return panel:text( {
			font_size = self.font_size,
			x = align_x, y = 0,
			align=text_align or row_item.align or "left", halign="left", vertical="center",
			font = row_item.font,
...
function MenuNodeGui:scroll_update( dt )
	...
	if math.round( self.item_panel:world_y() ) - math.round( self._item_panel_parent:world_y() ) < 0 then
		...
		self._list_arrows.up:set_color( self._list_arrows.up:color():with_alpha( 0.0 ) )
		-- self._list_arrows.up:set_visible( false )
	end
	...
	if math.round( self.item_panel:world_bottom() ) - self._item_panel_parent:world_bottom() < 4 then
		...
		-- self._list_arrows.down:set_visible( false )
	else
...
function MenuNodeGui:reload_item( item )
	...
	if not item:reload( row_item, self ) then
		...
		elseif type == "expand" then
			self:_reload_expand( item )
	-- 	elseif type == "multi_choice" then 			-- Moved to MenuItemMultiChoice
		elseif type == "friend" then
			self:_reload_friend( item )
	--	elseif type == "upgrade" then  				-- Moved to MenuItemUpgrade
	--	elseif type == "customize_controller" then  -- Moved to MenuItemCustomizeController
		else
...
function MenuNodeGui:_collaps_others( my_item )
	for _,row_item in ipairs( self.row_items ) do
		...
		if my_item ~= item then
			if type == "expand" or type == "weapon_expand" or type == "weapon_upgrade_expand" then
				if not item:is_parent_to_item( my_item ) then
...
function MenuNodeGui:_reload_expand( item )
	-- print( "MenuNodeGui:_reload_expand", item:expanded() )
	local row_item = self:row_item( item )
	-- print( inspect( item._items ) )
	-- print( row_item.node_gui )
	
	item:reload( row_item, self )
	--[[if item:expanded() then
		row_item.menu_unselected:set_color( Color( 1, 0.5, 0 ) )
	else
		row_item.menu_unselected:set_color( Color.white )
	end]]
	
	--[[local type = item:type()
	if type == "weapon_expand" then
		if not item:parameter( "unlocked" ) then
			return
		end
	end]]
	...
	--[[if item:can_expand() then
		print( item:name(), inspect( item._items ) )
		for i,eitem in ipairs( item._items ) do
			-- if eitem:visible() then
				--eitem:parameters().is_expanded = true
				--eitem:parameters().expand_value = item:expand_value() + (item:parameters().is_expanded and item:parameters().expand_value or 0)
				if item:expanded() then
					need_repos = true
				
					-- Insert correct in data node
					...
					-- Insert correct in gui node
					
					for j,ritem in pairs( self.row_items ) do
						if ritem == row_item then
							self:_insert_row_item( eitem, row_item.node, j+i )
						end
					end
					
					-- self:row_item( eitem ).is_expanded = true
				else
					-- print( "delete item", eitem:name() )
					local type = eitem:type()
					if type == "expand" or type == "weapon_expand" or type == "weapon_upgrade_expand" then
						if eitem:expanded() then
							eitem:toggle()
							self:_reload_expand( eitem )
						end
						-- eitem:set_expand( false )
						-- self:_reload_expand( item )
					end
					if row_item.node:delete_item( eitem:name() ) then
						need_repos = true
					end
					self:_delete_row_item( eitem )
				end
			--end
		end
	end]]
	...
	if need_repos then
		...
		-- managers.menu:active_menu().logic:select_item( item:name(), false )
...
function MenuNodeGui:_setup_level_size( row_item )
	...
	-- row_item.level_image:set_shape( 0, 0, row_item.gui_level_panel:w()/2,row_item.gui_level_panel:w()/2 )
	-- row_item.level_image:set_position( 0, 0 )
	-- local p = (row_item.level_image:texture_width() == 0 and 1) or row_item.level_image:texture_height()/row_item.level_image:texture_width()
	local w = row_item.gui_level_panel:w() - padding * 2
	-- row_item.level_image:set_center_x( row_item.gui_level_panel:w()/2 )
	-- row_item.level_image:set_bottom( self._item_panel_parent:h() - padding )
	-- row_item.level_text:set_shape( row_item.gui_level_panel:w()/2, 0, row_item.gui_level_panel:w()/2,row_item.gui_level_panel:w()/2 )
	row_item.level_title:set_shape( padding, 24, w, row_item.gui_level_panel:w() )
	row_item.level_text:set_shape( padding, 34 + 32, w, row_item.gui_level_panel:w())
	...
	--[[if row_item.level_movie then
		...
		row_item.level_movie:set_bottom( self._item_panel_parent:h() - 24 )
	end]]
end

--[[
function MenuNodeGui:_set_item_positions()
	MenuNodeGui.super._set_item_positions( self )
	
	for _,row_item in pairs( self.row_items ) do
		if( row_item.item:parameters().pd2_corner ) then
			row_item.gui_panel:set_rightbottom( self.ws:panel():w(), self.ws:panel():h() )
		end
	end
end
]]
...
function MenuNodeGui:_highlight_row_item( row_item, mouse_over )
	if row_item then
		...
		--[[if row_item.item:parameters().is_expanded then
			row_item.gui_panel:set_h( row_item.gui_panel:h()*2)
			-- self:_setup_size()
			-- self:scroll_setup()
			self:_set_item_positions()
			-- self:reload_item( row_item.item )
		end]]
	...
	--	if row_item.type == "slider" then  		-- Moved to CoreMenuItemSlider
	--	elseif row_item.type == "toggle" then 	-- Moved to CoreMenuItemToggle
	...
	--	elseif row_item.type == "customize_controller" then -- Moved to MenuItemCustomizeController
		...
		elseif( row_item.item:parameters().pd2_corner ) then
			...
			if not row_item.item:highlight_row_item( self, row_item, mouse_over ) then
				if row_item.gui_panel.set_text then
					row_item.gui_panel:set_font( (row_item.font and Idstring(row_item.font)) or tweak_data.menu.pd2_medium_font_id ) -- row_item.gui_panel:set_font( tweak_data.menu.default_font_no_outline_id )
				end
...
function MenuNodeGui:_align_marker( row_item )
	...
	if( row_item.item:parameters().pd2_corner ) then
		...
		-- self._marker_data.marker:set_height( row_item.gui_text:height() )
	...
	-- self._marker_data.marker:set_center_x( self:_mid_align() )
	...
	-- (row_item.item:parameters().is_expanded and 10 or 0)
	...
	-- Set color based on item enabled state	
	local item_enabled = row_item.item:enabled()
	if item_enabled then
		-- self._marker_data.gradient:set_gradient_points( { 0, tweak_data.menu.highlight_background_color_left, 1 ,  tweak_data.menu.highlight_background_color_right } )
	else
		-- self._marker_data.gradient:set_gradient_points( { 0, Color( 1, 0.33, 0.33, 0.33 ), 1 , Color( 1, 0.33, 0.33, 0.33 ) } )
	end
	
	-- Hide marker if it is the back button 
	...
	--		self._marker_data.back_marker:rect( { visible = false, x = 0, y = 0, w = 10, h = 10, layer = 1 } )
			self._marker_data.back_marker:gradient ( { x = 0, y = 0, h = row_item.gui_panel:height(), layer = 0, gradient_points = { 0, tweak_data.menu.highlight_background_color_left, 1 ,  tweak_data.menu.highlight_background_color_right } } )
		end
		...
		if self._marker_data.back_marker then
			...
			-- self._marker_data.back_marker = nil
		end
	end
	...
	elseif row_item.type == "nothing"  --[[row_item.type == "slider" or row_item.type == "kitslot" or row_item.type == "multi_choice" or row_item.type == "toggle"]] then
		...
		-- self._marker_data.gradient:set_gradient_points( { 0, self.row_item_color:with_alpha( 0 ), 0.5, self.row_item_color:with_alpha( 1 ), 1, self.row_item_color:with_alpha( 0 ) } )
		-- self._marker_data.gradient:set_gradient_points( { 0, Color.green:with_alpha( 1 ), 0.5, Color.red:with_alpha( 1 ), 1,  Color.blue:with_alpha( 0 ) } )
		-- self._marker_data.gradient:set_x( 0 )
	else
		-- self._marker_data.gradient:set_width( self._marker_data.marker:width()/2 )
		-- self._marker_data.gradient:set_gradient_points( { 0, self.row_item_color:with_alpha( 1 ), 1, self.row_item_color:with_alpha( 0 ), } )
		-- self._marker_data.gradient:set_x( self._marker_data.marker:width()/2 )
	end
...
function MenuNodeGui:_fade_row_item( row_item )
	...
	--	elseif row_item.type == "multi_choice" then	-- Moved to MenuItemMultiChoice
		elseif row_item.type == "chat" then
			-- row_item.gui_text:set_color( row_item.color )
			-- row_item.gui_panel:set_debug( false )
	...
	--	elseif row_item.type == "customize_controller" then -- Moved to MenuItemCustomizeController
		...
		elseif row_item.item:parameters().back then
			-- row_item.selected:set_visible( false )
		...
		elseif( row_item.item:parameters().pd2_corner ) then
			...
			if not row_item.item:fade_row_item( self, row_item ) then
				if row_item.gui_panel.set_text then
...
function MenuNodeGui:_align_item_gui_info_panel( panel )
	panel:set_shape( 	self._info_bg_rect:x() + tweak_data.menu.info_padding,  
...
function MenuNodeGui:_align_chat( row_item )
	...
	--[[row_item.input_text:set_bottom( row_item.gui_panel:h() )
	row_item.input_text:set_w( 300 )
	row_item.input_text:set_right( self:_left_align() )
	row_item.input_text:set_text( "" )]]
	...
	--[[local h = row_item.gui_panel:h() - row_item.input_text:h() - 4
	row_item.chat_text:set_w( 300 )
	row_item.chat_text:set_right( self:_left_align() )
	row_item.chat_text:set_h( h )
	row_item.chat_text:set_bottom( h )]]
...
function MenuNodeGui:close( ... )
	for _,row_item in ipairs( self.row_items ) do
		...
		--[[elseif row_item.level_movie then
			if alive( row_item.level_movie ) then 
				managers.video:remove_video( row_item.level_movie )
