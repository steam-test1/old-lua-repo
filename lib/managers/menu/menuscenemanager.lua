...
function MenuSceneManager:update(t, dt)
	...
	if self._fade_down_lights then
		local (for generator), (for state), (for control) = ipairs(self._fade_down_lights)
		do
			do break end
...
function MenuSceneManager:set_lobby_character_visible(i, visible, no_state)
	...
	do
		local (for generator), (for state), (for control) = ipairs(mask_unit:children())
		do
			do break end
...
function MenuSceneManager:_delete_character_mask(owner)
	...
	if alive(old_mask_unit) then
		do
			local (for generator), (for state), (for control) = ipairs(old_mask_unit:children())
			do
				do break end
				linked_unit:unlink()
...
function MenuSceneManager:remove_item()
	if alive(self._item_unit) then
		do
			local (for generator), (for state), (for control) = ipairs(self._item_unit:children())
			do
				do break end
...
function MenuSceneManager:_set_weapon_upgrades(weapon_id)
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.upgrades.visual.upgrade[weapon_id].objs)
		do
			do break end
	...
	local (for generator), (for state), (for control) = pairs(weapon_upgrades)
	do
		do break end
		if upgrade_data.attached then
			...
			if objs then
				local (for generator), (for state), (for control) = pairs(objs)
				do
					do break end
...
function MenuSceneManager:pre_unload()
	self._mask_names = {}
	do
		local (for generator), (for state), (for control) = pairs(self._mask_units)
		do
			do break end
	...
	do
		do break end
		local (for generator), (for state), (for control) = pairs(weapon_units)
		do
			do break end
...
function MenuSceneManager:unload()
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._mask_names)
		do
			do break end
