...
function BlackMarketGuiTabItem:init(main_panel, data, node, size_data, hide_select_rect, scroll_tab_table)
	...
	local max_y_index = 0
	do
		local (for generator), (for state), (for control) = ipairs(self._data)
		do
			do break end
	...
	(for control) = self._tab_panel:size() and BlackMarketGuiSlotItem
...
function BlackMarketGuiTabItem:destroy()
	local (for generator), (for state), (for control) = ipairs(self._slots)
	do
		do break end
		slot:destroy()
	end
...
function BlackMarketGuiTabItem:check_new_drop(first_time)
	...
	(for control) = nil and slot._data
...
12
	...
	if data.extra_bitmaps then
		local color, shape
		local (for generator), (for state), (for control) = ipairs(data.extra_bitmaps)
		do
			do break end
	...
	(for control) = custom_name_text.set_right and DB
	...
	if data.mini_icons then
		...
		do
			local (for generator), (for state), (for control) = ipairs(data.mini_icons)
			do
				do break end
	...
	if red_box then
		if self._bitmap then
			...
			do
				local (for generator), (for state), (for control) = pairs(self._extra_textures)
				do
					do break end
			...
			(for control) = Color.white:with_alpha(0.6) and bitmap.set_color
...
function BlackMarketGuiSlotItem:destroy()
	if self._data and self._data.mini_icons then
		local (for generator), (for state), (for control) = ipairs(self._data.mini_icons)
		do
			do break end
	...
	(for control) = nil and icon_data.stream
...
function BlackMarketGuiSlotItem:texture_loaded_clbk(texture_idstring)
	...
	if self._bitmap then
		...
		(for control) = "VertexColorTextured" and bitmap.set_color
		self._bitmap = self._panel:bitmap({
			name = "item_texture",
			texture = texture_idstring,
			blend_mode = "normal",
	...
	local shape
	do
		local (for generator), (for state), (for control) = ipairs(self._extra_textures)
		do
			do break end
	...
	(for control) = self._panel and self._data
	if self._post_load_alpha then
		...
		self._post_load_blend_mode = Color.white:with_alpha(self._post_load_alpha) and nil
	end
...
function BlackMarketGuiSlotItem:set_highlight(highlight, instant)
	if self._bitmap and not self._loading_texture then
		if highlight then
			...
			local animate_deselect = instant and function(o, panel, instant, width, height)
				local w = o:w()
				local h = o:h()
...
function BlackMarketGuiMaskSlotItem:init(main_panel, data, x, y, w, h)
	...
	if not current_match_with_true then
		...
		(for control) = Color.white:with_alpha(0.3) and bitmap.set_color
		if self._loading_texture then
...
function BlackMarketGuiMaskSlotItem:set_highlight(highlight, instant)
	if self._bitmap and not self._loading_texture then
		if highlight then
			...
			local animate_deselect = self._panel and function(o, panel, instant, width, height)
				local w = o:w()
				local h = o:h()
...
7
	...
	if self._btn_text_legends then
		local legend_string = ""
		do
			local (for generator), (for state), (for control) = ipairs(self._btn_text_legends)
			do
				do break end
				if i > 1 then
...
function BlackMarketGui:_setup(is_start_page, component_data)
	...
	(for control) = top_padding + 1 and data.on_create_func_name
	...
	if self._tabs[self._selected] then
		...
		local btn_x = 10
		do
			local (for generator), (for state), (for control) = pairs(BTNS)
			do
				do break end
		...
		self._info_texts, (for control) = {}, 8 and BlackMarketGuiButtonItem
		...
		self._info_texts_bg = {}
		do
			local (for generator), (for state), (for control) = ipairs(self._info_texts)
			do
				do break end
		...
		local longest_text_w = self._info_texts_panel:text({
			name = "info_text_5",
			font_size = small_font_size,
			font = small_font,
			layer = 1,
		...
		if self._data.info_callback then
			...
			do
				local (for generator), (for state), (for control) = ipairs(info_table)
				do
					do break end
			...
			(for control) = self._btn_panel:w() and info.name
			local (for generator), (for state), (for control) = ipairs(self._info_panel:children())
			do
				do break end
			...
			(for control) = self._info_panel:children() and string
			self._stats_shown = {
			...
			do
				local (for generator), (for state), (for control) = ipairs(self._stats_shown)
				do
					do break end
					...
					local (for generator), (for state), (for control) = ipairs(text_columns)
					do
						do break end
				...
				(for control) = Color.black:with_alpha(0.3) and panel.panel
			end
			...
			do
				...
				local (for generator), (for state), (for control) = ipairs(self._armor_stats_shown)
				do
					do break end
			...
			do
				...
				local (for generator), (for state), (for control) = ipairs(self._mweapon_stats_shown)
				do
					do break end
		...
		do
			local (for generator), (for state), (for control) = ipairs(self._tabs)
			do
				do break end
		...
		(for control) = prev_page:text_rect() and "set_tab_position"
		if not managers.menu:is_pc_controller() and #self._tabs > 1 then
	...
	if is_start_page then
		...
		local (for generator), (for state), (for control) = ipairs(new_givens)
		do
			do break end
			...
			if type(params.value) == "table" then
				local (for generator), (for state), (for control) = ipairs(params.value)
				do
					do break end
				...
				(for control) = unlocked_item[2] and "category"
				managers.menu:show_new_item_gained(params)
			end
...
function BlackMarketGui:_get_skill_stats(name, category, slot, base_stats, mods_stats, silencer, single_mod, auto_mod)
	local skill_stats = {}
	do
		local (for generator), (for state), (for control) = pairs(self._stats_shown)
		do
			do break end
...
function BlackMarketGui:_get_mods_stats(name, base_stats, equipped_mods)
	...
	do
		...
		do
			local (for generator), (for state), (for control) = ipairs(equipped_mods)
			do
				do break end
			...
			(for control) = nil and tweak_factory[mod]
		end

		local index
		(for control) = mods_stats[stat.name] and tweak_factory[mod]
		local (for generator), (for state), (for control) = pairs(self._stats_shown)
		do
			do break end
...
function BlackMarketGui:_get_melee_weapon_stats(name)
	...
	(for control) = 1 and stat.num_decimals
	do
		local (for generator), (for state), (for control) = ipairs(multiple_of)
		do
			do break end
	...
	do
		...
		do
			...
			if stat.name == "armor" then
				...
				skill_stats[stat.name] = {
					value = math.round(base_stats[stat.name].value * (managers.player:body_armor_skill_multiplier() - 1))
				}
			elseif stat.name == "concealment" then
				...
				base_stats[stat.name] = {
					value = math.round(base + mod)
				}
				skill_stats[stat.name] = {
					value = math.round(managers.blackmarket:concealment_modifier())
				}
			elseif stat.name == "movement" then
				...
				base_stats[stat.name] = {
					value = math.round(movement_penalty * base)
				}
...
function BlackMarketGui:_get_weapon_mod_stats(mod_name, weapon_name, base_stats, mods_stats, equipped_mods)
	...
	mod_stats.chosen.name = nil and mod_name
	...
	local index = nil and nil
	do
		local (for generator), (for state), (for control) = pairs(mod_stats)
		do
			do break end
			local (for generator), (for state), (for control) = pairs(self._stats_shown)
			do
...
function BlackMarketGui:set_stats_titles(...)
	...
	local (for generator), (for state), (for control) = ipairs(stat_title_changes)
	do
		do break end
...
18
	...
	if tweak_data.weapon[self._slot_data.name] then
		...
		if slot ~= equipped_slot then
			do
				local (for generator), (for state), (for control) = pairs(self._stats_titles)
				do
					do break end
					title:hide()
				end
		...
		if tweak_data.blackmarket.armors[self._slot_data.name] then
			...
			(for control) = {
				...
				alpha = 1,
				x = 120
			} and self._armor_stats_texts
			if tweak_data.blackmarket.melee_weapons[self._slot_data.name] then
				self:hide_armor_stats()
				self:hide_weapon_stats()
				...
				if self._slot_data.name ~= equipped_item then
					...
					(for control) = ipairs(self._armor_stats_shown) and title.hide
					self:set_stats_titles({name = "total", show = true}, {
						name = "equip",
						show = true,
						text_id = "bm_menu_equipped",
				...
				local equip, stat_changed = self._armor_stats_texts[stat.name].equip and nil, nil
				...
				(for control) = slot and title.hide
				...
				do
					...
					do
						local (for generator), (for state), (for control) = pairs(self._stats_texts[stat.name])
						do
							do break end
					...
					(for control) = utf8.to_upper(managers.localization:text("bm_menu_" .. stat.name)) and column.set_alpha
	...
	(for control) = utf8.to_upper(managers.localization:text("bm_menu_equipped")) and self._stats_texts
	local modslist_panel = self._stats_panel:child("modslist_panel")
	local y = 0
	if self._rweapon_stats_panel:visible() then
		local (for generator), (for state), (for control) = ipairs(self._rweapon_stats_panel:children())
		do
			do break end
		...
		(for control) = self._rweapon_stats_panel:children() and math
		if self._armor_stats_panel:visible() then
			local (for generator), (for state), (for control) = ipairs(self._armor_stats_panel:children())
			do
				do break end
				y = math.max(y, child:bottom())
			end
			...
			(for control) = self._armor_stats_panel:children() and math
			if self._mweapon_stats_panel:visible() then
				local (for generator), (for state), (for control) = ipairs(self._mweapon_stats_panel:children())
				do
					do break end
					y = math.max(y, child:bottom())
				end
	...
	(for control) = self._mweapon_stats_panel:children() and math
	...
	if identifier == self.identifiers.weapon then
		...
		if not slot_data.empty_slot then
			...
			if not slot_data.unlocked then
				...
				local dlc_based = tweak_data.lootdrop.global_values[slot_data.global_value] and tweak_data.lootdrop.global_values[slot_data.global_value].dlc and not managers.dlc:has_dlc(slot_data.global_value)
...
##]] .. managers.localization:to_upper_text("menu_bm_overwrite", {
		...
		if not managers.blackmarket:can_finish_customize_mask() then
			...
			local missed_mods = {}
			do
				local (for generator), (for state), (for control) = ipairs(mask_mod_info)
				do
					do break end
	...
	if self._desc_mini_icons then
		local (for generator), (for state), (for control) = pairs(self._desc_mini_icons)
		do
			do break end
	...
	self._desc_mini_icons = slot_data.name .. "_desc" and {}
	local desc_mini_icons = self._slot_data.desc_mini_icons
	local info_box_panel = self._panel:child("info_box_panel")
	if desc_mini_icons and 0 < table.size(desc_mini_icons) then
		do
			local (for generator), (for state), (for control) = pairs(desc_mini_icons)
			do
				do break end
				local new_icon = self._panel:bitmap({
		...
		(for control) = self._panel.remove and self._panel
	...
	(for control) = tweak_data.screen_colors.text and self.set_info_text
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._desc_mini_icons)
		do
			do break end
	...
	do break end
	if self._rename_info_text and self._rename_caret then
...
function BlackMarketGui:set_info_text(id, new_string, resource_color)
	...
	info_text:set_blend_mode("normal")
	...
	if resource_color then
		...
		(for control) = utf8.len(info_text:text()) and Idstring
...
function BlackMarketGui:_round_everything()
	...
	(for control) = self._panel:children() and self._rec_round_object
	if alive(self._fullscreen_panel) then
		local (for generator), (for state), (for control) = ipairs(self._fullscreen_panel:children())
		do
			do break end
			self:_rec_round_object(d)
		end
...
11
	...
	if update_select then
		local (for generator), (for state), (for control) = pairs(self._btns)
		do
			do break end
...
function BlackMarketGui:set_tab_positions()
	...
	(for control) = nil and tab.set_tab_position
...
function BlackMarketGui:show_btns(slot)
	...
	(for control) = nil and self._btns
	if not managers.menu:is_pc_controller() then
	...
	do
		local (for generator), (for state), (for control) = ipairs(btns)
		do
			do break end
	...
	local num_btns = #btns
	(for control) = back_btn and managers
...
function BlackMarketGui:populate_weapon_category(category, data)
	...
	if not last_weapon then
		...
		do
			local (for generator), (for state), (for control) = pairs(crafted_category)
			do
				do break end
		...
		last_unlocked_weapon = true or true
	end
	...
	do
		...
		do
			...
			local new_parts = {}
			do
				local (for generator), (for state), (for control) = pairs(managers.blackmarket:get_weapon_new_part_drops(crafted.factory_id) or {})
				do
					do break end
			...
			(for control) = crafted.factory_id and tweak_data
			...
			new_data.mini_icons = {}
			do
				local (for generator), (for state), (for control) = pairs(icon_list)
				do
					do break end
			...
			(for control) = crafted.factory_id and table
...
function BlackMarketGui:populate_melee_weapons(data)
	...
	table.sort(sort_data, function(x, y)
		...
		if l_tweak_data[x_gv].category then
		...
		if l_tweak_data[y_gv].category then
	...
	do
		local (for generator), (for state), (for control) = ipairs(sort_data)
		do
			do break end
	...
	(for control) = id and melee_weapon_data[1]
	for i = 1, max_items do
		if not data[i] then
			new_data = {}
			new_data.name = "empty"
...
function BlackMarketGui:populate_deployables(data)
	...
	local sort_data = {}
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.blackmarket.deployables)
		do
			do break end
			table.insert(sort_data, {id, d})
		end
	...
	(for control) = sort_data and "guis/"
	for i = 1, max_items do
		if not data[i] then
			new_data = {}
			new_data.name = "empty"
...
function BlackMarketGui:populate_masks(data)
	...
	do
		...
		do
			...
			if not is_locked then
				local (for generator), (for state), (for control) = pairs(crafted.blueprint)
				do
					do break end
	...
	(for control) = nil and i - start_crafted_item
	for i = 1, max_items do
		if not data[i] then
			index = i + start_crafted_item - 1
...
function BlackMarketGui:populate_armors(data)
	...
	do
		...
		(for control) = d.name_id and tweak_data
	end
	...
	(for control) = ipairs(data.upgrades) and armor_data[1]
...
function BlackMarketGui:populate_mod_types(data)
	...
	local guis_catalog = "guis/"
	do
		local (for generator), (for state), (for control) = pairs(data.on_create_data)
		do
			do break end
			new_data = {}
			new_data.name = type
...
function BlackMarketGui:populate_mods(data)
	...
	do
		...
		(for control) = small_font and mod.name
	end
...
8
	...
	do
		...
		do
			...
			do
				local (for generator), (for state), (for control) = ipairs(mask_blueprint)
				do
					do break end
			...
			(for control) = tostring(type) and data.name
			new_data.stream = true or ipairs(mask_blueprint)
...
function BlackMarketGui:_cleanup_blackmarket()
	...
	local (for generator), (for state), (for control) = pairs(blackmarket_inventory)
	do
		do break end
		local (for generator), (for state), (for control) = pairs(gv_table)
		do
			do break end
			if type_id ~= "weapon_mods" then
				local item_data = blackmarket_tweak_data[type_id]
				if item_data then
					local (for generator), (for state), (for control) = pairs(type_table)
					do
						do break end
		...
		(for control) = nil and item_data[item_id]
	end
...
function BlackMarketGui:choose_weapon_mods_callback(data)
	...
	local new_node_data = {}
	local sort_mods = nil and {}
	do
		local (for generator), (for state), (for control) = pairs(mods)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = ipairs(sort_mods)
		do
			do break end
			do
...
function BlackMarketGui:choose_weapon_buy_callback(data)
	...
	do
		local (for generator), (for state), (for control) = ipairs(blackmarket_items)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(item_categories)
		do
			do break end
	...
	local item_data
	do
		local (for generator), (for state), (for control) = ipairs(sorted_categories)
		do
			do break end
	...
	new_node_data.topic_id = tweak_data.weapon[item.weapon_id] and "bm_menu_buy_weapon_title"
	new_node_data.topic_params = {
...
9
	...
	do
		local (for generator), (for state), (for control) = ipairs(masks)
		do
			do break end
	...
	(for control) = nil and func_add_item
	do
		...
		do
			...
			if mask_id ~= "character_locked" and (mask.pc or mask.pcs) then
				...
				if mask.dlc or mask.dlcs then
					...
					do
						local (for generator), (for state), (for control) = ipairs(mask.dlcs or {})
						do
							do break end
					...
					local add_dlc = false
					local dlc_tweak, global_value_tweak, global_value = nil and nil, nil, nil
					...
					(for control) = dlc and mask.global_value
					if mask.global_value then
				...
				local (for generator), (for state), (for control) = ipairs(global_values)
				do
					do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(items)
		do
			do break end
	...
	local item_data
	do
		local (for generator), (for state), (for control) = ipairs(global_value_sort)
		do
			do break end
			item_data = {}
			do
				local (for generator), (for state), (for control) = ipairs(items[global_value])
				do
					do break end
	...
	new_node_data.topic_params, (for control) = {}, mask.global_value and {}
	new_node_data.blur_fade = self._data.blur_fade
	managers.menu:open_node(self._inception_node_name, {new_node_data})
end
...
function BlackMarketGui:show_available_mask_mods_callback(data)
	...
	do
		...
		do
			...
			do
				local (for generator), (for state), (for control) = ipairs(mods)
				do
					do break end
	...
	local params = {}
	params.color_table = "materials" and color_table
...
function BlackMarketGui:show_available_mods_callback(data)
	...
	local params = {}
	params.color_table = table.sort and color_table
	params.weapon_id = data.name
	params.text_block = text_block
...
function BlackMarketGui:destroy()
	local (for generator), (for state), (for control) = pairs(self._tabs)
	do
		do break end
		tab:destroy()
	end
