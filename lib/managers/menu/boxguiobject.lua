...
function BoxGuiObject:create_sides( panel, config )
	...
	self._panel = panel:panel( { halign = "grow", valign = "grow", layer=1 } )
...
function BoxGuiObject:_create_side( panel, side, type )
	...
	local side_panel = panel:panel( { name = side, w = w, h = h, halign = "grow", valign = "grow" } )
	...
	elseif type == 1 then
		...
		if left_or_right then
			...
			-- one:set_texture_coordinates( Vector3( x, y + 2, 0 ), Vector3( x, y, 0 ), Vector3( x + th, y + 2, 0 ), Vector3( x + th, y, 0 ) )
			...
			mvector3.set_static( mvector_bl, x+th, y+tw, 0 )
			mvector3.set_static( mvector_br, x+th, y, 0 )
			-- two:set_texture_coordinates( Vector3( x, y + 2, 0 ), Vector3( x, y, 0 ), Vector3( x + th, y + 2, 0 ), Vector3( x + th, y, 0 ) )
			one:set_texture_coordinates( mvector_tl, mvector_tr, mvector_bl, mvector_br )
			...
			-- one:set_texture_coordinates( Vector3( x, y, 0 ), Vector3( x + tw, y, 0 ), Vector3( x, y + 2, 0 ), Vector3( x + tw, y + 2, 0 ) )
			one:set_texture_coordinates( mvector_tl, mvector_tr, mvector_bl, mvector_br )
			...
			mvector3.set_static( mvector_bl, x, y+th, 0 )
			mvector3.set_static( mvector_br, x+tw, y+th, 0 )
			-- two:set_texture_coordinates( Vector3( x, y, 0 ), Vector3( x + tw, y, 0 ), Vector3( x, y + 2, 0 ), Vector3( x + tw, y + 2, 0 ) )
			one:set_texture_coordinates( mvector_tl, mvector_tr, mvector_bl, mvector_br )
	...
	elseif type == 2 then
		...
		if left_or_right then
			...
			-- full:set_texture_coordinates( Vector3( y, x + w, 0 ), Vector3( y, x, 0 ), Vector3( y + h, x + w, 0 ), Vector3( y + h, x, 0 ) )
			...
			-- full:set_texture_coordinates( Vector3( x, y, 0 ), Vector3( x + w, y, 0 ), Vector3( x, y + h, 0 ), Vector3( x + w, y + h, 0 ) )
			full:set_texture_coordinates( mvector_tl, mvector_tr, mvector_bl, mvector_br )
		end
	else
