...
function MenuInput:mouse_moved( o, x, y, mouse_ws )
	...
	if node_gui then
		...
		for _,row_item in pairs( node_gui.row_items ) do
			if( row_item.item:parameters().pd2_corner ) then		-- row_item.gui_panel is not used here
...
function MenuInput:mouse_pressed( o, button, x, y )
	...
	--[[ Test scroll in menu with mouse wheel
	if button == Idstring( "mouse wheel down" ) then
		local node_gui = managers.menu:active_menu().renderer:active_node_gui()
		if not node_gui then
			return
		end
		node_gui:wheel_scroll_start( -1 )
	...
	if button == Idstring( "0" ) then
		local node_gui = managers.menu:active_menu().renderer:active_node_gui()
		if not node_gui then -- Had a freak crash reported that I couldn't reproduce on this /Martin
			return
		end
		-- x, y = self:_modified_mouse_pos( x, y )
		
		--[[ Test detect mouse over legend
		if node_gui._legends_panel then
			local text = node_gui._legends_panel:child( 0 )
			if text then
		...
		for _,row_item in pairs( node_gui.row_items ) do
			...
			elseif row_item.gui_panel:inside( x, y ) then
				if row_item.type == "slider" then							-- Is slider
					...
					elseif row_item.gui_slider:inside( x, y ) then 				-- Slider bar
						local where = ((x-row_item.gui_slider:world_left())/(row_item.gui_slider:world_right()-row_item.gui_slider:world_left()) )
						local item = self._logic:selected_item()
						item:set_value_by_percentage( where*100 )
						self._logic:trigger_item( true, item )
				...
				elseif row_item.type == "chat" then
					local item = self._logic:selected_item()
					-- if row_item.input_text:inside( x, y ) then
...
function MenuInput:update( t, dt )
	...
	if( not MenuInput.super.update( self, t, dt ) and self._accept_input ) then		-- core input hijacked, running our own and send them to renderer -> menu component
		...
		if self._page_timer <= 0 then
			...
			if( self._accept_input and self._controller and self._controller:get_input_pressed( "confirm" ) ) then
				if managers.menu:active_menu().renderer:confirm_pressed() then
					managers.menu:active_menu().renderer:disable_input( 0.2 )
				end
			end
			
			if( self._accept_input and self._controller and self._controller:get_input_pressed( "back" ) ) then
				if managers.menu:active_menu().renderer:back_pressed() then
					managers.menu:active_menu().renderer:disable_input( 0.2 )
				end
			end
			
			if( self._accept_input and self._controller and self._controller:get_input_pressed( "cancel" ) ) then
				if managers.menu:active_menu().renderer:back_pressed() then
					managers.menu:active_menu().renderer:disable_input( 0.2 )
				end
			end
			...
			local special_btns = { "menu_respec_tree", "menu_modify_item", "menu_preview_item", "menu_remove_item", "menu_preview_item_alt", "menu_toggle_legends", "menu_toggle_filters", "menu_toggle_ready" }
			
			if self._controller then
				for _, button in ipairs( special_btns ) do
					if self._accept_input and self._controller:get_input_pressed( button ) then
			...
			-- <button name="menu_special_1" input="y"/>
			-- <button name="menu_special_2" input="x"/>
			-- <button name="menu_special_3" input="right_thumb"/>
			
			--[[
			if( self._controller and self._controller:get_input_pressed( "menu_special_1" ) ) then
				managers.menu:active_menu().renderer:special_btn_pressed( Idstring("1") )
			end
			
			if( self._controller and self._controller:get_input_pressed( "menu_special_2" ) ) then
				managers.menu:active_menu().renderer:special_btn_pressed( Idstring("2") )
			end
			
			if( self._controller and self._controller:get_input_pressed( "menu_special_3" ) ) then
				managers.menu:active_menu().renderer:special_btn_pressed( Idstring("3") )
			end
			
			if( self._controller and self._controller:get_input_pressed( "toggle_legends" ) ) then
				managers.menu:active_menu().renderer:special_btn_pressed( Idstring("legend") )
			end
			if( self._controller and self._controller:get_input_pressed( "toggle_filters" ) ) then
				managers.menu:active_menu().renderer:special_btn_pressed( Idstring("filter") )
			end]]
		end
	else	-- haxor to enable button in crimenet contract gui
		if( self._controller and self._controller:get_input_pressed( "menu_toggle_voice_message" ) and managers.menu:active_menu().renderer.special_btn_pressed ) then
			managers.menu:active_menu().renderer:special_btn_pressed( Idstring("voice_message") )
		end
	end
	
		-- NOTE: this will currently cause mouse_move to be updated once every frame, may cause performance issues (with exception when mouse pointer is inactive or mouse is still and arrow key/conirm button been pressed, but then it will start again when mouse is moved
...
function MenuInput:menu_up_pressed()
	return MenuInput.super.menu_up_pressed( self ) or 
	( self._axis_status.y == self.AXIS_STATUS_PRESSED and self:menu_axis_move().y > 0 )
end
...
function MenuInput:menu_down_pressed()
	return MenuInput.super.menu_down_pressed( self ) or 
	( self._axis_status.y == self.AXIS_STATUS_PRESSED and self:menu_axis_move().y < 0 )
end
...
-- Left. Added check for axis.
function MenuInput:menu_left_input_bool()
	return MenuInput.super.menu_left_input_bool( self ) or 
	( self:menu_axis_move().x < -self._move_axis_limit )
end
function MenuInput:menu_left_pressed()
	return MenuInput.super.menu_left_pressed( self ) or 
	( self._axis_status.x == self.AXIS_STATUS_PRESSED and self:menu_axis_move().x < 0 )
end
...
-- Right. Added check for axis.
function MenuInput:menu_right_input_bool()
	return MenuInput.super.menu_right_input_bool( self ) or 
	( self:menu_axis_move().x > self._move_axis_limit )
end
function MenuInput:menu_right_pressed()
	return MenuInput.super.menu_right_pressed( self ) or 
	( self._axis_status.x == self.AXIS_STATUS_PRESSED and self:menu_axis_move().x > 0 )
end
...
function MenuInput:_update_axis_status()
	...
	-- self:_update_axis_scroll_status()
end
