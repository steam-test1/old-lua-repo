...
function ImageBoxGui:_create_image_box( image_config )
	...
	if image_shapes then
		for _, shape in pairs ( image_shapes ) do
			...
			local new_shape = nil
			if type == "rect" then
				new_shape = image_panel:rect( { color = shape.color or Color.white, w = shape.width or shape.w or 0, h = shape.height or shape.h or 0, layer = shape.layer or 0 } )
			elseif type == "bitmap" then
				new_shape = image_panel:bitmap( { texture = shape.texture, color = shape.color or Color.white, w = shape.width or shape.w or 0, h = shape.height or shape.h or 0, layer = shape.layer or 0 } )
			end
