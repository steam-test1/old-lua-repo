...
function MenuBackdropGUI:init( ws, gui_data_manager, fixed_dt )
	...
	if( not ws ) then
		...
		self._black_bg_ws:panel():rect( { name="bg", color=Color.black, layer=-1000, halign="scale", valign="scale" } )
...
function MenuBackdropGUI:_create_particle()
	...
	local particle = particles_layer:bitmap( { w=32, h=32, texture=self._bitmap_texture, texture_rect={texture_rect_x, texture_rect_y, 32, 32}, alpha = 0 } )
	...
	local particle_animation = function( o, self )
		...
		over( 0.2, function( p ) o:set_alpha( math.lerp( 0, start_alpha, p ) ) end, self._fixed_dt)
		while true do
