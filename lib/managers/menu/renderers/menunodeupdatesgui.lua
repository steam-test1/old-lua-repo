...
function MenuNodeUpdatesGui:setup()	
	...
	panel:text( { text = title_text, align = "left", vertical = "top", font_size = tweak_data.menu.pd2_large_font_size, font = tweak_data.menu.pd2_large_font, color = tweak_data.screen_colors.text } )
	
	local back_button = panel:text( { name = "back_button", text = managers.localization:to_upper_text( "menu_back" ), align = "right", vertical = "bottom", font_size = tweak_data.menu.pd2_large_font_size, font = tweak_data.menu.pd2_large_font, color = tweak_data.screen_colors.button_stage_3 } )
	self:make_fine_text( back_button )
	back_button:set_right( panel:w() )
	back_button:set_bottom( panel:h() )
	back_button:set_visible( managers.menu:is_pc_controller() )
	if MenuBackdropGUI then
		local bg_text = panel:text( { text = title_text, align = "left", vertical = "top", font_size = tweak_data.menu.pd2_massive_font_size, font = tweak_data.menu.pd2_massive_font, color = tweak_data.screen_colors.button_stage_3, alpha = 0.4, layer = -1, rotation = 360 } )
		...
		if managers.menu:is_pc_controller() then
			local bg_back = panel:text( { text = managers.localization:to_upper_text( "menu_back" ), align = "right", vertical = "bottom", font_size = tweak_data.menu.pd2_massive_font_size, font = tweak_data.menu.pd2_massive_font, color = tweak_data.screen_colors.button_stage_3, alpha = 0.4, layer = -1, rotation = 360 } )
	...
	local latest_update_panel = panel:panel( { name = "lastest_content_update", w = panel:w() / 2, h = panel:w() / 4, x = 0, y = 70 } )
	local selected = BoxGuiObject:new( latest_update_panel, { sides = { 2, 2, 2, 2 } } )
	BoxGuiObject:new( latest_update_panel, { sides = { 1, 1, 1, 1 } } )
	...
	table.insert( self._requested_textures, { count = texture_count, texture = latest_update.image } )
	...
	local latest_desc_panel = panel:panel( { name = "latest_description", w = panel:w() - latest_update_panel:w() - 30, h = latest_update_panel:h(), x = latest_update_panel:right() + 30, y = latest_update_panel:top() } )
	BoxGuiObject:new( latest_desc_panel, { sides = { 1, 1, 1, 1 } } )
	...
	local title_text = latest_desc_panel:text( { name = "title_text", text = title_string, font = tweak_data.menu.pd2_large_font, font_size = tweak_data.menu.pd2_large_font_size, color = tweak_data.screen_colors.text, x = self.PADDING, y = self.PADDING } )
	local date_text = latest_desc_panel:text( { name = "date_text", text = date_string, font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text, x = self.PADDING } )
	local desc_text = latest_desc_panel:text( { name = "desc_text", text = desc_string, font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = tweak_data.screen_colors.text, x = self.PADDING, wrap = true, word_wrap = true } )
	...
	local previous_updates_panel = panel:panel( { name = "previous_content_updates", w = w, h = small_width / 2 + self.PADDING * 2, x = 0, y = latest_update_panel:bottom() + 40 } )
	
	
	local previous_update_text = panel:text( { name = "previous_update_text", text = managers.localization:to_upper_text( "menu_content_updates_previous" ), font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text } )
	...
	for index, data in ipairs( previous_updates ) do
		...
		local content_panel = previous_updates_panel:panel( { name = data.id, w = w, h = h, x = x, y = y } )
		...
		table.insert( self._requested_textures, { texture_count = texture_count, texture = data.image } )
		
		local text = panel:text( { name = data.name_id, text = managers.localization:to_upper_text( data.name_id ), font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = tweak_data.screen_colors.text } )
		...
		local selected = BoxGuiObject:new( content_panel, { sides = { 2, 2, 2, 2 } } )
		self._selects[ data.id ] = selected
	end
	BoxGuiObject:new( previous_updates_panel, { sides = { 1, 1, 1, 1 } } )
	...
	if #content_updates > num_previous_updates then
		local num_pages = self._num_pages
		self._prev_page = panel:panel( { name = "previous_page", w = tweak_data.menu.pd2_medium_font_size, h = tweak_data.menu.pd2_medium_font_size } )
		self._next_page = panel:panel( { name = "next_page", w = tweak_data.menu.pd2_medium_font_size, h = tweak_data.menu.pd2_medium_font_size } )
		
		local prev_text = self._prev_page:text( { name = "text_obj", text = managers.menu:is_pc_controller() and "<" or managers.localization:get_default_macro( "BTN_BOTTOM_L" ), font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, align = "center", vertical = "center" } )
		local next_text = self._next_page:text( { name = "text_obj", text = managers.menu:is_pc_controller() and ">" or managers.localization:get_default_macro( "BTN_BOTTOM_R" ), font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, align = "center", vertical = "center" } )
		
		local page_text = panel:text( { text = tostring( current_page ) .. "/" .. tostring( num_pages ), font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text, align = "center", vertical = "center" } )
...
function MenuNodeUpdatesGui:texture_done_clbk( panel, texture_ids )
	panel:bitmap( { name = "texture", texture = texture_ids, w = panel:w(), h = panel:h() } )
end
