...
function MenuNodeCrimenetCasinoGui:_set_cards( amount, card )
	...
	local height
	local width = math.round( (128/180) * self._betting_cards_panel:h() )
	...
	if amount == 0 then
		...
		x_offset = math.round( (128/180) * height / 2 )
	...
	for i = 1, 3 do
		...
		self._betting_cards[i]:set_w( math.round( (128/180) * height ) )
		...
		self._betting_cards[i]:set_visible( count > 0 )
		count = count - 1
	end
end
...
function MenuNodeCrimenetCasinoGui:_setup_layout()
	...
	local text_title = self._panel:text( { text = managers.localization:to_upper_text( "menu_casino_title_main" ), font_size = large_font_size, font = large_font, color = tweak_data.screen_colors.text, blend_mode = "add" } )
	local _, _, w, h = text_title:text_rect()
	
	self._main_panel = self._panel:panel( { x = 0, y = h, w = self._panel:w(), h = self._panel:h() - h } )
	
	
	
	local text_betting = self._main_panel:text( { text = managers.localization:to_upper_text( "menu_casino_title_betting" ), font_size = medium_font_size, font = medium_font, color = tweak_data.screen_colors.text, blend_mode = "add" } )
	text_betting:set_position( start_x, 0 )
	local _, _, _, h = text_betting:text_rect()
	text_betting:set_h( h )
	
	self._betting_panel = self._main_panel:panel( { y = text_betting:bottom(), w = width, h = height, layer = 1 } )
	self._betting_panel:set_x( text_betting:x() )
	BoxGuiObject:new( self._betting_panel, { sides = { 1, 1, 1, 1 } } )
	
	
	
	local text_options = self._main_panel:panel( { w = width - option_size, h = self.item_panel:h(), layer = 1 } )
	text_options:set_x( self._betting_panel:x() )
	text_options:set_y( self.item_panel:y() )
	
	local betting_titles = { {id = "prefer", text = "menu_casino_option_prefer_title"}, {id = "infamous", text = "menu_casino_option_infamous_title"}, {id = "safecards", text = "menu_casino_option_safecard_title"}, {skip = true}, {skip = true} }
	...
	for _, item in ipairs( self.row_items ) do
		if item.type ~= "divider" then
			if not betting_titles[i].skip then
				self._betting_titles[ betting_titles[i].id ] = text_options:text( { text=managers.localization:to_upper_text( betting_titles[i].text ), y=y, align="right", font_size=small_font_size, font=small_font, color=tweak_data.screen_colors.text, blend_mode="add" } )
			end
			i = i + 1
		end
	...
	self._betting_carddeck = { none = "downcard_overkill_deck", weapon_mods = "upcard_weapon", masks = "upcard_mask", materials = "upcard_material", textures = "upcard_pattern", colors = "upcard_color", cash = "upcard_cash", xp = "upcard_xp" }
	
	self._betting_cards_panel = self._betting_panel:panel( { x = content_offset, y = content_offset + 15, w = self._betting_panel:w() - content_offset*2, layer = 1 } )
	...
	for i = 1, 3 do
		self._betting_cards[i] = self._betting_cards_panel:bitmap( { name = "upcard", texture = texture, w = math.round( (128/180) * self._betting_cards_panel:h() ), h = self._betting_cards_panel:h(), blend_mode = "add", layer = 1, halign = "scale", valign = "scale" } )
		self._betting_cards[i]:set_rotation( math.random( 14 ) - 7 )
	end
	...
	self._stats_panel = self._main_panel:panel( { x = text_betting:x(), y = text_betting:bottom(), w = width, h = height/2 - space_y/2, layer = 1 } )
	self._stats_panel:set_x( self._betting_panel:right() + space_x )
	BoxGuiObject:new( self._stats_panel, { sides = {1, 1, 1, 1} } )
	
	local text_stats = self._main_panel:text( { text = managers.localization:to_upper_text( "menu_casino_title_stats" ), font_size = medium_font_size, font = medium_font, color = tweak_data.screen_colors.text, blend_mode = "add" } )
	...
	self._stats_cards = { "weapon_mods", "masks", "materials", "textures", "colors" }
	local stat_columns = { {name = "base", color = Color( 0.5, 0.5, 0.5 ), color_inf = Color( 1, 0.1, 1 )}, {name = "bets", color = tweak_data.screen_colors.risk}, {name = "skill", color = tweak_data.screen_colors.resource}, {name = "total", color = tweak_data.screen_colors.text, color_inf = Color(1, 0.1, 1)} }
	self._stat_values = {}
	...
	for _, column in pairs( stat_columns ) do
		self._stats_panel:text( { text=managers.localization:to_upper_text( "menu_casino_stat_" .. column.name ), x=x, y=y, font_size=small_font_size, font=small_font, color=column.color or tweak_data.screen_colors.text, blend_mode="add" } )
		x = x + column_width
	end
	...
	for _, stat in pairs( self._stats_cards ) do
		...
		for _, column in pairs( stat_columns ) do
			text_panel = self._stats_panel:panel( { layer=1, x=x, y=y, w=column_width, h=small_font_size } )
			self._stat_values[ stat ][ column.name ] = text_panel:text( { align="right", font_size=small_font_size, font=small_font, color=column.color or tweak_data.screen_colors.text, blend_mode="add", alpha=1 } )
			x = x + column_width
		end
	...
	y = content_offset + small_font_size + 10
	for _, stat in pairs( self._stats_cards ) do
		text_panel = self._stats_panel:panel( { layer=1, x=0, y=y, w=title_width, h=small_font_size } )
			self._stat_values[ stat ].title = text_panel:text( { text=managers.localization:to_upper_text( "menu_casino_stat_" .. stat ), font_size=small_font_size, font=small_font, align="right", color=tweak_data.screen_colors.text, blend_mode="add", alpha=1 } )
			y = y + small_font_size
	end
	...
	y = y + small_font_size
	text_panel = self._stats_panel:panel( { layer=1, x=0, y=y, w=title_width, h=small_font_size } )
	text_panel:text( { text = managers.localization:to_upper_text( "bm_global_value_infamous" ), font_size=small_font_size, font=small_font, align="right", color=Color(1, 0.1, 1), blend_mode = "add" } )
	
	x = title_width
	for _, column in pairs( stat_columns ) do
		text_panel = self._stats_panel:panel( { layer=1, x=x, y=y, w=column_width, h=small_font_size } )
		self._infamous_values[ column.name ] = text_panel:text( { align="right", font_size=small_font_size, font=small_font, color=column.color_inf or column.color or tweak_data.screen_colors.text, blend_mode="add", alpha=column.alpha or 1 } )
		x = x + column_width
	end
	...
	local infamous_chance = items_total > 0 and managers.lootdrop:infamous_chance() * ( items_infamous / items_total ) or 0
	...
	self._breakdown_panel = self._main_panel:panel( { w=width, h=self._betting_panel:h() - self._stats_panel:h() - space_y, layer=1 } )
	self._breakdown_panel:set_x( self._stats_panel:x() )
	self._breakdown_panel:set_top( self._stats_panel:bottom() + space_y )
	BoxGuiObject:new( self._breakdown_panel, { sides = {1, 1, 1, 1} } )
	
	local text_breakdown = self._main_panel:text( { text = managers.localization:to_upper_text( "menu_casino_title_breakdown" ), font_size=medium_font_size, font=medium_font, color=tweak_data.screen_colors.text, blend_mode="add" } )
	...
	self._breakdown_titles = self._breakdown_panel:text( { x=content_offset, y=content_offset, font_size=small_font_size, font=small_font, color=tweak_data.screen_colors.text, blend_mode="add" } )
	self._breakdown_costs = self._breakdown_panel:text( { x=self._breakdown_panel:w()*0.4, y=content_offset, font_size=small_font_size, font=small_font, color=tweak_data.screen_colors.risk, blend_mode="add" } )
	
	
	
	self._offshore_text = self._main_panel:text( { font_size=small_font_size, font=small_font, color=tweak_data.screen_colors.text, blend_mode="add" } )
	...
	self._total_bet = self._panel:text( { text=text_string, font_size=large_font_size, font=large_font, color=tweak_data.screen_colors.text, blend_mode="add", align="right" } )
...
function MenuNodeCrimenetCasinoGui:set_update_values( preferred_card, secured_cards, increase_infamous, infamous_enabled, safecards_enabled )
	...
	elseif nbr_types > 1 then
		...
		for _, card in pairs( self._stats_cards ) do
			...
			self._stat_values[ card ].bets:set_text( value==0 and "" or ( value > 0 and ( "+" .. string.format( "%.1f", value ) ) or string.format( "%.1f", value ) ) .. "%" )
...
function MenuNodeCrimenetContactInfoGui:init( node, layer, parameters )
	...
	self._file_icons = {}
	self._file_icons.selected = { 0, 0, 17, 23 }
	self._file_icons.unselected = { 20, 0, 17, 23 }
	self._file_icons.locked = { 40, 0, 17, 23 }
...
function MenuNodeCrimenetContactInfoGui:set_contact_info( id, name, files, override_file )
	...
	if num_files > 1 then
		...
		for i = 1, num_files do
			...
			files_menu:bitmap( { texture = "guis/textures/pd2/codex_pages", texture_rect = texture_rect, x = ( i - 1 ) * 20, y = 0, w = 17, h = 23 } )
		end
	else
		files_menu:set_h( 0 )
	end
...
function MenuNodeCrimenetContactInfoGui:set_file( index )
	...
	local video_panel = self._panel:child( "video_panel" )
	if video then
		video_panel:video( { name = "video", video = "movies/codex/" .. video, width = video_panel:w(), height = video_panel:h(), blend_mode = "add", loop = true, color = tweak_data.screen_colors.button_stage_2 } )
	end
...
function MenuNodeCrimenetContactInfoGui:_setup_layout()
	...
	local blur = self._fullscreen_panel:bitmap( { texture = "guis/textures/test_blur_df", w = self._fullscreen_panel:w(), h = self._fullscreen_panel:h(), render_template = "VertexColorTexturedBlur3D" } )
	...
	self._panel = panel:panel( { h = height, w = width, layer = 51 } )
	self._panel:set_center( panel:w() / 2, panel:h() / 2 )
	
	self._panel:rect( { color = Color.black, alpha = 0.6, layer = 0 } )
	BoxGuiObject:new( self._panel, { sides = { 1, 1, 1, 1 } } )
	
	
	local title_text = panel:text( { name = "title_text", text = self._codex_text, font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text, layer = 51 } )
	...
	local contact_title_text = self._panel:text( { name = "contact_title_text", text = self._codex_text .. ": ", font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text, layer = 52 } )
	make_fine_text( contact_title_text )
	...
	local video_panel = self._panel:panel( { name = "video_panel", layer = 2, w = self.WIDTH - self.MENU_WIDTH - self.PADDING * 5 } )
	video_panel:set_h( video_panel:w() / ( 16 / 9 ) )
	video_panel:set_top( contact_title_text:bottom() + self.PADDING )
	video_panel:set_left( contact_title_text:left() )
	local box = BoxGuiObject:new( video_panel, { sides = { 2, 2, 2, 2 } } )
	box:set_color( Color( 0.2, 1, 1, 1 ) )
	box:set_blend_mode( "add" )
	
	
	local contact_desc_title_text = self._panel:text( { name = "contact_desc_title_text", text = self._codex_text .. ": ", font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text, layer = 52 } )
	make_fine_text( contact_desc_title_text )
	...
	local files_menu = self._panel:panel( { name = "files_menu", x = contact_desc_title_text:x(), y = contact_desc_title_text:y(), h = 26, w = video_panel:w() } )
	
	
	
	
	local contact_desc_text = self._panel:text( { name = "contact_desc_text", text = "", font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = tweak_data.screen_colors.text, layer = 52, wrap = true, word_wrap = true } )
...
function MenuNodeCrimenetContactInfoGui:gui_node_custom( row_item )
	row_item.gui_panel = self._item_panel_parent:panel( { layer = self.layers.items, w = 3, h = 3 } )
