...
function MenuNodeCrimenetCasinoGui:_setup_layout()
	...
	do
		local (for generator), (for state), (for control) = pairs(stat_columns)
		do
			do break end
...
function MenuNodeCrimenetCasinoGui:set_update_values(preferred_card, secured_cards, increase_infamous, infamous_enabled, safecards_enabled)
	...
	do
		...
		do
			do break end
			do
				local (for generator), (for state), (for control) = pairs(self._stat_values[card])
				do
					do break end
...
function MenuNodeCrimenetContactInfoGui:set_file(index)
	...
	local files_menu = self._panel:child("files_menu")
	local (for generator), (for state), (for control) = ipairs(files_menu:children())
	do
		do break end
...
(]]
					...
					elseif #weapon_uses_part == 1 then
						...
						if need_string then
							local families = {}
							do
								local (for generator), (for state), (for control) = pairs(all_families)
								do
									do break end
									if #weapons == 0 then
	...
	elseif ids == Idstring("_summary") then
		...
		do
			local (for generator), (for state), (for control) = ipairs(self.node:items())
			do
				do break end
