...
function TextBoxGui:_setup_stats_panel(scroll_panel, stats_list, stats_text)
	...
	if has_stats then
		do
			local (for generator), (for state), (for control) = ipairs(stats_list)
			do
				do break end
				...
				elseif stats.type == "mods" then
					...
					local s = ""
					do
						local (for generator), (for state), (for control) = ipairs(stats.list)
						do
							do break end
							s = s .. mod .. "\n"
						end
...
function TextBoxGui:check_focus_button(x, y)
	do
		local (for generator), (for state), (for control) = ipairs(self._text_box_buttons_panel:children())
		do
			do break end
...
function TextBoxGui:_set_scroll_indicator()
	...
	if not self._no_scroll_legend then
		-- unhandled boolean indicator
	else
	end

	legend_scroll:set_visible(true)
