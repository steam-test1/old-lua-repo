...
function LootDropScreenGui:init( saferect_ws, fullrect_ws, lootscreen_hud, saved_state )
	...
	self._continue_button = self._panel:text( { name="ready_button", text=utf8.to_upper( managers.localization:text( "menu_l_waiting_for_all" ) ), h=32, align="right", vertical="center", font_size=tweak_data.menu.pd2_large_font_size, font=tweak_data.menu.pd2_large_font, color=tweak_data.screen_colors.button_stage_3, layer=1 } )
	...
	local big_text = self._fullscreen_panel:text( { name="continue_big_text", text=utf8.to_upper( managers.localization:text( "menu_l_waiting_for_all" ) ), h=90, align="right", vertical="bottom", font_size=tweak_data.menu.pd2_massive_font_size, font=tweak_data.menu.pd2_massive_font, color=tweak_data.screen_colors.button_stage_3, alpha=0.4 } )
	...
	self._time_left_text = self._panel:text( { name="time_left", text="30", align="right", vertical="bottom", font_size=tweak_data.menu.pd2_small_font_size, font=tweak_data.menu.pd2_small_font, color=tweak_data.screen_colors.text, layer=1 } )
	...
	-- self._lootdata = self._lootscreen_hud:fetch_local_lootdata()
	-- self:create_stars_giving_animation()
	...
	-- self._lootscreen_hud:create_stars_giving_animation()
end
...
function LootDropScreenGui:check_all_ready()
	...
	-- local text_id = (not is_server_or_trial and "") or ( (not self._completion_bonus_done and "menu_es_calculating_experience") or (managers.job:on_last_stage() and "menu_victory_goto_payday" or "menu_victory_goto_next_stage") )
		...
		-- self._fullscreen_panel:text( { name="continue_big_text", text=utf8.to_upper( managers.localization:text( "menu_waiting_for_server", { CONTINUE = managers.localization:btn_macro( "continue" ) } ) ), h=90, align="right", vertical="bottom", font_size=tweak_data.menu.pd2_massive_font_size, font=tweak_data.menu.pd2_massive_font, color=tweak_data.screen_colors.button_stage_3, alpha=0.4 } )
...
function LootDropScreenGui:on_peer_removed( peer, reason )
	...
	-- if Network:is_server() or Global.game_settings.single_player or self._is_alone then
	self:check_all_ready()
	-- end
end
...
function LootDropScreenGui:mouse_pressed( button, x, y )
	...
	if self._card_chosen and not self._button_not_clickable then
		if self._continue_button:inside( x, y ) then
...
function LootDropScreenGui:input_focus()
	return self._enabled -- and not self._card_chosen
end
...
function LootDropScreenGui:move_up()
	-- self._lootscreen_hud._peers_panel:set_debug(true)
end

function LootDropScreenGui:move_down()
	-- self._lootscreen_hud._peers_panel:set_debug(false)
end
