...
function NewsFeedGui:update( t, dt )
	...
	if self._news and #self._titles > 0 then
		...
		if self._sustain_t then
			if self._sustain_t < t then
				self._sustain_t = nil
...
function NewsFeedGui:_create_gui()
	...
	self._panel = self._ws:panel():panel( { name = "main", w = size.width/2, h = 44 } )
	-- self._panel:rect( { name = "bg_rect", color = Color( 0.5, 0.5, 0.5, 0.5 ) } )
	--self._title_panel = self._panel:panel( { name = "title_panel", layer = 1 } )
	self._panel:bitmap( { visible = false, name = "bg_bitmap", texture = "guis/textures/textboxbg", layer = 0, color = Color.black, w = self._panel:w(), h = self._panel:h() } )
	self._panel:text( { visible = false, rotation = 360, name = "title_announcement", text = managers.localization:to_upper_text( "menu_announcements" ), font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size,
						align="left", halign="left", vertical="top", hvertical="top", color = Color.white } )

	self._title_panel = self._panel:panel( { name = "title_panel", layer = 1 } )
	self._title_panel:text( { rotation = 360, name = "title", text = "", font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size,
						align="left", halign="left", vertical="bottom", hvertical="bottom", color = Color( 0.75, 0.75, 0.75 ) } )
	...
	self._panel:set_bottom( self._panel:parent():h() ) -- 635 )
end
