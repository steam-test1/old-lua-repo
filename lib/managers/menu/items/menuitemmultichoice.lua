...
function MenuItemMultiChoice:init( data_node, parameters )
	...
	--[[if self._parameters.category == "weapon" then
		...
		local selected_weapon = managers.player:weapon_in_slot( self._parameters.slot )-- Global.player_manager.kit.weapon_slots[ self._parameters.slot ]
	...
	elseif self._parameters.category == "equipment" then
		...
		local selected = managers.player:equipment_in_slot( self._parameters.slot ) -- Global.player_manager.kit.equipment_slots[ self._parameters.slot ]
		for i,option in ipairs( self._options ) do
...
function MenuItemMultiChoice:set_callback_handler( callback_handler )
	...
	-- self:_show_options( callback_handler )
end
...
-- Returns highest option index that is not excluded
...
-- Returns lowest option index that is not excluded
...
function MenuItemMultiChoice:next()
	...
	--[[while self._options[ self._current_index ].exclude do
		self._current_index =  self._current_index == #self._options and 1 or (self._current_index + 1)
	end]]
...
function MenuItemMultiChoice:setup_gui( node, row_item )
	...
	row_item.choice_text = row_item.choice_panel:text( {
				...
				x = 0, y = 0,
				align="center", vertical="center", -- , halign="center",
				font = row_item.font,
				color = node.row_item_hightlight_color, -- tweak_data.menu.default_changeable_text_color,
	...
	-- menu_arrows
	row_item.arrow_left = row_item.gui_panel:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, color = Color( 0.5,0.5,0.5 ), visible = self:arrow_visible(), x = 0, y = 0, layer = node.layers.items, blend_mode = node.row_item_blend_mode } )
	row_item.arrow_right = row_item.gui_panel:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {24,0,-24,24}, color = Color( 0.5,0.5,0.5 ), visible = self:arrow_visible(), x = 0, y = 0, layer = node.layers.items, blend_mode = node.row_item_blend_mode } )
...
function MenuItemMultiChoice:reload( row_item, node )
	...
	row_item.arrow_left:set_color( ( not self._enabled and row_item.disabled_color ) or ( self:left_arrow_visible() and tweak_data.screen_colors.button_stage_2 ) or tweak_data.screen_colors.button_stage_3 ) -- tweak_data.menu.arrow_available or tweak_data.menu.arrow_unavailable )	row_item.arrow_right:set_color( self:right_arrow_visible() and tweak_data.screen_color_blue or tweak_data.screen_color_blue:with_alpha(0.5) ) -- tweak_data.menu.arrow_available or tweak_data.menu.arrow_unavailable )
	row_item.arrow_right:set_color( ( not self._enabled and row_item.disabled_color ) or ( self:right_arrow_visible() and tweak_data.screen_colors.button_stage_2 ) or tweak_data.screen_colors.button_stage_3 ) -- tweak_data.menu.arrow_available or tweak_data.menu.arrow_unavailable )	row_item.arrow_right:set_color( self:right_arrow_visible() and tweak_data.screen_color_blue or tweak_data.screen_color_blue:with_alpha(0.5) ) -- tweak_data.menu.arrow_available or tweak_data.menu.arrow_unavailable )
...
function MenuItemMultiChoice:fade_row_item( node, row_item, mouse_over )
	row_item.gui_text:set_color( row_item.color )
	row_item.choice_text:set_color( (not self._enabled and row_item.disabled_color ) or self:selected_option():parameters().color or node.row_item_hightlight_color )
	-- row_item.gui_text:set_font( tweak_data.menu.default_font )
...
function MenuItemMultiChoice:_layout( node, row_item )
	...
	-- row_item.gui_panel:set_width( safe_rect.width - safe_rect.width/2 - xl_pad )
	...
	if( row_item.align == "right" ) then
		...
		row_item.arrow_right:set_left( row_item.arrow_left:right() + 2 *(1-tweak_data.scale.multichoice_arrow_multiplier) ) -- node._left_align( node ) - row_item.gui_panel:x() )
	else
		row_item.arrow_right:set_right( row_item.gui_panel:w() ) -- node._left_align( node ) - row_item.gui_panel:x() )
		row_item.arrow_left:set_right( row_item.arrow_right:left() + 2 *(1-tweak_data.scale.multichoice_arrow_multiplier) ) -- 3 )
	end
	
	-- row_item.gui_text:set_width( row_item.arrow_left:left() - node._align_line_padding * 2 )-- safe_rect.width/2 + row_item.gui_panel:x() )
	...
	if( row_item.align == "right" ) then
		row_item.choice_panel:set_left( row_item.arrow_left:right() + node._align_line_padding ) -- right_align - row_item.gui_panel:x() )
		row_item.arrow_right:set_left( row_item.choice_panel:right() )
	else
		row_item.choice_panel:set_right( row_item.arrow_right:left() - node._align_line_padding ) -- right_align - row_item.gui_panel:x() )
		row_item.arrow_left:set_right( row_item.choice_panel:left() )
	end
	
	-- row_item.choice_panel:set_right( row_item.gui_panel:right() )
	
	row_item.choice_text:set_w( row_item.choice_panel:w() )
	row_item.choice_text:set_h( h )
	row_item.choice_text:set_left( 0 )
	-- row_item.choice_text:set_right( row_item.arrow_left:left() - node._align_line_padding * 2 )
	-- row_item.choice_text:set_left( 24*tweak_data.scale.multichoice_arrow_multiplier + 4*tweak_data.scale.multichoice_arrow_multiplier )
	...
	-- row_item.gui_text:set_left( 0 ) -- safe_rect.width/2 )
	-- row_item.gui_text:set_left( safe_rect.width/2 - row_item.gui_panel:x() )
