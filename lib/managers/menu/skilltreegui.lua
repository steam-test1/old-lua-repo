...
function SkillTreeSkillItem:refresh( locked )
	...
	if selected then
		if not self._tier then
			if step == 1 then
				skill_text_string = managers.localization:text( "st_menu_unlock_profession", { profession=managers.localization:text(tweak_data.skilltree.trees[self._tree].name_id), points=Application:digest_value(tweak_data.skilltree.skills[self._skill_id][1].cost, false) } ) -- "UNLOCK PROFESSION"
			else
		...
		elseif step == 2 then
			local points = Application:digest_value( tweak_data.skilltree.skills[self._skill_id][2].cost, false )
		...
		elseif step == 1 then
			local points = Application:digest_value( tweak_data.skilltree.skills[self._skill_id][1].cost, false )
			local cost = managers.money:get_skillpoint_cost( self._tree, self._tier, points )
