...
function SkillTreeSkillItem:refresh( locked )
	...
	if selected then
		if not self._tier then
			if step == 1 then
				skill_text_string = managers.localization:text( "st_menu_unlock_profession", { profession=managers.localization:text(tweak_data.skilltree.trees[self._tree].name_id), points=managers.skilltree:get_skill_points( self._skill_id, 1 ) } ) -- "UNLOCK PROFESSION"
			else
...
function SkillTreeGui:_setup()
	...
	local tree_tabs_panel = self._panel:panel( { name = "tree_tabs_panel", h = tree_tab_h, y=71 } )
	-- Total skill points
...
function SkillTreeGui:set_selected_item( item, no_sound )
	...
	if( self._selected_item and self._selected_item._skill_panel ) then
		...
		if step > 1 then
			...
			local points = managers.skilltree:get_skill_points( skill_id, 1 ) or 0
			local money_cost = managers.money:get_skillpoint_cost( self._selected_item._tree, self._selected_item._tier, points )
		...
		if step > 2 then
			...
			local points = managers.skilltree:get_skill_points( skill_id, 2 ) or 0
			local money_cost = managers.money:get_skillpoint_cost( self._selected_item._tree, self._selected_item._tier, points )
		...
		if self._selected_item._tier then
			if not unlocked then
				...
				local tier_unlocks = Application:digest_value( tweak_data.skilltree.tier_unlocks[ self._selected_item._tier ], false ) or 0
