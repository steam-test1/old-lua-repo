...
-- Command line was: F:\SteamLibrary\SteamApps\common\PAYDAY 2\lua\lib\managers\menu\ingamecontractgui.luac 

if not IngameContractGui then
  IngameContractGui = class()
end
IngameContractGui.init = function(l_1_0, l_1_1)
  l_1_0._panel = l_1_1:panel():panel({w = math.round(l_1_1:panel():w() / 2.5999999046326), h = math.round(l_1_1:panel():h() / 1.2000000476837)})
  l_1_0._panel:set_y(CoreMenuRenderer.Renderer.border_height + tweak_data.menu.pd2_large_font_size)
  ...
  if job_data and managers.job:current_job_id() == "safehouse" and Global.mission_manager.saved_job_values.playedSafeHouseBefore then
    l_1_0._panel:set_visible(false)
  end
  local contract_text = l_1_0._panel:text({layer = 1, rotation = 360, vertical = "bottom", font = tweak_data.menu.pd2_large_font, font_size = tweak_data.menu.pd2_large_font_size, text = "", color = tweak_data.screen_colors.text})
  contract_text:set_text(l_1_0:get_text("cn_menu_contract_header") .. " " .. (job_data and l_1_0:get_text(job_data.name_id) or ""))
  contract_text:set_bottom(0)
  local text_panel = l_1_0._panel:panel({layer = 1, w = l_1_0._panel:w() - 32, h = l_1_0._panel:h() - 32})
  text_panel:set_center(l_1_0._panel:w() / 2, l_1_0._panel:h() / 2)
  ...
  briefing_title:set_text(l_1_0:get_text("menu_briefing"))
  ...
  local briefing_description = text_panel:text({name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size})
  managers.hud:make_fine_text(briefing_description)
  {name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size}.h, {name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size}.word_wrap, {name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size}.wrap, {name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size}.vertical, {name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size}.align, {name = "briefing_description", text = text, font = tweak_data.menu.pd2_small_font, font_size = font_size}.color = 128, true, true, "top", "left", Color(1, 1, 1, 1)
  ...
  if managers.job:is_current_job_professional() then
    local pro_warning_text = text_panel:text({name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size})
    managers.hud:make_fine_text(pro_warning_text)
    {name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size}.h, {name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size}.word_wrap, {name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size}.wrap, {name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size}.vertical, {name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size}.align, {name = "pro_warning_text", text = l_1_0:get_text("menu_pro_warning"), font = tweak_data.menu.pd2_small_font, font_size = font_size}.color = 128, true, true, "top", "left", Color(255, 255, 50, 53) / 255
  ...
  local risk_title = text_panel:text({font = tweak_data.menu.pd2_small_font, font_size = font_size, text = l_1_0:get_text("menu_risk"), color = risk_color})
  ...
  local risk_text = text_panel:text({w = text_panel:w(), h = text_panel:h(), name = "risk_text", text = l_1_0:get_text(menu_risk_id) .. " ", font = tweak_data.hud_stats.objective_desc_font, font_size = font_size})
  managers.hud:make_fine_text(risk_text)
  {w = text_panel:w(), h = text_panel:h(), name = "risk_text", text = l_1_0:get_text(menu_risk_id) .. " ", font = tweak_data.hud_stats.objective_desc_font, font_size = font_size}.word_wrap, {w = text_panel:w(), h = text_panel:h(), name = "risk_text", text = l_1_0:get_text(menu_risk_id) .. " ", font = tweak_data.hud_stats.objective_desc_font, font_size = font_size}.wrap, {w = text_panel:w(), h = text_panel:h(), name = "risk_text", text = l_1_0:get_text(menu_risk_id) .. " ", font = tweak_data.hud_stats.objective_desc_font, font_size = font_size}.vertical, {w = text_panel:w(), h = text_panel:h(), name = "risk_text", text = l_1_0:get_text(menu_risk_id) .. " ", font = tweak_data.hud_stats.objective_desc_font, font_size = font_size}.align, {w = text_panel:w(), h = text_panel:h(), name = "risk_text", text = l_1_0:get_text(menu_risk_id) .. " ", font = tweak_data.hud_stats.objective_desc_font, font_size = font_size}.color = true, true, "top", "left", risk_color
  risk_text:set_h(risk_text:h() + 10)
  risk_text:set_top(risk_title:bottom() + 10)
  local potential_rewards_title = text_panel:text({font = tweak_data.menu.pd2_small_font, font_size = font_size, text = l_1_0:get_text("menu_potential_rewards"), color = Color.white})
  managers.hud:make_fine_text(potential_rewards_title)
  potential_rewards_title:set_top(risk_text:bottom() + 4)
  local paygrade_title = text_panel:text({font = tweak_data.menu.pd2_small_font, font_size = font_size, text = l_1_0:get_text("cn_menu_contract_paygrade_header"), color = Color.white})
  managers.hud:make_fine_text(paygrade_title)
  paygrade_title:set_top(potential_rewards_title:bottom() + 4)
  local experience_title = text_panel:text({font = tweak_data.menu.pd2_small_font, font_size = font_size, text = l_1_0:get_text("menu_experience"), color = Color.white})
  managers.hud:make_fine_text(experience_title)
  experience_title:set_top(paygrade_title:bottom())
  local stage_cash_title = text_panel:text({font = tweak_data.menu.pd2_small_font, font_size = font_size, text = l_1_0:get_text("menu_cash_stage", {money = ""}), color = Color.white})
  managers.hud:make_fine_text(stage_cash_title)
  stage_cash_title:set_top(experience_title:bottom())
  local cash_title = text_panel:text({font = tweak_data.menu.pd2_small_font, font_size = font_size, text = l_1_0:get_text("menu_cash_job", {money = ""}), color = Color.white})
  ...
  if job_data then
    ...
    for i,name in ipairs({"risk_pd", "risk_swat", "risk_fbi", "risk_death_squad"}) do
      if i ~= 1 then
        ...
        if not active or i == 1 or not risk_color then
    ...
    if not job_data.professional or not tweak_data:get_value("experience_manager", "pro_day_multiplier", num_days) then
      local last_day_mul = tweak_data:get_value("experience_manager", "day_multiplier", num_days)
    end
    ...
    local payday_text = text_panel:text({font = tweak_data.menu.pd2_large_font, font_size = tweak_data.menu.pd2_large_font_size, text = l_1_0:get_text("menu_payday", {MONEY = managers.experience:cash_string(math.round(payday_value))}), color = Color.white})
    managers.hud:make_fine_text(payday_text)
    payday_text:set_bottom(text_panel:h())
  end
  l_1_0:_rec_round_object(l_1_0._panel)
  l_1_0._sides = BoxGuiObject:new(l_1_0._panel, {sides = {1, 1, 1, 1}})
end

IngameContractGui._rec_round_object = function(l_2_0, l_2_1)
...
IngameContractGui.set_layer = function(l_3_0, l_3_1)
  l_3_0._panel:set_layer(l_3_1)
end

IngameContractGui.get_text = function(l_4_0, l_4_1, l_4_2)
  return utf8.to_upper(managers.localization:text(l_4_1, l_4_2))
end

IngameContractGui._make_fine_text = function(l_5_0, l_5_1)
  local x, y, w, h = l_5_1:text_rect()
  l_5_1:set_size(w, h)
end

IngameContractGui.close = function(l_6_0)
  if l_6_0._panel and alive(l_6_0._panel) then
    l_6_0._panel:parent():remove(l_6_0._panel)
    l_6_0._panel = nil
  end
end
