	...
	if not node:item( "divider_end" ) then
		...
		for index, job_id in ipairs( tweak_data.narrative:get_jobs_index() ) do
			...
			if table.contains( contacts, contact ) then
				...
				if ( tweak_data.narrative.jobs[ job_id ].jc or 0 ) + ( tweak_data.narrative.jobs[ job_id ].professional and 1 or 0 ) <= max_jc and not tweak_data.narrative.jobs[ job_id ].wrapped_to_job then
					table.insert( jobs[ contact ], job_id )
				end
			end
		end
		for _, contracts in pairs( jobs ) do
			table.sort( contracts, function( x, y ) return tweak_data.narrative.jobs[ x ].jc < tweak_data.narrative.jobs[ y ].jc end )
		end
