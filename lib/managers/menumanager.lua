...
function MenuChallenges:modify_node(node, up)
	...
	do
		local (for generator), (for state), (for control) = pairs(managers.challenges:get_near_completion())
		do
			do break end
...
function MenuChallengesAwarded:modify_node(node, up)
	local new_node = up and node or deep_clone(node)
	do
		local (for generator), (for state), (for control) = pairs(managers.challenges:get_completed())
		do
			do break end
			local params = {
				name = data.id,
...
10
	...
	elseif type then
		...
		local (for generator), (for state), (for control) = pairs(locations)
		do
			do break end
