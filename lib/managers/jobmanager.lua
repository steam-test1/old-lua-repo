...
--[[function JobManager:set_interupt_stage( interupt )
	self._global.interupt_stage = interupt
end]]
...
-- DANGER!!
-- DANGER!! This function might not always be runned on clients (XBOX has a problem with it for example)
-- DANGER!!
function JobManager:next_stage()
	...
	self._global.interupt_stage = nil 	-- MAYBY FOR ESCAPE LAST
	

	-- is_job_finished
	if self:is_job_finished() and not self._global.next_interupt_stage then 	-- MAYBY FOR ESCAPE LAST
		-- print( "AWARD !!!!!" )
	...
	if not self._global.interupt_stage then
		-- Client may set this, but he will still get it from the server in sync_stage_settings
		self:set_current_stage( self._global.current_job.current_stage + 1 ) 
		-- self._global.current_job.current_stage = self._global.current_job.current_stage + 1
	end
	...
	if Network:is_server() then
		...
		-- local job_id_index = tweak_data.narrative:get_index_from_job_id( self:current_job_id() )
		...
		managers.network:session():send_to_peers( "sync_stage_settings", level_id_index, self._global.current_job.current_stage, self._global.alternative_stage or 0, interupt_level_id_index ) -- Let everyone know
		-- SYNC MISSION?
	end
end
...
function JobManager:current_stage_data()
	...
	if #stage > 0 then -- The stage has alternatives
		return stage[ self._global.alternative_stage or 1 ]
	end
	return stage -- job_data.chain[ self._global.current_job.current_stage ]
end
...
function JobManager:_check_add_to_cooldown()
	-- print( "JobManager:_check_add_to_cooldown()", self._global.start_time, TimerManager:wall_running():time(), self:current_job_id() )
	if Network:is_server() and self._global.start_time then
		...
		if cooldown_time > 0 then
			-- print( "ADD TO COOLDOWN", cooldown_time )
