...
function HUDManager:text_clone( text )
	return text:parent():text( { font = tweak_data.hud.medium_font_noshadow, font_size = text:font_size(), text = text:text(), 
							x = text:x(), y = text:y(), w = text:w(), h = text:h(),
							align = text:align(), vertical = text:vertical(),
							layer = text:layer(), color = text:color(), visible = text:visible() } )
end
...
--[[add_weapon	---------------------------------------------------------------------------------------------
13:03:09	| amount              | 24                                                                  |
13:03:09	| inventory_index     | 3                                                                   |
13:03:09	| is_equip            | false                                                               |
13:03:09	| unit                | [Unit 'units/weapons/mossberg/mossberg' Vector3(0, 0, 0) 11250380]  |
13:03:09	---------------------------------------------------------------------------------------------
]]
...
function HUDManager:add_weapon( data )
	...
	local weapon = teammate_panel:bitmap( { name = "weapon"..data.inventory_index, visible = data.is_equip, texture = icon, color = Color(1, 0.8, 0.8, 0.8), layer = 2, texture_rect = texture_rect, x = mask:right() + pad  } )
	...
	-- local weapon = teammate_panel:bitmap( { name = "weapon", layer = 1, color = Color.white, w = 64, h = 32, x = mask:right() + pad, y = y } )
	-- weapon:set_bottom( mask:bottom() )
	if data.is_equip then
		self:set_weapon_selected_by_inventory_index( data.inventory_index )
	end
	
	-- If it is not equip. Then it must be the second weapon	 
...
function HUDManager:_set_weapon( data )
	...
	--[[local teammate_panel = self._teammate_panels[ HUDManager.PLAYER_PANEL ]:panel()
	local weapon_panel = teammate_panel:child( "weapon_panel"..data.inventory_index ) 
	weapon_panel:child( "weapon" ):set_image( "guis/textures/pd2/weapons", 0, (data.inventory_index == 2 and 0) or 64, 64, 64 )
	print( "_set_weapon", data.inventory_index, (data.inventory_index == 2 and 0) or 64 )]]
	-- data.is_equip
end
...
function HUDManager:set_ammo_amount( selection_index, max_clip, current_clip, current_left, max )
	-- print( "set_ammo_amount", max_clip, current_clip, current_left, max )
	-- local type = selection_index == 1 and "secondary" or "primary"
	-- managers.player:update_synced_ammo_info_to_peers( self._hud.selected_weapon, max_clip, current_clip, current_left, max )
...
function HUDManager:set_teammate_ammo_amount( id, selection_index, max_clip, current_clip, current_left, max )
	local type = selection_index == 1 and "secondary" or "primary"
	-- print( "[HUDManager:set_teammate_ammo_amount]" )
	-- print( Application:stack_dump() )
	-- self._teammate_panels[ id ]:set_ammo_amount( type, max_clip, current_clip, current_left, max )
...
-- NEW
function HUDManager:set_teammate_callsign( i, id ) -- Id is between 1 and 4, should be peer id when player
...
-- NEW
function HUDManager:set_teammate_state( i, state ) -- state, ai, player
...
function HUDManager:add_teammate_special_equipment( i, data )
	-- print( "add_teammate_special_equipment", i, inspect( data ) )
	if not i then
...
function HUDManager:set_teammate_condition( i, icon_data, text )
	-- print( "set_teammate_condition", i, icon_data, text )
	if not i then
...
function HUDManager:_create_assault_corner()
	...
	-- self._hud_assault_corner:sync_start_assault()
end

-- NEW
function HUDManager:add_teammate_panel( character_name, player_name, ai, peer_id )
	--print( "[HUDManager:add_teammate_panel]", character_name, player_name, ai, peer_id )
 	for i,data in ipairs( self._hud.teammate_panels_data ) do
 		...
 		if not data.taken then
 			...
 			self:set_teammate_callsign( i, ai and 5 or peer_id ) -- managers.criminals:character_static_data_by_name( character_name ).color_id )
 			...
 			if peer_id then
				...
				for equipment,amount in pairs( peer_equipment ) do
					self:add_teammate_special_equipment( i, { id = equipment, icon = tweak_data.equipments.specials[ equipment ].icon, amount = amount } )
				end
				...
				if peer_deployable_equipment then
					...
					self:set_deployable_equipment( i, { icon = icon, amount = peer_deployable_equipment.amount } )
					-- self:add_teammate_special_equipment( i, { id = equipment, icon = tweak_data.equipments.specials[ equipment ].icon } )
				end
				...
				if peer_cable_ties then
					...
					self:set_cable_tie( i, { icon = icon, amount = peer_cable_ties.amount } )
					-- self:add_teammate_special_equipment( i, { id = equipment, icon = tweak_data.equipments.specials[ equipment ].icon } )
				end
				...
				if peer_grenades then
					...
					self:set_teammate_grenades( i, { icon = icon, amount = Application:digest_value( peer_grenades.amount, false ) } )
				end
 			end
			...
			if peer_ammo_info then
				...
				--[[local selection_index = alive( unit ) and unit:inventory():equipped_selection() or 1
				local ammo_info = peer_ammo_info[ selection_index ]
				self:set_teammate_ammo_amount( i, selection_index, unpack( ammo_info ) )]]
			end
	...
	-- self._teammate_panels[ id ]:set_visible( false )
end

-- NEW
function HUDManager:remove_teammate_panel( id )
	--print( "HUDManager:remove_teammate_panel( id )", id )
	--Application:stack_dump()
	...
	-- Case where we have dropped in. And a player has taken our panel.
	-- This is not nice, but having problem keeping track of characters/ai/players when dropping in.	
	if (self._teammate_panels[ HUDManager.PLAYER_PANEL ]._peer_id 
		and (self._teammate_panels[ HUDManager.PLAYER_PANEL ]._peer_id ~= managers.network:session():local_peer():id())) or is_ai then
		print( " MOVE!!!", self._teammate_panels[ HUDManager.PLAYER_PANEL ]._peer_id, is_ai )
		-- print( "someone else occupies local player panel" )
		...
		-- managers.criminals:character_data_by_peer_id( self._teammate_panels[ HUDManager.PLAYER_PANEL ]._peer_id ).panel_id = nil
		local peer_id = self._teammate_panels[ HUDManager.PLAYER_PANEL ]._peer_id
	...
	managers.hud._teammate_panels[ HUDManager.PLAYER_PANEL ]:add_panel() -- Readd our panel
...
function HUDManager:_create_teammates_panel( hud )
	...
	local teammates_panel = hud.panel:panel( { name = "teammates_panel", h = h, y = hud.panel:h() - h, halign = "grow", valign = "bottom" } )
	-- teammates_panel:set_debug( true )
...
function HUDManager:remove_interact()
	if not self._hud_interaction then
		return -- crash fix for case when server terminates the game under loading
	end
...
function HUDManager:sync_end_assault( result )
	...
	if result then -- No result when ending from simulation  
...
function HUDManager:show_stats_screen()
	...
	if not self:exists( safe ) then
		-- This triggers the setup code (HUDManager._setup_stats_screen)
...
function HUDManager:flash_point_of_no_return_timer( beep )
	-- print( "HUDManager:flash_point_of_no_return_timer", beep )
...
function HUDManager:activate_objective( data )
	-- print( "HUDManager:activate_objective", inspect( data ) )
	self._hud_objectives:activate_objective( data )
end

-- HIJACKED
function HUDManager:complete_sub_objective( data )
	-- print( "HUDManager:complete_sub_objective", inspect( data ) )
...
function HUDManager:complete_objective( data )
	-- print( "HUDManager:complete_objective", inspect( data ) )
...
-- status=false : off, status=[number] : notice progress 0-1
...
function HUDManager:_add_name_label( data )
	...
	local radius = 24
	local interact = CircleBitmapGuiObject:new( panel, { --[[image = "guis/textures/pd2/hud_progress_32px",]] use_bg = true, radius = radius, blend_mode = "add", color = Color.white, layer = 0 } )
	interact:set_visible( false ) 
	-- interact:set_position( self._blackscreen_panel:w() - self._circle_radius * 3, self._blackscreen_panel:h() - self._circle_radius * 3 )
	...
	local bag = panel:bitmap( { name = "bag", texture = tabs_texture, texture_rect = bag_rect, visible = false, layer = 0, color = (crim_color * 1.1):with_alpha( 1 ), x = 1, y = 1 } )
	local text = panel:text( { name = "text", text = utf8.to_upper( data.name ), font = tweak_data.hud.medium_font, font_size = tweak_data.hud.name_label_font_size, color = crim_color, align = "right", vertical = "top", layer = -1, w = 256, h = 18 } )
...
function HUDManager:teammate_progress( peer_id, type_index, enabled, tweak_data_id, timer, success )
	-- print( "HUDManager:teammate_progress", peer_id, enabled, tweak_data_id, timer, success )
	local name_label = self:_name_label_by_peer_id( peer_id )
	if name_label then
		...
		elseif type_index == 3 then -- Putting on mask
		...
		elseif success then
			...
			bitmap:set_position( name_label.interact:position() ) -- self._interact_circle:position() )
...
function HUDManager:setup_blackscreen_hud()
	-- print( "HUDManager:setup_blackscreen_hud()" )
...
function HUDManager:layout_mission_briefing_hud()
	if( self._hud_mission_briefing ) then
		self._hud_mission_briefing:update_layout()	-- change resolution
	end
end
...
function HUDManager:layout_endscreen_hud()
	if( self._hud_stage_endscreen ) then
		self._hud_stage_endscreen:update_layout()	-- change resolution
	end
end
...
function HUDManager:set_group_statistics_endscreen_hud( best_kills, best_kills_score,
	...
	if( self._hud_stage_endscreen ) then
		self._hud_stage_endscreen:set_group_statistics( best_kills, best_kills_score,
										best_special_kills, best_special_kills_score, 
										best_accuracy, best_accuracy_score, 
										most_downs, most_downs_score, 
										total_kills, total_specials_kills, total_head_shots, group_accuracy, group_downs )
...
function HUDManager:layout_lootscreen_hud()
	if( self._hud_lootscreen ) then
		self._hud_lootscreen:update_layout()	-- change resolution
	end
end
...
function HUDManager:_create_test_circle()
	...
	self._test_circle = CircleGuiObject:new( managers.hud:script( PlayerBase.PLAYER_INFO_HUD_PD2 ).panel, { radius = 10, sides = 64, current = 10, total = 10 } )
