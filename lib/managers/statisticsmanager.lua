...
function StatisticsManager:_setup(reset)
	...
	do
		local (for generator), (for state), (for control) = ipairs(tweak_data.levels._level_index)
		do
			do break end
...
function StatisticsManager:_flush_log()
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._data_log)
		do
			do break end
			local type = line[1]
			local time = line[2]
...
function StatisticsManager:publish_to_steam(session, success, completion)
	...
	do
		local (for generator), (for state), (for control) = pairs(session.shots_by_weapon)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(session.killed_by_weapon)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(session.killed_by_melee)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(session.killed)
		do
			do break end
...
function StatisticsManager:publish_skills_to_steam()
	...
	do
		local (for generator), (for state), (for control) = ipairs(tree_data)
		do
			do break end
...
function StatisticsManager:_calculate_average()
	...
	do
		local (for generator), (for state), (for control) = pairs(self._global.average.killed)
		do
			do break end
	...
	local (for generator), (for state), (for control) = pairs(self._global.average.experience)
	do
		do break end
		if data.actions then
			data.count = math.round(data.count / t)
			local (for generator), (for state), (for control) = pairs(data.actions)
			do
				do break end
...
- Experience -]])
	do
		local (for generator), (for state), (for control) = pairs(self._global.experience)
		do
			do break end
...
function StatisticsManager:_check_loaded_data()
	...
	do
		local (for generator), (for state), (for control) = pairs(self._defaults.killed)
		do
			do break end
...
function StatisticsManager:favourite_weapon()
	...
	local count = 0
	do
		local (for generator), (for state), (for control) = pairs(self._global.killed_by_weapon)
		do
			do break end
...
function StatisticsManager:session_favourite_weapon()
	...
	local count = 0
	do
		local (for generator), (for state), (for control) = pairs(self._global.session.killed_by_weapon)
		do
			do break end
			if count < data.count then
				count = data.count
...
function StatisticsManager:session_used_weapons()
	...
	if self._global.session.shots_by_weapon then
		local (for generator), (for state), (for control) = pairs(self._global.session.shots_by_weapon)
		do
			do break end
