...
function StatisticsManager:init()
	self:_setup()
	self:_reset_session() -- Always make sure that session is clean when starting
end

function StatisticsManager:_setup( reset )
	self._defaults = {}
	self._defaults.killed = {
		other = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		civilian = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		civilian_female = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		security = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		cop = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		swat = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		heavy_swat = { count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		total = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		shield = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		spooc = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		tank = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		taser = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		gangster = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		fbi = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
		sniper = {	count = 0, head_shots = 0, melee = 0, explosion = 0, tied = 0 },
	...
	for _,lvl in ipairs( tweak_data.levels._level_index ) do
		self._defaults.sessions.levels[ lvl ] = { started = 0, completed = 0, quited = 0, drop_in = 0, from_beginning = 0, time = 0 }
	end	
	...
	self._defaults.downed 		= { bleed_out = 0, fatal = 0, incapacitated = 0, death = 0 }
...
function StatisticsManager:update( t, dt )
	if self._data_log then
		...
		if self._log_timer <= 0 and alive( managers.player:player_unit() ) then
			...
			table.insert( self._data_log, { 1, Application:time(), managers.player:player_unit():position(), 1 / dt } )
			
			if Network:is_server() then
				for u_key, u_data in pairs( managers.enemy:all_enemies() ) do
					table.insert( self._data_log, { 2, Application:time(), u_data.unit:position(), 1, u_key } )
				end
				for u_key, u_data in pairs( managers.groupai:state()._ai_criminals ) do
					table.insert( self._data_log, { 2, Application:time(), u_data.unit:position(), 2, u_key } )
				end
				for u_key, u_data in pairs( managers.enemy:all_civilians() ) do
					table.insert( self._data_log, { 2, Application:time(), u_data.unit:position(), 3, u_key } )
				end
			end
...
function StatisticsManager:start_session( data )
	...
	--[[
	local d = os.date( "*t" )
	self._data_log_name = d.year .. "-" .. d.month .. "-" .. d.day .. " " .. d.hour .. "." .. d.min .. "." .. d.sec .. " -- " .. Global.level_data.level_id .. " -- " .. managers.network.account:player_id() .. ".txt"
	...
	if Global.level_data.level_id then -- else probably in editor
	...
	self._global.session = deep_clone( self._defaults ) -- Start new session, reset to default values
...
function StatisticsManager:stop_session( data )
	...
	self._global.last_session = deep_clone( self._global.session )  -- Stop session, save session to last session
	self:_calculate_average()
...
function StatisticsManager:_increment_misc( name, amount )
	...
	if self._data_log and alive( managers.player:player_unit() ) then
		table.insert( self._data_log, { 3, Application:time(), managers.player:player_unit():position(), name, amount } )
	end
end
...
function StatisticsManager:_get_stat_tables()
	local level_list = {	"safehouse",
												"jewelry_store", "four_stores", "nightclub", "mallcrasher", "ukrainian_job",
												"branchbank",
												...
												"escape_cafe_day", "escape_park_day", "escape_cafe", "escape_park", "escape_street", "escape_overpass", "escape_garage",
												"family",
												"arm_cro", "arm_und", "arm_hcm", "arm_par", "arm_fac", "arm_for"
											}
	
	local job_list = {	"jewelry_store", "four_stores", "nightclub", "mallcrasher", "ukrainian_job_prof",
...
function StatisticsManager:publish_to_steam( session, success )
	...
	-- stats.player_time = { type = "int", method = "set", value = current_time }

	local time_found = false
	local play_times = { 1000, 500, 250, 200, 150, 100, 80, 40, 20, 10, 0 }

	for _, play_time in ipairs( play_times ) do
		if not time_found and play_time <= current_time then
			stats[ "player_time_" .. play_time .. "h" ] = { type = "int", method = "set", value = 1 }
			time_found = true
		else
			stats[ "player_time_" .. play_time .. "h" ] = { type = "int", method = "set", value = 0 }
		end
	end
	...
	stats[ "player_level" ] = { type = "int", method = "set", value = current_level }

	for i = 0, 100, 10 do
		stats[ "player_level_" .. i ] = { type = "int", method = "set", value = 0 }
	end

	local level_range = current_level >= 100 and 100 or ( math.floor( current_level / 10 ) * 10 )
	stats[ "player_level_" .. level_range ] = { type = "int", method = "set", value = 1 }



	local current_cash = managers.money:offshore()
	-- stats[ "player_cash" ] = { type = "int", method = "set", value = current_cash }
	...
	for i = 0, 9 do
		if not cash_found and cash_amount <= current_cash then
			stats[ "player_cash_" .. cash_amount .. "k" ] = { type = "int", method = "set", value = 1 }
			cash_found = true
		else
			stats[ "player_cash_" .. cash_amount .. "k" ] = { type = "int", method = "set", value = 0 }
		end

		cash_amount = cash_amount / 10
	end

	stats[ "player_cash_0k" ] = { type = "int", method = "set", value = cash_found and 0 or 1 }
	...
	stats[ "gadget_used_ammo_bag" ] = 		{ type = "int", value = session.misc.deploy_ammo or 0 }
	stats[ "gadget_used_doctor_bag" ] = 	{ type = "int", value = session.misc.deploy_medic or 0 }
	stats[ "gadget_used_trip_mine" ] = { type = "int", value = session.misc.deploy_trip or 0 }
	stats[ "gadget_used_sentry_gun" ] = { type = "int", value = session.misc.deploy_sentry or 0 }
	stats[ "gadget_used_ecm_jammer" ] = { type = "int", value = session.misc.deploy_jammer or 0 }
	...
	stats[ "heist_success" ] = { type = "int", value = success and 1 or 0 }
	stats[ "heist_failed" ] = { type = "int", value = success and 0 or 1 }
...
function StatisticsManager:publish_skills_to_steam()
	...
	for tree_index, tree in ipairs( skill_data ) do
		...
		for _, tier in ipairs( tree.tiers ) do
			for _, skill in ipairs( tier ) do
				...
				stats[ "skill_" .. tree.skill .. "_" .. skill ] = { type = "int", method = "set", value = skill_bought }
				stats[ "skill_" .. tree.skill .. "_" .. skill .. "_ace" ] = { type = "int", method = "set", value = skill_aced }
	...
	for tree_index, tree in ipairs( skill_data ) do
		stats[ "skill_" .. tree.skill ] = { type = "int", method = "set", value = skill_amount[ tree_index ] }

		for i = 0, 35, 5 do
			stats[ "skill_" .. tree.skill .. "_" .. i ] = { type = "int", method = "set", value = 0 }
		end

		stats[ "skill_" .. tree.skill .. "_" .. math.ceil( skill_amount[ tree_index ] / 5 ) * 5 ] = { type = "int", method = "set", value = 1 }
	end
...
function StatisticsManager:clear_statistics()
	...
	stats.player_time = {type = "int", method = "set", value = 0}
	...
	stats[ "player_level" ] = { type = "int", method = "set", value = 0 }
	...
	stats[ "player_cash" ] = { type = "int", method = "set", value = 0 }
	...
	stats[ "player_cash_0k" ] = {type = "int", method = "set", value = 0}
	...
	stats[ "gadget_used_ammo_bag" ] = { type = "int", method = "set", value = 0 }
	stats[ "gadget_used_doctor_bag" ] = { type = "int", method = "set", value = 0 }
	stats[ "gadget_used_trip_mine" ] = { type = "int", method = "set", value = 0 }
	stats[ "gadget_used_sentry_gun" ] = { type = "int", method = "set", value = 0 }
	stats[ "gadget_used_ecm_jammer" ] = { type = "int", method = "set", value = 0 }
	...
	for _, difficulty in pairs( tweak_data.difficulties ) do
		stats[ "difficulty_" .. difficulty ] = { type = "int", method = "set", value = 0 }
	end



	stats[ "heist_success" ] = { type = "int", method = "set", value = 0 }
	stats[ "heist_failed" ] = { type = "int", method = "set", value = 0 }
	
	
	
	stats[ "payday2" ] = { type = "int", method = "set", value = 0 }
...
function StatisticsManager:clear_skills_statistics()
	...
	for tree_index, tree in ipairs( skill_data ) do
		for _, tier in ipairs( tree.tiers ) do
			for _, skill in ipairs( tier ) do
				stats[ "skill_" .. tree.skill .. "_" .. skill ] = { type = "int", method = "set", value = 0 }
				stats[ "skill_" .. tree.skill .. "_" .. skill .. "_ace" ] = { type = "int", method = "set", value = 0 }
			end
		end
	end
	...
	for tree_index, tree in ipairs( skill_data ) do
		stats[ "skill_" .. tree.skill ] = { type = "int", method = "set", value = 0 }
	...
	for _, level_id in ipairs( level_list ) do
		stats[ "level_"..level_id ] = { type = "int", method = "set", value = 0 }
	end
	
	
	
	for _, job_id in ipairs( job_list ) do
		stats[ "job_"..job_id ] = { type = "int", method = "set", value = 0 }
	end
...
function StatisticsManager:debug_estimate_steam_players()
	...
	local play_times = { 0, 10, 20, 40, 80, 100, 150, 200, 250, 500, 1000 }
	for _, play_time in ipairs( play_times ) do
...
function StatisticsManager:debug_print_stats( global_flag, days )
	...
	-- table.insert( stats, { name = "player_time", loc = account:get_stat( "player_time" ), glo = account:get_global_stat( "player_time", days ) } )

	local play_times = {0, 10, 20, 40, 80, 100, 150, 200, 250, 500, 1000}

	for _, play_time in ipairs( play_times ) do
		key = "player_time_" .. play_time .. "h"
		table.insert( stats, { name = key, loc = account:get_stat( key ), glo = account:get_global_stat( key, days ) } )
	end



	table.insert( stats, { name = "player_level", loc = account:get_stat( "player_level" ), glo = account:get_global_stat( "player_level", days ) } )
	...
	-- table.insert( stats, { name = "player_cash", loc = account:get_stat( "player_cash" ), glo = account:get_global_stat( "player_cash", days ) } )

	table.insert( stats, { name = "player_cash_0k", loc = account:get_stat( "player_cash_0k" ), glo = account:get_global_stat( "player_cash_0k", days ) } )
	...
	table.insert( stats, { name = "gadget_used_ammo_bag", loc = account:get_stat( "gadget_used_ammo_bag" ), glo = account:get_global_stat( "gadget_used_ammo_bag", days ) } )
	table.insert( stats, { name = "gadget_used_doctor_bag", loc = account:get_stat( "gadget_used_doctor_bag" ), glo = account:get_global_stat( "gadget_used_doctor_bag", days ) } )
	table.insert( stats, { name = "gadget_used_trip_mine", loc = account:get_stat( "gadget_used_trip_mine" ), glo = account:get_global_stat( "gadget_used_trip_mine", days ) } )
	table.insert( stats, { name = "gadget_used_sentry_gun", loc = account:get_stat( "gadget_used_sentry_gun" ), glo = account:get_global_stat( "gadget_used_sentry_gun", days ) } )
	table.insert( stats, { name = "gadget_used_ecm_jammer", loc = account:get_stat( "gadget_used_ecm_jammer" ), glo = account:get_global_stat( "gadget_used_ecm_jammer", days ) } )
	...
	table.insert( stats, { name = "heist_success", loc = account:get_stat( "heist_success" ), glo = account:get_global_stat( "heist_success", days ) } )
	table.insert( stats, { name = "heist_failed", loc = account:get_stat( "heist_failed" ), glo = account:get_global_stat( "heist_failed", days ) } )
	...
	table.insert( stats, { name = "payday2", loc = account:get_stat( "payday2" ), glo = account:get_global_stat( "payday2", days ) } )
...
function StatisticsManager:killed( data )
	...
	if not self._global.killed[ data.name ] then
		...
		data.name = "other"
		-- Application:stack_dump_error( "Bad name id applied to killed, "..tostring( data.name ).."." )
		-- return
	end
...
function StatisticsManager:_bullet_challenges( data )
	...
	-- managers.challenges:count_up( "kill" )
	if data.head_shot then
		managers.challenges:count_up( data.type.."_head_shot" )
	--	managers.challenges:count_up( "head_shot" )
	else
...
function StatisticsManager:_explosion_challenges( data )
	...
	if weapon_id == "m79" then
		...
		if data.name == "shield" or 
			data.name == "spooc" or 
			data.name == "tank" or 
			data.name == "taser" then
...
function StatisticsManager:tied( data )
	...
	if self._data_log and alive( managers.player:player_unit() ) then
		table.insert( self._data_log, { 3, Application:time(), managers.player:player_unit():position(), "tiedown", 1 } )
	end
end
...
function StatisticsManager:revived( data )
	...
	-- managers.experience:perform_action( "revive" )
	...
	if self._data_log and alive( managers.player:player_unit() ) then
		table.insert( self._data_log, { 3, Application:time(), managers.player:player_unit():position(), "revive", 1 } )
	end
end
...
function StatisticsManager:objective_completed( data )
	-- Only count objectives if we have left the waiting/equip screen
...
function StatisticsManager:downed( data )
	...
	if self._data_log and alive( managers.player:player_unit() ) then
		table.insert( self._data_log, { 3, Application:time(), managers.player:player_unit():position(), "downed", 1 } )
	end
end
...
function StatisticsManager:reloaded( data )
	...
	if self._data_log and alive( managers.player:player_unit() ) then
		table.insert( self._data_log, { 3, Application:time(), managers.player:player_unit():position(), "reloaded", 1 } )
	end
end

function StatisticsManager:recieved_experience( data )
	self._global.experience[ data.size ] = self._global.experience[ data.size ] or { count = 0, actions = {} }
	...
	self._global.session.experience[ data.size ] = self._global.session.experience[ data.size ] or { count = 0, actions = {} }
...
function StatisticsManager:print_stats()
	...
	for name,data in pairs( self._global.killed ) do
		...
		.." Head shots: "..self:_amount_format(data.head_shots).."/"..self:_amount_format(self._global.average.killed[ name ].head_shots, true)
		.." Melee: "..self:_amount_format(data.melee).."/"..self:_amount_format(self._global.average.killed[ name ].melee, true)
		.." Explosion: "..self:_amount_format(data.explosion).."/"..self:_amount_format(self._global.average.killed[ name ].explosion, true).." "..name )
	end
...
function StatisticsManager:_print_experience_stats()
	...
	for size,data in pairs( self._global.experience ) do
		...
		for action,count in pairs( data.actions ) do
			...
			print( "\tAction: "..action ) -- .." "..self:_amount_format(count).."/"..self:_amount_format(count, true) )
...
function StatisticsManager:_amount_format( amount, left )
	...
	local s = ""
	for i = 6 - string.len( amount ), 0, -1 do s = s.." " end
...
function StatisticsManager:_time_text( time, params )
	...
	local days = no_days and 0 or math.floor( time/(3600 * 24) )
...
function StatisticsManager:favourite_level()
	...
	local c_name = nil
	for name,data in pairs( self._global.sessions.levels ) do
		c_name = ((data.started > started) and name) or c_name
		started = ((data.started > started) and data.started) or started
	end
...
function StatisticsManager:session_total_specials_kills()
	return self._global.session.killed.shield.count +
	self._global.session.killed.spooc.count +
	self._global.session.killed.tank.count +
	self._global.session.killed.taser.count
end
...
function StatisticsManager:session_total_law_enforcer_kills()
	return self._global.session.killed.total.count 
		- self._global.session.killed.civilian.count
		- self._global.session.killed.civilian_female.count
		- self._global.session.killed.gangster.count
		- self._global.session.killed.other.count
end
