...
function ChallengesManager:_setup(reset)
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.challenges)
		do
			do break end
			local (for generator), (for state), (for control) = pairs(challenges)
			do
				do break end
				if managers.dlc:has_full_game() or not challenge.awards_achievment and challenge.in_trial then
					...
					if challenge.reset_criterias then
						local (for generator), (for state), (for control) = ipairs(challenge.reset_criterias)
						do
							do break end
...
function ChallengesManager:check_active_challenges()
	...
	do
		local (for generator), (for state), (for control) = pairs(self._challenges_map)
		do
			do break end
...
function ChallengesManager:_check_depends_on(name)
	...
	if self._challenges_map[name].depends_on.challenges then
		local (for generator), (for state), (for control) = ipairs(self._challenges_map[name].depends_on.challenges)
		do
			do break end
...
function ChallengesManager:count_up(counter_id)
	...
	local (for generator), (for state), (for control) = ipairs(self._counter_map[counter_id])
	do
		do break end
...
function ChallengesManager:set_flag(flag_id)
	...
	local (for generator), (for state), (for control) = ipairs(self._flag_map[flag_id])
	do
		do break end
...
function ChallengesManager:session_stopped(...)
	local actives = clone(self._global.active)
	local (for generator), (for state), (for control) = ipairs(self._session_stopped_map)
	do
		do break end
		if actives[name] and (not self._global.completed[name] or actives[name].already_awarded) then
...
function ChallengesManager:reset(criteria)
	...
	local (for generator), (for state), (for control) = ipairs(self._reset_map[criteria])
	do
		do break end
		if self._global.active[name] then
...
function ChallengesManager:get_near_completion()
	local t = {}
	do
		local (for generator), (for state), (for control) = pairs(self._global.active)
		do
			do break end
...
function ChallengesManager:get_completed()
	local t = {}
	do
		local (for generator), (for state), (for control) = pairs(self._global.completed)
		do
			do break end
...
function ChallengesManager:_correct_difficulty(difficulty)
	...
	if type(difficulty) == "table" then
		do
			local (for generator), (for state), (for control) = ipairs(difficulty)
			do
				do break end
...
function ChallengesManager:check_still_active_available()
	local actives = clone(self._global.active)
	local (for generator), (for state), (for control) = pairs(actives)
	do
		do break end
		if not self._challenges_map[name] then
			self._global.active[name] = nil
