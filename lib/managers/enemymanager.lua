...
30
	if self._gfx_lod_data.enabled and managers.navigation:is_data_ready() then
		...
		if camera_rot then
			...
			do
				local (for generator), (for state), (for control) = ipairs(states)
				do
					do break end
				...
				repeat
					if states[i] then
						...
						elseif not world_in_view_with_options(World, com[i], 0, 120, 18000) then
							...
							local previous_prio
							do
								local (for generator), (for state), (for control) = ipairs(imp_i_list)
								do
									do break end
									if i == i_entry then
										previous_prio = prio
...
function EnemyManager:_remove_i_from_lod_prio(i, anim_lod)
	...
	local (for generator), (for state), (for control) = ipairs(self._gfx_lod_data.prio_i)
	do
		do break end
		if i == i_entry then
...
function EnemyManager:_update_queued_tasks(t)
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._queued_tasks)
		do
			do break end
...
function EnemyManager:remove_delayed_clbk(id)
	local all_clbks = self._delayed_clbks
	do
		local (for generator), (for state), (for control) = ipairs(all_clbks)
		do
			do break end
...
17
	...
	do
		local (for generator), (for state), (for control) = pairs(to_dispose)
		do
			do break end
...
function EnemyManager:save(data)
	...
	if not managers.groupai:state():enemy_weapons_hot() then
		my_data = my_data or {}
		local (for generator), (for state), (for control) = pairs(self._enemy_data.corpses)
		do
			do break end
...
function EnemyManager:load(data)
	...
	if my_data.corpses then
		...
		local (for generator), (for state), (for control) = pairs(my_data.corpses)
		do
			do break end
