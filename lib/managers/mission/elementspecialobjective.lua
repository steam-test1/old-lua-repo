...
ElementSpecialObjective._pathing_types = { "destination", "precise", "coarse" }
...
function ElementSpecialObjective:init( ... )
	...
	if self._values.follow_up_id then -- backwards compatibility
		self._values.followup_elements = { self._values.follow_up_id }
...
function ElementSpecialObjective:clbk_objective_administered( unit )
	if self._values.needs_pos_rsrv then
		...
		if unit_rsrv then
			...
			unit_rsrv = { position = self._values.align_position and self._values.position or unit:position(), radius = 30 }
...
function ElementSpecialObjective:clbk_verify_administration( unit )
	if self._values.needs_pos_rsrv then
		self._tmp_pos_rsrv = self._tmp_pos_rsrv or { position = self._values.position, radius = 30 }
...
function ElementSpecialObjective:on_executed( instigator )
	...
	if not ( managers.groupai:state():is_AI_enabled() or Application:editor() ) then
		--skip
	elseif self._values.spawn_instigator_ids then	-- collect units from spawn elements
	...
	elseif self._values.use_instigator then
		...
		elseif type_name( instigator ) == "Unit" and alive( instigator ) then
			if instigator:brain() then
				if not ( instigator:character_damage() and instigator:character_damage():dead() ) and managers.navigation:check_access( self._values.SO_access, instigator:brain():SO_access(), 0 ) then
	...
	elseif self:_is_nav_link() then
		...
		if objective then
			...
			local so_descriptor = {
				...
				interval = self._values.interval,		-- -1 is a special value. It means the SO will be attempted once and then discarded
				search_dis_sq = search_dis_sq,
				search_pos = self._values.search_position,
				usage_amount = self._values.trigger_times, -- nil means the SO may be triggered infinite times
...
-- This function is called both internally and externally by groupAI when executing queued SOs
function ElementSpecialObjective:get_objective( instigator )
	...
	local objective = {
		type = false, -- gets filled in later
		pos = pos,
		rot = rot,
		path_data = false, -- gets filled in later
		...
		no_retreat = not ( interrupt_dis or interrupt_health ),
	...
	if is_AI_SO then	-- AI SO
		local objective_type = string.sub( self._values.so_action, 4 ) -- substring consisting of the ending of "AI_..."
	...
	--print( "[ElementSpecialObjective:get_objective]", inspect( objective ) )
	return objective
end
...
function ElementSpecialObjective:_get_misc_SO_params()
	...
	if values.interrupt_dis == -1 then -- any distance
		interrupt_dis = -1
	elseif values.interrupt_dis ~= 0 then -- convert m to cm
		interrupt_dis = values.interrupt_dis * 100
	end -- nil = no interrupt on distance
...
function ElementSpecialObjective:_select_units_from_spawners()
	...
	for _, element_id in ipairs( self._values.spawn_instigator_ids ) do
		...
		for _, unit in ipairs( spawn_element:units() ) do
			if alive( unit ) and not ( unit:character_damage() and unit:character_damage():dead() ) and managers.navigation:check_access( self._values.SO_access, unit:brain():SO_access(), 0 ) then
	...
	if self._values.forced or unit:brain():is_available_for_assignment( objective ) or not unit:brain():objective() then
		if objective.area then
			...
			if u_data and u_data.assigned_area then
				...
				--print("!!ASSIGNED DUDE TO GROUP AI!!")
			end
		end
...
-----------------------------------------------------------------------------------
-- also called from ElementSpecialObjectiveGroup
-- skip_element_ids: table=these mission elements are to be excluded. nil=I am the current objective element. I decide if i want to exclude myself
function ElementSpecialObjective:choose_followup_SO( unit, skip_element_ids )
	--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "unit", unit, "skip_element_ids", skip_element_ids and inspect( skip_element_ids ) )
	...
	--print( "skip_element_ids", skip_element_ids and inspect( skip_element_ids ) )
	...
	for _, followup_element_id in ipairs( self._values.followup_elements ) do
		--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "examining", followup_element_id )
		local weight
		...
		if followup_element:enabled() then
			...
			if followup_element and followup_element:enabled() then
				--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "weight", weight )
				if weight > 0 then
	...
	if not next( pool ) or total_weight <= 0 then
		--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "empty pool" )
		return
	end
	
	local lucky_w = math.random() * total_weight
	--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "lucky_w", lucky_w, "total_weight", total_weight )
	local accumulated_w = 0
	for i, followup_data in ipairs( pool ) do
		accumulated_w = accumulated_w + followup_data.weight
		--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "accumulated_w", accumulated_w )
		if accumulated_w >= lucky_w then
			--print( "[ElementSpecialObjective:choose_followup_SO] self._id", self._id, "selecting", inspect( pool[ i ].element ) )
...
function ElementSpecialObjective:get_as_followup( unit, skip_element_ids )
	--print( "[ElementSpecialObjective:get_as_followup] self._id", self._id, unit, skip_element_ids and inspect( skip_element_ids ) )
