...
function ObjectInteractionManager:remove_object(obj)
	local (for generator), (for state), (for control) = pairs(self._interactive_objects)
	do
		do break end
		if v == obj then
...
function ObjectInteractionManager:_raycheck_ok(obj, camera_pos)
	...
	do
		local (for generator), (for state), (for control) = ipairs(check_objects)
		do
			do break end
