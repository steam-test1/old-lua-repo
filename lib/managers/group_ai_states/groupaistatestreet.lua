...
function GroupAIStateStreet:_upd_blockade_task(t)
	...
	do
		local (for generator), (for state), (for control) = ipairs(event.blockade_points)
		do
			do break end
			local blockade_pos = blockade_point[2]
			local (for generator), (for state), (for control) = pairs(relevant_players)
			do
				do break end
	...
	if not self._task_data.regroup.active then
		if spawn_threshold > 0 then
			...
			if not flank_assault.sneak_unit_key and t > flank_assault.next_dispatch_t then
				...
				do
					local (for generator), (for state), (for control) = pairs(relevant_areas)
					do
						do break end
...
function GroupAIStateStreet:_upd_reenforce_tasks(t)
	local (for generator), (for state), (for control) = ipairs(self._task_data.reenforce.tasks)
	do
		do break end
		...
		do
			local (for generator), (for state), (for control) = pairs(target_area_data.police.units)
			do
				do break end
				local objective = u_data.unit:brain():objective()
		...
		elseif undershot > 0 then
			...
			if existing_cops then
				do
					local (for generator), (for state), (for control) = ipairs(existing_cops)
					do
						do break end
						local new_objective = {
							type = "defend_area",
...
function GroupAIStateStreet:_upd_assault_task(t)
	...
	do
		local (for generator), (for state), (for control) = pairs(area_data.criminal.units)
		do
			do break end
...
24
	...
	if nav_segs then
		local (for generator), (for state), (for control) = pairs(nav_segs)
		do
			do break end
	...
	repeat
		...
		do
			local (for generator), (for state), (for control) = pairs(nav_segs or all_nav_segs)
			do
				do break end
				if nav_segs then
...
function GroupAIStateStreet:_on_area_safety_status(seg, event)
	...
	do
		local (for generator), (for state), (for control) = pairs(area_data.police.units)
		do
			do break end
	...
	if area_data.neighbours then
		...
		do
			do break end
			local (for generator), (for state), (for control) = pairs(all_areas[neighbour_data.seg].police.units)
			do
				do break end
				local unit = unit_data[u_key].unit
				unit:brain():on_area_safety(seg, safe, event)
	...
	if force and force > 0 then
		...
		do
			...
			if force_factor and force_factor.id == id then
				...
				do
					local (for generator), (for state), (for control) = ipairs(tasks)
					do
						do break end
						if task_data.target_area == nav_seg then
...
function GroupAIStateStreet:_map_ai_blockade_spots()
	...
	local (for generator), (for state), (for control) = ipairs(all_spots)
	do
		do break end
