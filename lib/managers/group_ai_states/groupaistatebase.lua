...
GroupAIStateBase._nr_important_cops = 3 -- # of high AI resolution enemies each human player is allowed


-- Max 255 blames
GroupAIStateBase.BLAME_SYNC = {		-- this table doubles as a weighting system, with the most important in the begining and the least in the end
	"empty",						-- always the most important
...
function GroupAIStateBase:set_AI_enabled( state )
	...
	if Network:is_server() then
		for u_key, u_data in pairs( managers.enemy:all_enemies() ) do
			...
			if state and not is_active or not state and is_active then	-- the unit is operational
...
function GroupAIStateBase:_init_misc_data()
	...
	self._criminals = {}
	self._ai_criminals = {}	-- data tables of AI criminals
	self._player_criminals = {}	-- data tables of player criminals
	self._special_units = {}	-- a table of various special unit categories
	self._special_unit_types = { tank = true, spooc = true, shield = true, taser = true }
...
function GroupAIStateBase:add_alert_listener( id, clbk, filter_num, types, m_pos )
	local listener_data = { clbk = clbk, filter = filter_num, types = types, m_pos = m_pos }
...
-----------------------------------------------------------------------------------
-- alert_data:
-- if alert_type == "footstep", "vo_ntl", "vo_cbt", "vo_intimidate", "vo_distress", "aggression", "bullet" : { alert_type, epicenter, effective_radius, alert_filter, alerting_unit }
function GroupAIStateBase:propagate_alert( alert_data )
	--print( "[GroupAIStateBesiege:propagate_alert]", inspect( alert_data ) )
	...
	if listeners_by_type then
		...
		if alert_epicenter then
			-- Draw:brush( Color(0.1, 0, 1, 0), 2 ):sphere( alert_data[2], alert_data[3] )
			local alert_rad_sq = alert_data[3] * alert_data[3]
			
			proximity_chk_func = function( listener_pos )
				return mvec3_dis_sq( alert_epicenter, listener_pos ) < alert_rad_sq
			end
		else
			proximity_chk_func = function()
				return true
			end
		end
		...
		for filter_str, listeners_by_type_and_filter in pairs( listeners_by_type ) do
			local key, listener = next( listeners_by_type_and_filter, nil ) -- take first listener
			...
			if access_func( nav_manager, filter_num, alert_filter, nil ) then
				for id, listener in pairs( listeners_by_type_and_filter ) do
					--print( "testing listener", id )
					if proximity_chk_func( listener.m_pos ) then
						--print( "proximity ok" )
						-- Draw:brush( Color(0.1, 1, 0, 0), 2 ):sphere( listener.m_pos, 150 )
						clbks = clbks or {}
						table.insert( clbks, listener.clbk ) -- collect all callbacks here. do not call from within the loop
					--else
						--print( "proximity failed" )
					end
				end
			end
		end
...
function GroupAIStateBase:_on_drama_zone_change()
	--print( "[GroupAIStateBase:_on_drama_zone_change]", self._drama_data.zone )
end
		...
		for crim_key, crim in pairs( self:all_AI_criminals() ) do
			if not crim.unit:movement():cool() then
				...
				if not crim.unit:anim_data().upper_body_empty then
					crim.unit:movement():action_request( { type = "idle", body_part = 3, sync = true } )
				end
...
function GroupAIStateBase:on_police_called( called_reason )
	--print( "[GroupAIStateBase:on_police_called]", self._police_call_clbk_id )
...
function GroupAIStateBase:on_enemy_weapons_hot( is_delayed_callback )
	--print( "[GroupAIStateBase:on_enemy_weapons_hot]", self._police_call_clbk_id, is_delayed_callback )
	if not self._ai_enabled then
		return
	end
	...
	if not self._enemy_weapons_hot then
		...
		-- self:notify_bain_weapons_hot( self._called_reason )
...
function GroupAIStateBase:_clbk_switch_enemies_to_not_cool()
	for u_key, unit_data in pairs( self._police ) do
		if unit_data.unit:movement():cool() then
			if unit_data.assigned_area then
	...
	self:propagate_alert( { "vo_cbt", nil, nil, self._unit_type_filter.civilian, nil } )
...
function GroupAIStateBase:_hostage_hint_clbk()
	...
	if self._hostage_headcount == 0 then
		if self._first_hostage_hint then
			-- managers.hint:show_hint( "control_civilians", nil, nil, { BTN_INTERACT = managers.localization:btn_macro( "interact" ) } )
			self._first_hostage_hint = nil
			managers.enemy:add_delayed_clbk( "_hostage_hint_clbk", self._hstg_hint_clbk, Application:time() + 120 )
		else
			-- managers.hint:show_hint( "take_hostages", nil, nil, { BTN_INTERACT = managers.localization:btn_macro( "interact" ) } )
		end
	else
		self._hstg_hint_clbk = nil
	end
...
function GroupAIStateBase:teammate_comment( trigger_unit, message, pos, pos_based, radius, sync )
	--print( "[GroupAIStateBase:teammate_comment]", trigger_unit, message, pos, pos_based, radius, sync )
	...
	local close_criminal = nil
	local close_criminal_d = nil
	...
	if trigger_unit and alive( trigger_unit ) then
		...
		for u_key, u_data in pairs( self._criminals ) do
			if not u_data.is_deployable and ( allow_first_person or not u_data.unit:base().is_local_player ) then 
				if alive( u_data.unit ) and not u_data.unit:movement():downed() and not u_data.unit:sound():speaking() then
					...
					if not radius or ed < radius * radius then
						if not close_criminal_d or d < close_criminal_d then
...
function GroupAIStateBase:_get_spawn_unit_name( weights, wanted_access_type )
	...
	for cat_name, cat_weights in pairs( weights ) do
		...
		--print( "cat_name", cat_name, cat_weight )
		...
		if suitable and cat_data.max_amount then
			...
			if nr_active >= cat_data.max_amount then
				--print( "unsuitable. too many of that type" )
				suitable = false
			end
		end
		if suitable and cat_data.special_type then
			if not self._special_units[ cat_name ] then
				...
				if nr_boss_types_present >= tweak_data.group_ai.max_nr_simultaneous_boss_types then
					--print( "unsuitable. too many boss types" )
					suitable = false
				end
			end
		end
	...
	--print( "lucky_nr", lucky_nr )
	local i_candidate = 1
	while candidate_weights[ i_candidate ] < lucky_nr do
		--print( "skipping", i_candidate, candidates[ i_candidate ] )
...
-----------------------------------------------------------------------------------
-- this is called when a cop detects a criminal. unit is the criminal
...
function GroupAIStateBase:on_criminal_nav_seg_change( unit, nav_seg_id )
	...
	if not u_sighting then
		return -- when the unit spawns it is not yet registered in group ai
	end
...
-----------------------------------------------------------------------------------
-- Returns data about a particular criminal by their key
function GroupAIStateBase:criminal_record( u_key )
	return self._criminals[ u_key ]
end

-----------------------------------------------------------------------------
-- Called when an AI unit engages another unit in combat
...
-----------------------------------------------------------------------------
-- Called when an AI unit disengages a unit it was engaged in combat with
...
function GroupAIStateBase:on_tase_start( cop_key, criminal_key )
	...
	if managers.player:player_unit() and alive( self._criminals[ criminal_key ].unit ) then
...
-----------------------------------------------------------------------------
-- Called when an AI unit ends the tasing process on a criminal
...
-----------------------------------------------------------------------------
-- Called when an AI unit gives a warning to a criminal
...
-----------------------------------------------------------------------------
-- Called when an AI unit ends arresting a criminal
...
function GroupAIStateBase:on_simulation_started()
	...
	local drama_tweak = tweak_data.drama
	self._drama_data = { decay_period = tweak_data.drama.decay_period
												,last_calculate_t = 0
												,amount = 0
												,zone = "low"	-- also nil and "high"
												,low_p = drama_tweak.low
...
function GroupAIStateBase:on_enemy_unregistered( unit )
	...
	-- Check if the unit was killed close to its spawn point. add delay in that case
...
-----------------------------------------------------------------------------
-- a police unit has noticed a criminal performing a hostile action
...
function GroupAIStateBase:register_criminal( unit )
	...
	--local i_vis_group = managers.navigation:get_vis_group_from_tracker( tracker )
	local is_AI
	if unit:base()._tweak_table then	-- this is not a player. its an AI
		is_AI = true
	end
	...
	local u_sighting = { 
		...
		engaged_force = 0,
		dispatch_t = 0,
		--assault_t = nil, -- Last aggression time from the criminal's part
		arrest_timeout = -100,	-- Timeout counter for not attempting to arrest this criminal the whole time
		important_enemies = not is_AI and {} or nil, -- closest enemies
...
function GroupAIStateBase:check_gameover_conditions()
	...
	-- Should be redundant because of the presence check
...
-----------------------------------------------------------------------------
-- Called by the enemy unit's brain when he gets tided by a criminal
function GroupAIStateBase:on_enemy_tied( u_key )
end

-----------------------------------------------------------------------------
-- Called by the enemy unit's brain when he exits the "hostage" state
...
function GroupAIStateBase:_debug_draw_drama( t )
	...
	local t_span = draw_data.t_span -- timespan of the histogram
	
	local drama_hist = draw_data.drama_hist
	if t > drama_hist[ #drama_hist ][2] then
		-- check if drama value has changed
		if #drama_hist == 1 then
			table.insert( drama_hist, { drama_data.amount, t } )
		else -- if the tangent hasn't changed overwrite the time of the previosu entry
			...
			else-- add new drama entry
				table.insert( drama_hist, { drama_data.amount, t } )
			end
		end
	end
	while drama_hist[2] and t - drama_hist[2][2] > t_span do -- remove old drama entries
	...
	for i, entry in ipairs( drama_hist ) do -- Draw drama histogram
	...
	if t > last_entry[2] then -- add new drama entry
		...
		if pop_hist_size > 1 and last_entry[1] == police_force and pop_hist[ pop_hist_size - 1 ][1] == police_force then
			--print( "no population change" )
			last_entry[2] = t
		else
			--print( "appending", self._police_force, t )
			table.insert( pop_hist, { police_force, t } )
		end
	end
	while pop_hist[2] and t - pop_hist[2][2] > t_span do -- remove old population entries
		--print( "disposing", pop_hist[2][2], t )
		table.remove( pop_hist, 1 )
	end
	
	local max_force = 25
	local pop_pen = draw_data.population_pen
	for i, entry in ipairs( pop_hist ) do -- Draw drama histogram
		local new_x = right_limit - width * ( t - entry[2] ) / t_span
	...
	local _draw_events = function( event_brush, event_list )
		while event_list[1] and event_list[1][2] and t - event_list[1][2] > t_span do -- remove old tasks
			table.remove( event_list, 1 )
		end
		for i, entry in ipairs( event_list ) do -- Draw task histogram
...
function GroupAIStateBase:set_drama_draw_state( state )
	if state then
		...
		local width = 1
		local height = 0.3
		local low_zone_color = Color( 1, 0.2, 0.2, 0.7 ) -- Alpha doesnt seem to be taken to account
		...
		self._draw_drama = { background_brush = Draw:brush( background_color )
													...
													,offset_y = offset.y
													,start_t = self._t
													,drama_hist = {{ self._drama_data.amount, self._t }}
													,pop_hist = {{ self._police_force, self._t }}
													,assault_hist = {}
													,regroup_hist = {}
													,t_span = 3 * 60
												}
		...
		if self._task_data then
			for _, task_type in ipairs( { "assault", "blockade" } ) do
				if self._task_data[ task_type ] and self._task_data[ task_type ].active then
					table.insert( self._draw_drama.assault_hist, { self._task_data[ task_type ].start_t } )
					break
				end
			end
			if self._task_data.regroup and self._task_data.regroup.active then
				table.insert( self._draw_drama.regroup_hist, { self._task_data.regroup.start_t } )
			end
		end
	else
...
function GroupAIStateBase:task_names()
	return { "any", "assault", "blockade", "recon", "reenforce", "rescue" }
end

-----------------------------------------------------------------------------

function GroupAIStateBase:add_spawn_event( id, event_data )
	--print( "[GroupAIStateBase:add_spawn_event]", id, inspect( event_data ) )
...
function GroupAIStateBase:on_objective_failed( unit, objective )
	--debug_pause_unit( unit, "[GroupAIStateBase:on_objective_failed]", unit:key(), inspect( objective ) )
	...
	local new_objective
	if unit:brain():objective() == objective then	-- If the unit has not received a new objective then set it free
		local u_key = unit:key()
		local u_data = self._police[ u_key ]
		if u_data and unit:brain():is_active() and not unit:character_damage():dead() then
			new_objective = { type = "free", scan = true, is_default = true, attitude = objective.attitude }
...
function GroupAIStateBase:add_special_objective( id, objective_data )
	...
	local interval = objective_data.chance_inc >= 0 and objective_data.interval >= 0 and objective_data.interval
	...
	if not objective_data.access then	-- backwards compatibility and script SO
		objective_data.access = managers.navigation:convert_SO_AI_group_to_access( objective_data.AI_group )
		--print( "[GroupAIStateBase:add_special_objective] auto-creating access", objective_data.access )
	end
...
function GroupAIStateBase:_execute_so( so_data, so_rooms, so_administered )
	...
	if ai_group == "enemies" then
		for e_key, enemy_unit_data in pairs( self._police ) do
			if enemy_unit_data.assigned_area and not ( so_administered and so_administered[ e_key ] ) then
				if ( so_objective.forced or enemy_unit_data.unit:brain():is_available_for_assignment( so_objective ) )
				and ( not so_data.verification_clbk or so_data.verification_clbk( enemy_unit_data.unit ) ) 
				and access_f( nav_manager, so_access, enemy_unit_data.so_access, 0 ) then
					local dis = max_dis and mvec3_dis_sq( enemy_unit_data.m_pos, pos )
					if ( not closest_dis or closest_dis > dis ) and ( not max_dis or dis < max_dis ) then
	...
	elseif ai_group == "friendlies" then
		for u_key, u_unit_data in pairs( self._ai_criminals ) do
			if not ( so_administered and so_administered[ u_key ] ) then
				if ( so_objective.forced or u_unit_data.unit:brain():is_available_for_assignment( so_objective ))
				and (not so_data.verification_clbk or so_data.verification_clbk( u_unit_data.unit ))
				and access_f( nav_manager, so_access, u_unit_data.so_access, 0 ) then
	...
	if closest_u_data then
		local objective_copy = self.clone_objective( so_objective ) -- cannot deep_clone with a callback object...
	...
	if objective and objective.type == "follow" then
		...
		if ( is_drop_in or spawn_on_unit ) and not self:whisper_mode() then
			...
			if not tracker:lost() then
				...
				local ray_params = { tracker_from = tracker
															,allow_entry = false
															,pos_to = search_pos
															,trace = true
														}
		...
		-- local unit_name = Idstring( "units/payday2/characters/npc_criminal_suit_1/npc_criminal_suit_1" )
		...
		unit:brain():set_spawn_ai( { init_state = "idle", params = { scan = true }, objective = objective } )
		return unit
	end
end
...
function GroupAIStateBase:remove_one_teamAI( name_to_remove, replace_with_player )
	...
	if u_key then
		...
		-- Notify enemies that the criminal is being removed so they can clean up their detection tables
...
function GroupAIStateBase:sync_remove_one_teamAI( name, replace_with_player )
	...
	if replace_with_player then -- player spawns in custody
		return managers.trade:replace_ai_with_player( name, name )
	else -- player spawns free
...
function GroupAIStateBase:_determine_spawn_objective_for_criminal_AI()
	...
	if #valid_criminals > 0 then
		local follow_unit = self._player_criminals[ valid_criminals[ math.random( #valid_criminals ) ] ].unit -- pick a random player
		new_objective = { 
			type = "follow",
...
function GroupAIStateBase:_determine_objective_for_criminal_AI( unit )
	local new_objective
	local closest_dis, closest_record
...
--	SoundDevice:set_state( "whisper", enabled and "true" or "false" )
...
function GroupAIStateBase:hostage_killed( killer_unit )
	...
	if not self._hunt_mode then
		...
		elseif self._hostages_killed >= 7 and self._hostage_killed_warning_lines == 2 then
			if self:sync_hostage_killed_warning( 3 ) then
...
function GroupAIStateBase:on_AI_criminal_death( criminal_name, unit )
	managers.hint:show_hint( "teammate_dead", nil, false, {TEAMMATE=unit:base():nick_name()} )
...
function GroupAIStateBase:amount_of_winning_ai_criminals()
	...
	for _,u_data in pairs( self._ai_criminals ) do
		if alive( u_data.unit ) and 
			not u_data.unit:character_damage():bleed_out() and
			not u_data.unit:character_damage():fatal() and
			not u_data.unit:character_damage():arrested() and
			not u_data.unit:character_damage():dead()
			then
...
-----------------------------------------------------------------------------------
-- a character has just run a detection check and is reporting its importance weight to all human players
-- wgt_report : { pl_unit_key_1, weight_1, pl_unit_key_2, weight_2... }
function GroupAIStateBase:set_importance_weight( u_key, wgt_report )
	...
	for i_dis_rep = #wgt_report - 1, 1, -2 do
		...
		local i_imp = #imp_dis	-- rank new importance
		while i_imp > 0 do
		...
		if i_imp < max_nr_imp then	-- this cop is important for this criinal
			i_imp = i_imp + 1	-- the position we are inserting
			while #imp_enemies >= max_nr_imp do -- list is full. we need to pop the last guy
			...
			if not was_imp then	-- the cop has become important for this criminal
				imp_adj = imp_adj + 1
			end
		elseif was_imp then	-- the cop is no longer important for this criminal
			imp_adj = imp_adj - 1
		end
	end
	
	if imp_adj ~= 0 then -- this enemy's importance counter has changed
...
function GroupAIStateBase:_adjust_cop_importance( e_key, imp_adj )
	...
	if old_imp == 0 or e_data.importance == 0 then	-- our importance status has changed
...
------------------------------------------------------------------------------------------
--	events: an indexed table of event types you are interested receiving callbacks from. ex: { "player_weapons_hot", "enemy_weapons_hot" }
...
function GroupAIStateBase:sync_hostage_headcount( nr_hostages )
	if nr_hostages then	-- nr_hostages from server 
		self._hostage_headcount = nr_hostages
	elseif Network:is_server() then	-- The check is needed because network is stopped before units are destroyed
...
function GroupAIStateBase:chk_area_leads_to_enemy( start_nav_seg_id, test_nav_seg_id, enemy_is_criminal )
	...
	repeat
		...
		for neighbour_seg_id, door_list in pairs( neighbours ) do
			if not ( all_nav_segs[ neighbour_seg_id ].disabled or found_nav_segs[ neighbour_seg_id ] ) then
...
function GroupAIStateBase:chk_say_enemy_chatter( unit, unit_pos, chatter_type )
	--print( "[GroupAIStateBase:chk_say_enemy_chatter]", unit, unit_pos, chatter_type )
	...
	if not chatter_type_hist then	--	First time this event happens
		chatter_type_hist = {
			cooldown_t = 0,
			events = {}
		}
	...
	if t < chatter_type_hist.cooldown_t then
		--print( "[GroupAIStateBase:chk_say_enemy_chatter] too_soon" )
		return	-- too soon
	end
	...
	for i_event, event_data in pairs( chatter_type_hist.events ) do
		if event_data.expire_t < t then
			...
			if mvector3.distance( unit_pos, event_data.epicenter ) < chatter_tweak.radius then
				if nr_events_in_area == chatter_tweak.max_nr - 1 then
					--print( "[GroupAIStateBase:chk_say_enemy_chatter] too many in area" )
					return
				else
	...
	if group_requirement and group_requirement > 1 then
		...
		if nr_in_group < group_requirement then
			--print( "[GroupAIStateBase:chk_say_enemy_chatter] group nr insufficient" )
			return
		end
	end
	
	--print( "[GroupAIStateBase:chk_say_enemy_chatter] SUCCESS" )
	
	chatter_type_hist.cooldown_t = t + math.lerp( chatter_tweak.interval[1], chatter_tweak.interval[2], math.random() )	-- global chatter cooldown
	
	local new_event = {
		epicenter = mvector3.copy( unit_pos ),
		expire_t = t + math.lerp( chatter_tweak.duration[1], chatter_tweak.duration[2], math.random() )	-- event instance expiry
	}
...
function GroupAIStateBase:chk_say_teamAI_combat_chatter( unit )
	if not self:is_detection_persistent() then
		--print( "[GroupAIStateBase:chk_say_teamAI_combat_chatter] non-persistent" )
		return
	end
	
	local drama_amount = self._drama_data.amount
	local frequency_lerp = drama_amount--( drama_amount - 0.6 ) / 0.4
	local delay = math.lerp( 5, 0.5, frequency_lerp )
	local delay_t = self._teamAI_last_combat_chatter_t + delay
	
	--print( "[GroupAIStateBase:chk_say_teamAI_combat_chatter] drama_amount", drama_amount, "frequency_lerp", frequency_lerp, "delay", delay )
	
	if self._t < delay_t then
		--print( "[GroupAIStateBase:chk_say_teamAI_combat_chatter] too soon" )
		return -- too soon
	end
	...
	if math.random() > chance then
		--print( "[GroupAIStateBase:chk_say_teamAI_combat_chatter] no luck", chance )
		return -- no luck
	end
	--print( "YEY!!" )
	--Application:set_pause( true )
	--Application:draw_sphere( unit:position(), 50, 1,0,0 )
...
function GroupAIStateBase:on_nav_link_unregistered( element_id )
	local all_ai = { self._police, self._ai_criminals, managers.enemy:all_civilians() }
...
function GroupAIStateBase:_get_anticipation_duration( anticipation_duration_table, is_first )
	local anticipation_duration = anticipation_duration_table[1][1] -- default is first
	if not is_first then
		...
		for i, setting in pairs( anticipation_duration_table ) do
			...
			if accumulated_chance >= rand then
...
-----------------------------------------------------------------------------------
-- adds a spawn point group. spawn points belonging to the same group may be located on diffrent areas
	...
	for _, spawn_grp_element in ipairs( spawn_groups ) do
		...
		if spawn_points and next( spawn_points ) and spawn_group_names and next( spawn_group_names ) then
			...
			local new_spawn_group_data = {
				id = id
				,pos = Vector3() -- mean position. calculated later
				,nav_seg = nav_seg
				,area = area
...
function GroupAIStateBase:_remove_preferred_spawn_point_from_area( area, sp_data )
	...
	for i, sp_data_ in ipairs( area.spawn_points ) do
		if sp_data_ == sp_data then
			area.spawn_points[i] = area.spawn_points[ #area.spawn_points ] -- swap-erase
...
function GroupAIStateBase:_remove_preferred_spawn_group_from_area( area, sp_data )
	...
	for i, sp_data_ in ipairs( area.spawn_groups ) do
		if sp_data_ == sp_data then
			area.spawn_groups[i] = area.spawn_groups[ #area.spawn_groups ] -- swap-erase
...
-----------------------------------------------------------------------------------
-- go through all area data and remove spawn points with the given id
function GroupAIStateBase:remove_preferred_spawn_points( id )
	if self._spawn_points[ id ] then -- individual spawn points
			...
			if area_spawn_points then
				...
				while i_sp > 0 do
					...
					if sp_data.id == id then
						area_spawn_points[ i_sp ] = area_spawn_points[ #area_spawn_points ]	-- swap-erase
	...
	elseif self._spawn_groups[ id ] then -- spawn group
		for nav_seg, area_data in pairs( self._area_data ) do
			local area_spawn_points = area_data.spawn_groups
			if area_spawn_points then
				local i_sp = #area_spawn_points
...
-----------------------------------------------------------------------------
-- returns true if the group is completely out
...
function GroupAIStateBase:add_area( area_id, nav_segs, area_pos )
	...
	-- if any of the new area's nav_segments were individual areas themselves, remove them and their references
	for _, seg_id in ipairs( nav_segs ) do
				...
				if not neighbour_nav_seg.disabled then
					for other_area_id, other_area in pairs( all_areas ) do
						if other_area.nav_segs[ neighbour_seg_id ] then
							new_area.neighbours[ other_area_id ] = other_area -- he is my neighbour
							if neighbour_nav_seg.neighbours[ seg_id ] then
								other_area.neighbours[ new_area.id ] = new_area -- i am his neighbour
							end
						end
					end
				end
...
-----------------------------------------------------------------------------------
-- default setting: make each nav_segment into its own area
function GroupAIStateBase:_create_area_data()
	...
	-- create basic area data and map accessible neighbours
...
-----------------------------------------------------------------------------------
-- get all areas that include this nav_seg
...
-----------------------------------------------------------------------------------
-- Inform neighbouring areas that they have a new neighbour or that a neighbour is no longer available
function GroupAIStateBase:on_nav_segment_state_change( changed_seg_id, state )
	--debug_pause( "[GroupAIStateBase:on_nav_segment_state_change]", changed_seg_id, state )
	local all_nav_segs = managers.navigation._nav_segments
				...
				if not all_nav_segs[ seg_id ].disabled then
					...
					for other_nav_seg_id, door_list in pairs( nav_seg_neighbours ) do
						...
						if not other_nav_seg.disabled then
							for other_area_id, other_area in pairs( self._area_data ) do
								if other_area_id ~= area_id and other_area.nav_segs[ other_nav_seg_id ] then
									--print( "he is my new neighbour", other_area_id )
									area.neighbours[ other_area_id ] = other_area
									if other_nav_seg.neighbours[ seg_id ] then
										--print( "I am his new neighbour", other_area_id )
										other_area.neighbours[ area_id ] = area
									end
								end
							end
...
-- To un-assign send nil as area_data parameter
function GroupAIStateBase:set_enemy_assigned( area, unit_key )
	-- First unassign from previous area
...
-----------------------------------------------------------------------------------
-- jam_settings: { call = true, camera = true, ... }
...
function GroupAIStateBase:_init_unit_type_filters()
	...
	local law_enforcer_filter = { "security", "security_patrol", "cop", "fbi", "swat", "murky", "sniper", "spooc", "shield", "tank", "taser" }
	...
	local all_enemy_filter = { "gangster", "security", "security_patrol", "cop", "fbi", "swat", "murky", "sniper", "spooc", "shield", "tank", "taser" }
	all_enemy_filter = convert_f( nav_manager, all_enemy_filter )
	local criminal_filter = { "teamAI1", "teamAI2", "teamAI3", "teamAI4" }
	criminal_filter = convert_f( nav_manager, criminal_filter )
	local criminals_and_enemies_filter = { "gangster", "security", "security_patrol", "cop", "fbi", "swat", "murky", "sniper", "spooc", "shield", "tank", "taser", "teamAI1", "teamAI2", "teamAI3", "teamAI4" }
	criminals_and_enemies_filter = convert_f( nav_manager, criminals_and_enemies_filter )
	local criminals_enemies_civilians_filter = { "civ_male", "civ_female", "gangster", "security", "security_patrol", "cop", "fbi", "swat", "murky", "sniper", "spooc", "shield", "tank", "taser", "teamAI1", "teamAI2", "teamAI3", "teamAI4" }
	criminals_enemies_civilians_filter = convert_f( nav_manager, criminals_enemies_civilians_filter )
	local civilians_enemies_filter = { "civ_male", "civ_female", "gangster", "security", "security_patrol", "cop", "fbi", "swat", "murky", "sniper", "spooc", "shield", "tank", "taser" }
...
function GroupAIStateBase:sync_event( event_id, blame_id )
	...
	elseif event_name == "enemy_weapons_hot" then
		...
		managers.music:post_event( tweak_data.levels:get_music_event( "control" ) )
		self:_call_listeners( "enemy_weapons_hot" )
		-- self:notify_bain_weapons_hot( blame_name )
...
-----------------------------------------------------------------------------------
	-- tweak_data.blame
	-- default, mrgreen, mrblue, mrbrown, mrorange, drill, saw, sentry_gun, trip_mine, ecm_jammer, alarm_pager, body_bag, hostage, escort, disturbance
function GroupAIStateBase:notify_bain_weapons_hot( called_reason )
	...
	if( blame ) then
		...
		-- managers.dialog:queue_dialog( blame, {} )


		-- local title = managers.localization:text( "hud_payback_success_title" )
		-- local text = managers.localization:text( "hud_payback_success_text", { revives=tostring(blame) } ) 
		-- local icon = nil
...
function GroupAIStateBase:fetch_highest_giveaway( ... )
	local giveaways  = {...}
	local highest_giveaway_id = #self.BLAME_SYNC	-- with highest I mean highest ranked, most important, i.e. the one with lowest index in BLAME_SYNC
	...
	-- Application:debug( "fetch_highest_giveaway", highest_giveaway, inspect(giveaways) )
	return highest_giveaway
end

function GroupAIStateBase.analyse_giveaway( trigger_string, giveaway_unit, additional_info )		-- string of triggerer, unit of what triggerer saw
	if managers.groupai:state():enemy_weapons_hot() then
			-- police are on their way and blame has already been given
		return nil
	end
	if trigger_string == "empty" then	-- ElementAiGlobalEvent
	...
	if GroupAIStateBase.unique_triggers[trigger_string] then
		-- Application:debug( "UNIQUE", GroupAIStateBase.unique_triggers[trigger_string] )
	...
	if giveaway_string then
		...
		elseif giveaway_prefix then
			-- Application:debug( tostring(giveaway_prefix) .. "_" .. tostring(giveaway_string), trigger_string )
			return tostring(giveaway_prefix) .. "_" .. tostring(giveaway_string) -- giveaway_result
		end
	end
...
function GroupAIStateBase.investigate_unit( giveaway_unit, additional_info )
	local investigate_coolness = false	-- only check for given giveway in giveaway_unit if the alert tells me to
	...
	local investigate_brain = true			-- dissect the brain, if the unit got one 
	local investigate_further = true		-- 
	
	-- local scared_and_mindless = (not investigate_coolness) 	-- to prevent spam in console
	
	-- Application:debug( "GroupAIStateBase.analyse_giveaway", "giveaway_unit", giveaway_unit, "additional_info", inspect(additional_info) )
	if investigate_further then
		if additional_info then
			-- Application:debug( "MORE", inspect(additional_info) )
	...
	if investigate_coolness then
			-- investigating if the giveaway unit got giveaway reason to share
		...
		if unit_movement and unit_movement.coolness_giveaway then
			...
			if coolness_giveaway then
					-- something gave away the giveaway unit's coolness, spread it to me
				if coolness_giveaway == "default" or coolness_giveaway == "empty" then
					return coolness_giveaway
				end
				return string.sub( coolness_giveaway, 5 ) -- coolness_giveaway, true
			end
			-- if coolness_giveaway == false then 
			-- 	scared_and_mindless = true 
			-- end
		end
	end
	...
	if investigate_brain then
		...
		if unit_brain then
			if unit_brain.is_hostage and unit_brain:is_hostage() then
				return "hostage"
			end
		end
	end
	...
	if investigate_criminals then
			-- investigating if the giveaway unit is a criminal (player/ai)
		local criminal_color_id = managers.criminals:character_color_id_by_unit( giveaway_unit )
		if criminal_color_id then
			return "criminal" -- tweak_data.peer_colors[ criminal_color_id ]
		end
	end
	
	
	if investigate_base then
			-- investigating if the giveaway unit got a base extension, then investigate parameters to determine what it is 
	...
	if investigate_attention then
			-- investigating if the giveaway unit got a attention extension
	...
	if investigate_data then
		...
		if unit_data then
			if unit_data.blame_giveaway then
	...
	-- if not scared_and_mindless then
		-- Application:error( "GroupAIStateBase.analyse_giveaway: Investigation failed, did not find anything to blame:", "giveaway_unit", giveaway_unit, "additional_info", inspect(additional_info) )
		-- Application:stack_dump()
		-- return
	-- end
...
function GroupAIStateBase:get_sync_blame_id( blame_name )
	...
	return #self.BLAME_SYNC	-- default
end
...
function GroupAIStateBase:on_nav_seg_neighbours_state( changed_seg_id, neighbours, state )
	--debug_pause( "[GroupAIStateBase:on_nav_seg_neighbours_state]", nav_seg_id, inspect( neighbours ), state )
	local all_nav_segs = managers.navigation._nav_segments
	local changed_seg = all_nav_segs[ changed_seg_id ]
	local changed_seg_neighbours = changed_seg.neighbours
...
function GroupAIStateBase:on_criminal_suspicion_progress( u_suspect, u_observer, status )
	...
	if not obs_susp_data and status and self._whisper_mode then
		...
		local icon = managers.hud:add_waypoint( icon_id, { icon = icon_name, distance = false, position = icon_pos, no_sync = true, present_timer = 0, state = "sneak_present", radius = 100, color = color, blend_mode = "add" }  )
		obs_susp_data = { u_observer = u_observer, icon_id=icon_id, suspects={}, icon_pos = icon_pos }
	...
	if status then
		obs_susp_data.suspects[ susp_key ] = { status = status, u_suspect = u_suspect, last_status_t = self._t }
	else
		obs_susp_data.suspects[ susp_key ] = nil
	end
	
	if not next( obs_susp_data.suspects ) then -- this observer is not detecting anything any more
...
function GroupAIStateBase:_upd_criminal_suspicion_progress()
	...
	local susp_data = self._suspicion_hud_data
	if not ( next( susp_data ) and self._ai_enabled ) then
		return
	end
	
	-- check if any critical icons have expired
		...
		if Network:is_server() then
			...
			for susp_key, suspect_data in pairs( obs_susp_data.suspects ) do
				if suspect_data.status == true then
					if t - suspect_data.last_status_t > 5 then -- expired critical icon
						to_remove = to_remove or {}
						table.insert( to_remove, susp_key )
					else	-- valid critical icon
						critical_icon = true
					end
			...
			if not next( obs_susp_data.suspects ) then -- this observer is not detecting anything any more
				...
				if critical_icon then
					if not obs_susp_data.critical_icon then -- set critical icon
				...
				elseif obs_susp_data.critical_icon then	-- set standard icon
...
function GroupAIStateBase:sync_suspicion_hud( u_observer, status )
	...
	if status then
		...
		if not obs_susp_data then
			...
			local icon = managers.hud:add_waypoint( icon_id, { icon = icon_name, distance = false, position = icon_pos, no_sync = true, present_timer = 0, state = "sneak_present", radius = 100, color = color, blend_mode = "add" }  )
			obs_susp_data = { u_observer = u_observer, icon_id=icon_id, icon_pos = icon_pos }
			susp_data[ obs_key ] = obs_susp_data
		else
	...
	elseif obs_susp_data then -- this observer is not detecting anything any more
		managers.hud:remove_waypoint( obs_susp_data.icon_id )
		susp_data[ obs_key ] = nil
	end
end
...
-----------------------------------------------------------------------------------
-- plocks out consecutive nav_points whose nav_segments belong to the same area, so that each nav point is a different area from its adjucent ones
...
-----------------------------------------------------------------------------------
-- called by a units that need to be removed at end of editor simulation
