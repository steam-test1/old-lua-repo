...
function GroupAIStateBase:calm_ai()
	...
	if Network:is_server() then
		local (for generator), (for state), (for control) = pairs(self:all_AI_criminals())
		do
			do break end
		...
		(for control) = self:all_AI_criminals() and data.unit
	end

	(for control) = self:all_AI_criminals() and crim.unit
	local (for generator), (for state), (for control) = pairs(self:all_char_criminals())
	do
		do break end
...
function GroupAIStateBase:_radio_chatter_clbk()
	if self._ai_enabled and not self:get_assault_mode() then
		...
		do
			...
			do
				do break end
				local (for generator), (for state), (for control) = ipairs(c_record.important_enemies)
				do
					do break end
			...
			(for control) = nil and self._police
		end
...
10
	...
	local message_id
	do
		local (for generator), (for state), (for control) = ipairs(self.teammate_comment_names)
		do
			do break end
...
8
	...
	if state then
		do
			local (for generator), (for state), (for control) = ipairs(self._hostage_keys)
			do
				do break end
				if key == h_key then
...
17
	...
	do
		local (for generator), (for state), (for control) = pairs(weights)
		do
			do break end
			...
			if suitable and wanted_access_type then
				suitable = false
				local (for generator), (for state), (for control) = ipairs(cat_data.access)
				do
					do break end
...
function GroupAIStateBase:on_simulation_ended()
	...
	self._converted_police = {}
	do
		local (for generator), (for state), (for control) = pairs(self._criminal_AI_respawn_clbks)
		do
			do break end
	...
	self._criminal_AI_respawn_clbks = nil and {}
...
12
	...
	if e_data.importance > 0 then
		...
		do
			...
			local (for generator), (for state), (for control) = ipairs(imp_keys)
			do
				do break end
	...
	(for control) = nil and c_data.important_enemies
	do
		local (for generator), (for state), (for control) = pairs(self._ai_criminals)
		do
			do break end
	...
	(for control) = nil and record.unit
	local unit_type = unit:base()._tweak_table
	if self._special_unit_types[unit_type] then
			...
			if mvector3.distance(spawn_pos, u_pos) < 700 and math.abs(spawn_pos.z - u_pos.z) < 300 then
				...
				do
					...
					if area_spawn_points then
						do
							local (for generator), (for state), (for control) = ipairs(area_spawn_points)
							do
								do break end
...
function GroupAIStateBase:unregister_criminal(unit)
	...
	do break end
	if record.minions then
		local minions = clone(record.minions)
		do
			local (for generator), (for state), (for control) = pairs(minions)
			do
				do break end
	...
	if record.ai then
		...
		if Network:is_server() then
			local (for generator), (for state), (for control) = ipairs(record.important_enemies)
			do
				do break end
		...
		self._player_criminals[u_key] = unit and nil
	end
...
function GroupAIStateBase:check_gameover_conditions()
	...
	local ai_disabled = inspect(self._player_criminals) and true
	do
		local (for generator), (for state), (for control) = pairs(self._ai_criminals)
		do
			do break end
	...
	(for control) = u_data.status and true
...
function GroupAIStateBase:_debug_draw_drama(t)
	...
	do
		local (for generator), (for state), (for control) = ipairs(drama_hist)
		do
			do break end
	...
	local pop_hist_size = nil and #pop_hist
	...
	do
		local (for generator), (for state), (for control) = ipairs(pop_hist)
		do
			do break end
			local new_x = right_limit - width * (t - entry[2]) / t_span
	...
	local mvec3_setx = t and mvector3.set_x
	...
	local function _draw_events(event_brush, event_list)
		...
		local (for generator), (for state), (for control) = ipairs(event_list)
		do
			do break end
...
function GroupAIStateBase:set_drama_draw_state(state)
	if state then
		...
		if self._task_data then
			do
				local (for generator), (for state), (for control) = ipairs({"assault", "blockade"})
				do
					do break end
			...
			(for control) = "blockade" and self._task_data
...
function GroupAIStateBase:_try_use_task_spawn_event(t, target_area, task_type, target_pos, force)
	...
	local (for generator), (for state), (for control) = pairs(self._spawn_events)
	do
		do break end
...
function GroupAIStateBase:add_special_objective(id, objective_data)
	...
	if not objective_data.repeatable then
...
16
	...
	if ai_group == "enemies" then
		...
		(for control) = nil and enemy_unit_data.assigned_area
		if ai_group == "friendlies" then
			local (for generator), (for state), (for control) = pairs(self._ai_criminals)
			do
				do break end
...
function GroupAIStateBase:_upd_grp_SO()
	if self._grp_SO then
		...
		do
			local (for generator), (for state), (for control) = pairs(self._grp_SO)
			do
				do break end
		...
		self._grp_SO = nil
		local task_queue = nil and self._grp_SO_task_queue
		self._grp_SO_task_queue = nil
		local (for generator), (for state), (for control) = ipairs(task_queue)
		do
			do break end
	...
	(for control) = nil and task_info[1]
	if self._recurring_grp_SO and not next(self._spawning_groups) then
		local (for generator), (for state), (for control) = pairs(self._recurring_grp_SO)
		do
			do break end
...
function GroupAIStateBase:_process_recurring_grp_SO(recurring_id, data)
	if data.groups then
		local junk_groups
		do
			local (for generator), (for state), (for control) = pairs(data.groups)
			do
				do break end
				if group.has_spawned then
					...
					if not is_junk then
						is_junk = true
						do
							local (for generator), (for state), (for control) = pairs(group.units)
							do
								do break end
		...
		do break end
		do
			local (for generator), (for state), (for control) = ipairs(junk_groups)
			do
				do break end
		...
		(for control) = nil and data.groups
	...
	(for control) = math.lerp(data.interval[1], data.interval[2], math.random()) and element.value
	...
	local grp_objective = element:value("base_chance") and element:get_grp_objective()
...
11
	...
	if not self._point_of_no_return_areas then
		...
		local (for generator), (for state), (for control) = ipairs(element._values.elements)
		do
			do break end
	...
	(for control) = nil and get_mission_script_element
	...
	if plr_unit and alive(plr_unit) then
		local (for generator), (for state), (for control) = ipairs(self._point_of_no_return_areas)
		do
			do break end
	...
	(for control) = managers.criminals:local_character_name() and area._shape
	...
	if self._point_of_no_return_timer <= 0 then
		...
		if Network:is_server() then
			if managers.platform:presence() == "Playing" then
				...
				do
					local (for generator), (for state), (for control) = pairs(self._peers_inside_point_of_no_return)
					do
						do break end
		...
		(for control) = false and get_mission_script_element
...
function GroupAIStateBase:remove_one_teamAI(name_to_remove, replace_with_player)
	...
	if u_key then
		...
		unit = u_data.unit
		(for control) = uk and data.unit
	elseif not name_to_remove then
		local unit
		do
			local (for generator), (for state), (for control) = pairs(managers.criminals._characters)
			do
				do break end
...
function GroupAIStateBase:_determine_spawn_objective_for_criminal_AI()
	...
	(for control) = nil and pl_record.status
...
function GroupAIStateBase:_draw_enemy_importancies()
	...
	(for control) = nil and e_data.importance
	local (for generator), (for state), (for control) = pairs(self._player_criminals)
	do
		do break end
		local imp_enemies = c_data.important_enemies
		local (for generator), (for state), (for control) = ipairs(imp_enemies)
		do
			do break end
...
function GroupAIStateBase:_set_rescue_state(state)
	...
	(for control) = nil and civ_data.unit
	local (for generator), (for state), (for control) = pairs(self._police)
	do
		do break end
...
function GroupAIStateBase:chk_area_leads_to_enemy(start_nav_seg_id, test_nav_seg_id, enemy_is_criminal)
	local enemy_areas = {}
	do
		local (for generator), (for state), (for control) = pairs(enemy_is_criminal and self._criminals or self._police)
		do
			do break end
	...
	repeat
		...
		(for control) = nil and all_nav_segs[neighbour_seg_id]
...
function GroupAIStateBase:chk_say_enemy_chatter(unit, unit_pos, chatter_type)
	...
	do
		local (for generator), (for state), (for control) = pairs(chatter_type_hist.events)
		do
			do break end
	...
	(for control) = nil and event_data.expire_t
...
function GroupAIStateBase:on_nav_link_unregistered(element_id)
	...
	local (for generator), (for state), (for control) = pairs(all_ai)
	do
		do break end
		local (for generator), (for state), (for control) = pairs(ai_group)
		do
			do break end
...
function GroupAIStateBase:_get_anticipation_duration(anticipation_duration_table, is_first)
	...
	if not is_first then
		...
		local (for generator), (for state), (for control) = pairs(anticipation_duration_table)
		do
			do break end
...
function GroupAIStateBase:_map_spawn_groups_to_respective_areas(id, spawn_groups)
	local nav_manager = managers.navigation
	local (for generator), (for state), (for control) = ipairs(spawn_groups)
	do
		do break end
		...
		if spawn_points and next(spawn_points) and spawn_group_names and next(spawn_group_names) then
			...
			(for control) = nil and spawn_pt_element.value
...
function GroupAIStateBase:_remove_preferred_spawn_point_from_area(area, sp_data)
	...
	local (for generator), (for state), (for control) = ipairs(area.spawn_points)
	do
		do break end
...
function GroupAIStateBase:_remove_preferred_spawn_group_from_area(area, sp_data)
	...
	local (for generator), (for state), (for control) = ipairs(area.spawn_groups)
	do
		do break end
		if sp_data_ == sp_data then
...
function GroupAIStateBase:remove_preferred_spawn_points(id)
	if self._spawn_points[id] then
		...
		self._spawn_points[id] = nil and nil
	elseif self._spawn_groups[id] then
		...
		self._spawn_groups[id] = nil and nil
	end
...
function GroupAIStateBase:on_AI_attention_changed(unit_key)
	...
	local (for generator), (for state), (for control) = pairs(self._attention_objects)
	do
		do break end
...
function GroupAIStateBase:get_AI_attention_objects_by_filter(filter)
	if not self._attention_objects[filter] then
		...
		do
			local (for generator), (for state), (for control) = pairs(self._attention_objects.all)
			do
				do break end
		...
		self._attention_objects[filter] = nil and new_attention_set
	end
...
function GroupAIStateBase:add_area(area_id, nav_segs, area_pos)
	...
	do
		local (for generator), (for state), (for control) = ipairs(nav_segs)
		do
			do break end
	...
	(for control) = true and new_area.nav_segs
	do
		local (for generator), (for state), (for control) = ipairs(nav_segs)
		do
			do break end
	...
	(for control) = true and self._nav_seg_to_area_map
	do
		local (for generator), (for state), (for control) = ipairs(nav_segs)
		do
			do break end
		...
		(for control) = nil and neighbour_area.neighbours
	end

	(for control) = true and all_areas[seg_id]
	local all_nav_segs = managers.navigation._nav_segments
	do
		local (for generator), (for state), (for control) = ipairs(nav_segs)
		do
			...
			if not nav_seg.disabled then
				...
				(for control) = nil and other_area.nav_segs
			end
...
function GroupAIStateBase:_create_area_data()
	...
	do
		local (for generator), (for state), (for control) = pairs(all_nav_segs)
		do
			do break end
			local new_area = self:_empty_area_data()
			new_area.nav_segs[seg_id] = true
			...
			if not nav_seg.disabled then
				...
				local (for generator), (for state), (for control) = pairs(seg_neighbours)
				do
					do break end
			...
			all_areas[seg_id] = nil and new_area
		end
...
function GroupAIStateBase.get_nav_seg_id_from_area(area)
	do
		local (for generator), (for state), (for control) = pairs(area.nav_segs)
		do
			do break end
...
function GroupAIStateBase:_on_area_safety_status(area, event)
	...
	do
		local (for generator), (for state), (for control) = pairs(area.police.units)
		do
			do break end
	...
	do
		do break end
		local (for generator), (for state), (for control) = pairs(other_area.police.units)
		do
			do break end
			local unit = unit_data[u_key].unit
...
19
	...
	do
		...
		if area.nav_segs[changed_seg_id] then
			if state then
				local (for generator), (for state), (for control) = pairs(changed_seg_neighbours)
				do
					do break end
					local neighbour_nav_seg = all_nav_segs[neighbour_seg_id]
					if not neighbour_nav_seg.disabled then
...
function GroupAIStateBase:is_ecm_jammer_active(medium)
	local (for generator), (for state), (for control) = pairs(self._ecm_jammers)
	do
		do break end
...
function GroupAIStateBase:fetch_highest_giveaway(...)
	...
	do
		local (for generator), (for state), (for control) = pairs(giveaways)
		do
			do break end
			if giveaway then
...
function GroupAIStateBase.investigate_unit(giveaway_unit, additional_info)
	...
	if investigate_attention then
		...
		if unit_attention and unit_attention._attention_data then
			local (for generator), (for state), (for control) = pairs(unit_attention._attention_data)
			do
				do break end
...
function GroupAIStateBase:get_sync_event_id(event_name)
	local (for generator), (for state), (for control) = ipairs(self.EVENT_SYNC)
	do
		do break end
...
4
	do
		local (for generator), (for state), (for control) = ipairs(self.BLAME_SYNC)
		do
			do break end
...
function GroupAIStateBase:register_rescueable_hostage(unit, rescue_area)
	...
	(for control) = unit:movement():nav_tracker():nav_segment() and area.hostages
...
function GroupAIStateBase:_upd_criminal_suspicion_progress()
	...
	local (for generator), (for state), (for control) = pairs(susp_data)
	do
		do break end
...
function GroupAIStateBase:_clear_criminal_suspicion_data()
	local (for generator), (for state), (for control) = pairs(self._suspicion_hud_data)
	do
		do break end
...
function GroupAIStateBase:_get_balancing_multiplier(balance_multipliers)
	local nr_players = 0
	do
		local (for generator), (for state), (for control) = pairs(self:all_player_criminals())
		do
			do break end
	...
	(for control) = self:all_player_criminals() and u_data.status
