...
function GroupAIStateBesiege:_init_misc_data()
	...
	if managers.navigation:is_data_ready() then
		...
		self._task_data = {}
		self._task_data.reenforce = {	tasks = {}, next_dispatch_t = 0 }
		self._task_data.recon = {	tasks = {}, next_dispatch_t = 0 }
		...
		for u_key, u_data in pairs( self._police ) do
			if not u_data.assigned_area then
				...
				self:set_enemy_assigned( self:get_area_from_nav_seg_id( nav_seg ), u_key )	-- assign
			end
		end
	end
end
...
function GroupAIStateBesiege:update( t, dt )
	...
	--Application:draw_sphere( data.unit:position() + math.UP * 120, 20 + 30 * (t%2), 1, 1, 0 )
	--[[
	for _, unit in pairs(managers.groupai:state():fleeing_civilians()) do
		Application:draw_sphere( unit:position() + math.UP * 25, 30,  1,0,1 )
	end
	]]
...
-----------------------------------------------------------------------------
-- this is called from ElementSpawnEnemyDummy.lua for force-spawned units that should participate in the level scenario
function GroupAIStateBesiege:assign_enemy_to_group_ai( unit )
	...
	local group_desc = {
		type = u_category or "custom", -- this changes later to the type of unit spawned. the group ID however is labelled "custom1"
		size = 1
	}
...
function GroupAIStateBesiege:_upd_police_activity()
	self._police_upd_task_queued = false	-- this is to make the code crash-safe
...
function GroupAIStateBesiege:_upd_SO()
	...
	for id, so in pairs( self._special_objectives ) do
		if t > so.delay_t then
			...
			if so.data.interval < 0 then	-- "try once" SO
				trash = trash or {}
				table.insert( trash, id )
			end
		end
...
function GroupAIStateBesiege:_begin_new_tasks()
	-- First check if we need to re-take a lost area
	local all_areas = self._area_data
	...
	--print( "reenforce_data.next_dispatch_t", reenforce_data.next_dispatch_t and reenforce_data.next_dispatch_t - t )
	local recon_candidates
	local are_recon_candidates_safe
	local recon_data = task_data.recon
	if recon_data.next_dispatch_t and t > recon_data.next_dispatch_t and not ( task_data.assault.active or task_data.regroup.active ) then
		recon_candidates = {}
	end
	--print( "recon_data.next_dispatch_t", recon_data.next_dispatch_t and recon_data.next_dispatch_t - t )
	...
	--print( "assault_data.next_dispatch_t", assault_data.next_dispatch_t and assault_data.next_dispatch_t - t )
	
	if not ( reenforce_candidates or recon_candidates or assault_candidates ) then
		--print( "[GroupAIStateBesiege] no need to initiate a task" )
		return
	end
	...
	if #to_search_areas == 0 then
		--print( "[GroupAIStateBesiege] no spawn points!" )
		return
	end
	
	-- if assault_candidates and (task_data.phase == "sustain" or task_data.phase == "fade" or self._hunt_mode) then
	...
	-- Bredth first search for areas that neighbour police-controlled areas
	local i = 1
	repeat
		...
		if nr_criminals == 0 then
			for neighbour_area_id, neighbour_area in pairs( area.neighbours ) do
				if not found_areas[ neighbour_area_id ] then
					--print( "safe: adding to search", neighbour_seg )
	...
	--print( "assault_candidates", assault_candidates and inspect( assault_candidates) )
	if assault_candidates and #assault_candidates > 0 then
		--local lucky_i_candidate = math.random( #assault_candidates )
		--local assault_area = assault_candidates[ lucky_i_candidate ]
		self:_begin_assault_task( assault_candidates )
		recon_candidates = nil
		--reenforce_candidates = nil
	end
...
function GroupAIStateBesiege:_upd_assault_task()
	...
	--print( "force_pool", force_pool, "task_spawn_allowance", task_spawn_allowance )
	...
	if task_data.phase == "anticipation" then
		if task_spawn_allowance <= 0 then
			...
			task_data.phase_end_t = t + tweak_data.group_ai.besiege.assault.fade_duration -- allow time before ending assault
	...
	elseif task_data.phase == "sustain" then
		...
		elseif t > task_data.phase_end_t then
			if not self._hunt_mode then	-- We do not expire during hunt mode
	...
	else -- task_data.phase == "fade"
	-- ULF : Added a check for enemies left here, trying the simple stuff first.
		
		local enemies_left = self:_count_police_force( "assault" )
		
		if (enemies_left < 7) or (t > (task_data.phase_end_t + 350)) then -- The 1000 here is just a safety
			...
			elseif t > task_data.phase_end_t then
				if self._drama_data.amount < tweak_data.drama.assault_fade_end and self:_count_criminals_engaged_force( 4 ) <= 3 then
			...
			-- Debug
			-- print("kill more enemies to end fase", enemies_left)
		end
	end
	
	-- Too low drama. reveal the positions of the players to enemies
	...
	if self:is_area_safe( primary_target_area ) then
		--print( "assault area is not under threat" )
		local target_pos = primary_target_area.pos
		--Application:draw_sphere( target_pos, 210, 0,0,0.7)
		--Application:set_pause( true )
		...
		if nearest_area then
			--print( "new assault area", nearest_area )
			primary_target_area = nearest_area
			task_data.target_areas[1] = nearest_area
			--local target_pos = primary_target_area.pos
			--Application:draw_sphere( target_pos, 230, 0.7,0,0)
			--Application:set_pause( true )
		end
	end
	--local target_pos = primary_target_area.pos
	...
	if task_data.phase == "anticipation" then
		nr_wanted = nr_wanted - 5 -- reserve a few places for when build begins
	end
	
	if nr_wanted > 0 and task_data.phase ~= "fade" then -- If this is positive then we have too few cops in the level. spawn instead of re-assigning
		local used_event
		...
		if not used_event then
			if next( self._spawning_groups ) then -- we are in the process of spawning a group
			else
				...
				--print( "spawn_group", spawn_group and inspect( spawn_group ), "spawn_group_type", spawn_group_type )
				if spawn_group then
					local grp_objective = {
						type = "assault_area",
	...
	if area.is_safe then
		...
		for criminal_key, c_data in pairs( self._criminals ) do
			if not c_data.status then
				if mvector3.distance( sp_data.pos, c_data.m_pos ) < 2500 and math.abs( sp_data.pos.z - c_data.m_pos.z ) < 300 then
					return	-- a criminal is near
				end
			end
		end
	end
...
function GroupAIStateBesiege:_begin_regroup_task()
	...
	if self._draw_drama then
		table.insert( self._draw_drama.regroup_hist, { self._t } )
	end
...
function GroupAIStateBesiege:_find_nearest_safe_area( start_area, start_pos )
	...
	local found_areas = { [group.objective.area] = "init" } -- map: found_area, from_area. for first found area "from_area" is "init"
	repeat
		local search_area = table.remove( to_search_areas, 1 )
		--print( "examining area", search_area.id )
		if next( search_area.criminal.units ) then
			...
			for other_area_id, other_area in pairs( search_area.neighbours ) do
				if not found_areas[ other_area ] then
					...
					--print( "found_area", other_area.id )
				end
			end
		end
	...
	for c_key, c_data in pairs( self._criminals ) do
		...
		if ( not my_enemy_pos or my_dis > my_enemy_dis_sq ) and math.abs( mvector3.z( c_data.m_pos ) - mvector3.z( start_pos ) ) < 300 then
			my_enemy_pos = c_data.m_pos
			my_enemy_dis_sq = my_dis
		end
	end
	if not my_enemy_pos or my_enemy_dis_sq > 3000 * 3000 then
		return
	end
	...
	for neighbour_seg_id, door_list in pairs( start_neighbours ) do
		...
		if not next( neighbour_area.criminal.units ) then
			...
			if not neighbour_nav_seg.disabled and mvec3_dis_sq( my_enemy_pos, neighbour_nav_seg.pos ) > my_enemy_dis_sq then
...
function GroupAIStateBesiege:_upd_recon_tasks()
	...
	--Application:draw_sphere( target_pos, 220, 0,0,0.7)
...
function GroupAIStateBesiege:_find_spawn_points_near_area( target_area, nr_wanted, target_pos, max_dis, verify_clbk )
	...
	--[[for _, sp_data in ipairs( s_points ) do
		Application:draw_sphere( sp_data.pos, 230, 0,1,1)
	end]]
...
function GroupAIStateBesiege:_find_spawn_group_near_area( target_area, nr_wanted, allowed_groups, target_pos, max_dis, verify_clbk )
	--print( "[GroupAIStateBesiege:_find_spawn_group_near_area] target_area", target_area.id, "nr_wanted", nr_wanted, "allowed_groups", inspect( allowed_groups ), "target_pos", target_pos, "max_dis", max_dis, "verify_clbk", verify_clbk )
	--Application:set_pause( true )
	local all_areas = self._area_data
	local mvec3_dis = mvector3.distance_sq
	if max_dis then -- work with sqared distances
			...
			for _, spawn_group in ipairs( spawn_groups ) do
				if spawn_group.delay_t <= t and ( not verify_clbk or verify_clbk( spawn_group ) ) then
					...
					if not max_dis or my_dis < max_dis then
						...
						--print( "added group", inspect( spawn_group ), my_dis )
					end
				--else
					--print( "failed check", spawn_group.delay_t, t, verify_clbk )
				end
			end
		end
	...
	for i, dis in ipairs( valid_spawn_group_distances ) do
		...
		--print( "calculating weight for", i, "my_wgt", my_wgt )
		...
		for _, group_type in ipairs( my_group_types ) do
			--print( "checking group_type", group_type )
			...
			if cat_weights then
				...
				--print( "calculated: mod_weight", mod_weight, "total_weight", total_weight )
			--else
				--print( "not allowed" )
			end
		end
...
function GroupAIStateBesiege:_spawn_in_individual_groups( grp_objective, spawn_points, task )
	for i_sp, spawn_point in ipairs( spawn_points ) do
		local group_desc = {
			type = "custom", -- this changes later to the type of unit spawned. the group ID however is labelled "custom1"
			size = 1
		}
...
-----------------------------------------------------------------------------------
-- this resolves and expands a subtable of tweak_data.group_ai.enemy_spawn_groups[ something_here ]
function GroupAIStateBesiege._extract_group_desc_structure( spawn_entry_outer, valid_unit_entries )
	--print( "[GroupAIStateBesiege._extract_group_desc_structure] spawn_entry_outer ", inspect( spawn_entry_outer ) )
	for spawn_entry_key, spawn_entry in ipairs( spawn_entry_outer ) do -- indexed entries are definite
		if spawn_entry.unit then
			--print( " unit entry", spawn_entry_key, inspect( spawn_entry ) )
			table.insert( valid_unit_entries, clone( spawn_entry ) )
		else
			--print( " sub entry", spawn_entry_key, inspect( spawn_entry ) )
	...
	for spawn_entry_key, spawn_entry in pairs( spawn_entry_outer ) do
		if type( spawn_entry_key ) ~= "number" or spawn_entry_key > #spawn_entry_outer then -- non-indexed entries are randomizers
			--print( " rand_entry", spawn_entry_key, inspect( spawn_entry ) )
			if #spawn_entry ~= 0 then
				local i_rand = math.random( #spawn_entry )
				local rand_branch = spawn_entry[ i_rand ]
				if rand_branch.unit then
					--print( " rand unit entry", i_rand, inspect( rand_branch ) )
					table.insert( valid_unit_entries, clone( rand_branch ) )
				else
					--print( " rand sub_entry", i_rand, inspect( rand_branch ) )
...
function GroupAIStateBesiege:_spawn_in_group( spawn_group, spawn_group_type, grp_objective, ai_task )
	--print( "\n[GroupAIStateBesiege:_spawn_in_group] spawn_group", inspect( spawn_group ), "spawn_group_type", spawn_group_type, "\ngrp_objective", inspect( grp_objective ) )
	...
	--print( "wanted_nr_units", wanted_nr_units )
	
	
	local valid_unit_types = {}
	--print( "start expansion", spawn_group_desc.spawn )
	self._extract_group_desc_structure( spawn_group_desc.spawn, valid_unit_types )
	
	--print( "valid_unit_types", inspect( valid_unit_types ) )
	...
	local _add_unit_type_to_spawn_task = function( i, spawn_entry )
	...
	local i = 1
	while i <= #valid_unit_types do -- add unit types with minimum amount requirement
		...
		if i <= #valid_unit_types and wanted_nr_units > 0 and spawn_entry.amount_min and spawn_entry.amount_min > 0 and ( not spawn_entry.amount_max or spawn_entry.amount_max > 0 ) then
			if not _add_unit_type_to_spawn_task( i, spawn_entry ) then -- if this index was removed from valid_unit_types then the next index is again i
				i = i + 1
			end
			--print( "added min unit type", i, inspect( spawn_entry ), "wanted_nr_units", wanted_nr_units )
		else
			i = i + 1
		end
	end
	...
	while wanted_nr_units > 0 and #valid_unit_types ~= 0 do
		--print( "iterating non-min. wanted_nr_units", wanted_nr_units, "valid_unit_types\n", inspect( valid_unit_types ) )
		local rand_wght = math.random() * total_wgt
		--print( "rand_wght", rand_wght )
		local rand_i = 1
		local rand_entry 
		repeat
			rand_entry = valid_unit_types[ rand_i ]
			--print( "rand_i", rand_i, "rand_entry\n", inspect( rand_entry ) )
			rand_wght = rand_wght - rand_entry.freq
			--print( "rand_wght", rand_i )
			if rand_wght <= 0 then
				break
			else
				rand_i = rand_i + 1
		...
		--print( "added unit type", rand_i, inspect( rand_entry ), "wanted_nr_units", wanted_nr_units, "total_wgt", total_wgt )
	end
...
function GroupAIStateBesiege:_upd_group_spawning()
	...
	--print( "\n[GroupAIStateBesiege:_upd_group_spawning] spawn_task\n", inspect( spawn_task ) )
	--[[
	local spawn_task = {
		units_remaining = {},
		spawn_group = spawn_group,
	...
	local _try_spawn_unit = function( u_type_name, spawn_entry )
		--print( "_try_spawn_unit", u_type_name )
		if nr_units_spawned >= GroupAIStateBesiege._MAX_SIMULTANEOUS_SPAWNS then
			--print( "nr_units_spawned >= GroupAIStateBesiege._MAX_SIMULTANEOUS_SPAWNS" )
			return
		end
		local hopeless = true -- if true then this type of unit cannot spawn 
		for _, sp_data in ipairs( spawn_points ) do
			local category = group_ai_tweak.unit_categories[ u_type_name ]
			if ( sp_data.accessibility == "any" or category.access[ sp_data.accessibility ] ) and ( not sp_data.amount or sp_data.amount > 0 ) then -- this spawn point is suitable for me
				hopeless = false -- this spawn point is suitable for me. if i can't spawn now, I can spawn later
				if self._t > sp_data.delay_t then
					...
					--Application:draw_sphere( spawned_unit:position(), 50, 1,0,1 )
					...
					--print( "spawned. nr_units_spawned", nr_units_spawned )
					return true
				--else
					--print( "category.access\n", inspect( category.access ), "sp_data.accessibility", sp_data.accessibility )
				end
			end
		end
		if hopeless then
			...
			return true -- do not try to respawn this unit
		end
	end
	
	for u_type_name, spawn_info in pairs( spawn_task.units_remaining ) do -- iterate first through units that are more picky about spawn points
		if not group_ai_tweak.unit_categories[ u_type_name ].access.acrobatic then
			--print( "picky unit", u_type_name )
			while spawn_info.amount > 0 do
				--print( "amount", spawn_info.amount )
	...
	for u_type_name, spawn_info in pairs( spawn_task.units_remaining ) do -- iterate through rest of the units
		while spawn_info.amount > 0 do
			--print( "spawning non-picky", u_type_name, amount )
			local success = _try_spawn_unit( u_type_name, spawn_info.spawn_entry )
			if success then
				spawn_info.amount = spawn_info.amount - 1
			else
	...
	--else
		--print( "spawning not complete\n", inspect( spawn_task.units_remaining ) )
	end
end
...
function GroupAIStateBesiege:_upd_reenforce_tasks()
	...
	while i > 0 do
		...
		--print( "area", task_data.target_area.id )
		--local target_pos = task_data.target_area.pos
		--Application:draw_sphere( target_pos, 230, 0.6,0.6,0)
		...
		if force_required then			
			...
			if undershot > 0 and not ( self._task_data.regroup.active or self._task_data.assault.phase == "fade" ) and t > self._task_data.reenforce.next_dispatch_t and self:is_area_safe( task_data.target_area ) then
				local used_event
				if task_data.use_spawn_event then
					task_data.use_spawn_event = false
				...
				if used_event or used_group then -- if we spawned soem units. wait until the next reenforce dispatch
			...
			elseif undershot < 0 then
				...
				if overshot > 0 then
					...
					for group_id, group in pairs( self._groups ) do
						if group.has_spawned and ( group.objective.target_area or group.objective.area ) == task_data.target_area and group.objective.type == "reenforce area" then
							if ( not closest_group_size or closest_group_size < group.size ) and group.size <= overshot then
								closest_group = group
...
function GroupAIStateBesiege:on_objective_complete( unit, objective )
	--print( "[GroupAIStateBesiege:on_objective_complete]", unit:key(), inspect( objective ) )
	...
	if new_objective then
		if new_objective.nav_seg then
			local u_key = unit:key()
			local u_data = self._police[ u_key ]
			-- Don't assign a unit to the groupai if he isn't already in it
		...
		if self:rescue_state() and tweak_data.character[ unit:base()._tweak_table ].rescue_hostages then	-- try rescue civilians
		...
		if not new_objective then	-- give guard objective instead
			...
			elseif objective.type == "free" then
				new_objective = { type = "free", is_default = true, attitude = objective.attitude }
			end
		end
...
function GroupAIStateBesiege:on_cop_jobless( unit )
	local u_key = unit:key()
	if not self._police[ u_key ].assigned_area then -- this unit does not participate in group AI
		--Application:draw_sphere( unit:position(), 200, 0,0,0 )
		--Application:set_pause(true)
		return
	end
	...
	if undershot and undershot > 0 then
		--[[print( "[GroupAIStateBesiege:on_cop_jobless] defend" )
		Application:draw_sphere( unit:position(), 200, 0,1,1 )
		Application:draw_cylinder( unit:position(), managers.navigation._nav_segments[ nav_seg ].pos, 20, 0,1,1 )
		Application:draw_sphere( managers.navigation._nav_segments[ nav_seg ].pos, 150, 0,1,0 )
		Application:set_pause(true)]]
		local new_objective = { type = "defend_area", nav_seg = nav_seg, attitude = "avoid", stance = "hos", interrupt_dis = 700, interrupt_health = 0.75, in_place = true, scan = true, is_default = true }
		self:set_enemy_assigned( self._area_data[ nav_seg ], u_key )
		unit:brain():set_objective( new_objective )
		return true
	end
	if not area.is_safe then
		--[[print( "[GroupAIStateBesiege:on_cop_jobless] recon" )
		Application:draw_sphere( unit:position(), 200, 0,1,1 )
		Application:draw_cylinder( unit:position(), managers.navigation._nav_segments[ nav_seg ].pos, 20, 0,1,1 )
		Application:draw_sphere( managers.navigation._nav_segments[ nav_seg ].pos, 150, 0,1,0 )
		Application:set_pause(true)]]
		local new_objective = { type = "free", nav_seg = nav_seg, attitude = "avoid", stance = "hos", in_place = true, scan = true, is_default = true }
		self:set_enemy_assigned( self._area_data[ nav_seg ], u_key )
		unit:brain():set_objective( new_objective )
		return true
	end
	--[[print( "occupation not found", nav_seg )
	Application:draw_sphere( unit:position(), 200, 1,0,0 )
	Application:set_pause(true)]]
end
...
function GroupAIStateBesiege:_draw_enemy_activity( t )
	...
	local _f_draw_logic_name = function( u_key, l_data, draw_color )
		...
		if logic_name_text then
			...
			logic_name_text = panel:text{ name="text", text=text_str, font = tweak_data.hud.medium_font, font_size = 20, color=draw_color, layer=1 }
	...
	local _f_draw_obj_pos = function( unit )
		local brush
		local objective = unit:brain():objective()
	...
	local _f_draw_attention_on_player = function( l_data )
	...
	local groups = { { group=self._police, color=Color(1,1,0,0) }, { group=managers.enemy:all_civilians(), color=Color(1,0.75,0.75,0.75) }, { group=self._ai_criminals, color=Color(1,0,1,0) } }
...
function GroupAIStateBesiege:find_occupation_in_area( nav_seg )
	--print( "find_occupation_in_area", nav_seg )
	...
	-- Create a table with all door weights
	local guarded_doors = {}
	...
	-- Find the door with the highest weight
	local best_door
	local best_door_weight
	local best_door_nav_seg
...
-----------------------------------------------------------------------------------
-- Check if a police unit needs to change guard door
-- reurns nil if no change is needed. a new guard object otherwise
...
function GroupAIStateBesiege:safe_flee_point( start_nav_seg )
	...
	repeat
		...
		if search_area.flee_points and next( search_area.flee_points ) then
			...
			for other_area_id, other_area in pairs( search_area.neighbours ) do
				if not ( found_areas[ other_area ] or next( other_area.criminal.units ) ) then
					table.insert( to_search_areas, other_area )
					found_areas[ other_area ] = true
				end
			end
...
-----------------------------------------------------------------------------------
-- id: the same key that you will provide for removal
-- pos: world position from which the area data will be extracted from
-- force: right now all police units have a drama value of 1 so this is the number of units
...
-----------------------------------------------------------------------------------
-- With hunt mode enabled the enemy manager always knows where the player is and concentrates efforts on those areas
-- this info is not shared with individual AI. They need to be able to see the players in order to engage
...
-----------------------------------------------------------------------------------
-- this means that enemies do not lose track of the criminals once they have detected them
function GroupAIStateBesiege:is_detection_persistent()
	return self._task_data.assault.active
end

-----------------------------------------------------------------------------------
-- sets assault objectives to enemy groups
function GroupAIStateBesiege:_assign_enemy_groups_to_assault( phase )
	for group_id, group in pairs( self._groups ) do
		if group.has_spawned and group.objective.type == "assault_area" then
			--print( "examining group", group_id )
			if group.objective.moving_out then
				local done_moving
				for u_key, u_data in pairs( group.units ) do
					local objective = u_data.unit:brain():objective()
					if not objective or objective.grp_objective ~= group.objective then -- wrong objective
					else -- he has the correct objective
...
function GroupAIStateBesiege:_set_recon_objective_to_group( group )
	...
	--print( "\n[GroupAIStateBesiege:_set_recon_objective_to_group]\n", inspect( group ), "objective\n", inspect( current_objective ) )
	...
	if not ( target_area.loot or target_area.hostages ) or not current_objective.moving_out and current_objective.moved_in and group.in_place_t and self._t - group.in_place_t > 15 then
		--print( "target area fruitless", target_area.id )
		local recon_area -- the new target area
		-- bredth first search that searches for the nearest criminal area
		local to_search_areas = { current_objective.area }
		local found_areas = { [current_objective.area] = "init" } -- map: found_area, from_area. for first found area "from_area" is "init"
		repeat
			local search_area = table.remove( to_search_areas, 1 )
			--print( "examining area", search_area.id )
			if search_area.loot or search_area.hostages then
				...
				for test_group_id, test_group in pairs( self._groups ) do
					--print( "checking occupied. test_group_id", test_group_id, "area id", ( group.objective.target_area or group.objective.area ).id )
		...
		if recon_area then
			--Application:set_pause( true )
			--print( "new recon_area", recon_area.id )
			--Application:draw_cylinder( recon_area.pos, recon_area.pos + math.UP * 1500, 130, 1, 1, 0 )
			
			-- create a coarse path
			...
			while found_areas[ last_added_area ] ~= "init" do
				...
				--print( "adding", last_added_area.id )
			end
			...
			--for i, nav_point in ipairs( coarse_path ) do
				--print( i, nav_point[1] )
				--if i ~= 1 then
					--Application:draw_cylinder( nav_point[2], coarse_path[ i - 1 ][2], 25, 1, 0, 1 )
				--end
			--end
	...
	if current_objective.target_area then -- we have still not given order to move in to the target area
		if current_objective.moving_out and not current_objective.moving_in and current_objective.coarse_path then -- we are moving to an intermediate pos. make sure the path is safe
			--print( "checking coarse path safety\n", inspect( current_objective.coarse_path ) )
			local forwardmost_i_nav_point = self:_get_group_forwardmost_coarse_path_index( group )
			--print( "forwardmost_i_nav_point", forwardmost_i_nav_point )
			--Application:draw_sphere( current_objective.coarse_path[ forwardmost_i_nav_point ], 300, 1,1,1 )
			if forwardmost_i_nav_point and forwardmost_i_nav_point > 1 then
				for i = forwardmost_i_nav_point + 1, #current_objective.coarse_path do --start checking from the area in front the forwardmost nav_seg
					local nav_point = current_objective.coarse_path[ forwardmost_i_nav_point ]
					if not self:is_nav_seg_safe( nav_point[1] ) then
						--debug_pause( "enemies in nav_seg", nav_point[1] )
						--Application:draw_sphere( nav_point[ 2 ], 250, 0,0,1 )
						...
						--print( "current_objective.coarse_path", inspect( current_objective.coarse_path ) )
						local grp_objective = {
							type = "recon_area",
		...
		if not current_objective.moving_out and not current_objective.area.neighbours [ current_objective.target_area.id ] then -- we just finished spawning. need to travel to an intermediate area
			local search_params = {
			...
			if coarse_path then
				self:_merge_coarse_path_by_area( coarse_path ) -- remove nav_points that are in the same area as others
				
				table.remove( coarse_path ) -- remove the target_area
				
				--[[print( "moving to intermediate pos" )
				for i_nav_point, nav_point in ipairs( coarse_path ) do
					local area = self:get_area_from_nav_seg_id( nav_point[1] )
					print( "area:", area.id )
				end
				Application:set_pause( true )
				Application:draw_cylinder( self:get_area_from_nav_seg_id( coarse_path[ #coarse_path ][1] ).pos, current_objective.target_area.pos, 100, 0,0,0.5 )
				Application:draw_cylinder( self:get_area_from_nav_seg_id( coarse_path[ #coarse_path ][1] ).pos, current_objective.area.pos, 100, 0,0,1 )
				]]
				local grp_objective = {
					type = "recon_area",
				...
				self:_set_objective_to_enemy_group( group, grp_objective )
			--[[else
				debug_pause( "failed coarse path to intermediate pos!!!", group.id )
				Application:draw_cylinder( current_objective.area.pos, current_objective.target_area.pos, 100, 1,0,0 )
				]]
			end
		end
	
		if not current_objective.moving_out and current_objective.area.neighbours [ current_objective.target_area.id ] then -- we have arrived at the intermediate stop point. check if we should move in
			--print( "moving in!!!" )
			--Application:set_pause( true )
			--Application:draw_cylinder( current_objective.target_area.pos, current_objective.target_area.pos + math.UP * 1000, 100, 0.5,0,0 )
			local grp_objective = {
				type = "recon_area",
			...
			if next( current_objective.target_area.criminal.units ) then
				self:_chk_group_use_smoke_grenade( group, { use_smoke = true, target_areas = { grp_objective.area } } )
			end
		end
	end
end
...
function GroupAIStateBesiege:_set_objective_to_enemy_group( group, grp_objective )
	--print( "[GroupAIStateBesiege:_set_objective_to_enemy_group]", group.id, inspect( grp_objective ) )
	--Application:stack_dump()
	group.objective = grp_objective
...
function GroupAIStateBesiege:_upd_groups()
	for group_id, group in pairs( self._groups ) do
		...
		for u_key, u_data in pairs( group.units ) do
			...
			if ( not current_objective or current_objective.is_default or current_objective.grp_objective and current_objective.grp_objective ~= group.objective ) then
				if not group.objective.follow_unit or alive( group.objective.follow_unit ) then -- check for destroyed follow_unit
					...
					if brain:is_available_for_assignment( objective ) then
						--debug_pause_unit( u_data.unit, "_upd_groups, wrong objective", u_data.unit, "current_objective", current_objective and inspect( current_objective ), "current_objective.grp_objective", current_objective and current_objective.grp_objective, "group.objective", group.objective )
...
function GroupAIStateBesiege:_set_assault_objective_to_group( group, phase )
	...
	--print( "\n[GroupAIStateBesiege:_set_assault_objective_to_group]\n", inspect( group ), "objective\n", inspect( current_objective ), self._t )
	
	local approach, open_fire, push, pull_back, charge -- possible decisions
	
	-- if we are moving to position and a criminal is in the area, abort
	local obstructed_area = self:_chk_group_areas_tresspassed( group )
	
	-- get the highest ranking unit
	local group_leader_u_key, group_leader_u_data = self._determine_group_leader( group.units )
	--print( "leader", group_leader_u_data and group_leader_u_data.unit:name():s(), "tactics:\n", inspect( group_leader_u_data and group_leader_u_data.tactics ) )
	local tactics_map
	if group_leader_u_data and group_leader_u_data.tactics then
		...
		-- check if the current tactic is still an option
		...
		for i_tactic, tactic_name in ipairs( group_leader_u_data.tactics ) do
			if tactic_name == "deathguard" and not phase_is_anticipation then
				-- verify deathguard objective
				if current_objective.tactic == tactic_name then
					for u_key, u_data in pairs( self._char_criminals ) do
						if u_data.status and current_objective.follow_unit == u_data.unit then -- downed criminal
				...
				-- acquire new deathguard objective
				local closest_crim_u_data, closest_crim_dis_sq
				for u_key, u_data in pairs( self._char_criminals ) do
					if u_data.status then -- downed criminal
				...
				if closest_crim_u_data then
					...
					if coarse_path then
						...
						self:_set_objective_to_enemy_group( group, grp_objective )
						--debug_pause_unit( closest_crim_u_data.unit, "tactic: applying deathguard", closest_crim_u_data.unit )
			...
			elseif tactic_name == "charge" and not current_objective.moving_out and group.in_place_t and ( self._t - group.in_place_t > 15 or self._t - group.in_place_t > 4 and self._drama_data.amount <= tweak_data.drama.low ) and next( current_objective.area.criminal.units ) and group.is_chasing and not current_objective.charge then
				--debug_pause_unit( group_leader_u_data.unit, "CHAAAAAAAAARGE!!!!" )
				charge = true
			end
		end
	end
	...
	if obstructed_area then
		--print( "obstructed_area", obstructed_area.id )
		if current_objective.moving_out then
			if not current_objective.open_fire then
				open_fire = true -- we are moving quietly and we run into someone. fire!
				--print( "open_fire1" )
			end
		elseif not current_objective.pushed or charge and not current_objective.charge then
			--print( "push1" )
			push = true -- we were static and someone run into us. move in!
		end
	else
		-- check if our path is obstructed
		...
		elseif not current_objective.moving_out then
			...
			if charge then
				push = true
				--print( "push" )
			elseif not has_criminals_close or not group.in_place_t then -- We need to get closer to the fight. ( or we havent moved since we spawned )
				approach = true
				--print( "approach" )
			elseif not ( phase_is_anticipation or current_objective.open_fire ) then
				open_fire = true
				--print( "open_fire" )
			elseif not phase_is_anticipation and group.in_place_t and ( group.is_chasing or not ( tactics_map and tactics_map.ranged_fire ) or self._t - group.in_place_t > 15 ) then -- we need to move in
				push = true
				--print( "push" )
	...
	elseif approach or push then
		...
		local found_areas = { [ objective_area ] = "init" } -- map: found_area, from_area. for first found area "from_area" is "init"
		repeat
			local search_area = table.remove( to_search_areas, 1 )
			--print( "examining area", search_area.id )
			if next( search_area.criminal.units ) then
				--print( "criminals present" )
				...
				if not push and tactics_map and tactics_map.flank then
					...
					--print( "checking flank. assault_from_area", assault_from_area.id )
					if assault_from_area ~= "init" then -- if assault_from_area
						--print( "area is not init" )
						...
						for u_key, u_data in pairs( cop_units ) do
							if u_data.group and u_data.group ~= group and u_data.group.objective.type == "assault_area" then
								--Application:set_pause( true )
								--print( "bad flank" )
								--Application:draw_cylinder( u_data.m_pos, u_data.m_pos + math.UP * 5000, 20, 0, 0, 1 )
								-- bad flank
								assault_from_here = false
								if not alternate_assault_area or math.random() < 0.5 then -- don't always choose the 
									--print( "setting as alternate_assault_area. alternate_assault_area_from:", assault_from_area.id )
									alternate_assault_area = search_area -- remember this path
									alternate_assault_area_from = assault_from_area
								end
								found_areas[ search_area ] = nil -- re-visit this area if more paths lead to it
								break
							end
						end
					end
		...
		if not assault_area and alternate_assault_area then
			--print( "using alternate area", alternate_assault_area.id, "alternate_assault_area_from", alternate_assault_area_from.id )
		...
		if assault_area then
			...
			--print( "assault_area", assault_area.id, "target_area", target_area.id )
			-- create a coarse path
			local coarse_path = push and { { assault_area.pos_nav_seg, assault_area.pos } } or {} -- if we push then add criminal area too
			...
			--Application:draw_cylinder( coarse_path[ #coarse_path ][2], coarse_path[ #coarse_path ][2] + math.UP * 1500, 130, 1, 0, 0 )
			--[[print( "coarse path" )
			for i, nav_point in ipairs( coarse_path ) do
				print( i, nav_point[1] )
				if i ~= 1 then
	...
	elseif pull_back then
		...
		if not ( retreat_area or do_not_retreat ) and current_objective.coarse_path then
			--retreat_area, nearest_safe_nav_seg_id = self:_find_nearest_safe_area( current_objective.area, current_objective.area.pos )
			local forwardmost_i_nav_point = self:_get_group_forwardmost_coarse_path_index( group )
...
function GroupAIStateBesiege:_assign_groups_to_retire( allowed_groups, suitable_grp_func )
	--print( "[GroupAIStateBesiege:_assign_groups_to_retire] allowed_groups", inspect( allowed_groups ) )
	--Application:set_pause( true )
	for group_id, group in pairs( self._groups ) do
		...
		elseif suitable_grp_func and allowed_groups[ group.type ] then
			--print( "[GroupAIStateBesiege:_assign_groups_to_retire] switch_objective", group.id )
			suitable_grp_func( group )
		--else
			--print( "[GroupAIStateBesiege:_assign_groups_to_retire] unchanged", group.id, suitable_grp_func )
		end
	end
end
...
function GroupAIStateBesiege:_assign_group_to_retire( group )
	--print( "[GroupAIStateBesiege:_assign_group_to_retire] retire", group.id )
	--Application:stack_dump()
	local retire_area, retire_pos
	local to_search_areas = { group.objective.area }
	local found_areas = { [group.objective.area] = true }
	repeat
		local search_area = table.remove( to_search_areas, 1 )
		if search_area.flee_points and next( search_area.flee_points ) then
	...
	local grp_objective = {
		...
		coarse_path = { { retire_area.pos_nav_seg, retire_area.pos } },
		pos = retire_pos,
	}
	self:_set_objective_to_enemy_group( group, grp_objective )
end
...
function GroupAIStateBesiege:_assign_assault_groups_to_retire()
	local suitable_grp_func = function( group )
	...
	for group_id, group in pairs( self._groups ) do
		if group.has_spawned and group.objective.type == "reenforce_area" then
			...
			if group.objective.moving_out then
				...
				for u_key, u_data in pairs( group.units ) do
					local objective = u_data.unit:brain():objective()
					if not objective or objective.is_default or objective.grp_objective and objective.grp_objective ~= group.objective then -- wrong objective
						if objective then
					...
					elseif not objective.in_place then -- he has the correct objective but is not here yet
						done_moving = false
					end
				end
				if done_moving then
			...
			if not group.objective.moving_in then
				if locked_up_in_area and locked_up_in_area ~= group.objective.area then
					--debug_pause( "locked up in area", locked_up_in_area.id )
...
function GroupAIStateBesiege:_set_reenforce_objective_to_group( group )
	...
	--print( "\n[GroupAIStateBesiege:_set_reenforce_objective_to_group]\n", inspect( group ), "objective\n", inspect( current_objective ) )
	--[[local group_leader_u_key, group_leader_u_data
	
	if current_objective.moving_out then
	else -- not moving. check special tactics.
		-- get the highest ranking unit
	...
	if current_objective.target_area then -- we have still not given order to move in to the target area
		if current_objective.moving_out and not current_objective.moving_in then -- we are moving to an intermediate pos. make sure the path is safe
			--print( "checking coarse path safety\n", inspect( current_objective.coarse_path ) )
			local forwardmost_i_nav_point = self:_get_group_forwardmost_coarse_path_index( group )
			--print( "forwardmost_i_nav_point", forwardmost_i_nav_point )
			--Application:draw_sphere( current_objective.coarse_path[ forwardmost_i_nav_point ], 300, 1,1,1 )
		...
		if not current_objective.moving_out and current_objective.area.neighbours [ current_objective.target_area.id ] then -- we have arrived at the intermediate stop point. check if we should move in
			if not next( current_objective.target_area.criminal.units ) then -- move in carefully
				--print( "moving in!!!" )
				local grp_objective = {
					type = "reenforce_area",
					area = current_objective.target_area,
				...
				group.objective.moving_in = true
			--else
				--print( "target_area still occupied" )
			end
		end
	end
end
	...
	while forwardmost_i_nav_point > 0 do
		...
		for u_key, u_data in pairs( group.units ) do
			if area.nav_segs[ u_data.unit:movement():nav_tracker():nav_segment() ] then
				--print( "forwardmost nav_seg", nav_seg )
