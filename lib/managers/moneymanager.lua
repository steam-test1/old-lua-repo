...
function MoneyManager:get_civilian_deduction()
	...
	return math.round(self:get_tweak_value("money_manager", "killing_civilian_deduction", job_and_difficulty_stars))
end
...
function MoneyManager:get_money_by_job(job_id, difficulty)
	...
	if tweak_data.narrative.jobs[job_id].payout and tweak_data.narrative.jobs[job_id].payout[difficulty] then
		...
		local risk_payout = 0
		do
			local (for generator), (for state), (for control) = pairs(tweak_data.narrative.jobs[job_id].chain)
			do
				do break end
...
function MoneyManager:_get_pc_entry(entry)
	...
	if not pcs then
		...
		pc_value = pcs[1]
		local (for generator), (for state), (for control) = ipairs(pcs)
		do
			do break end
...
function MoneyManager:get_mask_sell_value(mask_id, global_value, blueprint)
	...
	do
		local (for generator), (for state), (for control) = pairs(bonuses)
		do
			do break end
...
function MoneyManager:get_cost_of_premium_contract(job_id, difficulty_id)
	...
	if tweak_data.narrative.jobs[job_id].payout then
	else
	end

	value = value + ((tweak_data.narrative.jobs[job_id].payout[difficulty_id - 1] or 0) / self:get_tweak_value("money_manager", "offshore_rate") or 0)
...
function MoneyManager:_check_multipliers(amount)
	do
		local (for generator), (for state), (for control) = pairs(self._active_multipliers)
		do
			do break end
...
function MoneyManager:actions()
	local t = {}
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.money_manager.actions)
		do
			do break end
...
function MoneyManager:multipliers()
	local t = {}
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.money_manager.multipliers)
		do
			do break end
