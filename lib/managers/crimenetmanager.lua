...
function CrimeNetManager:_find_online_games_ps3( friends_only )
				...
				if managers.network.matchmake:is_server_ok( friends_only, room.owner_id, attributes_numbers ) then
					...
					if name_id then -- Check that the tweak data for the level exists
						if not self._active_server_jobs[ name_str ] then
							if (table.size( self._active_jobs ) + table.size( self._active_server_jobs )) < tweak_data.gui.crime_net.job_vars.total_active_jobs then
								self._active_server_jobs[ name_str ] = { added = false, alive_time = 0, room_id = room.room_id }
	...
	local done_verify_func = function()
		managers.network.matchmake:register_callback( "search_lobby", f )
	...
	local updated_session_attributes = function( active_info_list )
			...
			for _, info in ipairs( active_info_list ) do
				...
				for i, room in ipairs( room_list ) do
					...
					if (not friends_only or is_friend) and managers.network.matchmake:is_server_ok( friends_only, room.owner_id, attributes_numbers ) then
						...
						if name_id then
							if self._active_server_jobs[ name_str ] then
		...
		for i,room in ipairs( room_list ) do
			...
			if managers.network.matchmake:is_server_ok( friends_only, room.owner_id, attributes_numbers ) then
				...
				if name_id then -- Check that the tweak data for the level exists
					if not self._active_server_jobs[ room.room_id ] then
						if (table.size( self._active_jobs ) + table.size( self._active_server_jobs )) < tweak_data.gui.crime_net.job_vars.total_active_jobs then
						if table.size ( self._active_server_jobs ) < self._max_active_server_jobs then
							self._active_server_jobs[ room.room_id ] =  { added = false, alive_time = 0 }
							-- print( "ADD", name_str )
							-- print( "   ", host_name, level_id, name_id, level_name, difficulty, state_string_id, state_name, state, num_plrs )
							-- self:_crimenet_gui():add_server_job( { id = room.room_id, level_id = level_id, difficulty = difficulty, difficulty_id = difficulty_id, num_plrs = num_plrs, host_name = host_name } )
							managers.menu_component:add_crimenet_server_job( { room_id = room.room_id, id = room.room_id, level_id = level_id, difficulty = difficulty, difficulty_id = difficulty_id, num_plrs = num_plrs, host_name = host_name, state_name = state_name, state = state, level_name = level_name, job_id = job_id, is_friend = is_friend, kick_option = kick_option } )
						end	end
					else
						managers.menu_component:update_crimenet_server_job( { room_id = room.room_id, id = room.room_id, level_id = level_id, difficulty = difficulty, difficulty_id = difficulty_id, num_plrs = num_plrs, host_name = host_name, state_name = state_name, state = state, level_name = level_name, job_id = job_id, is_friend = is_friend, kick_option = kick_option } )
						-- print( "Found same, need update", name_str )
						-- print( "   ", host_name, level_id, name_id, level_name, difficulty, state_string_id, state_name, state, num_plrs )
					end
				end
...
function CrimeNetGui:init( ws, fullscreeen_ws, node )
	...
	self._fullscreen_panel:bitmap( { name="blur_top", 		texture="guis/textures/test_blur_df", w=self._fullscreen_ws:panel():w(), h=full_16_9.convert_y*2, x=0, y=-full_16_9.convert_y, render_template="VertexColorTexturedBlur3D", layer=1001, rotation=360  } )
	self._fullscreen_panel:bitmap( { name="blur_right", 	texture="guis/textures/test_blur_df", w=full_16_9.convert_x*2, h=self._fullscreen_ws:panel():h(), x=self._fullscreen_ws:panel():w()-full_16_9.convert_x, y=0, render_template="VertexColorTexturedBlur3D", layer=1001, rotation=360  } )
...
function CrimeNetGui:add_special_contracts( no_casino )
	for index, special_contract in ipairs( tweak_data.gui.crime_net.special_contracts ) do
		...
		if id and not self._jobs[ id ] and
				( not special_contract.unlock or special_contract.unlock and managers.experience:current_level() >= tweak_data:get_value( special_contract.id, special_contract.unlock ) )
				and not ( special_contract.id == "casino" and no_casino ) then
...
--[[
	...
	side_panel:set_h( math.max( job_name:bottom(), info_name:bottom(), difficulty_name:bottom(), stars_panel:bottom() ) )
	...
	local glow_center = glow_panel:bitmap( { texture="guis/textures/pd2/crimenet_marker_glow", w=192, h=192, blend_mode="add", alpha=0.55, color=data.pulse_color or ( is_professional and pro_color ) or regular_color } )
	local glow_stretch = glow_panel:bitmap( { texture="guis/textures/pd2/crimenet_marker_glow", w=64*15, h=50, blend_mode="add", alpha=0.55, color=data.pulse_color or ( is_professional and pro_color ) or regular_color } )
	...
	local marker_dot = marker_panel:bitmap( { name="marker_dot", texture = marker_dot_texture, color=color, w=32, h=64, x=2, y=2, layer=1 } )
	...
	if( is_server ) then
		...
		icon_panel = self._pan_panel:panel( { layer = 26, alpha = 0, h = 18, w = 18 } )
		local kick_icon = icon_panel:bitmap( { name="kick_icon", texture="guis/textures/pd2/cn_kick_marker" } )
	...
	local job = { room_id = data.room_id, job_id = data.job_id, level_id = level_id, level_data = level_data, marker_panel = marker_panel, peers_panel = peers_panel, kick_option = data.kick_option,
				--[[marker_rect = marker_rect,]] timer_rect = timer_rect, side_panel = side_panel, --[[host_name = host_name, info_text = info_text, job_name = job_name, stars_panel = stars_panel, ]]icon_panel = icon_panel, focus = focus,
