...
function SecretAssignmentManager:_setup()
	...
	do
		local (for generator), (for state), (for control) = pairs(tweak_data.secret_assignment_manager)
		do
			do break end
...
function SecretAssignmentManager:unregister_unit(unit, failed)
	...
	do
		...
		if name == id then
			if data.type == "interact" then
				do
					local (for generator), (for state), (for control) = ipairs(self._assignments[name].available_units)
					do
						do break end
						if u == unit then
...
function SecretAssignmentManager:_fail_assignment(name)
	...
	if tweak_data.secret_assignment_manager[name].type == "interact" then
		local (for generator), (for state), (for control) = ipairs(self._current_assignment.units)
		do
			do break end
...
function SecretAssignmentManager:_get_available_assignments()
	...
	local t = {}
	do
		local (for generator), (for state), (for control) = pairs(self._assignments)
		do
			do break end
...
function SecretAssignmentManager:_check_level_filter(name)
	...
	if level_filter.include then
		do
			local (for generator), (for state), (for control) = ipairs(level_filter.include)
			do
				do break end
	...
	if level_filter.exclude then
		do
			local (for generator), (for state), (for control) = ipairs(level_filter.exclude)
			do
				do break end
				if lvl_id == Global.level_data.level_id then
					return false
