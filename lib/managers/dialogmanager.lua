...
function DialogManager:_load_dialog_data( name )
	...
	for _,node in ipairs( data ) do
		if( node._meta == "dialog" ) then
			if node.id then
				self._dialog_list[ node.id ] = { id = node.id, character = node.character, sound = node.sound, string_id = node.string_id, priority = node.priority and tonumber( node.priority ) or tweak_data.dialog.DEFAULT_PRIORITY }
			else
