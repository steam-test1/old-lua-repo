...
function WeaponFactoryManager:_preload_part( factory_id, part_id, forbidden, override, parts, third_person, need_parent, only_record )
	...
	if factory[ factory_id ].adds then
		if factory[ factory_id ].adds[ part_id ] then
	...
	if( parts[ part_id ] ) then	-- part already added
		return
	end
	
		-- need_parent needed for preload?
	if part.parent then
		if not self:get_part_from_weapon_by_type( part.parent, parts ) then
	...
	if not package then
		parts[ part_id ] = { ids_unit, ids_unit_name, "packages/dyn_resources", false }
		if not only_record then		-- if only_record then whoever wanted to preload the blueprint got to load the resources, is he wants to split it up over frames.
...
function WeaponFactoryManager:assemble_default( factory_id, p_unit, third_person, done_cb, skip_queue )
	...
	-- self:_assemble( factory_id, p_unit, blueprint, third_person, done_cb )
...
function WeaponFactoryManager:_assemble( factory_id, p_unit, blueprint, third_person, done_cb, skip_queue )
	...
	local factory = tweak_data.weapon.factory
	-- This should be from user specifications later
	local factory_weapon = factory[ factory_id ]

	local forbidden = self:_get_forbidden_parts( factory_id, blueprint )
	
	-- self:_add_parts( p_unit, factory_id, factory_weapon, blueprint, forbidden, third_person, done_cb )
...
function WeaponFactoryManager:_get_forbidden_parts( factory_id, blueprint )
	...
	for _,part_id in ipairs( blueprint ) do
		...
		--[[if factory.parts[ part_id ].forbids then
			for _,forbidden_id in ipairs( factory.parts[ part_id ].forbids ) do
				forbidden[ forbidden_id ] = true 
			end
		end]]
	end
...
function WeaponFactoryManager:_add_parts( p_unit, factory_id, factory_weapon, blueprint, forbidden, third_person, done_cb, skip_queue )
	...
	-- self._uses_tasks = false
	if self._uses_tasks and not skip_queue then
		table.insert( self._tasks, { 	done_cb = done_cb,
										p_unit = p_unit, 
	...
	--[[for _,part_id in ipairs( blueprint ) do
		self:_add_part( p_unit, factory_id, part_id, forbidden, override, parts, third_person )
	end
	
	for _,part_id in ipairs( self._need_parent ) do
		self:_add_part( p_unit, factory_id, part_id, forbidden, override, parts, third_person )
	end]]
	
	-- self._need_parent = nil
...
function WeaponFactoryManager:_part_data( part_id, factory_id, override )
	...
	if factory[ factory_id ].override then
		-- print( "HAS override", factory[ factory_id ].override )
		if factory[ factory_id ].override[ part_id ] then
			-- print( "  for part", part_id, inspect( factory[ factory_id ].override[ part_id ] ) )
			-- print( "  org", inspect( part ) )
			for d,v in pairs( factory[ factory_id ].override[ part_id ] ) do
				part[ d ] = v
			end
			-- print( "  aft", inspect( part ) )
		end
	end
	if override then
...
function WeaponFactoryManager:_add_part( p_unit, factory_id, part_id, forbidden, override, parts, third_person, need_parent )
	...
	if part.parent then
		...
		-- print( "part.parent", part.parent, parent_part )
		if parent_part then
			-- print(" HAS UNIT PARETN" )
			link_to_unit = parent_part.unit
		else
			-- table.insert( self._need_parent, part_id )
			table.insert( need_parent, part_id )
			return
			-- print(" HAS NOT UNIT PARETN" )
		end
	end
	...
	-- print( " --- spawn part", unit_name, part.a_obj )
	...
	parts[ part_id ] = { unit = unit, animations = part.animations, name = u_name, package = package }
end
...
-- Works as an add_part function as well
function WeaponFactoryManager:change_part( p_unit, factory_id, part_id, parts, blueprint )
	...
	-- print( "WeaponFactoryManager:change_part", p_unit, factory_id, part_id, parts, blueprint )
	
	local type = part.type
	if self._parts_by_weapon[ factory_id ][ type ] then
		-- print( "self._parts_by_weapon", self._parts_by_weapon[ factory_id ], self._parts_by_weapon[ factory_id ][ type ]  )
		if table.contains( self._parts_by_weapon[ factory_id ][ type ], part_id ) then
			-- print( "PART", part_id, "IS ALLOWED FOR WEAPON", factory_id )
...
-- Checks if there are parts that needs to be removed if the part is added
function WeaponFactoryManager:get_removes_parts( factory_id, part_id, blueprint )
	...
	-- print( inspect( self:_get_forbidden_parts( factory_id, blueprint ) ) )
	
	return removes
end

-- Check if a part can be added to the current blueprint, returns which parts, if any, that forbids it.
function WeaponFactoryManager:can_add_part( factory_id, part_id, blueprint )
	...
	for _,b_id in ipairs( blueprint ) do
		...
		if part.forbids then
			if table.contains( part.forbids, part_id ) then
...
function WeaponFactoryManager:remove_part( p_unit, factory_id, part_id, parts, blueprint )
	...
	-- Somehow part to remove cannot be anything. Only some parts can be optional, barrel extension, vertical grips, gadget.
...
function WeaponFactoryManager:get_stats( factory_id, blueprint )
	...
	local forbidden = self:_get_forbidden_parts( factory_id, blueprint )
	local override = self:_get_override_parts( factory_id, blueprint )
	-- print( "forbidden", inspect( forbidden ) ) 

	local stats = {}
	-- Need to check forbidden
	for _,part_id in ipairs( blueprint ) do
		if not forbidden[ part_id ] then
			if factory.parts[ part_id ].stats then
				local part = self:_part_data( part_id, factory_id )
...
function WeaponFactoryManager:get_sound_switch( switch_group, factory_id, blueprint )
	...
	for _,part_id in ipairs( blueprint ) do
		if not forbidden[ part_id ] then
			if factory.parts[ part_id ].sound_switch then
				if factory.parts[ part_id ].sound_switch[ switch_group ] then
...
function WeaponFactoryManager:load( data )
	-- Will need safety checks for added or removed content
