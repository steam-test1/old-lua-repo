...
SavefileManager.MAX_PROFILE_SAVE_INTERVAL = 5 * 60		-- Seconds (TCR #136)

-- Task types:
SavefileManager.IDLE_TASK_TYPE = 1
SavefileManager.LOAD_TASK_TYPE = 2
SavefileManager.SAVE_TASK_TYPE = 3
SavefileManager.REMOVE_TASK_TYPE = 4		-- TODO
SavefileManager.CHECK_SPACE_REQUIRED_TASK_TYPE = 5
SavefileManager.DEBUG_TASK_TYPE_NAME_LIST = { "Idle", "Loading", "Saving", "Removing", "CheckSpaceRequired" }
SavefileManager.RESERVED_BYTES = 200 * 1024
...
function SavefileManager:init()
	...
	if( not Global.savefile_manager ) then
		Global.savefile_manager = {
			current_game_cache_slot = nil,		-- Used to reset old cache (prevents the game from having more than one cache stored)
			meta_data_list = {},
			setting_changed = nil,
			safe_profile_save_time = nil		-- Time when it is safe to save to the profile without violating TCR #136 (Xbox)
		}
	end

	self._workspace = managers.gui_data:create_saferect_workspace() --  Overlay:gui():create_screen_workspace()
...
function SavefileManager:storage_changed()
	-- print( " !!!! function SavefileManager:storage_changed()" )
	...
	if( storage_device_selected ) then
		...
		self._loading_save_games = { [self.SETTING_SLOT] = true, [self.PROGRESS_SLOT] = true }
		cat_print( "savefile_manager", "[SavefileManager] Loading meta data list." )

		local param_map = { user_index = managers.user:get_platform_id() }
...
function SavefileManager:update( t, dt )
	...
	if self._loading_sequence then
		if not next( self._loading_save_games ) then
...
function SavefileManager:get_save_info_list( include_empty_slot )
	...
	for slot,meta_data in pairs( Global.savefile_manager.meta_data_list ) do
		if( meta_data.is_synched_text and ( not include_empty_slot or ( slot ~= self.AUTO_SAVE_SLOT ) ) and ( slot ~= self.SETTING_SLOT ) and ( slot ~= self.PROGRESS_SLOT ) ) then
			table.insert( data_list, { slot = slot, text = meta_data.text, sort_list = meta_data.sort_list } )
		end
	end
...
function SavefileManager:_meta_data_slot_detected_done_callback()
	...
	if not self._has_meta_list then
		...
		--[[local size = 0
		for _,slot in ipairs( self._has_meta_list ) do
			print( "had meta for slot", slot )
			size = size + self.RESERVED_BYTES / 1024
		end]]
		--[[if SystemInfo:platform() == Idstring("PS3") then
	...
	if not ( self._has_meta_list and table.contains( self._has_meta_list, self.PROGRESS_SLOT ) ) and self._backup_data then
...
function SavefileManager:_parse_text( save_data )
	if( not save_data ) then
		...
		text = managers.localization:text( "savefile", { TEXT = text, DATE = save_data:date( "%c" ) } )
	...
	if( cache_only ) then
		...
		-- elseif( is_progress_slot ) then
			...
			if( is_setting_slot ) then
				...
				subtitle = managers.localization:text( "savefile_setting", { VERSION = self.LOWEST_COMPATIBLE_VERSION } )
				description = managers.localization:text( "savefile_setting_description" )
			else -- Progress save file
				title = managers.localization:text( "savefile_game_title" )
			...
			--[[elseif SystemInfo:platform() == Idstring("X360") then
				save_data:set_title( title )
				save_data:set_small_icon_by_path( "gameicon.png" )
				param_map.use_small_icon = true]]
			end
			-- save_data:corrupt( 25 )
			...
			-- SystemInfo:platform() == Idstring("PS3")
...
function SavefileManager:_save_platform_setting_map_callback( success )
	...
	if not success then
		...
		self:_set_setting_changed( false ) -- Yes it failed, normaly we would try again next time. This stops it from trying without actually changing a setting?
	end
end
...
function SavefileManager:_save_callback( save_data )
	...
	--[[print( inspect( save_data:subtitle() ) )
	print( inspect( save_data:title() ) )
	print( inspect( save_data:details() ) )
	print( inspect( save_data:information() ) )
	print( inspect( save_data:information() ) )]]
...
function SavefileManager:_save_cache( slot )
	...
	-- Create cache:
	local cache = { version = SavefileManager.VERSION, version_name = SavefileManager.VERSION_NAME }
	if( is_setting_slot ) then
...
function SavefileManager:_load( slot, cache_only, save_system )
	...
	if( cache_only or ( meta_data.is_synched_cache and meta_data.cache ) ) then
		self:_load_done( slot, cache_only )	-- Skip loading from file since the cache is up to date.
	else
		-- Remove old cache:
		if( is_setting_slot ) then
			self:_set_cache( slot, nil )
...
function SavefileManager:_load_callback_done()
	--[[print( "SavefileManager:_load_callback_done" )
	print( inspect( self._save_sizes ) )
	local extra_file_size = 0
	for _,s in ipairs( self._save_sizes ) do
		extra_file_size = extra_file_size + s
	end
	print( "extra_file_size", extra_file_size )
	if SystemInfo:platform() == Idstring("PS3") then
		Application:check_sufficient_hdd_space_to_launch( extra_file_size/1000 )
	end]]
end
...
function SavefileManager:_load_done( slot, cache_only, wrong_user, wrong_version )
	...
	elseif( not success ) then
		self._try_again = self._try_again or {} -- Table to track if we have tried reloading /Martin
		local dialog_data = {}
		...
		if ( is_setting_slot or is_progress_slot ) then
			...
			if not self._try_again[ slot ] then
				...
				if is_setting_slot then
					yes_button.callback_func = function() self:load_setting() end
				elseif is_progress_slot then
					yes_button.callback_func = function() self:load_progress() end
				end
				no_button.callback_func = 
					function()
						if is_progress_slot and self._backup_data then
							...
							local ok_button = {}
							ok_button.text = managers.localization:text( "dialog_ok" )
							ok_button.callback_func = function() self:_remove( slot ) end
			...
			-- NewSave:remove( { save_slots = {is_setting_slot} }, function( ... ) print( "did something", ... ) end  )
			if at_init then
...
function SavefileManager:_remove_callback( ... )
	-- print( "SavefileManager:_remove_callback", ... ) 
end
...
function SavefileManager:_meta_data( slot )
	...
	if( not meta_data ) then
		meta_data = {
			slot = slot,
			is_corrupt = false,
			is_synched_text = false,	-- Whether the text is synched with the filesystem or not.
			text = nil,
			is_synched_cache = false,	-- Whether the cache is synched with the filesystem or not.
			cache = nil,
		}
...
function SavefileManager:_add_task_handler( task_handler )
	if task_handler:unique_id() then -- A unique task can only exist once in the task list, and it is always be the latest one
		if self._task_handler_list and #self._task_handler_list > 0 then
...
function SavefileManager:_set_current_task_type( task_type )
	...
	if( old_task_type ~= task_type ) then
		...
		if( ( task_type == self.SAVE_TASK_TYPE ) or ( task_type == self.REMOVE_TASK_TYPE ) -- ) then	-- Write task type begun. 
				or (use_load_task_type) )then	-- Load task type begun on PS3.
		...
		elseif( self._show_gui_time ) then															-- Write task type done.
			if( main_time - self._show_gui_time > check_t ) then
				self._hide_gui_time = main_time					-- "Writes longer than three seconds require the standard message be displayed for the length of the write."
			elseif( main_time - self._show_gui_time > 1 ) then
				self._hide_gui_time = self._show_gui_time + check_t	-- "Writes longer than one second require the standard message be displayed for three seconds."
			else
				self._hide_gui_time = self._show_gui_time + check_t	-- "Writes that last one second or less require the shorter message be displayed for one second or the standard message for three seconds.". Used standard message.
			end
...
function SavefileManager:_is_saving_progress_allowed()
	-- Saving progress when not signed in is not allowed
	if not managers.user:is_signed_in( nil ) then
		return false
	end
	
	-- Saving progress when no storage i snot allowed
	if not managers.user:is_storage_selected( nil ) then
		return false
	end
	
	-- Don't attempt to save if progress is loaded from wrong user (PS3) 
...
function SavefileManager:_is_saving_setting_allowed( is_user_initiated_action )
	-- Saving settings when not signed in is not allowed
	if not managers.user:is_signed_in( nil ) then
		return false
	end
...
function SavefileManager:_ask_load_backup( reason, dialog_at_init, load_params )
	...
	-- This is no when loading with "no_progress"
...
function SavefileTaskHandler:init( task, task_type, callback_func, done_callback_func, unique_id )
	...
	if self._task_type == SavefileManager.REMOVE_TASK_TYPE then
		...
		self._remove_rect = self._ws:panel():rect( {color = Color(0.97,0,0,0), layer= 1200 } )
	end 
end
...
function SavefileTaskHandler:update()
	...
	if self._task:done() then
		if alive( self._remove_rect ) then
...
function LoadPlatformSettingMapTask:init()
	...
	self._done = nil
	self._has_next = true
	self._platform_setting_map = nil
	]]
