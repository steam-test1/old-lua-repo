...
5
	...
	do
		...
		do
			...
			elseif (tweak_data.preplanning.types[type].category or "default") == optional_category then
				table.insert(t, type)
			end
...
function PrePlanningManager:categories_with_mission_elements()
	local t = {}
	do
		local (for generator), (for state), (for control) = ipairs(self:types_with_mission_elements())
		do
			do break end
