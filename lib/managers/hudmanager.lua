...
function HUDManager:_set_chat_alpha(scrollus, scrolllines, alpha)
	...
	local (for generator), (for state), (for control) = ipairs(scrolllines)
	do
		do break end
...
function HUDManager:clear_items()
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._hud.items)
		do
			do break end
...
function HUDManager:set_weapon_selected_by_inventory_index(inventory_index)
	local (for generator), (for state), (for control) = pairs(self._hud.weapons)
	do
		do break end
...
function HUDManager:remove_mugshot_equipment(id, equipment)
	...
	do
		...
		if data.id == id then
			do
				local (for generator), (for state), (for control) = ipairs(data.equipment)
				do
					do break end
...
function HUDManager:_layout_mugshots()
	...
	do
		...
		local alpha = 0.5
		do
			local (for generator), (for state), (for control) = ipairs(mugshot.panel:children())
			do
				do break end
...
function HUDManager:_layout_crosshair()
	...
	local (for generator), (for state), (for control) = ipairs(self._hud.crosshair_parts)
	do
		do break end
...
function HUDManager:_update_pressed_d_pad(t, dt)
	local (for generator), (for state), (for control) = pairs(self._hud.pressed_d_pad)
	do
		do break end
...
function HUDManager:activate_objective(data)
	...
	text:set_size(w, h)
	do
		local (for generator), (for state), (for control) = pairs(data.sub_objectives)
		do
			do break end
