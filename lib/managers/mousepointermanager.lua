...
function MousePointerManager:_setup()
	...
	-- 0,0,19,23  20,0 . 40,0   
	self._mouse = self._ws:panel():panel( { name_s = "mouse", name = "mouse", x = x, y = y, w = 19, h = 23, layer = tweak_data.gui.MOUSE_LAYER } )
	self._mouse:bitmap( { name = "pointer", texture = "guis/textures/mouse_pointer", texture_rect = {0,0,19,23}, x = -7, y = -2, w = 19, h = 23, color = Color( 1, 0.7, 0.7, 0.7 ), rotation = 360 } )
	self._ws:hide()
	-- self:_set_size()
