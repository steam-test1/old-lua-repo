...
function TradeManager:update( t, dt )
	...
	if not self._hostage_remind_t or t > self._hostage_remind_t then
		if not self._trading_hostage and 
			not self._hostage_trade_clbk and
			#self._criminals_to_respawn > 0 and
			managers.groupai:state():hostage_count() <= 0 and
			managers.groupai:state():is_AI_enabled() then -- TODO(nikmyr): is_AI_enabled is to see if its not in the end screen, surely this can be done better?
	...
	if self._trade_countdown then
		if 	Network:is_server() and
			not self._trading_hostage and 
		   	not self._hostage_trade_clbk and
		   	#self._criminals_to_respawn > 0 and
			not managers.groupai:state():whisper_mode() then
			...
			if trade then
		   		if managers.groupai:state():hostage_count() <= 0 then
		   			-- Remind player to tie down hostages?
		   		else
					self._cancel_trade = nil
		   		
					-- TODO: Add dialog about trading hostages?
...
function TradeManager:sync_set_trade_death( criminal_name, respawn_penalty, hostages_killed, from_local )
	...
	if managers.criminals:local_character_name() == criminal_name and not Network:is_server() then
...
--	print("[TradeManager:sync_set_trade_spawn]", criminal_name)
...
function TradeManager:on_AI_criminal_death( criminal_name, respawn_penalty, hostages_killed, skip_netsend )
	...
	local crim = { id = criminal_name, ai = true, respawn_penalty = respawn_penalty, hostages_killed = hostages_killed }
...
function TradeManager:on_player_criminal_death( criminal_name, respawn_penalty, hostages_killed, skip_netsend )
	...
	local crim = { id = criminal_name, ai = false, respawn_penalty = respawn_penalty, hostages_killed = hostages_killed, peer_id = managers.criminals:character_peer_id_by_name( criminal_name ) }
	local inserted = false
	
	for i, crim_to_respawn in ipairs(self._criminals_to_respawn) do
		if crim_to_respawn.ai or crim_to_respawn.respawn_penalty > respawn_penalty then
...
--	print( "[TradeManager:remove_from_trade]", criminal )
...
function TradeManager:cancel_trade( )
	...
	if self._criminal_respawn_clbk then
		managers.enemy:remove_delayed_clbk( self._criminal_respawn_clbk )
...
function TradeManager:begin_hostage_trade_dialog( i )
	...
	elseif i == 2 then
		-- Byt dessa mot h02abcd
		--[[
		if #self._criminals_to_respawn == 1 then
			print("Playing Play_ban_h17x")
			managers.dialog:queue_dialog( "Play_ban_h17x", {} )
			i = 2
		else
			print("Playing Play_ban_h18x")
			managers.dialog:queue_dialog( "Play_ban_h18x", {} )
			i = 3
		end
		]]
...
function TradeManager:begin_hostage_trade( )
	...
	-- TODO: Ask clients if best_hostage is dead
	...
	if best_hostage then
		...
		if not rescuing_criminal then
			-- No player alive to rescue hostage, tough 
		end
		
		-- TODO: Else, notifications
	end
...
function TradeManager:clbk_respawn_criminal( trading_unit )
	...
	if not alive( spawn_on_unit ) then
		...
		if #possible_criminals <= 0 then
			-- Should be game over here anyway
			return
		end
...
--		else
		...
		--[[
			local id = respawn_criminal.id
			managers.network:session():send_to_peers_synched_except( id, "sync_teammate_helped_hint", 1, spawned_unit, trading_unit )
			
			if id ~= 1 then
				self:sync_teammate_helped_hint( spawned_unit, trading_unit, 1 )
				managers.network:session():peer( id ):send_queued_sync( "sync_you_helped_hint", 1, trading_unit )
			else
				self:sync_you_helped_hint( trading_unit, 1 )
			end
		]] 
--		end
	end
...
function TradeManager:sync_teammate_helped_hint( helped_unit, helping_unit, hint )
	...
	if hint_id then
		managers.hint:show_hint( hint_id, nil, false, {TEAMMATE=helped_unit:base():nick_name(), HELPER=helping_unit:base():nick_name()} )
	end
end
