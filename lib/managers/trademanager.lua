...
function TradeManager:save(save_data)
	...
	local (for generator), (for state), (for control) = ipairs(self._criminals_to_respawn)
	do
		do break end
...
function TradeManager:update(t, dt)
	...
	(for control) = "Play_ban_h22x" and managers
...
function TradeManager:play_custody_voice(criminal_name)
	...
	if #self._criminals_to_respawn == 3 then
		...
		(for control) = managers.groupai:state():all_char_criminals() and crim_data.unit
...
function TradeManager:clbk_begin_hostage_trade()
	...
	(for control) = managers.groupai:state():all_player_criminals() and u_data.status
	...
	do
		local (for generator), (for state), (for control) = ipairs(managers.groupai:state():all_hostages())
		do
			do break end
	...
	do break end
	do
		local (for generator), (for state), (for control) = pairs(managers.groupai:state():all_converted_enemies())
		do
			do break end
...
6
	...
	if not alive(spawn_on_unit) then
		...
		(for control) = managers.groupai:state():all_char_criminals() and u_data.status
