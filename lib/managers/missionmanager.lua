...
function MissionManager:init( ... )
	...
	self:set_global_event_list( { "bankmanager_key", "keycard", "start_assault", "end_assault", "police_called", "police_weapons_hot", "civilian_killed", "loot_lost", "loot_exploded",
																"pku_gold", "pku_money", "pku_jewelry", "pku_painting", "pku_meth", "pku_cocaine", "pku_weapons", "pku_toolbag", "pku_atm", "pku_folder", "pku_poster" } )
	
	self._mission_filter = {}
	
	if not Global.mission_manager then
		Global.mission_manager = {}
		Global.mission_manager.stage_job_values = {} -- During the stage, values are stored here. Cleared on failed, moved to job_values on success.
...
function MissionManager:on_reset_profile()
	Global.mission_manager.saved_job_values.playedSafeHouseBefore = nil -- Remove save value used in Safehouse
end
...
function MissionManager:on_retry_job_stage()
	-- print( "  MissionManager:on_retry_job_stage()" )
	Global.mission_manager.stage_job_values = {}
end

function MissionManager:on_stage_success()
	-- print( "  MissionManager:on_stage_success()" )
...
-- Called from client to tell the server that he entered an area
...
-- Called from client to tell the server that he exited an area
...
-- Called from client to tell the server that he has accessed a camera feed
