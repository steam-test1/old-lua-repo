...
function HUDMissionBriefing:hide()
	...
	if alive( self._background_layer_two ) then
		self._background_layer_two:clear() -- remove the video
	end
	--[[
	if self._delayed_dialog then
		managers.enemy:remove_delayed_clbk( "HUDMissionBriefing_brf_dialog" )
		self._delayed_dialog = nil
	else
		managers.dialog:stop_current_dialog()
	end]]
end
...
function HUDMissionBriefing:set_player_slot( nr, params )
	...
	slot:child("name"):set_text( params.name ) -- .. " (" .. tostring(params.level) .. ")  "  )
...
function HUDMissionBriefing:set_slot_not_ready( peer, peer_id )
	...
	--[[
	x, y, w, h = slot:child("status"):text_rect()
	slot:child("invert"):show()
	slot:child("invert"):set_alpha( 1 )
	slot:child("invert"):set_w( w )
	slot:child("invert"):set_h( h )
	slot:child("invert"):set_color( slot:child("status"):color() )
	slot:child("invert"):set_position( slot:child("status"):position() )
	-- slot:child("status"):set_color( Color.white )
	slot:child("status"):set_blend_mode( "add" )
	]]
end
...
function HUDMissionBriefing:remove_player_slot_by_peer_id( peer, reason )
	...
	-- slot:child("name"):set_selection( 0, 0 )
