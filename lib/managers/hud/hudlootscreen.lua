...
function HUDLootScreen:init( hud, workspace, saved_lootdrop, saved_selected, saved_chosen )
	...
	local loot_text = self._foreground_layer_safe:text( { name="loot_text", text=lootscreen_string, align="center", vertical="top", font_size=large_font_size, font=large_font, color=tweak_data.screen_colors.text } )
	self:make_fine_text( loot_text )
	
	local bg_text = self._background_layer_full:text( { text=loot_text:text(), h=90, align="left", vertical="top", font_size=massive_font_size, font=massive_font, color=tweak_data.screen_colors.button_stage_3, alpha=0.4 } )
	...
	-- self._pattern_panel = self._backdrop:set_pattern()
	-- self._pattern_panel:bitmap( { name="pattern", texture=self._hud.pattern, wrap_mode="wrap", texture_rect={ 0, 0, self._pattern_panel:w()*0.25, self._pattern_panel:h()*0.25 }, w=self._pattern_panel:w(), h=self._pattern_panel:h(), color=Color.white:with_alpha(0.2) } )
	--[[
	local lootscreen_string = managers.localization:to_upper_text("menu_l_lootscreen")
	local loot_text = self._foreground_layer_safe:text( { name="loot_text", text=lootscreen_string, align="center", vertical="top", font_size=large_font_size, font=large_font, color=tweak_data.screen_colors.text } )
	
	self:make_fine_text( loot_text )
	loot_text:set_center_x( self._foreground_layer_safe:w()/2 )
	
	local bg_text = self._background_layer_full:text( { name="loot_text", text=lootscreen_string, align="center", vertical="center", font_size=massive_font_size, font=massive_font, color=tweak_data.screen_colors.button_stage_2, alpha=0.4 } )
	
	self:make_fine_text( bg_text )
	bg_text:set_world_center( loot_text:world_center() )]]
	-- bg_text:move( 0, -3 )
	
	-- self._backdrop:animate_bg_text( bg_text )
	
	-- self._foreground_layer_safe:text( { name="firstpass1", text="(WIP)", align="center", vertical="top", font_size=text_font_size, font=text_font, color=tweak_data.screen_color_white } )
	-- self._foreground_layer_safe:text( { name="firstpass2", text="(WIP)", align="left", vertical="bottom", font_size=text_font_size, font=text_font, color=tweak_data.screen_color_white } )
	-- self._foreground_layer_safe:text( { name="firstpass3", text="(WIP)", align="right", vertical="bottom", font_size=text_font_size, font=text_font, color=tweak_data.screen_color_white } )
	
	-- self._video = self._foreground_layer_full:video( { video = "movies/loot_drop01", loop = false } ) -- , x = x, y = y } )
	-- self._video:pause()
	...
	-- self._wait_t = { false, false, false, false }
end
...
function HUDLootScreen:create_peer( peers_panel, peer_id )
	...
	local peer_infamy = peer_info_panel:bitmap( { name = "peer_infamy", w = 16, h = 32, texture = infamy_icon, visible = false, color = color } )
	
	self:make_fine_text(peer_name)
	peer_name:set_right( peer_info_panel:w() )
	peer_name:set_center_y( peer_info_panel:h() * 0.5 )
	
	local max_quality = peer_info_panel:text( { name="max_quality", font=small_font, font_size=small_font_size-1, text=" ", h=medium_font_size, w=1, color=tweak_data.screen_colors.text, blend_mode="add", visible=false } )
	...
	for i=1, 3 do
		...
		local downcard = card_panel:bitmap( { name="downcard", texture = self._downcard_icon_path, texture_rect = self._downcard_texture_rect, w=math.round(128/180*card_panel:h()), h=card_panel:h(), blend_mode="add", rotation=math.random(4)-2, layer=1, halign="scale", valign="scale" } )
		if downcard:rotation() == 0 then
			downcard:set_rotation( 1 )
		end
		
		if not is_local_peer then
			downcard:set_size( math.round(128/180*card_panel:h()*0.85), math.round(card_panel:h()*0.85) )
		end 
		downcard:set_center( card_panel:w()*0.5, card_panel:h()*0.5 )
		
		local bg = card_panel:bitmap( { name="bg", texture = self._downcard_icon_path, texture_rect = self._downcard_texture_rect, color=tweak_data.screen_colors.button_stage_3, halign="scale", valign="scale" } )
		...
		card_panel:hide()
		-- card_panel:rect( { color = i==1 and Color.red or i==2 and Color.green or Color.blue, blend_mode="add" } )
	end
...
function HUDLootScreen:create_selected_panel( peer_id )
	...
	local glow_circle = selected_panel:bitmap( { texture="guis/textures/pd2/crimenet_marker_glow", w=200, h=200, blend_mode="add", color=tweak_data.screen_colors.button_stage_3, alpha=0.4, rotation=360 } )
	local glow_stretch = selected_panel:bitmap( { texture="guis/textures/pd2/crimenet_marker_glow", w=500, h=200, blend_mode="add", color=tweak_data.screen_colors.button_stage_3, alpha=0.4, rotation=360 } )
...
function HUDLootScreen:show()
	if not self._video and not (SystemInfo:platform() == Idstring( "X360" )) then
		local variant = math.random(8)
		self._video = self._baselayer_two:video( { video = "movies/lootdrop"..tostring(variant), loop = false, speed = 1, blend_mode="add", alpha=0.2 } ) -- , x = x, y = y } )
		
		managers.video:add_video( self._video )
		-- self._video:animate( callback( self, self, "_wait_for_video" ) )
	end
	...
	local fade_rect = self._foreground_layer_full:rect( { layer=10000, color=Color.black } )
	local fade_out_anim = function(o)
...
function HUDLootScreen:feed_lootdrop( lootdrop_data )
	...
	-- self._lootdrops[ peer_id ] = lootdrop_data
	...
	local anim_fadein = function(o)
		over(1, function(p) o:set_alpha(p) end )
	end
	...
	if category == "weapon_mods" then	-- name converter
		category = "mods"
	end
	...
	if category == "colors" then
		...
		local texture_loaded_clbk = callback( self, self, "texture_loaded_clbk", {peer_id, (category == "textures") and true or false} )
		local texture_path
...
function HUDLootScreen:begin_choose_card( peer_id, card_id )
	...
	if item_pc == 0 then
		self._peer_data[ peer_id ].joker = tweak_data.lootdrop.joker_chance > 0
	end
	...
	for i, pc in ipairs( cards ) do
		...
		local upcard = card_panel:bitmap( { name="upcard", texture=texture, w = math.round(128/180*card_panel:h()), h=card_panel:h(), blend_mode="add", layer=1, halign="scale", valign="scale" } )
		...
		if joker then
			...
			-- card_panel:panel( { name="joker", w=0, h=0 } )
		end
...
function HUDLootScreen:flipcard( card_panel, timer, done_clbk, peer_id )
	...
	if done_clbk then
		...
		if max_pc == 0 then
			-- nothing, joker already included in timer
	...
	if Application:production_build() then
		downcard:set_rotation( math.random(4)-2 )
		downcard:set_w( math.round(128/180*card_panel:h()) )
		downcard:set_h( card_panel:h() )
		downcard:set_center( card_panel:w()*0.5, card_panel:h()*0.5 )
	end]]
end
...
function HUDLootScreen:update( t, dt )
	-- for peer_id, wait_t in ipairs( self._wait_t ) do
	for peer_id=1, 4 do
		if self._peer_data[ peer_id ].wait_t then
			...
			if self._peer_data[ peer_id ].wait_t == 0 then
				-- for i=1, 3 do
				-- 	local card = panel:child("card"..i)
				-- 	card:child("downcard"):hide()
				-- 	card:child("upcard"):show()
				-- end
				main_text:set_text( managers.localization:to_upper_text( "menu_l_choose_card_chosen_suspense" ) )
				local joker = self._peer_data[ peer_id ].joker -- panel:child( "card"..self._selected[peer_id] ):child("joker") and true or false
...
function HUDLootScreen:create_stars_giving_animation()
	...
	-- max_pc: how many stars I can get, the end amounts of stars
	-- job_stars: Jobs stars
	-- player_stars: My star level
	-- bonus_stars: gives +1 star
	...
	-- Application:debug( "max_pc", max_pc, "job_stars", job_stars, "difficulty_stars", difficulty_stars, "player_stars", player_stars )
	
	local max_number_of_stars = job_stars -- math.max( max_pc, job_stars + bonus_stars )
	...
	local star_reason_text = self._stars_panel:text( { font=tweak_data.menu.pd2_medium_font, font_size=tweak_data.menu.pd2_medium_font_size, text="" } )
	...
	-- self._stars_panel:set_debug(true)
	
	
	local animation_func = function(o)
		...
		for i=1, max_number_of_stars do
			...
			-- local star = self._stars_panel:rect( { name="star_"..tostring(i), w=60, h=60, color=i>(max_number_of_stars-difficulty_stars) and tweak_data.screen_colors.risk or tweak_data.screen_colors.text } )
			local star = self._stars_panel:bitmap( { name="star_"..tostring(i), texture=texture, texture_rect=rect, w=32, h=32, color=i>(max_number_of_stars-difficulty_stars) and tweak_data.screen_colors.risk or tweak_data.screen_colors.text, blend_mode="add" } )
			...
			over( 0.45, function(p)
				...
				star:set_color( math.lerp( star_color, star_color, p ) - math.clamp( math.sin( p * 180 ), 0, 1 ) * Color( 1.0, 1.0, 1.0 ) )
		...
		--[[
		wait( 0.25 )
		for i=max_number_of_stars, (player_stars+level_stars+1), -1 do
			wait( 0.1 )
			
			star_reason_text:set_text( managers.localization:to_upper_text( "menu_es_alive_low_level_bonus" ) )
			star_reason_text:set_color( tweak_data.screen_colors.text )
			local star = self._stars_panel:child( "star_" .. tostring(i) )
			over( 0.28, function(p)
				star:set_alpha( math.lerp( 1, 0.2, p ) )
				star_reason_text:set_alpha( math.lerp( 0.2, 1, p ) )
			end)
			latest_star = i
		end
		
		local max_number_of_stars = math.max( max_pc, job_stars + bonus_stars )
		
		wait( 0.30 )
		]]
		
		--[[
		star_reason_text:set_text( "" )
		
		for i=latest_star+1, max_pc do
			wait( 0.1 )
			
			managers.menu_component:post_event( "Play_star_hit" )
			star_reason_text:set_text( managers.localization:to_upper_text( "menu_es_alive_players_bonus" ) )
			star_reason_text:set_color( tweak_data.screen_colors.friend_color )
			star_reason_text:set_left( max_number_of_stars * 35 )
			
			local star = self._stars_panel:child( "star_" .. tostring(i) ) or false
			if not star then
				star = self._stars_panel:bitmap( { name="star_"..tostring(i), texture=texture, texture_rect=rect, w=32, h=32, color=tweak_data.screen_colors.text, blend_mode="add" } )
				star:set_alpha(0)
				star:set_x( (i-1) * 35 )
				star:set_world_center_y( math.round( self._foreground_layer_safe:child( "loot_text" ):world_center_y() ) )
			end
			local star_alpha = star:alpha()
			over( 0.4, function(p)
				star:set_alpha( math.min( math.lerp( star_alpha, 10, p ), 1 ) )
				star:set_color( Color.white - math.clamp( math.sin( p * 180 ), 0, 1 ) * Color( 0.5, 0.5, 0.5 ) )
				star:set_color( star:color():with_alpha(1) )
				
				star_reason_text:set_alpha( 1-math.sin( p * 180 ) )
			end)
			latest_star = i
		end
		
		wait( 0.30 )
		local haxed_max_quality_string = managers.localization:to_upper_text( "menu_l_max_quality", {quality=""} )
		haxed_max_quality_string = string.sub( haxed_max_quality_string, 1, -3 )
		star_reason_text:set_text( haxed_max_quality_string )
		star_reason_text:set_alpha( 0.2 )
		for i=1, max_pc do
			wait( 0.05 )
			-- local star_fill = self._stars_panel:rect( { name="star_fill_"..tostring(i), w=58, h=58, color=Color.black, alpha=0, layer=1 } )
			local star = self._stars_panel:child( "star_" .. tostring(i) )
			local star_fill = self._stars_panel:bitmap( { name="star_fill_"..tostring(i), texture=texture, texture_rect=rect, w=30, h=30, color=Color.black, alpha=0, layer=1 } )
			
			star_fill:set_center( star:center() )
			over( 0.11, function(p)
				star_fill:set_alpha( p )
				star_reason_text:set_alpha( math.lerp( star_reason_text:alpha(), 1, p ) )
			end )
		end
		
		local deduct_timer = 0
		for i=max_pc+1, max_number_of_stars do
			wait( 0.04 )
			
			local star = self._stars_panel:child( "star_" .. tostring(i) )
			over( 0.10, function(p)
				star:set_alpha( 0.2 * ( 1-p ) )
			end )
			deduct_timer = deduct_timer + 0.08
		end
		
		wait( 0.85 - deduct_timer )]]
		over( 0.5, function(p) star_reason_text:set_alpha(1-p) end )
	end
	
	-- menu_es_risk_bonus
	-- menu_es_alive_low_level_bonus
...
function HUDLootScreen:check_inside_local_peer( x, y )
	...
	local panel = self._peers_panel:child("peer"..tostring(peer_id))
	-- self._peers_panel:set_debug( true )
...
function HUDLootScreen:reload()
	...
	--[[
	local saved_lootdrop = {}
	for i, d in pairs( self._lootdrops ) do
		table.insert( saved_lootdrop, d )
	end
	
	local saved_selected = {}
	for i, d in ipairs( self._selected ) do
		table.insert( saved_selected, d )
	end
	]]
