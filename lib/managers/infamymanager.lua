...
function InfamyManager:_setup(reset)
	if not Global.infamy_manager or reset then
		...
		local (for generator), (for state), (for control) = pairs(tweak_data.infamy.tree)
		do
			do break end
...
function InfamyManager:unlock_item(item)
	...
	if self:points() >= Application:digest_value(infamy_item.cost, false) then
		do
			local (for generator), (for state), (for control) = ipairs(infamy_item.upgrades)
			do
				do break end
...
function InfamyManager:load(data, version)
	...
	if state then
		...
		do
			local (for generator), (for state), (for control) = pairs(state.unlocks)
			do
				do break end
