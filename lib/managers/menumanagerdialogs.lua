...
function MenuManager:show_does_not_own_heist_info( heist, player )
	local dialog_data = {}
	dialog_data.title = managers.localization:text( "dialog_error_title" )
	dialog_data.text = managers.localization:text( "dialog_does_not_own_heist_info", { HEIST = string.upper( heist ), PLAYER = string.upper( player ) } )
	local ok_button = {}
	ok_button.text = managers.localization:text( "dialog_ok" )
	dialog_data.button_list = { ok_button }
	managers.system_menu:show( dialog_data )
...
function MenuManager:show_person_joining( id, nick )
	local dialog_data = {}
	dialog_data.title = managers.localization:text( "dialog_dropin_title", { USER = string.upper( nick ) } )
...
-- Hijack this dialog for remove dead player
...
function MenuManager:show_default_option_dialog()
	...
	yes_button.callback_func = function() managers.user:reset_setting_map() end
	local no_button = {}
...
function MenuManager:show_err_not_signed_in_dialog()
	...
	local ok_button = {}
	ok_button.text = managers.localization:text( "dialog_ok" )
	ok_button.callback_func =  function() self._showing_disconnect_message = nil end
	dialog_data.button_list = { ok_button }
	managers.system_menu:show( dialog_data )
end
...
function MenuManager:show_confirm_skillpoints( params )
	...
	dialog_data.text = managers.localization:text( params.text_string, {skill=params.skill_name_localized, points=params.points, remaining_points=params.remaining_points, cost=managers.experience:cash_string(params.cost)} )
...
function MenuManager:show_confirm_blackmarket_sell_no_slot( params )
	...
	dialog_data.text = 	managers.localization:text( "dialog_blackmarket_item", {item=params.name} ) .. "\n\n" ..
...
function MenuManager:show_confirm_blackmarket_buy( params )
	...
	if( num_of_same > 0 ) then
		num_in_inventory = managers.localization:text("dialog_blackmarket_num_in_inventory", {item=params.name, amount=num_of_same})
	end
	-- 
	...
	dialog_data.text = managers.localization:text( "dialog_blackmarket_buy_item", 
								{ item=params.name, money=params.money, num_in_inventory=num_in_inventory } )
	dialog_data.focus_button = 2
...
function MenuManager:show_confirm_blackmarket_mod( params )
	...
	dialog_data.text = 	l_local:text( "dialog_blackmarket_slot_item", {slot=params.slot, item=params.weapon_name} ) .. "\n\n" ..
	...
	if( params.removes and #params.removes > 0 ) then
		...
		for _, mod_name in ipairs( params.removes ) do
			mods = mods .. "\n" .. managers.weapon_factory:get_part_name_by_part_id( mod_name ) -- managers.localization:text( tweak_data.weapon.factory.parts[  ].name_id )
		end
...
function MenuManager:show_confirm_blackmarket_assemble( params )
	...
	dialog_data.text = managers.localization:text( "dialog_blackmarket_assemble_item", 
								{ item=params.name, num_in_inventory=num_in_inventory } )
	dialog_data.focus_button = 2
...
function MenuManager:show_confirm_blackmarket_abort( params )
	local dialog_data = {}
	dialog_data.title = managers.localization:text( "dialog_bm_mask_custom_abort" )	-- title
...
function MenuManager:show_confirm_blackmarket_finalize( params )
	...
	dialog_data.text = managers.localization:text( "dialog_blackmarket_finalize_item", 
								{ money=params.money, ITEM=managers.localization:text("dialog_blackmarket_slot_item", {item=params.name, slot=params.slot}) } )
	dialog_data.focus_button = 2
...
function MenuManager:show_confirm_mission_asset_buy( params )
	...
	dialog_data.text = managers.localization:text( "dialog_mission_asset_buy", { asset_desc=managers.localization:text(asset_tweak_data.unlock_desc_id or "menu_asset_unknown_unlock_desc"), cost=managers.experience:cash_string(managers.money:get_mission_asset_cost_by_id(params.asset_id)) } )
	dialog_data.focus_button = 2
...
function MenuManager:show_confirm_buy_premium_contract( params )
	...
	dialog_data.text = managers.localization:text( "menu_cn_premium_buy_fee", { contract_fee = params.contract_fee, offshore = params.offshore } )
	dialog_data.focus_button = 2
...
function MenuManager:show_confirm_pay_casino_fee( params )
	...
	dialog_data.text = managers.localization:text( "menu_cn_casino_pay_fee", { contract_fee = params.contract_fee, offshore = params.offshore } )
	dialog_data.focus_button = 2
...
function MenuManager:show_accept_crime_net_job( params )		-- NIU
	local dialog_data = {}
...
function MenuManager:show_storage_removed_dialog( params )
	...
	-- ok_button.callback_func = callback( self, self, "continue_without_saving_yes_callback" )
...
--[[function MenuManager:show_question_open_crimenet( params )
	local dialog_data = {}
	dialog_data.focus_button = 1
	dialog_data.title = managers.localization:text( "dialog_open_crimenet_title" )
	dialog_data.text = managers.localization:text( "dialog_open_crimenet_text" )
	local yes_button = {}
	yes_button.text = managers.localization:text( "dialog_yes" )
	yes_button.callback_func = params.yes_func
	local no_button = {}
