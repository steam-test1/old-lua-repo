...
-- HEALTH
	...
	self.security.ecm_vulnerability = 1 -- chance of puking when exposed to ecm feedback
	self.security.ecm_hurts = { ears = { min_duration = 5, max_duration = 7 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.cop.ecm_vulnerability = 0.95 -- chance of puking when exposed to ecm feedback
	self.cop.ecm_hurts = { ears = { min_duration = 5, max_duration = 7 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.fbi.ecm_vulnerability = 0.95 -- chance of puking when exposed to ecm feedback
	self.fbi.ecm_hurts = { ears = { min_duration = 5, max_duration = 7 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.swat.ecm_vulnerability = 0.9 -- chance of puking when exposed to ecm feedback
	self.swat.ecm_hurts = { ears = { min_duration = 5, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH	
	...
	self.heavy_swat.ecm_vulnerability = 0.9 -- chance of puking when exposed to ecm feedback
	self.heavy_swat.ecm_hurts = { ears = { min_duration = 5, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.fbi_swat.ecm_vulnerability = 0.85 -- chance of puking when exposed to ecm feedback
	self.fbi_swat.ecm_hurts = { ears = { min_duration = 5, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.fbi_heavy_swat.ecm_vulnerability = 0.85 -- chance of puking when exposed to ecm feedback
	self.fbi_heavy_swat.ecm_hurts = { ears = { min_duration = 5, max_duration = 5 } }  -- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.sniper.ecm_vulnerability = 0.85 -- chance of puking when exposed to ecm feedback
	self.sniper.ecm_hurts = { ears = { min_duration = 3, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.gangster.ecm_vulnerability = 0.95 -- chance of puking when exposed to ecm feedback
	self.gangster.ecm_hurts = { ears = { min_duration = 5, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
function CharacterTweakData:_init_tank( presets )
	...
	self.tank.weapon.saiga.focus_delay = 5
	...
	self.tank.weapon.saiga.melee_speed = 0.7
	self.tank.weapon.saiga.melee_dmg = 6
	self.tank.weapon.saiga.range = { close = 1000, optimal = 2000, far = 5000 }
	self.tank.weapon.saiga.FALLOFF = {
		{ r=500, acc={0.6, 0.9}, dmg_mul=1.75, recoil={0.4,0.7}, mode={ 0, 3, 3, 1 } },
		{ r=1000, acc={0.4, 0.8}, dmg_mul=1.5, recoil={0.45,0.8}, mode={ 1, 2, 2, 0 } },
		{ r=2000, acc={0.4, 0.55}, dmg_mul=1.25, recoil={0.45,0.8}, mode={ 3, 2, 2, 0 } },
		{ r=3000, acc={0.1, 0.35}, dmg_mul=1, recoil={1,1.2}, mode={ 3, 1, 1, 0 } }
	}
	
	self.tank.detection = presets.detection.normal
	
-- HEALTH
	self.tank.HEALTH_INIT = 275
	...
	self.tank.ecm_vulnerability = 0.85 -- chance of puking when exposed to ecm feedback
	self.tank.ecm_hurts = { ears = { min_duration = 3, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.spooc.ecm_vulnerability = 0.5 -- chance of puking when exposed to ecm feedback
	self.spooc.ecm_hurts = { ears = { min_duration = 3, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.shield.ecm_vulnerability = 0.85 -- chance of puking when exposed to ecm feedback
	self.shield.ecm_hurts = { ears = { min_duration = 3, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
	...
	self.shield.weapon.mp9.range = { close = 500, optimal = 1200, far = 3000 }
	
	self.shield.weapon.mp9.FALLOFF = {
		{ r=0, acc={0.5, 0.95}, dmg_mul=4, recoil={0.35,0.55}, mode={ 0.2, 2, 4, 10 } },
		{ r=700, acc={0.4, 0.75}, dmg_mul=3, recoil={0.35,0.55}, mode={ 0.2, 2, 4, 10 } },
		{ r=1000, acc={0.3, 0.65}, dmg_mul=2, recoil={0.35,0.55}, mode={ 0.2, 2, 4, 10 } },
		{ r=2000, acc={0.2, 0.5}, dmg_mul=2, recoil={0.35,1.2}, mode={ 2, 5, 6, 4 } },
		{ r=4000, acc={0.1, 0.25}, dmg_mul=1, recoil={0.35,1.5}, mode={ 6, 4, 2, 0 } }
	}
	...
	self.shield.weapon.c45.range = { close = 500, optimal = 900, far = 3000 }
	
	self.shield.weapon.c45.FALLOFF = {
		{ r=0, acc={0.5, 0.9}, dmg_mul=4, recoil={0.35,0.55}, mode={ 1, 0, 0, 0 } },
		{ r=700, acc={0.4, 0.8}, dmg_mul=1.5, recoil={0.35,0.55}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.35, 0.6}, dmg_mul=1, recoil={0.35,0.55}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.15, 0.5}, dmg_mul=1, recoil={0.35,0.75}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0, 0.25}, dmg_mul=1, recoil={0.35,1.5}, mode={ 1, 0, 0, 0 } }
	}
...
function CharacterTweakData:_init_taser( presets )
	...
	self.taser.weapon = {
		m4 = {
			...
			range = { close = 1000, optimal = 2000, far = 5000 },
			FALLOFF = {
				{ r=500, acc={0.75, 0.95}, dmg_mul=2.5, recoil={0.35,0.7}, mode={ 0, 3, 3, 1 } },
				{ r=1000, acc={0.65, 0.95}, dmg_mul=2, recoil={0.35,0.75}, mode={ 1, 2, 2, 0 } },
				{ r=2000, acc={0.65, 0.8}, dmg_mul=1.25, recoil={0.4,1.2}, mode={ 3, 2, 2, 0 } },
				{ r=3000, acc={0.45, 0.6}, dmg_mul=1, recoil={1.5,3}, mode={ 3, 1, 1, 0 } }
			}
		}
	}
...
-- HEALTH
	...
	self.taser.headshot_dmg_mul = self.taser.HEALTH_INIT/20	-- damage multiplier on received headshots. nil means insta-death
	...
	self.taser.ecm_vulnerability = 0.85 -- chance of puking when exposed to ecm feedback
	self.taser.ecm_hurts = { ears = { min_duration = 5, max_duration = 5 } } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
-- HEALTH
	...
	self.civilian.ecm_vulnerability = 0 -- chance of puking when exposed to ecm feedback
	self.civilian.ecm_hurts = { } 					-- what type of hurts ecm feedback can inflict (puking, covering ears ect) and their durations
...
---- NORMAL ----
	presets.weapon.normal = { beretta92={}, c45={}, raging_bull={}, m4={}, ak47={}, r870={}, mossberg={}, mp5={}, mac11={}, raging_bull={} }
	...
	presets.weapon.normal.beretta92.FALLOFF = {
		{ r=500, acc={0.4, 0.85}, dmg_mul=1.5, recoil={0.1, 0.25}, mode={ 1, 0, 0, 0 } },
	...
	presets.weapon.normal.c45.FALLOFF = {
		{ r=500, acc={0.4, 0.85}, dmg_mul=1.5, recoil={0.15, 0.25}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.375, 0.55}, dmg_mul=1, recoil={0.15, 0.3}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.25, 0.45}, dmg_mul=1, recoil={0.3, 0.7}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.01, 0.35}, dmg_mul=1, recoil={0.4, 1}, mode={ 1, 0, 0, 0 } }
	}
	...
	presets.weapon.normal.r870.FALLOFF = {
		{ r=500, acc={0.4, 0.9}, dmg_mul=2, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
	...
	presets.weapon.normal.mp5.FALLOFF = {
		{ r=500, acc={0.4, 0.9}, dmg_mul=2, recoil={0.1,0.3}, mode={ 0, 3, 3, 1 } },
	...
	presets.weapon.normal.mac11.FALLOFF = {
		{ r=500, acc={0.1, 0.85}, dmg_mul=2, recoil={0.5,0.65}, mode={ 0, 1, 3, 1 } },
		{ r=1000, acc={0.2, 0.5}, dmg_mul=1, recoil={0.55,0.85}, mode={ 2, 1, 3, 0 } },
	...
	presets.weapon.normal.raging_bull.melee_speed = 1.5
	presets.weapon.normal.raging_bull.melee_dmg = 6
	presets.weapon.normal.raging_bull.range = { close = 1000, optimal = 2000, far = 5000 }
	presets.weapon.normal.raging_bull.FALLOFF = {
		{ r=500, acc={0.4, 0.85}, dmg_mul=1.5, recoil={0.4,0.8}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.375, 0.55}, dmg_mul=1, recoil={0.4,0.8}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.25, 0.45}, dmg_mul=0.75, recoil={1,1.2}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.01, 0.35}, dmg_mul=0.5, recoil={1.5,2}, mode={ 1, 0, 0, 0 } }
	}
...
---- GOOD ----
	...
	presets.weapon.good.beretta92.focus_delay = 5
	...
	presets.weapon.good.beretta92.RELOAD_SPEED = 1.3
	...
	presets.weapon.good.beretta92.FALLOFF = {
		{ r=500, acc={0.5, 0.9}, dmg_mul=2, recoil={0.1,0.25}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.375, 0.55}, dmg_mul=1, recoil={0.15,0.3}, mode={ 1, 0, 1, 0 } },
		{ r=2000, acc={0.15, 0.45}, dmg_mul=1, recoil={0.3,0.7}, mode={ 1, 0, 1, 0 } },
		{ r=3000, acc={0.1, 0.35}, dmg_mul=1, recoil={0.4,1.0}, mode={ 1, 0, 0, 0 } }
	}
	
	presets.weapon.good.c45.aim_delay = { 0.1, 0.1 }
	presets.weapon.good.c45.focus_delay = 5
	presets.weapon.good.c45.focus_dis = 200
	presets.weapon.good.c45.spread = 20
	presets.weapon.good.c45.miss_dis = 50
	presets.weapon.good.c45.RELOAD_SPEED = 1.3
	...
	presets.weapon.good.c45.FALLOFF = {
		{ r=500, acc={0.5, 0.85}, dmg_mul=2, recoil={0.15,0.25}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.375, 0.55}, dmg_mul=1.5, recoil={0.15,0.40}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.25, 0.45}, dmg_mul=1.25, recoil={0.4,0.9}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.01, 0.35}, dmg_mul=1, recoil={0.4,1}, mode={ 1, 0, 0, 0 } }
	}
	
	presets.weapon.good.m4.aim_delay = { 0.1, 0.1 }
	presets.weapon.good.m4.focus_delay = 5
	...
	presets.weapon.good.m4.FALLOFF = {
		{ r=500, acc={0.4, 0.9}, dmg_mul=2, recoil={0.45,0.8}, mode={ 0, 3, 3, 1 } },
		{ r=1000, acc={0.2, 0.8}, dmg_mul=1.5, recoil={0.35,0.75}, mode={ 1, 2, 2, 0 } },
		{ r=2000, acc={0.2, 0.5}, dmg_mul=1.25, recoil={0.4,1.2}, mode={ 3, 2, 2, 0 } },
		{ r=3000, acc={0.01, 0.35}, dmg_mul=1, recoil={1.5,3}, mode={ 3, 1, 1, 0 } }
	}
	...
	presets.weapon.good.r870.FALLOFF = {
		...
		{ r=1000, acc={0.2, 0.75}, dmg_mul=1.5, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.01, 0.25}, dmg_mul=0.75, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.05, 0.35}, dmg_mul=0.33, recoil={1.5,2}, mode={ 1, 0, 0, 0 } }
	}
	
	presets.weapon.good.mp5.aim_delay = { 0.0, 0.2 }
	presets.weapon.good.mp5.focus_delay = 5
	presets.weapon.good.mp5.focus_dis = 200
	presets.weapon.good.mp5.spread = 15
	presets.weapon.good.mp5.miss_dis = 10
	presets.weapon.good.mp5.RELOAD_SPEED = 1.2
	...
	presets.weapon.good.mp5.FALLOFF = {
		...
		{ r=1000, acc={0.2, 0.75}, dmg_mul=1.75, recoil={0.35,0.5}, mode={ 0, 3, 3, 0 } },
		{ r=2000, acc={0.1, 0.45}, dmg_mul=1.25, recoil={0.35,0.6}, mode={ 0, 3, 3, 0 } },
		{ r=3000, acc={0.1, 0.35}, dmg_mul=1, recoil={0.5,0.6}, mode={ 1, 3, 2, 0 } }
	}
	
	presets.weapon.good.mac11.aim_delay = { 0.1, 0.1 }
	presets.weapon.good.mac11.focus_delay = 5
	presets.weapon.good.mac11.focus_dis = 200
	presets.weapon.good.mac11.spread = 15
	presets.weapon.good.mac11.miss_dis = 10
	presets.weapon.good.mac11.RELOAD_SPEED = 1.2
	...
	presets.weapon.good.mac11.FALLOFF = { 
		{ r=500, acc={0.1, 0.7}, dmg_mul=2, recoil={0.5,0.65}, mode={ 0, 1, 3, 1 } },
		{ r=1000, acc={0.2, 0.55}, dmg_mul=1.25, recoil={0.55,0.85}, mode={ 2, 1, 3, 0 } },
		{ r=2000, acc={0.05, 0.4}, dmg_mul=1, recoil={0.65,1.00}, mode={ 2, 1, 3, 0 } },
		{ r=3000, acc={0, 0.2}, dmg_mul=1, recoil={.65,1.2}, mode={ 4, 1, 0, 0 } }
	}
...
---- EXPERT ----
	...
	presets.weapon.expert.beretta92.focus_delay = 5
	...
	presets.weapon.expert.beretta92.RELOAD_SPEED = 1.3
	...
	presets.weapon.expert.beretta92.FALLOFF = {
		{ r=0, acc={0.5, 0.95}, dmg_mul=7, recoil={0.1,0.25}, mode={ 1, 0, 0, 0 } },
		{ r=700, acc={0.375, 0.55}, dmg_mul=6, recoil={0.15,0.30}, mode={ 1, 0, 1, 0 } },
		{ r=3000, acc={0.05, 0.15}, dmg_mul=6, recoil={0.30,0.70}, mode={ 1, 0, 1, 0 } },
		{ r=10000, acc={0, 0.1}, dmg_mul=1, recoil={0.4,1}, mode={ 1, 0, 0, 0 } }
	}

	presets.weapon.expert.c45.aim_delay = { 0.1, 0.1 }
	presets.weapon.expert.c45.focus_delay = 5
	presets.weapon.expert.c45.focus_dis = 300
	presets.weapon.expert.c45.spread = 20
	presets.weapon.expert.c45.miss_dis = 50
	presets.weapon.expert.c45.RELOAD_SPEED = 1.3
	...
	presets.weapon.expert.c45.FALLOFF = {
		{ r=500, acc={0.6, 0.95}, dmg_mul=8, recoil={0.15,0.25}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.5, 0.7}, dmg_mul=8, recoil={0.15,0.3}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.3, 0.55}, dmg_mul=7.5, recoil={0.4,0.9}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.1, 0.35}, dmg_mul=7, recoil={0.4,1}, mode={ 1, 0, 0, 0 } }
	}
		
	presets.weapon.expert.m4.aim_delay = { 0.1, 0.1 }
	presets.weapon.expert.m4.focus_delay = 5
	presets.weapon.expert.m4.focus_dis = 200
	presets.weapon.expert.m4.spread = 20
	presets.weapon.expert.m4.miss_dis = 40
	presets.weapon.expert.m4.RELOAD_SPEED = 1
	presets.weapon.expert.m4.melee_speed = 0.9
	presets.weapon.expert.m4.melee_dmg = 6
	presets.weapon.expert.m4.range = { close = 1000, optimal = 2000, far = 5000 }
	presets.weapon.expert.m4.FALLOFF = {
		{ r=500, acc={0.55, 0.9}, dmg_mul=8, recoil={0.45,0.8}, mode={ 0, 3, 3, 1 } },
		{ r=1000, acc={0.525, 0.8}, dmg_mul=7.5, recoil={0.35,0.75}, mode={ 1, 2, 2, 0 } },
		{ r=2000, acc={0.5, 0.7}, dmg_mul=7.5, recoil={0.4,1.2}, mode={ 3, 2, 2, 0 } },
		{ r=3000, acc={0.4, 0.65}, dmg_mul=7, recoil={1.5,3}, mode={ 3, 1, 1, 0 } }
	}
	
	presets.weapon.expert.r870.aim_delay = { 0.1, 0.1 }
	presets.weapon.expert.r870.focus_delay = 10
	...
	presets.weapon.expert.r870.melee_speed = 0.9
	presets.weapon.expert.r870.melee_dmg = 8
	presets.weapon.expert.r870.range = { close = 1000, optimal = 2000, far = 5000 }
	presets.weapon.expert.r870.FALLOFF = { 
		{ r=500, acc={0.5, 0.95}, dmg_mul=7, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.4, 0.75}, dmg_mul=7, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.1, 0.55}, dmg_mul=6, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.1, 0.35}, dmg_mul=3, recoil={1.5,2}, mode={ 1, 0, 0, 0 } }
	}
	
	presets.weapon.expert.mp5.aim_delay = { 0.0, 0.2 } -- 0.4, 0.5
	presets.weapon.expert.mp5.focus_delay = 5
	presets.weapon.expert.mp5.focus_dis = 200
	presets.weapon.expert.mp5.spread = 15
	presets.weapon.expert.mp5.miss_dis = 10
	presets.weapon.expert.mp5.RELOAD_SPEED = 1.2 -- 1.5
	...
	presets.weapon.expert.mp5.FALLOFF = {
		{ r=500, acc={0.6, 0.95}, dmg_mul=6.5, recoil={0.1,0.3}, mode={ 0, 3, 3, 1 } },
		{ r=1000, acc={0.5, 0.75}, dmg_mul=6.5, recoil={0.35,0.5}, mode={ 0, 3, 3, 0 } },
		{ r=2000, acc={0.45, 0.65}, dmg_mul=6.5, recoil={0.35,0.6}, mode={ 0, 3, 3, 0 } },
		{ r=3000, acc={0.25, 0.55}, dmg_mul=6, recoil={0.5,1.2}, mode={ 1, 3, 2, 0 } }
	}
	
	presets.weapon.expert.mac11.aim_delay = { 0.1, 0.1 }
	presets.weapon.expert.mac11.focus_delay = 5
	...
	presets.weapon.expert.mac11.FALLOFF = {
		{ r=500, acc={0.5, 0.8}, dmg_mul=7, recoil={0.5,0.65}, mode={ 0, 1, 3, 1 } },
		{ r=1000, acc={0.35, 0.65}, dmg_mul=6.5, recoil={0.55,0.85}, mode={ 2, 1, 3, 0 } },
		{ r=2000, acc={0.35, 0.6}, dmg_mul=6.5, recoil={0.65,1}, mode={ 2, 1, 3, 0 } },
		{ r=3000, acc={0.2, 0.55}, dmg_mul=6, recoil={0.65,1.2}, mode={ 4, 1, 0, 0 } }
	}
	...
	presets.weapon.expert.beretta92.range = presets.weapon.normal.beretta92.range
	presets.weapon.expert.beretta92.FALLOFF = {
		{ r=0, acc={0.5, 0.95}, dmg_mul=4, recoil={0.1,0.25}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.375, 0.55}, dmg_mul=2, recoil={0.15,0.30}, mode={ 1, 0, 1, 0 } },
		{ r=2000, acc={0.05, 0.5}, dmg_mul=2, recoil={0.30,0.70}, mode={ 1, 0, 1, 0 } },
		{ r=3000, acc={0, 0.3}, dmg_mul=2, recoil={0.4,1}, mode={ 1, 0, 0, 0 } }
	}
	...
	presets.weapon.expert.c45.range = presets.weapon.normal.c45.range
	presets.weapon.expert.c45.FALLOFF = {
		{ r=500, acc={0.5, 0.9}, dmg_mul=4, recoil={0.15,0.3}, mode={ 1, 0, 1, 0 } },
		{ r=1000, acc={0.4, 0.65}, dmg_mul=3.5, recoil={0.15,0.3}, mode={ 1, 0, 1, 0 } },
		{ r=2000, acc={0.3, 0.5}, dmg_mul=3, recoil={0.4,0.9}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.1, 0.25}, dmg_mul=2.5, recoil={0.4,1.4}, mode={ 1, 0, 0, 0 } }
	}
	...
	presets.weapon.expert.m4.range = { close = 1000, optimal = 2000, far = 5000 }
	presets.weapon.expert.m4.FALLOFF = {
		{ r=500, acc={0.55, 0.95}, dmg_mul=6, recoil={0.45,0.8}, mode={ 0, 3, 3, 1 } },
		{ r=1000, acc={0.525, 0.8}, dmg_mul=6, recoil={0.35,0.75}, mode={ 1, 2, 2, 0 } },
		{ r=2000, acc={0.5, 0.7}, dmg_mul=6, recoil={0.4,1.2}, mode={ 3, 2, 2, 0 } },
		{ r=3000, acc={0.2, 0.4}, dmg_mul=5, recoil={1.5,3}, mode={ 3, 1, 1, 0 } }
	}
	...
	presets.weapon.expert.r870.range = { close = 1000, optimal = 2000, far = 5000 }
	presets.weapon.expert.r870.FALLOFF = { 
		{ r=500, acc={0.5, 0.95}, dmg_mul=5, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=1000, acc={0.4, 0.75}, dmg_mul=5, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=2000, acc={0.1, 0.55}, dmg_mul=5, recoil={1.5,2}, mode={ 1, 0, 0, 0 } },
		{ r=3000, acc={0.1, 0.35}, dmg_mul=4, recoil={1.5,2}, mode={ 1, 0, 0, 0 } }
	}
	...
	presets.weapon.expert.mp5.range = presets.weapon.normal.mp5.range
	presets.weapon.expert.mp5.FALLOFF = {
		{ r=500, acc={0.6, 0.95}, dmg_mul=4, recoil={0.1,0.3}, mode={ 0, 3, 3, 1 } },
		{ r=1000, acc={0.4, 0.65}, dmg_mul=3.5, recoil={0.35,0.5}, mode={ 0, 3, 3, 0 } },
		{ r=2000, acc={0.4, 0.6}, dmg_mul=3, recoil={0.35,0.7}, mode={ 0, 3, 3, 0 } },
		{ r=3000, acc={0.2, 0.35}, dmg_mul=2.5, recoil={0.5,1.5}, mode={ 1, 3, 2, 0 } }
	}
	...
	presets.weapon.expert.mac11.FALLOFF = {
		...
		{ r=1000, acc={0.3, 0.65}, dmg_mul=3, recoil={0.55,0.85}, mode={ 2, 1, 3, 0 } },
...
---- SNIPER ----
	...
	presets.weapon.sniper.m4.FALLOFF = {
		{ r=700, acc={0.4, 0.95}, dmg_mul=5, recoil={3,6}, mode={ 1, 0, 0, 0 } },
		{ r=3500, acc={0.1, 0.75}, dmg_mul=3, recoil={3,7}, mode={ 1, 0, 0, 0 } },
		{ r=10000, acc={0, 0.25}, dmg_mul= 1.5, recoil={4,8}, mode={ 1, 0, 0, 0 } }
	}
...
function CharacterTweakData:_create_table_structure()
	self.weap_ids = { "beretta92", "c45", "raging_bull", "m4", "ak47", "r870", "mossberg", "mp5", "mp9", "mac11", "m14_sniper_npc", "saiga" } -- these are the names of the tweak_tables ( in WeaponTweakData.lua ) of the respective weapons
...
-- TEAM MEMBERS
	self.presets.gang_member_damage.REGENERATE_TIME = 1.5									-- Amount of time to pass before regenerate
	self.presets.gang_member_damage.REGENERATE_TIME_AWAY = 0.2						-- Amount of time to pass before regenerate when this criminal is far away from players
...
-- TEAM MEMBERS
	...
	self.presets.gang_member_damage.REGENERATE_TIME_AWAY = 0.4						-- Amount of time to pass before regenerate when this criminal is far away from players
...
-- TEAM MEMBERS
	...
	self.presets.gang_member_damage.REGENERATE_TIME_AWAY = 0.6						-- Amount of time to pass before regenerate when this criminal is far away from players
	...
	self.sniper.weapon.m4.FALLOFF = {
		{ r=700, acc={0.4, 0.95}, dmg_mul=7, recoil={3,6}, mode={ 1, 0, 0, 0 } },
		{ r=3500, acc={0.1, 0.85}, dmg_mul=5, recoil={4,6}, mode={ 1, 0, 0, 0 } },
		{ r=10000, acc={0, 0.3}, dmg_mul=2, recoil={4,8}, mode={ 1, 0, 0, 0 } }
	}
...
-- TEAM MEMBERS
	...
	self.sniper.weapon.m4.FALLOFF = {
		{ r=700, acc={0.4, 0.95}, dmg_mul=10, recoil={3,6}, mode={ 1, 0, 0, 0 } },
		{ r=3500, acc={0.1, 0.85}, dmg_mul=8, recoil={4,6}, mode={ 1, 0, 0, 0 } },
		{ r=10000, acc={0, 0.3}, dmg_mul=3, recoil={4,8}, mode={ 1, 0, 0, 0 } }
	}
...
function CharacterTweakData:_set_characters_weapon_preset( preset )
	local all_units = {"security", "cop", "fbi", "swat", "heavy_swat", "gangster"}
	for _, name in ipairs( all_units ) do
