...
function NetworkGame:init()
	...
	self._dropin_pause_info = {} -- Counts how many times we have been asked to pause and for whom
	self._old_players = {}
end
...
function NetworkGame:load( game_data )
	if managers.network:session():is_client() then
		...
		--print( "[NetworkGame:load] pausing!" )
		--self._dropin_pause_info[ managers.network:session():local_peer():id() ] = true
...
function NetworkGame:on_entered_lobby()
	...
	-- Reuse member if allready exists
...
function NetworkGame:on_load_complete()
	...
	if managers.hud then
		for _,peer in pairs( managers.network:session():peers() ) do
			...
			--[[print( "join complete", managers.menu:active_menu() )
			
			if managers.menu:active_menu() and managers.menu:active_menu().renderer.remove_player_slot_by_peer_id then
				managers.menu:active_menu().renderer:set_slot_joining( peer, peer_id )
			end]]
		end
	end
...
function NetworkGame:ps3_disconnect( connected )
	...
	-- Don't care to change game state when playing single player (other things regarding PSN might need to be handled though) /Martin
	if Global.game_settings.single_player then
		return
	end
	...
	if(not connected and not PSN:is_online())then
		...
		-- This is only a faked test of PSN signout in game /Martin
		if game_state_machine:current_state().on_disconnected then
			game_state_machine:current_state():on_disconnected()
		end
...
function NetworkGame:check_peer_preferred_character( preferred_character )
	...
	-- Find free
	--[[for _,name in ipairs( managers.criminals:character_names() ) do
		if not used_characters[ name ] then
			print( "Player will be", name, "instead of", preferred_character )
			return name
		end
	end]]
end

-- This should only be called to the server
function NetworkGame:on_peer_request_character( peer_id, character )
	...
	-- managers.menu:get_menu( "lobby_menu" ).renderer:set_character( peer_id, character )
...
function NetworkGame:on_peer_sync_complete( peer, peer_id )
	...
	-- managers.hud:script( IngameWaitingForPlayersState.GUI_SAFERECT ):member_ready( peer:name() )
	if peer:ip_verified() then
		Global.local_member:sync_lobby_data( peer )
		Global.local_member:sync_data( peer )
	end
	-- print( "peer_id", peer_id )
	-- managers.hint:show_hint( "drop_in_join" )
...
-- Check if all members are ready, and start the game (if in waiting for players screen)
...
function NetworkGame:on_statistics_recieved( peer_id, peer_kills, peer_specials_kills, peer_head_shots, accuracy, downs )
	...
	for _,member in pairs( self._members ) do
		-- Ready means that he has spawned and are able to enter mission end screen and can send his stats.
		-- We don't need to wait for his stats otherwise.
	...
	if game_state_machine:current_state().on_statistics_result then
		game_state_machine:current_state():on_statistics_result( 	best_killer.peer_id, best_killer.score,
																	best_special_killer.peer_id, best_special_killer.score,
																	best_accuracy.peer_id, best_accuracy.score, 
																	most_downs.peer_id, most_downs.score, 
																	total_kills, total_specials_kills, total_head_shots, group_accuracy, group_downs )
	end
	managers.network:session():send_to_peers( "sync_statistics_result", best_killer.peer_id, best_killer.score,
																		best_special_killer.peer_id, best_special_killer.score,
																		best_accuracy.peer_id, best_accuracy.score, 
																		most_downs.peer_id, most_downs.score, 
																		total_kills, total_specials_kills, total_head_shots, group_accuracy, group_downs )
...
function NetworkGame:on_peer_removed( peer, peer_id, reason )
	if self._members[ peer_id ] then
		...
		-- Might check in_lobby on peer when the states works
		-- if managers.menu:active_menu().renderer.remove_player_slot_by_peer_id then
		-- if peer:in_lobby() then
		-- local menu = managers.menu:get_menu( "lobby_menu" ) or managers.menu:get_menu( "kit_menu" )
		-- menu.renderer:remove_player_slot_by_peer_id( peer_id, reason )
		...
		-- Check the ready state of all remaining members
		if Network:is_server() then
			self:_check_start_game_intro()
		end
		...
		if Network:multiplayer() then
			...
			elseif Network:is_server() then
				...
				if player_left then
					...
					self._old_players[ peer_ident ] = { t = Application:time(), member_downed = member_downed, health = member_health, used_deployable = member_used_deployable, used_cable_ties = member_used_cable_ties, member_dead = member_dead }
				...
				-- This will search for trip mines placed by the leaving peer and destroy them.
				local deployed_equipment = World:find_units_quick( "all", 14, 25, 26 ) -- Use items (grenades, trip mines etc), sentrygun, broken sentrygun
				for _,equipment in ipairs( deployed_equipment ) do
					if equipment:base() and equipment:base().server_information then
						...
						if server_information then
							if server_information.owner_peer_id == peer_id then
...
-- Returns id a peer allready has been added as client
...
function NetworkGame:on_peer_loading( peer, state )
	...
	if Network:is_server() and not state then -- Peer loaded completed
	...
	-- print( "  state", state, peer == managers.network:session():server_peer() ) 
	if state and (peer == managers.network:session():server_peer()) then
		...
		if managers.network:session():local_peer():in_lobby() then
			...
			-- managers.menu:active_menu().renderer:set_server_state( "loading" )
		end
	end
...
function NetworkGame:spawn_players( is_drop_in )
	...
	if Network:is_server() then
		if not Global.local_member then
			-- This means that we have saved the network settings because we are going to load something. We can't spawn players then.
			return
		end
...
function NetworkGame:spawn_dropin_player( peer_id )
	...
	-- local character = member:peer():character()
	-- member:spawn_unit( 0, true, character ~= "random" and character or nil )
...
------------------------------------------------------------------
-- The host wants us to pause or unpause so that a new peer can drop in
