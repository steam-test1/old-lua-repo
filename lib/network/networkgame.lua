...
function NetworkGame:load(game_data)
	...
	if game_data then
		local (for generator), (for state), (for control) = pairs(game_data.members)
		do
			do break end
...
function NetworkGame:save()
	...
	do
		local (for generator), (for state), (for control) = pairs(self._members)
		do
			do break end
...
function NetworkGame:on_peer_request_character(peer_id, character)
	...
	(for control) = kit_menu.renderer.set_character and member.peer
	...
	if self._members[peer_id] then
		...
		if Network:multiplayer() then
			...
			elseif Network:is_server() then
				...
				local (for generator), (for state), (for control) = ipairs(deployed_equipment)
				do
					do break end
...
function NetworkGame:_create_spawn_point_beanbag()
	...
	do
		local (for generator), (for state), (for control) = pairs(spawn_points)
		do
			do break end
...
function NetworkGame:chk_create_dropin_dialog()
	local (for generator), (for state), (for control) = pairs(self._dropin_pause_info)
	do
		do break end
		managers.menu:show_person_joining(peer_id, nickname)
		return true
