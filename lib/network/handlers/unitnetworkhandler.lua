...
function UnitNetworkHandler:set_unit( unit, character_name, outfit_string, peer_id )
	...
	if peer_id == 0 then -- Special case. its an AI
		...
		if not crim_data or not crim_data.ai then
			...
			--unit:inventory():on_character_set( character_name )
		else
...
function UnitNetworkHandler:action_walk_start( unit, first_nav_point, nav_link_yaw, nav_link_act_index, from_idle, haste_code, end_yaw, no_walk, no_strafe )
	if not ( self._verify_character( unit ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
	...
	if nav_link_act_index ~= 0 then
		...
		nav_link.element.value = function ( element, name ) return element[ name ] end
		nav_link.element.nav_link_wants_align_pos = function ( element ) return element.from_idle end
...
function UnitNetworkHandler:action_walk_nav_point( unit, nav_point, sender )
	if not ( self._verify_character_and_sender( unit, sender ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
...
function UnitNetworkHandler:friendly_fire_hit( subject_unit )
	if not ( self._verify_character( subject_unit ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
...
function UnitNetworkHandler:from_server_damage_incapacitated( subject_unit, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( subject_unit ) ) then
		return
	end
...
function UnitNetworkHandler:shot_blank( shooting_unit, impact, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character_and_sender( shooting_unit, sender ) ) then
		return
	end
...
function UnitNetworkHandler:reload_weapon( unit, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character_and_sender( unit, sender ) ) then
		return
	end
...
function UnitNetworkHandler:sync_teammate_progress( type_index, enabled, tweak_data_id, timer, success, sender )
	...
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and sender_peer ) then
		return
	end
...
function UnitNetworkHandler:action_aim_start( cop )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( cop ) ) then
		return
	end
	local shoot_action = {
		type = "shoot",
...
function UnitNetworkHandler:action_hurt_end( unit )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( unit ) ) then
		return
	end
...
function UnitNetworkHandler:set_attention( unit, target_unit, reaction, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( unit ) and alive( target_unit ) ) then
		return
	end
	...
	if not ( handler or target_unit:movement() and target_unit:movement().m_head_pos ) then
		debug_pause_unit( target_unit, "[UnitNetworkHandler:set_attention] no attention handler or m_head_pos", target_unit )
		return
	end
	--print( "[UnitNetworkHandler:set_attention]", unit, target_unit, reaction, handler )
	unit:movement():synch_attention( { unit = target_unit, u_key = target_unit:key(), handler = handler, reaction = reaction } )
end

--------------------------------------------------------------------------------------

function UnitNetworkHandler:cop_set_attention_unit( unit, target_unit )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( unit ) and self._verify_character( target_unit ) ) then
		return
	end
...
function UnitNetworkHandler:long_dis_interaction( target_unit, amount, aggressor_unit )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( target_unit ) and self._verify_character( aggressor_unit ) ) then
		return
	end
	...
	local target_is_civilian = not target_is_criminal and target_unit:in_slot( 21 ) -- non-tied civilians
	...
	if target_is_criminal then
		if aggressor_is_criminal then
			...
			elseif amount == 1 then -- 1 means "morale_boost"
				target_unit:movement():on_morale_boost( aggressor_unit ) -- local player only
			end
		else
			target_unit:brain():on_intimidated( amount / 10, aggressor_unit )
		end
	else
		if amount == 0 and target_is_civilian and aggressor_is_criminal then
...
function UnitNetworkHandler:alert( alerted_unit, aggressor )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( alerted_unit ) and self._verify_character( aggressor ) ) then
		return
	end
	...
	alerted_unit:brain():on_alert( { "aggression", aggressor_pos, false, nil, aggressor } )
end
...
function UnitNetworkHandler:revive_player( revive_health_level, sender )
	local peer = self._verify_sender( sender )
	if not ( self._verify_gamestate( self._gamestate_filter.need_revive ) and peer ) then
		return
	end
	
	if revive_health_level > 0 then
		-- managers.player:activate_temporary_upgrade( "temporary", "revive_health_boost" )
...
function UnitNetworkHandler:start_revive_player( timer, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.downed ) and self._verify_sender( sender ) ) then
		return
	end
...
function UnitNetworkHandler:start_free_player( sender )
	if not ( self._verify_gamestate( self._gamestate_filter.arrested ) and self._verify_sender( sender ) ) then
		return
	end
	local player = managers.player:player_unit()
...
function UnitNetworkHandler:revive_unit( unit, reviving_unit )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character( unit ) and alive( reviving_unit ) ) then
		return
	end
...
--------------------------------------------------------------------------------------
-- I am a client and requested from the server to place an ecm jammer for me
function UnitNetworkHandler:from_server_ecm_jammer_placed( unit, rpc )
	...
	unit:base():set_owner( managers.player:player_unit() ) -- Only I and the server know that I am the owner
end
...
function UnitNetworkHandler:sync_unit_event_id_8( unit, ext_name, event_id, rpc )
	local peer = self._verify_sender( rpc )
	if not ( peer and alive( unit ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
...
function UnitNetworkHandler:m79grenade_explode_on_client( position, normal, user, damage, range, curve_pow, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character_and_sender( user, sender ) ) then
		return
	end
...
function UnitNetworkHandler:from_server_sentry_gun_place_result( owner_peer_id, equipment_selection_index, sentry_gun_unit, rot_speed_mul, spread_mul, shield, rpc )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_sender( rpc ) and alive( sentry_gun_unit ) and managers.network:session():peer( owner_peer_id ) ) then
		return
	end
...
function UnitNetworkHandler:sync_money_wrap_money_taken( unit, sender ) --, amount )
	if not ( alive( unit ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_sender( sender ) ) then
		return
	end
	unit:base():sync_money_taken() --  amount )
end
...
function UnitNetworkHandler:unit_sound_play( unit, event_id, source, sender )
	if not ( alive( unit ) and self._verify_sender( sender ) ) then
		return
	end
...
function UnitNetworkHandler:say( unit, event_id, sender )
	...
	if unit:in_slot( managers.slot:get_mask( "all_criminals" ) ) and not managers.groupai:state():is_enemy_converted_to_criminal( unit ) then
		unit:sound():say( event_id, true, nil ) -- sound_name, important_say, sync
	else
		unit:sound():say( event_id, nil, true ) -- sound_name, sync, skip_prefix
	end
end
...
function UnitNetworkHandler:criminal_hurt( criminal_unit, attacker_unit, damage_ratio, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character_and_sender( criminal_unit, sender ) ) then
		return
	end
	if not alive( attacker_unit ) or criminal_unit:key() == attacker_unit:key() then	-- Special case when the attacker cannot be sent
		attacker_unit = nil
	end
...
function UnitNetworkHandler:assign_secret_assignment( assignment ) --, unit )
	if not self._verify_gamestate( self._gamestate_filter.any_ingame ) then
		return
	end
	managers.secret_assignment:assign( assignment ) -- , unit )
end
...
--------------------------------------------------------------------------------------
-- message from the host that an AI unit has been disarmed/arrested
...
--------------------------------------------------------------------------------------
-- local player has been discovered after suspicion
...
function UnitNetworkHandler:sync_carry_data( unit, carry_id, carry_value, dye_initiated, has_dye_pack, dye_value_multiplier, position, dir, throw_distance_multiplier_upgrade_level, sender  )
	...
	-- unit:interaction():set_carry_id( carry_id )
	-- unit:interaction():set_carry_value( carry_value )
end
...
function UnitNetworkHandler:sync_secure_loot( carry_id, carry_value, silent, sender )
	if not ( (self._verify_gamestate( self._gamestate_filter.any_ingame ) or self._verify_gamestate( self._gamestate_filter.any_end_game ) )and self._verify_sender( sender ) ) then
		return
	end
...
function UnitNetworkHandler:server_place_mission_door_device( unit, player, sender  )
	...
	-- unit:interaction():sync_placed_mission_door_device()
...
function UnitNetworkHandler:set_armor( unit, percent, sender )
	...
	if character_data and character_data.panel_id then
		managers.hud:set_teammate_armor( character_data.panel_id, { current = percent/100, total = 1, max = 1 } )
	else	
...
function UnitNetworkHandler:set_health( unit, percent, sender )
	...
	local character_data = managers.criminals:character_data_by_peer_id( peer_id )
	if character_data and character_data.panel_id then
		managers.hud:set_teammate_health( character_data.panel_id, { current = percent/100, total = 1, max = 1 } )
	else
...
function UnitNetworkHandler:sync_teammate_helped_hint( hint, helped_unit, helping_unit, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_character_and_sender( helped_unit, sender ) and self._verify_character( helping_unit, sender ) ) then
		return
	end
...
function UnitNetworkHandler:mission_ended( win, num_is_inside, sender )
	...
	if managers.platform:presence() == "Playing" then
		if win then
			game_state_machine:change_state_by_name( "victoryscreen", { num_winners = num_is_inside,
...
function UnitNetworkHandler:sync_player_kill_statistic( tweak_table_name, is_headshot, weapon_unit, variant, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and self._verify_sender( sender ) and alive( weapon_unit ) ) then
		return
	end
	local data = { name = tweak_table_name, head_shot = is_headshot, weapon_unit = weapon_unit, variant = variant }
...
function UnitNetworkHandler:link_attention_no_rot( parent_unit, attention_object, parent_object, local_pos, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and alive( parent_unit ) and alive( attention_object ) ) then
		return
	end
...
function UnitNetworkHandler:unlink_attention( attention_object, sender )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and alive( attention_object ) ) then
		return
	end
...
function UnitNetworkHandler:suspicion_hud( observer_unit, status )
	if not ( self._verify_gamestate( self._gamestate_filter.any_ingame ) and alive( observer_unit ) ) then
		return
	end
...
function UnitNetworkHandler:camera_yaw_pitch( cam_unit, yaw_255, pitch_255 )
	if not ( alive( cam_unit ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
...
function UnitNetworkHandler:loot_link( loot_unit, parent_unit, sender )
	if not ( alive( loot_unit ) and alive( parent_unit ) and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
