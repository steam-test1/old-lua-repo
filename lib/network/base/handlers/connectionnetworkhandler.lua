...
------------------------------------------------------------------
-- Someone ( the sender ) is looking for running servers. inform the network manager
...
------------------------------------------------------------------
-- Someone ( the sender ) has replied to our host discovery request. inform the network manager
function ConnectionNetworkHandler:discover_host_reply( sender_name, level_id, level_name, my_ip, state, difficulty, sender )
	...
	if level_name == "" then	-- We can either receive a level id or a name. both will do for production.
...
function ConnectionNetworkHandler:peer_exchange_info( peer_id, sender )
	--print( "[ConnectionNetworkHandler:peer_exchange_info]", peer_id )	
	local sender_peer = self._verify_sender( sender )
	if not sender_peer then
		return
	end
	...
	if self._verify_in_client_session() then
		if sender_peer:id() == 1 then -- from host
			managers.network:session():on_peer_requested_info( peer_id ) -- we are client and the hsot wants us to exchange info with this dude
		elseif peer_id == sender_peer:id() then
			managers.network:session():send_to_host( "peer_exchange_info", peer_id ) -- we are client and that dude is telling us that he received all our info. tell the server that we are good 2 go
		end
	elseif self._verify_in_server_session() then
		managers.network:session():on_peer_connection_established( sender_peer, peer_id ) -- we are host and the sender dude has just received all info from this dude
	end
end

------------------------------------------------------------------
-- sender now knows the IP address of peer_id
...
------------------------------------------------------------------
-- the host informs us that we have mutual connection with other_peer_id
...
------------------------------------------------------------------
-- a peer is informing us that he has just removed a peer
...
function ConnectionNetworkHandler:ok_to_load_level( sender )
	...
	if not self:_verify_in_client_session() then --  and self._verify_gamestate( self._gamestate_filter.menu ) ) then
		return
	end
...
function ConnectionNetworkHandler:sync_game_settings( job_index, level_id_index, difficulty_index, sender )
	-- print( "ConnectionNetworkHandler:sync_game_settings", job_index, level_id_index, difficulty_index )
	local peer = self._verify_sender( sender )
	if not peer then
		return
	end
	...
	-- print( "job_id", job_id, "level_id", level_id, "difficulty", difficulty )
	if managers.menu_component then
...
function ConnectionNetworkHandler:lobby_sync_update_level_id( level_id_index )
	-- if not self._verify_gamestate( self._gamestate_filter.menu ) then
	--	return
	-- end
	local level_id = tweak_data.levels:get_level_name_from_index( level_id_index )
	-- local menu = managers.menu:get_menu( "lobby_menu" ) or managers.menu:get_menu( "kit_menu" )
	-- menu.renderer:sync_update_level_id( level_id )
...
function ConnectionNetworkHandler:lobby_sync_update_difficulty( difficulty )
	...
	-- end
	-- local menu = managers.menu:get_menu( "lobby_menu" ) or managers.menu:get_menu( "kit_menu" )
	-- menu.renderer:sync_update_difficulty( difficulty )
...
function ConnectionNetworkHandler:lobby_info( peer_id, level, character, mask_set, ass_progress, sha_progress, sup_progress, tech_progress, sender )
	...
	if peer then
		...
		-- local menu = managers.menu:get_menu( "lobby_menu" ) or managers.menu:get_menu( "kit_menu" )
		-- menu.renderer:_set_player_slot( slot, { name = peer:name(), peer_id = peer:id(), level = level, character = character, progress = progress  } )
...
function ConnectionNetworkHandler:sync_chat_message( message, sender )
	-- if not ( self._verify_gamestate( self._gamestate_filter.player_slot ) and self._verify_sender( sender ) ) then
	local peer = self._verify_sender( sender )
	if not peer then
		return
	end
...
function ConnectionNetworkHandler:request_character( peer_id, character, sender )
	-- if not ( self._verify_gamestate( self._gamestate_filter.menu ) and self._verify_sender( sender ) ) then
	if not ( self._verify_sender( sender ) ) then
		return
	end
...
function ConnectionNetworkHandler:set_mask_set( peer_id, mask_set, sender )
	...
	peer:set_mask_set( mask_set )
	-- managers.menu:active_menu().renderer:set_character( peer_id, peer:character() )
	local lobby_menu = managers.menu:get_menu( "lobby_menu" )
	if lobby_menu and lobby_menu.renderer:is_open() then
...
function ConnectionNetworkHandler:request_character_response( peer_id, character, sender )
	...
	-- local menu = managers.menu:get_menu( "lobby_menu" ) or managers.menu:get_menu( "kit_menu" )
	-- menu.renderer:set_character( peer_id, character )
	...
	-- end
	-- managers.network:session():peers()
end
...
function ConnectionNetworkHandler:request_drop_in_pause( peer_id, nickname, state, sender )	-- peer who is dropping in, pause state
...
------------------------------------------------------------------
-- Somebody is asking us if we still have him as a client
...
------------------------------------------------------------------
-- Somebody is replying to a status request we sent. It means he is not our server.
function ConnectionNetworkHandler:sanity_check_network_status_reply( sender )
	if not self._verify_in_client_session() then	-- I am nobodys client any more
		return
	end
	
	local sender_peer = self._verify_sender( sender )	-- I do not have this peer
	if not sender_peer then
		return
	end
	
	local session = managers.network:session()
	if sender_peer ~= session:server_peer() then	-- The reply did not come from my server
		return
	end
...
------------------------------------------------------------------
-- Host informs us on the dropin progress of another client
function ConnectionNetworkHandler:dropin_progress( dropin_peer_id, progress_percentage, sender )
	if not ( self._verify_in_client_session() and self._verify_gamestate( self._gamestate_filter.any_ingame ) ) then
		return
	end
...
function ConnectionNetworkHandler:set_member_ready( ready, sender )
	...
	if Network:is_server() then
		if game_state_machine:current_state().start_game_intro then
			-- game_state_machine:current_state():set_peer_ready( peer_id, ready )
		elseif ready then
			--[[ Test to spawn all at the same time /MW
			for other_peer_id, other_peer in pairs( managers.network:session()._peers ) do
				managers.network:session():chk_spawn_member_unit( other_peer, other_peer_id )
			end]]
...
function ConnectionNetworkHandler:re_open_lobby_request( state, sender )
	local peer = self._verify_sender( sender )
	if not peer then
		sender:re_open_lobby_reply( false ) -- fuck off
		return
	end
	local session = managers.network:session()
...
function ConnectionNetworkHandler:set_selected_lootcard( selected, sender )
	...
	if managers.hud then	-- else client is in lobby menu
