...
function NetworkPeer:init( name, rpc, id, loading, synced, in_lobby, character, user_id )
	...
	self._dlcs = { dlc1 = false, dlc2 = false, dlc3 = false, dlc4 = false }
	
	self:chk_enable_queue()
	
	-- self._character = "random"
	self._character = character -- or "dallas" -- CriminalsManager:character_workname_by_peer_id( id )
	...
	-- self.chk_timeout = self.pre_handshake_chk_timeout
...
function NetworkPeer:load( data )
	...
	self._rpc = data.rpc -- Network:handshake( data.ip )	--data.rpc
	self._steam_rpc = data.steam_rpc
	self._id = data.id
	if self._rpc then
		self._ip = self._rpc:ip_at_index(0) -- data.ip
	end
...
function NetworkPeer:save( data )
	...
	data.id = self._id
	-- data.ip = self._rpc:ip_at_index(0) -- self._ip
...
function NetworkPeer:name()		return self._name end

--------------------------------------------------------------------

function NetworkPeer:ip() 		return self._ip end

--------------------------------------------------------------------

--function NetworkPeer:ping()		return self._ping_time end

--------------------------------------------------------------------

function NetworkPeer:id()		return self._id end

--------------------------------------------------------------------

function NetworkPeer:rpc()		return self._rpc end

--------------------------------------------------------------------

function NetworkPeer:steam_rpc()		return self._steam_rpc end

--------------------------------------------------------------------

function NetworkPeer:connection_info()
	return self._name, self._id, self._user_id or "", self._in_lobby, self._loading, self._synced, self._character, "remove", self._xuid, self._xnaddr -- self._mask_set
end

--------------------------------------------------------------------

function NetworkPeer:synched() return self._synced end

--------------------------------------------------------------------

function NetworkPeer:loading() return self._loading end

--------------------------------------------------------------------

function NetworkPeer:loaded() return self._loaded end

--------------------------------------------------------------------

function NetworkPeer:in_lobby() return self._in_lobby end

--------------------------------------------------------------------

function NetworkPeer:character() return self._character end

--------------------------------------------------------------------

-- function NetworkPeer:set_mask_set( mask_set ) self._mask_set = mask_set end

--------------------------------------------------------------------

-- function NetworkPeer:mask_set() return self._mask_set end

--------------------------------------------------------------------

function NetworkPeer:used_deployable() return self._used_deployable end

--------------------------------------------------------------------

function NetworkPeer:set_used_deployable( used ) self._used_deployable = used end
...
function NetworkPeer:waiting_for_player_ready() return self._waiting_for_player_ready end
...
function NetworkPeer:set_loading( state )
	...
	if self._loading and not state then -- We where loading, and not anymore
		self._loaded = true
	end 
	...
	if state then
		...
		if self._rpc and managers.network.voice_chat.on_member_added then
			managers.network.voice_chat:on_member_added( self )
		end
	end
end
...
-- Only for sever peer. Needed for clients to know what state their server is really in without triggering events
...
-- Only for sever peer. Needed for cleints to know what state their server is really in without triggering events
...
function NetworkPeer:send( func_name, ... )
	...
	-- print( "[NetworkPeer:send]", self._id, func_name, ... )
	local rpc = self._rpc
	rpc[ func_name ]( rpc, ... )
	--cat_print( "multiplayer_base", "[NetworkPeer:send]", func_name, ... )
end

--------------------------------------------------------------------

function NetworkPeer:_send_queued( queue_name, func_name, ... )
	--print( "[NetworkPeer:_send_queued]", queue_name, self._id, func_name, ... )
	if self._msg_queues and self._msg_queues[ queue_name ] then
		--print( "_push_to_queue" )
...
function NetworkPeer:_chk_flush_msg_queues()
	if not ( self._msg_queues and self._ip_verified ) then
		return
	end
...
function NetworkPeer:_flush_queue( queue_name )
	--print( "[NetworkPeer:_flush_queue]", queue_name, self._id )
	...
	for i, msg in ipairs( msg_queue ) do
		...
		if ok then
			-- print( "    queue sending", unpack( msg ) )
			self:send( unpack( msg ) )
		--else
			--print( "queue discarding", unpack( msg ) )
		end
	end
end
...
function NetworkPeer:set_id( my_id )
	self._id = my_id
	-- self._character = CriminalsManager:character_workname_by_peer_id( self._id )
end
...
--------------------------------------------------------------------
-- Call from session before the network sends this peer's RPCS
...
--------------------------------------------------------------------
-- This peer is expected to confirm that he has paused for dropin_peer_id to drop in
...
--------------------------------------------------------------------
-- this peer must verify to us that he has paused for dropin_peer_id to drop in
-- returns nil, "asked" or "paused"
...
--------------------------------------------------------------------
-- I am waiting for drop-in pause sequence initiation
...
--------------------------------------------------------------------
-- I am waiting for drop-in sequence
...
--------------------------------------------------------------------
-- status: nil = no handshake done,
-- "asked"=we have send the handshake and awaiting confirmation
-- "exchanging_info"=we have send the handshake and awaiting confirmation
-- true=we have received confirmation
-- "removing"=we asked this peer to remove introduced_peer_id
