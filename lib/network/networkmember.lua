...
function NetworkMember:peer()	return self._peer end

--------------------------------------------------------------------

function NetworkMember:unit()	return self._unit end
...
function NetworkMember:spawn_unit( spawn_point_id, is_drop_in, spawn_as ) --Server only.
	if self._unit then
		return
	end
	...
	local old_unit
	trade_entry, old_unit = managers.groupai:state():remove_one_teamAI( character_name, member_downed or member_dead ) -- removes AI if name collides
	...
	if alive( old_unit ) then
		ai_is_downed = old_unit:character_damage():bleed_out() or
			old_unit:character_damage():fatal() or
			old_unit:character_damage():arrested() or
			old_unit:character_damage():need_revive() or
			old_unit:character_damage():dead()
		World:delete_unit( old_unit )
	end
	
	-- jag var downad and ( ersätter inte AI or ersätter downad team AI )
	...
	--[[local unit_name = tweak_data.blackmarket.characters[ "npc_criminal_casual_1_var03" ].fps_unit
	if self._peer:id() ~= 1 then
		unit_name = tweak_data.blackmarket.characters[ "npc_criminal_suit_1_var02" ].fps_unit
	end]]
	
	--[[local unit_name = Idstring( "units/multiplayer/mp_fps_mover/mp_fps_mover_"..unit_name_suffix )
	if self._peer:id() ~= 1 then
		unit_name = Idstring( "units/multiplayer/mp_fps_mover/mp_fps_mover_casual" )
	end]]
	-- local unit_name = Idstring( "units/multiplayer/mp_fps_mover/mp_fps_mover_casual" )
...
function NetworkMember:set_unit( unit, character_name )
	...
	if is_new_unit then
		if unit:damage() then
			...
			unit:damage():run_sequence_simple( sequence ) -- tweak_data.blackmarket.characters[ self._peer:character_id() ].sequence )
...
function NetworkMember:sync_lobby_data( peer )
	...
	-- local slot = managers.menu:active_menu().renderer:get_player_slot_nr_by_peer_id( peer_id )
	...
	if Network:is_server() then
		...
		-- managers.network:session():send_to_peers( "lobby_sync_update_difficulty", difficulty )
	end
end
...
function NetworkMember:sync_data( peer )
	...
	if self._unit then
--		peer:send( "set_unit", self._unit, managers.criminals:character_name_by_id( self._peer:id() ), self._peer:id() )
	end
end
