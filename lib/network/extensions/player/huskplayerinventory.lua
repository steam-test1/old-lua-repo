...
function HuskPlayerInventory:init( unit )
	...
	self._align_places.right_hand = { obj3d_name = Idstring("a_weapon_right_front"), on_body = true }
	self._align_places.left_hand = { obj3d_name = Idstring("a_weapon_left_front"), on_body = true }
...
function HuskPlayerInventory:_send_equipped_weapon() -- husk does not send its equipped weapon
end
...
function HuskPlayerInventory:synch_equipped_weapon( weap_index, blueprint_string )
	...
	if type( weapon_name ) == "string" then -- New weapon
...
function HuskPlayerInventory:add_peer_blackmarket_outfit( peer )
	...
	if blackmarket_outfit then
		...
		if primary then
			...
			table.insert( self._peer_weapons, { factory_name_npc, false, true, primary.blueprint } )
			
			-- self:add_unit_by_factory_blueprint( factory_name_npc, false, true, primary.blueprint )
		end
		...
		if secondary then
			...
			table.insert( self._peer_weapons, { factory_name_npc, false, true, secondary.blueprint } )
			-- self:add_unit_by_factory_blueprint( factory_name_npc, true, true, secondary.blueprint )
		end
	end
end
