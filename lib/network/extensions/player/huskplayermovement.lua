...
HuskPlayerMovement._walk_anim_velocities = {
	stand = {
		ntl = {
			walk = { fwd = 183.48, bwd = 156.4, l = 150.36, r = 152.15 },
			run = { fwd = 381.35, bwd = 402.62, l = 405.06, r = 405.06 }
		},
		cbt = {
...
HuskPlayerMovement._walk_anim_lengths = {
	stand = {
		ntl = {
			walk = { fwd = 31, bwd = 31, l = 29, r = 31 },
			run = { fwd = 26, bwd = 17, l = 20, r = 20 },
		},
		cbt = {
...
HuskPlayerMovement._look_modifier_name = Idstring( "action_upper_body" ) -- gets overriden att first
...
function HuskPlayerMovement:init( unit )
	...
	self._is_weapon_gadget_on = nil -- t.ex. flashlight
	
	local stance = {}
	self._stance = stance
	
	stance.names = self._stance_names
	stance.values = { 1, 0, 0 } -- ntl, hos, cbt
	stance.blend = { 0.8, 0.5, 0.3 }	-- blend in for ntl, hos, cbt
	stance.code = 1
	stance.name = "ntl"
	...
	self._tase_effect_table = { effect = Idstring( "effects/payday2/particles/character/taser_hittarget" ), parent = self._unit:get_object( Idstring( "e_taser" ) ) }
	
	self._sequenced_events = {}
	
	self._synced_suspicion = false -- this is useful only when server. it saves the value of suspicion we have synced to the client
...
function HuskPlayerMovement:post_init()
	...
	self._unit:inventory():synch_equipped_weapon( 2 ) -- beretta
...
function HuskPlayerMovement:set_character_anim_variables()
	...
	--local char_index = self._char_name_to_index[ char_name ]
	local char_index = 1 -- We lack animations for other variants
	self._machine:set_global( "husk"..tostring( char_index ), 1 )
...
function HuskPlayerMovement:set_visual_carry( carry_id )
	...
	elseif alive( self._current_visual_carry_object ) then
		-- print( "hide visual carry object" )
...
function HuskPlayerMovement:update( unit, t, dt )
	...
	if not self._movement_updator and self._move_data and ( self._state == "standard" or self._state == "mask_off" or self._state == "clean" or self._state == "carry" )  then -- safety check
...
function HuskPlayerMovement:enable_update() -- empty for now. called from CopBase
end
...
--	RETURNS MUTABLE!!!!!
function HuskPlayerMovement:m_pos()	-- unit root pos
	return self._m_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function HuskPlayerMovement:m_stand_pos()	-- head position if unit was standing
	return self._m_stand_pos
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function HuskPlayerMovement:m_com()	-- current centre of mass ( mid height ) of the unit
	return self._m_com
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function HuskPlayerMovement:m_head_rot()	-- look rotation of unit
	return self._m_head_rot
end

-------------------------------------------------------------------------------
--	RETURNS MUTABLE!!!!!
function HuskPlayerMovement:m_head_pos() -- current position of unit's head/look point
...
function HuskPlayerMovement:set_need_revive( need_revive, down_time )
	...
	if Network:is_server() then -- Notify AI teammates that we want to be revived
		if need_revive and not ( self._revive_SO_id or self._revive_rescuer ) then
...
function HuskPlayerMovement:_register_revive_SO()
	--print( "[HuskPlayerMovement:_register_revive_SO]", self._revive_rescuer, self._revive_SO_id )
	--Application:stack_dump()
...
function HuskPlayerMovement:_unregister_revive_SO()
	--print( "[HuskPlayerMovement:_unregister_revive_SO]", self._revive_rescuer, self._revive_SO_id )
...
function HuskPlayerMovement:set_need_assistance( need_assistance )
	...
	--print( "[HuskPlayerMovement:set_need_assistance]", need_assistance, self._need_assistance, self._assist_SO_id )
...
function HuskPlayerMovement:on_revive_SO_administered( receiver_unit )
	--print( "[HuskPlayerMovement:on_revive_SO_administered]", receiver_unit, self._revive_rescuer, self._revive_SO_id )
...
-----------------------------------------------------------------------------
-- the teammate that was coming to our rescue failed his objective
function HuskPlayerMovement:on_revive_SO_failed( rescuer )
	--print( "[HuskPlayerMovement:on_revive_SO_failed]", rescuer, self._revive_rescuer, self._revive_SO_id )
	--Application:stack_dump()
	if self._revive_rescuer then
		self._revive_rescuer = nil	-- this teammate failed to rescue us. re-register the SO immediately
		self:_register_revive_SO()
	end
end
...
function HuskPlayerMovement:_upd_attention_mask_off( dt )
	...
	if self._sync_look_dir then
		...
		--print( "fwd angle",fwd:to_polar_with_reference( self._look_dir, math.UP ).spin )
	...
	--Application:draw_cylinder( self._unit:position() + math.UP * 150, self._unit:position() + math.UP * 150 + self._look_dir * 500, 1, 0, 1, 0 )
	--Application:draw_cylinder( self._unit:position() + math.UP * 50, self._unit:position() + math.UP * 50 + fwd * 500, 1, 0, 0.5, 0 )
end
...
function HuskPlayerMovement:_upd_attention_standard( dt )
	...
	if self._sync_look_dir then
		...
		if hips_err_spin > max_spin or hips_err_spin < min_spin then
			--print( "hips_err_spin", hips_err_spin )
			wait_for_turn = true
			--local head_pos = self._unit:position() + math.UP * 150
			--Application:draw_cylinder( head_pos, head_pos + tar_look_dir * 300, 2, 1,1,1)
			if hips_err_spin > max_spin then
				mvector3.rotate_with( tar_look_dir, Rotation( max_spin - hips_err_spin ) )
				--print( "correction", max_spin - hips_err_spin )
			else
				mvector3.rotate_with( tar_look_dir, Rotation( min_spin - hips_err_spin ) )
				--print( "correction", min_spin - hips_err_spin )
			end
			--Application:draw_cylinder( head_pos, head_pos + tar_look_dir * 300, 2, 1,0,0)
		end
...
function HuskPlayerMovement:_upd_sequenced_events( t, dt )
	...
	if next_event.commencing then -- we are still busy with this event
		--print( "current_event is commencing", t )
		return
	end
	...
	if event_type == "move" then
		--print( "starting to move", inspect( next_event ) )
...
function HuskPlayerMovement:_upd_stance( t )
	if self._aim_up_expire_t then
		if t > self._aim_up_expire_t then
			self._aim_up_expire_t = nil
...
function HuskPlayerMovement:_upd_slow_pos_reservation( t, dt )
	...
	if mvec3_norm( tmp_vec2 ) > slow_dist then
...
function HuskPlayerMovement:_upd_move_standard( t, dt )
	...
	if self._move_data then
		...
		if data.path_len > max_dis or not self:_chk_groun_ray() then
			max_velocity = self:_get_max_move_speed( true ) * 1.1
			wanted_str8_vel = max_velocity
		elseif data.path_len > slowdown_dis or not self:_chk_groun_ray() then
		...
		if data.velocity_len > wanted_str8_vel then
		...
		if mvector3.z( new_pos ) < mvector3.z( self._m_pos ) then
			last_z_vel = last_z_vel - 971 * dt -- a bit less gravity than 1G to avoid catching up with the real unit in case of lag
		...
		--[[if self._move_data then
			managers.navigation:draw_path( data.path, { 0.2, 0.6, 0.8, 0.1 }, { 0.5, 0.1, 0.8, 0.7 }, 0 )
		end]]
		...
		if math.abs( waist_twist_new ) > waist_twist_max then
		...
		if self._move_data then
			...
			if stance_name == "ntl" then
				...
				else -- walking or standing motionless
					if vel_len > 300 then
						anim_velocity = "run"
					else
						anim_velocity = "walk"
			...
			if not self._walk_anim_velocities[pose] or
				not self._walk_anim_velocities[pose][stance] or
				not self._walk_anim_velocities[pose][stance][anim_velocity] or
				not self._walk_anim_velocities[pose][stance][anim_velocity][anim_side] then
	...
	elseif self._ext_anim.idle_full_blend and not self._turning then
		if waist_twist > 40 or waist_twist < -65 then
			...
			if redir_res then
				self._turning = true
				-- Animations are in a mixed state
...
function HuskPlayerMovement:_adjust_move_anim( side, speed )
	...
	if move_side and ( side == move_side or self._matching_walk_anims[ side ][ move_side ] ) then
		...
		local pose = self._ext_anim.pose
		local stance = self._stance.name
		if not self._walk_anim_lengths[pose] or
			not self._walk_anim_lengths[pose][stance] or
			not self._walk_anim_lengths[pose][stance][speed] or
			not self._walk_anim_lengths[pose][stance][speed][side] then
...
function HuskPlayerMovement:sync_action_walk_nav_point( pos )
	--print( "[HuskPlayerMovement:sync_action_walk_nav_point]", pos, TimerManager:game():time() )
	if Network:is_server() then
		if not self._pos_reservation then
			self._pos_reservation = { position = mvector3.copy( pos ), radius = 100, filter = self._pos_rsrv_id }
			self._pos_reservation_slow = { position = mvector3.copy( pos ), radius = 100, filter = self._pos_rsrv_id }
	...
	if nr_seq_events == 1 and self._move_data then -- we are now moving and nothing else to do afterwards. append
		--print( "appended to current action" )
		local path = self._move_data.path
 		local vec = tmp_vec1
		mvector3.set( vec, pos )
		mvector3.subtract( vec, path[ #path ] )
		if mvector3.z( vec ) < 0 then -- going downwards. do not count z in distance
			mvector3.set_z( vec, 0 )
		end
		self._move_data.path_len = self._move_data.path_len + mvector3.length( vec )
		table.insert( path, pos )
	elseif nr_seq_events > 0 and self._sequenced_events[ nr_seq_events ].type == "move" then -- append to last move action
		--print( "appended to last action" )
		table.insert( self._sequenced_events[ #self._sequenced_events ].path, pos )
	else -- sequence new move action
...
function HuskPlayerMovement:_start_movement( path )
	...
	data.path = path	-- all the nav points. the first one is behind us, the second one is in front of us
	data.velocity_len = 0
	...
	while i < nr_nodes do
		...
		if mvector3.z( tmp_vec1 ) < 0 then -- going downwards. do not count z in distance
...
function HuskPlayerMovement:_start_standard( event_desc )
	-- print( "HuskPlayerMovement:_start_standard( event_desc )" )
...
function HuskPlayerMovement:_adjust_walk_anim_speed( dt, target_speed )
	...
	local min = 0.05
	local new_speed
	if target_speed > cur_speed and cur_speed < max then	-- We want to speed up
		new_speed = target_speed--math.clamp( cur_speed * ( 1 + 5 * dt ), min, max )
	elseif target_speed < cur_speed and cur_speed > min then	-- We want to slow down
		new_speed = target_speed--math.clamp( cur_speed * ( 1 - 5 * dt ), min, max )
	end
	if new_speed then
...
function HuskPlayerMovement:sync_shot_blank( impact )
	...
	self:_change_stance( 3, delay and { impact = impact } ) --cbt
end

-----------------------------------------------------------------------------------

-- Used when gadget is turned on and off
function HuskPlayerMovement:set_cbt_permanent( on )
	-- print( "HuskPlayerMovement:set_cbt_permanent( on )" )
...
function HuskPlayerMovement:_shoot_blank( impact )
	...
	if equipped_weapon and equipped_weapon:base().fire_blank then
		...
		if self._aim_up_expire_t ~= -1 then -- Cbt is set to permanent
			self._aim_up_expire_t = TimerManager:game():time() + 2
		end
	end
...
function HuskPlayerMovement:_change_pose( pose_code )
	...
	local enter_t
	local move_side = self._ext_anim.move_side	-- the direction of our current playing animation
	if move_side then	
		--Some animation combinations match well enough to allow us to continue with the new animation from where the old one currently is
		local seg_rel_t = self._machine:segment_relative_time( Idstring( "base" ) )
		...
		if not self._walk_anim_lengths[pose] or
			not self._walk_anim_lengths[pose][stance] or
			not self._walk_anim_lengths[pose][stance][speed] or
			not self._walk_anim_lengths[pose][stance][speed][move_side] then
...
-----------------------------------------------------------------------------------
-- called from CopLogicArrest
function HuskPlayerMovement:on_cuffed()
	self._unit:network():send_to_unit( { "sync_player_movement_state", self._unit, "arrested", 0, self._unit:id() } )
end

-----------------------------------------------------------------------------------

function HuskPlayerMovement:on_uncovered( enemy_unit )
	self._unit:network():send_to_unit( { "suspect_uncovered", enemy_unit } )
end
...
-----------------------------------------------------------------------------------
-- currently listens only to equip events
...
function HuskPlayerMovement:on_civ_revive_failed( sympathy_civ )
	if self._sympathy_civ then
		self._sympathy_civ = nil	-- this teammate failed to rescue us. re-register the SO immediately
	end
end
...
function HuskPlayerMovement:on_civ_revive_completed( sympathy_civ )
	...
	if self._unit:base():upgrade_value( "player", "civilian_gives_ammo" ) then
		managers.game_play_central:spawn_pickup( { name = "ammo", position = sympathy_civ:position(), rotation = Rotation() } )
	end
end
...
function HuskPlayerMovement:_chk_change_stance()
	...
	if self._is_weapon_gadget_on then
		wanted_stance_code = 3 -- cbt
	elseif self._aim_up_expire_t then
		wanted_stance_code = 3 -- cbt
	else
		wanted_stance_code = self._stance.owner_stance_code -- whatever the player ( not husk ) wanted for stance
	end
...
function HuskPlayerMovement:_get_max_move_speed( run )
	...
	if self._pose_code == 2 then -- crouch
	...
	-- move_speed = move_speed * morale_boost_bonus
	return move_speed
end
...
function HuskPlayerMovement._walk_spline( move_data, pos, walk_dis )
	...
	while true do
		...
		if move_data.seg_len == 0 or prog_in_seg >= move_data.seg_len then	--	we reached the next nav point
			if #path == 2 then
				move_data.prog_in_seg = move_data.seg_len
				return mvector3.copy( path[2] ), true -- new_pos, complete
			else
				table.remove( path, 1 )
		...
		else -- we didn't reach the next nav_point
			...
			--[[mvec3_set( return_vec, seg_dir )
			mvec3_mul( return_vec, move_data.prog_in_seg )
			mvec3_add( return_vec, path[1] )]]
			
			return return_vec, nil -- new_pos, not complete
		end
	end
end
