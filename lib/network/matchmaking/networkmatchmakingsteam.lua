...
NetworkMatchMakingSTEAM._BUILD_SEARCH_INTEREST_KEY = "payday2_beta_v1.9.2"
...
function NetworkMatchMakingSTEAM:get_friends_lobbies()
	...
	local function f(updated_lobby)
		...
		if num_updated_lobbies >= #lobbies then
			...
			do
				local (for generator), (for state), (for control) = ipairs(lobbies)
				do
					do break end
...
function NetworkMatchMakingSTEAM:search_lobby(friends_only)
	...
	if friends_only then
		...
		do
			local (for generator), (for state), (for control) = pairs(self._lobby_filters)
			do
				do break end
