...
function NetworkAccountSTEAM:init()
	...
	-- Steam:overlay_listener( NetworkAccountSTEAM._on_open_overlay, NetworkAccountSTEAM._on_close_overlay )
...
--[[	
	if managers.network:session() and managers.network:session():_local_peer_in_lobby() then
		managers.menu:steam_disconnected()
	elseif managers.network:game() then
		managers.network:game():steam_disconnected()
	end
]]
end
...
--	managers.network.matchmake:join_server( ip, pw )
end
...
function NetworkAccountSTEAM.output_global_stats( file )
	...
	local get_lvl_stat = function( diff, heist, stat, i )
		if i == 0 then
	...
	local get_weapon_stat = function( weapon, stat, i )
		if i == 0 then
			local st = weapon .. " - "
			if type( stat ) == "string" then
				return st .. stat
...
--[[
	out = out .. "," .. get_lvl_stat( diff, heist, "plays", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "wins", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "cash", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "time", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "shots", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "kills", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "health_lost", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "downs", i )
	out = out .. "," .. get_lvl_stat( diff, heist, "custody", i )
]]
	local lines = {}
	for i = 0, #invalid do
