...
-- HACK WARNING:
-- Work-in-progress. State machine setup, etc. is highly transitional at the moment.
-- Expect this file to change radically! /Mats
...
function Setup:init_category_print()
	...
	if not Global.interface_sound then
		--Global.interface_sound = SoundDevice:load_bank( "interface" )
	end
	...
	local cat_list = {
		...
		-- Units:
				"player_intel",			--Player unit Intel extension
				"player_damage",		--Player unit Damage extension
				"player_action",		--Player unit actions. Mostly used in movement states
				
				"AI_action",			--AI unit actions. Mostly used in movement states
				"bt",					-- Behavior tree related
				"dummy_ai",				-- dummy brain prints 
				"dummy_block_chance",	-- Details of block/parry/fail chance when attacked
				
		-- People:
				"johan",			-- Johan Svensson
				"george",			-- Georgios Giannakos
				"qa",				-- All the people working with quality assurance
				"bob",				-- Benny
		-- Sound
				"sound_placeholder", -- Placeholder prints for sounds
	...
	catprint_load()	-- Load custom user settings if any.
end
...
function Setup:_start_loading_screen()
	...
	if Global.load_level then
		...
		-- Load Package containing loading image
		...
		load_level_data.gui_data = {  	safe_rect_pixels 	= safe_rect_pixels,
										safe_rect 			= safe_rect,
										aspect_ratio		= aspect_ratio,
										res 				= res,
										workspace_size		= { x = 0, y = 0, w = res.x, h = res.y },
										saferect_size		= { x = safe_rect.x, y = safe_rect.y, w = safe_rect.width, h = safe_rect.height },
...
function Setup:_setup_loading_environment()
	...
	-- dummy_vp:get_post_processor_effect("World", Idstring("hdr_post_processor")):modifier(Idstring("post_composite")):material()
	...
	LoadingEnvironment:viewport():set_post_processor_effect( "World", Idstring("bloom_combine_post_processor"), Idstring( "bloom_combine_empty" ) )
		...
		-- material:set_variable(Idstring("noise_animation"), Vector3() ) -- Vector3(math.sin(0*600),math.cos(0*600), 1))
		-- material:set_variable(Idstring("noise_settings"), Vector3() ) -- Vector3(0.035+(0.035*(1-1)),255,0)) -- 0.03 10 0
		
		-- Color correction
		-- material:set_variable(Idstring("tgl_r"), Vector3( 1, 1, 1 ) ) -- Vector3(0, 1, 1*(1+(1-1))))
		-- material:set_variable(Idstring("tgl_g"), Vector3( 1, 1, 1 ) ) -- Vector3(0, 1, 1.1*1))
		-- material:set_variable(Idstring("tgl_b"), Vector3( 1, 1, 1 ) ) -- Vector3(0, 1, 0.9*1))
		...
		-- HDR_settings
		-- material:set_variable(Idstring("HDR_size"), 0.25)
		-- material:set_variable(Idstring("HDR_adaption"), 0.9875)
		-- material:set_variable(Idstring("HDR_upscale"), 1.5)
		
	--[[
	for effect_name,effect_data in pairs( env_map ) do
		local effect = dummy_vp:get_post_processor_effect( "World", Idstring( effect_name ) )
]]
		--[[ 
			Asplund@engine [2009-07-15]: remove the nil checks later on when lrb is up and running.
			Needed now since I change the layer setup + post processors heavily to strip down the
			render pipe.
		]]
	--[[	if (effect ~= nil) then		
			for modifier_name,modifier_data in pairs( effect_data ) do
				local modifier = effect:modifier( Idstring( modifier_name ) )
				if (modifier ~= nil) then
					local material = modifier:material()

					for variable_name,variable_value in pairs( modifier_data ) do
						material:set_variable( Idstring( variable_name ), variable_value )
					end
				end
			end
		end
...
-- PD2 -- FIXME, use add_end_frame_clbk
...
function Setup:destroy()
	...
	-- Destroy references owned by the setup-instance:
...
function Setup:load_level( level, mission, world_setting, level_class_name, level_id ) --, video_goto_time )
	...
	-- Global.level_data.video_goto_time = video_goto_time
	self:exec( level )
end
...
function Setup:load_start_menu()
	...
	-- Global.level_data.video_goto_time = nil
	self:exec( nil )
end
...
function Setup:block_exec()
	...
	--[[ BLOCK EXEC TEST
	print( "block_exec", self._blocker_t )
	if not self._blocker_t then
		if not self._block_complete then
			self._blocker_t = Application:time() + 10
			print( "SET BLOCK TIME", self._blocker_t )
			return true
		end
	else
		if self._blocker_t > Application:time() then
			return true
		else
			print( "self._blocker_t", self._blocker_t )
			self._block_complete = true
		end
	end]]
