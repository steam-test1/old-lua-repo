...
function GameSetup:load_packages()
	...
	-- Load a specific level package, loads a debug package for test levels
	local level_package
...
function GameSetup:init_managers( managers )
	...
	if SystemInfo:platform() == Idstring( "X360" ) then
		-- managers.blackmarket:load_all_crafted_weapons()
...
function GameSetup:init_game()
	...
	if( not Application:editor() ) then
		...
		engine_package:unload_all_temp(); -- shaders etc
		...
		if( level ) then
			...
			local mission_params = {
						...
						stage_name = "stage1"		-- Deprecated, backward compatibility only
						}
...
function GameSetup:init_finalize()
	...
	if not Application:editor() then
		managers.navigation:on_game_started()	-- removes dummy units. registers covers
	end
	if( not Application:editor() ) then
...
function GameSetup:update( t, dt )
	...
	managers.navigation:update( t, dt ) 		-- 0.2
	managers.hud:update( t, dt )				-- 1.1
	managers.killzone:update( t, dt )			-- 0.05
	managers.secret_assignment:update( t, dt ) 	-- 0.1
	managers.game_play_central:update( t, dt )	-- 1.1
...
function GameSetup:destroy()
	...
	if PackageManager:loaded( "packages/base" ) then
		PackageManager:unload( "packages/base" )
	end]]
...
function GameSetup:end_update( t, dt )
	...
	-- local id = Profiler:start( "game_play_central:end_update" )
...
-- toggle application pause state
function GameSetup:_update_debug_input()
	-- if not Application:production_build() or Application:editor() and not Global.running_simulation then
	...
	if not ( editor_ok and debug_on_ok ) then
		return
	end
	
	if self._keyboard then
		if self._keyboard:pressed( 59 ) then -- F1
		...
		elseif self._keyboard:pressed( 60 ) then -- F2
