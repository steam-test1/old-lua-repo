...
function CoreAiArea:find_spawnpoints(xml)
	local (for generator), (for state), (for control) = xml:children()
	do
		do break end
...
function CoreSpawnSystem:get_linked_unit_list()
	...
	if self._linked_unit_map then
		local (for generator), (for state), (for control) = pairs(self._linked_unit_map)
		do
			do break end
...
function CoreSpawnSystem:update(unit, t, dt)
	if self._delayed_var_and_cb_init then
		do
			local (for generator), (for state), (for control) = pairs(self._delayed_var_and_cb_init)
			do
				do break end
	...
	do
		...
		do
			...
			if lv3:name() == "variables" and lv3:parameter("extension") ~= "" then
				...
				local (for generator), (for state), (for control) = lv3:children()
				do
					do break end
...
function CoreSpawnSystem:read_spawn_xml()
	...
	if xml_data then
		local (for generator), (for state), (for control) = xml_data:children()
		do
			do break end
