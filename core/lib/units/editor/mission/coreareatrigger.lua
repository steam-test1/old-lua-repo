...
function CoreAreaTriggerUnitElement:draw_links(t, dt, selected_unit, all_units)
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._hed.spawn_unit_elements)
		do
			do break end
	...
	if self._hed.unit_ids then
		local (for generator), (for state), (for control) = ipairs(self._hed.unit_ids)
		do
			do break end
...
function CoreAreaTriggerUnitElement:_check_removed_units(all_units)
	if self._hed.use_shape_element_ids then
		local (for generator), (for state), (for control) = ipairs(clone(self._hed.use_shape_element_ids))
		do
			do break end
			local unit = all_units[id]
...
function CoreAreaOperatorUnitElement:init(unit)
	...
	local (for generator), (for state), (for control) = ipairs(self._apply_on_checkboxes)
	do
		do break end
