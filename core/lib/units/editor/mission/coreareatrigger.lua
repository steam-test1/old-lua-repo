...
--	self._shapes_types = { "box", "sphere", "plane" }
...
-- 	self._hed.interval = managers.area:default_interval()
...
function CoreAreaTriggerUnitElement:add_element()
	...
	if ray and ray.unit then
		if string.find( ray.unit:name():s(), "point_spawn_unit", 1 ,true ) then -- ray.unit:name() == "me_spawn_unit" then
...
function CoreAreaTriggerUnitElement:create_values_ctrlrs( panel, panel_sizer, disable )
	...
	if not disable or not disable.trigger_type then
		...
		local trigger_types_params = {
			...
			panel							= panel,
			sizer							= panel_sizer,
			options						= { "on_enter", "on_exit", "both", "on_empty", "while_inside" },
	...
	if not disable or not disable.instigator then
		...
		local f = function( ctrlr ) if ctrlr:get_value() == "criminals" then EWS:message_box( Global.frame_panel, "Criminals is deprecated, you should probably use local_criminals. Ask Martin or Ilija why.", "Instigator Warning", "ICON_WARNING", Vector3( -1, -1, 0 ) ) end end
	...
	use_disabled_shapes:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = use_disabled_shapes, value = "use_disabled_shapes" } )
	panel_sizer:add( use_disabled_shapes, 0, 0, "EXPAND" )
	self._use_disabled_shapes = use_disabled_shapes
end

-- Builds the gui panel for the unit
function CoreAreaTriggerUnitElement:_build_panel( panel, panel_sizer, disable_params )
	...
	local trigger_types_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= { "on_enter", "on_exit", "both", "on_empty" },
		value 				= self._hed.trigger_on,
		tooltip 			= "Select a trigger on type from the combobox",
		name_proportions 	= 1,
		ctrlr_proportions 	= 2,
...
function CoreAreaOperatorUnitElement:add_element()
	...
	if ray and ray.unit then
		if string.find( ray.unit:name():s(), "trigger_area", 1, true ) then
			local id = ray.unit:unit_data().unit_id
