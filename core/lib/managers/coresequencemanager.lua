...
function SequenceManager:init( area_damage_mask, target_world_mask, beings_mask )
	...
	self:register_event_element_class( AnimationGroupElement )				-- "animation_group"
	self:register_event_element_class( AnimationRedirectElement )			-- "animation_redirect"
	self:register_event_element_class( AreaDamageElement )					-- "area_damage"
	self:register_event_element_class( AlertElement )						-- "alert"
	self:register_event_element_class( AttentionElement )						-- "attention"
	self:register_event_element_class( BodyElement )						-- "body"
	self:register_event_element_class( ConstraintElement )					-- "constraint"
	self:register_event_element_class( DebugElement )						-- "debug"
	self:register_event_element_class( DecalMeshElement )					-- "decal_mesh"
	self:register_event_element_class( EffectElement )						-- "effect"
	self:register_event_element_class( EffectSpawnerElement )				-- "effect_spawner"
	self:register_event_element_class( EnemyKilledElement )					-- "enemy_killed"
	self:register_event_element_class( FunctionElement )					-- "function"
	self:register_event_element_class( GraphicGroupElement )				-- "graphic_group"
	self:register_event_element_class( LightElement )						-- "light"
	self:register_event_element_class( ObjectElement )						-- "object"
	self:register_event_element_class( MaterialConfigElement )				-- "material_config"
	self:register_event_element_class( MaterialElement )					-- "material"
	self:register_event_element_class( MorphExpressionElement )				-- "morph_expression"
	self:register_event_element_class( MorphExpressionMovieElement )		-- "morph_expression_movie"
	self:register_event_element_class( PhantomElement )						-- "phantom"
	self:register_event_element_class( PhysicEffectElement )				-- "physic_effect"
	self:register_event_element_class( ProjectDecalElement )				-- "project_decal"
	self:register_event_element_class( RemoveStartTimeElement )				-- "remove_start_time"
	self:register_event_element_class( RunSequenceElement )					-- "run_sequence"
	self:register_event_element_class( RunSpawnSystemSequenceElement )		-- "run_spawn_system_sequence"
	self:register_event_element_class( SetDamageElement )					-- "set_damage"
	self:register_event_element_class( SetExtensionVarElement )				-- "set_extension_var"
	self:register_event_element_class( SetGlobalVariableElement )			-- "set_global_variable"
	self:register_event_element_class( SetGlobalVariablesElement )			-- "set_global_variables"
	self:register_event_element_class( SetInflictElement )					-- "set_inflict"
	self:register_event_element_class( SetPhysicEffectElement )				-- "set_physic_effect"
	self:register_event_element_class( SetProximityElement )				-- "set_proximity"
	self:register_event_element_class( SetSaveDataElement )					-- "set_save_data"
	self:register_event_element_class( SpawnSystemUnitEnabledElement )		-- "set_spawn_system_unit_enabled"
	self:register_event_element_class( SetVariableElement )					-- "set_variable"
	self:register_event_element_class( SetVariablesElement )				-- "set_variables"
	self:register_event_element_class( SetWaterElement )					-- "set_water"
	self:register_event_element_class( ShakeCameraElement )					-- "shake_camera"
	self:register_event_element_class( SlotElement )						-- "slot"
	if SoundDevice:device_info() == "Wwise device" then
		self:register_event_element_class( WwiseElement )						-- "sound"
	else
		self:register_event_element_class( SoundElement )						-- "sound"
	end
		
	self:register_event_element_class( SpawnUnitElement )					-- "spawn_unit"
	self:register_event_element_class( StopPhysicEffectElement )			-- "stop_physic_effect"
	self:register_event_element_class( TriggerElement )						-- "trigger"

	self._filter_element_class_map = {}
	self:register_filter_element_class( CheckFilterElement )				-- "check"
	self:register_filter_element_class( SideFilterElement )					-- "side"
	self:register_filter_element_class( ZoneFilterElement )					-- "zone"

	self._inflict_element_class_map = {}
	self:register_inflict_element_class( InflictElectricityElement )		-- "electricity"
	self:register_inflict_element_class( InflictFireElement )				-- "fire"
	...
	-- Units that can receive inflict updator damage. It has the structure "_inflict_updator_body_map.damage_type.unit_key.body_key.body_ext" (optimization):
...
function SequenceManager:get_global_filter( filter_name )
	return ( self._global_unit_element and self._global_unit_element._filters[ filter_name ] ) or ( self._global_core_unit_element and self._global_core_unit_element._filters[ filter_name ] )
end
...
function SequenceManager:parse_event( node, unit_element )
	-- local element_class = self._event_element_class_map[ node:name() ]
	...
	-- elseif( ( node:name() == "xdefine" ) or ( #node:name() == 0 ) ) then	-- Left overs from xdefines.
	elseif( ( node._meta == "xdefine" ) ) then	-- Left overs from xdefines.
		return nil
	else
		Application:throw_exception( "Unit \"" .. tostring( unit_element._name ) .. "\" has an invalid element \"" .. tostring( node._meta ) .. "\"." )
		-- Application:throw_exception( "Unit \"" .. tostring( unit_element._name ) .. "\" has an invalid element \"" .. tostring( node:name() ) .. "\"." )
	end
end
...
function SequenceManager:update( time, delta_time )
	...
	for id,time_callback in self._time_callback_map:bottom_top_iterator() do
		if( time_callback.time <= time ) then
...
function SequenceManager:_add_sequences_from_unit_data( unit_data )
	...
	if seq_manager_filename then
		...
		-- If the same file allready been parsed by an other unit, we skip it and set the unit element data for the unit.
		...
		if DB:has( self.SEQUENCE_FILE_EXTENSION, seq_manager_filename ) then
			...
			--[[if PackageManager:has( (self.SEQUENCE_FILE_EXTENSION):id(), seq_manager_filename ) then
				print( "had sequence file", seq_manager_filename )
				manager_node = PackageManager:script_data( (self.SEQUENCE_FILE_EXTENSION):id(), (seq_manager_filename) )
			else
				print( "DID NOT HAVE SEQUENCE FILE", seq_manager_filename )
				manager_node = PackageManager:editor_load_script_data( (self.SEQUENCE_FILE_EXTENSION):id(), (seq_manager_filename) )
			end]]
			-- manager_node = PackageManager:editor_load_script_data( (self.SEQUENCE_FILE_EXTENSION):id(), (seq_manager_filename):id() )
		else
	...
	--[[local unit_name = unit_data:name()
	local root_node = unit_data:model_script_data()
	
	if root_node == nil then
		Application:throw_exception( "Unable to retrieve model-xml-file for unit \"" .. unit_name:t() .. "\"." )
		return
	end
	...
	for manager_node in root_node:children() do
		if manager_node:name() == "sequence_manager" then
			local file = manager_node:parameter( "file" )

			if file then
				if DB:has( self.SEQUENCE_FILE_EXTENSION, file ) then
					self._sequence_file_map[ unit_name:key() ] = file
					if PackageManager:has( (self.SEQUENCE_FILE_EXTENSION):id(), (file):id() ) then
						print( "had sequence file", file )
						manager_node = PackageManager:script_data( (self.SEQUENCE_FILE_EXTENSION):id(), (file):id() )
					else
						print( "DID NOT HAVE SEQUENCE FILE", file )
						manager_node = PackageManager:editor_load_script_data( (self.SEQUENCE_FILE_EXTENSION):id(), (file):id() )
					end
				else
					Application:error( "Unit \"" .. unit_name:t() .. "\" refers to the external sequence manager file \"" .. file:t() .. "." .. self.SEQUENCE_FILE_EXTENSION .. "\", but it doesn't exist." )
				end
			end
...
function SequenceManager:test_unit_variations( unit )
	if( alive( unit ) and unit:damage() ) then
		...
		if( #variation_list > 0 ) then
			...
			local param_list = { "change_state", unit, unit, nil, math.UP, unit:position(), math.DOWN, 0, Vector3(), nil }
...
function SequenceManager:internal_load()
	...
	--[[
	if( DB:has( self.SEQUENCE_FILE_EXTENSION, self.GLOBAL_CORE_SEQUENCE_PATH ) ) then
		local core_xml_node = DB:load_node( self.SEQUENCE_FILE_EXTENSION, self.GLOBAL_CORE_SEQUENCE_PATH )
		if( core_xml_node and ( core_xml_node:name() == "sequence_manager" ) ) then
			SequenceEnvironment:init_static_env()
			self._global_core_unit_element = UnitElement:new( core_xml_node, "[Global core sequence]", true )
			self._global_core_unit_element.get_sequence_element = managers.sequence.get_global_sequence		-- Global project sequences overrides global core sequences.
		end
	end
	
	if( DB:has( self.SEQUENCE_FILE_EXTENSION, self.GLOBAL_SEQUENCE_PATH ) ) then
		local xml_node = DB:load_node( self.SEQUENCE_FILE_EXTENSION, self.GLOBAL_SEQUENCE_PATH )
		if( xml_node and ( xml_node:name() == "sequence_manager" ) ) then
			SequenceEnvironment:init_static_env()
			self._global_unit_element = UnitElement:new( xml_node, "[Global sequence]", true )
		end
	end]]
...
-- Recieves the following parameters: id, killed_being_map, damaged_being_map, killed_prop_map, damaged_prop_map
...
function SequenceManager:do_area_damage_on_unit( unit, body, unit_extension, damage_type, attack_unit, normal, pos, dir, body_damage, velocity, ignore_unit, direct_attack_unit )
	...
	local return_param_list = { unit_extension:damage_by_area( damage_type, attack_unit, body, normal, pos, dir, body_damage, velocity or Vector3() ) }
...
SequenceEnvironment.print = callback( nil, _G, "cat_print", "sequence" )	-- For internal debugging.
...
function SequenceEnvironment.within_box( pos, box_pos1, box_pos2 )
	return ( ( ( box_pos1.x <= box_pos2.x ) and ( pos.x >= box_pos1.x ) and ( pos.x <= box_pos1.x ) ) or
				   ( ( pos.x >= box_pos2.x ) and ( pos.x <= box_pos2.x ) ) ) and
				 ( ( ( box_pos1.y <= box_pos2.y ) and ( pos.x >= box_pos1.y ) and ( pos.x <= box_pos1.y ) ) or
				   ( ( pos.y >= box_pos2.y ) and ( pos.y <= box_pos2.y ) ) ) and
				 ( ( ( box_pos1.z <= box_pos2.z ) and ( pos.z >= box_pos1.z ) and ( pos.z <= box_pos1.z ) ) or
				   ( ( pos.z >= box_pos2.z ) and ( pos.z <= box_pos2.z ) ) )
end
...
BaseElement.BASE_ATTRIBUTE_MAP = BaseElement.BASE_ATTRIBUTE_MAP or { start_time = true, repeat_nr = true, filter = true, delayed_filter = true }
...
function BaseElement:init( node, unit_element )
	...
	local __filter_name
	local __delayed_filter_name

	if( node ) then
		self._element_name = node._meta -- self._element_name = node:name()
		self._node_file = self:retrieve_node_file( node )
		self._node_line = self:retrieve_node_line( node )
		-- self._parameters = ( node.parameter_map and node:parameter_map() ) or node:parameters()
		-- self._parameters = node -- GENERATES TO MUCH DATA
		self._parameters = {}
	...
	if( ( __filter_name ~= nil ) and unit_element ) then
		self._is_allowed_func = function( self, env )
									...
									if( filter ) then
										...
										return filter_name	-- Treats the filter attribute as an if-statement.
									end
								end
	end
...
function BaseElement:retrieve_node_line( node )
	...
	if( line and ( ( line <= 0 ) or ( line >= 276577824 ) ) ) then		-- if the xml isn't compiled this execution.
		return nil
	else
		return line
	end
	...
	if( value ~= nil ) then
		...
		if( __func ) then
			if( set_function ) then
				return function( env, ... ) setfenv( __func, env ) set_function( self, env, __func(), ... ) end
			else
				return function( env, ... ) setfenv( __func, env ) return __func( env, ... ) end
			end
		else
...
function BaseElement:activate( env )
	...
	if( alive( env.dest_unit ) and self:filter_callback( env ) ) then
		...
		if( start_time > 0 ) then
			local params = env.params		-- Backup parameters.
			...
			local func = function()
								if( self:delayed_filter_callback( env ) and alive( env.dest_unit ) ) then
									env.params = params		-- Restore parameters.
									SequenceEnvironment.self = env
									SequenceEnvironment.element = self
...
function BaseElement:get_model_xml_file()
	...
	elseif( self._unit_element ) then
		if DB:has( "unit", self._unit_element:get_name():id() ) then
...
function UnitElement:init( node, name, is_global )
	...
	if( node ) then
		...
		for _,data in ipairs( node ) do
			...
			elseif( element_name == "water" ) then -- NOT FIXED WHEN SWITCH TO SCRIPTSERIALIZED /Martin
				table.insert( water_node_list, child_node ) 
			elseif( element_name == "proximities" ) then -- NOT FIXED WHEN SWITCH TO SCRIPTSERIALIZED /Martin
			...
			elseif( ( element_name ~= "variables" ) and ( element_name ~= "global_variables" ) ) then
				self:check_invalid_node( data, { "trigger", "filter", "sequence", "body", "proximities" } )
			end
		end
		
		-- Initializes sequences, water elements and bodies last since they can refer to filters:
	...
	--[[if( node ) then
		for child_node in node:children() do
			...
			if( element_name == "variables" ) then
				for variable_node in child_node:children() do
					local name = variable_node:name()
					self._set_variables[ name ] = self:get_static( "value", variable_node:parameter( "value" ), nil, child_node )
			...
			elseif( element_name == "global_variables" ) then
				for variable_node in child_node:children() do
					local name = variable_node:name()
					self._global_vars[ name ] = self:get_static( "value", variable_node:parameter( "value" ), nil, child_node )
		...
		for child_node in node:children() do
			...
			if( element_name == "trigger" ) then
				local trigger = TriggerDeclarationElement:new( child_node, self )
				self._triggers[ trigger._name ] = trigger
			elseif( element_name == "filter" ) then
				local filter = FilterElement:new( child_node, self )
				self._filters[ filter._name ] = filter
			elseif( element_name == "sequence" ) then
				local name = self:get_static( "name", child_node:parameter( "name" ), nil, child_node )
				name = name and name( SequenceEnvironment )

				if( not name ) then
					self:print_attribute_error( "name", name, nil, false, nil, child_node )
				else
					sequence_nodes[ name ] = child_node
				end
			elseif( element_name == "body" ) then
				table.insert( body_nodes, child_node )
			...
			elseif( ( element_name ~= "variables" ) and ( element_name ~= "global_variables" ) ) then
				self:check_invalid_node( child_node, { "trigger", "filter", "sequence", "body", "proximities" } )
			end
		end
...
function FilterElement:init( node, unit_element )
	...
	--[[for child_node in node:children() do
		local element_class = filter_element_class_map[ child_node:name() ]

		if( element_class ) then
			local element = element_class:new( child_node, unit_element )
			table.insert( self._elements, element )
		else
			self:check_invalid_node( child_node, filter_element_class_map )
		end
	end]]
end
...
function ZoneFilterElement:init( node, unit_element )
	...
	for i=1, #name do
		...
		elseif( ( __chr == "x" ) or ( __chr == "y" ) or ( __chr == "z" ) ) then
			...
			if( negative ) then
				func = function( zone_vector ) return zone_vector[ __chr ] < 0 end
			else
				func = function( zone_vector ) return zone_vector[ __chr ] >= 0 end
			end
...
function SequenceElement:init( node, unit_element )
	...
	--[[for child_node in node:children() do
		...
		if( self:is_valid_xml_node( child_node ) ) then
			local element = managers.sequence:parse_event( child_node, unit_element )

			if( element ) then
				table.insert( self._elements, element )
			else
				self:check_invalid_node( child_node, managers.sequence:get_event_element_class_map() )
			end
		end
	end]]
end
...
function SequenceElement:activate_callback( env )
	...
	if( not damage_ext._runned_sequences or not damage_ext._runned_sequences[ self._name ] ) then
		...
		-- print( "ENV PARAMS", inspect( env ) )
		-- print( "env.params", inspect( env.params ) )
...
function SequenceContainerElement:init( node, unit_element )
	...
	for child_node in node:children() do
		if( child_node:name() == "run_sequence" ) then
			...
			self:check_invalid_node( child_node, { "run_sequence" } )
		end
	end
end
	...
	for _,data in ipairs( node ) do
		...
		elseif( name == "endurance" ) then
			...
			for k,v in pairs( element._endurance ) do
				...
				elseif( self._first_endurance[ k ]._endurance[ k ] > v ) then	-- Insert at the beginning of the linked list.
					element._next[ k ] = self._first_endurance[ k ]
					self._first_endurance[ k ] = element
				else	-- Insert the element so that the linked list is sorted after endurance.
	...
	--[[for child_node in node:children() do
		...
		if( name == "inflict" ) then
			if( self._inflict_element ) then
				...
				self._inflict_element = RootInflictElement:new( child_node, unit_element )
			end
		elseif( name == "endurance" ) then
			element = EnduranceElement:new( child_node, unit_element )
			...
			self:check_invalid_node( child_node, { "inflict", "endurance" } )
		end
	end]]
...
function InflictElement:init( node, unit_element )
	...
	for child_node in node:children() do
		...
		elseif( child_name == "exit" ) then
			...
			self:check_invalid_node( child_node, { "enter", "damage", "exit" } )
		end
	end
end
...
function EnduranceElement:init( node, unit_element )
	...
	for k, v in pairs( self._parameters ) do
		-- WOULDN'T NEED META AND TABLE CHECK IF PARAMETERS WASN'T BLOATED /Martin
		if( k ~= "can_skip" ) then -- and k ~= "_meta" and CoreClass.type_name( v ) ~= "table" ) then
			if( string.sub( k, -4 ) ~= "_abs" ) then
	...
	for _,data in ipairs( node ) do
		...
		if( data._meta == "run_sequence" ) then
			...
			self:check_invalid_node( data, { "run_sequence" } )
		end
	end
end
...
function EnduranceElement:activate( env )
	...
	if( not skip ) then
		...
		next_endurance_element = self._next[ env.damage_type ]	-- Does this again since it can have changed.
	end
	...
	if( not once or not env.dest_unit:anim_is_playing( ids_name ) ) then
		...
		if( loop ) then
			...
			if( self.SAVE_STATE ) then
				self:set_cat_state( env.dest_unit, name, { "anim_play_loop", start_loop_time, end_time, speed } )
			end
		else
			if( duration ) then
			...
			if( end_time ) then
				...
				if( self.SAVE_STATE ) then
					self:set_cat_state( env.dest_unit, name, { "anim_play_to", end_time, speed } )
				end
			else
...
function AreaDamageElement:init( node, unit_element )
	...
	self._falloff_func_map[ "linear" ] = nil	-- do_area_damage( ... ) does this.
	...
	if( not damage_type ) then
		...
		if( not position ) then
			...
			if( range and ( range > 0 ) ) then
				...
				elseif( not damage_callback_func_name ) then
					damage_callback_func_name = self._falloff_func_map[ "keys" ]
				end
...
-- This function exists so that it can be overrided:
...
BodyElement.VALID_MOTION_MAP = BodyElement.VALID_MOTION_MAP or { fixed = "set_fixed", keyframed = "set_keyframed", dynamic = "set_dynamic" }
BodyElement.FUNC_MAP = BodyElement.FUNC_MAP or { motion = "set_motion", enabled = "set_enabled", body_collision = "set_body_collision", mover_collision = "set_mover_collision", pushed_by_mover = "set_pushed_by_mover", mover = "set_mover", interpolate = "interpolate", add_ray_type = "add_ray_type", remove_ray_type = "remove_ray_type" }
...
function BodyElement:set_enabled( env, enabled, body )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "enabled", { "set_enabled", enabled } )
	end
end
...
function BodyElement:add_ray_type( env, ray_type, body )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "add_ray_type", { "add_ray_type", ray_type } )
	end
end
...
function BodyElement:remove_ray_type( env, ray_type, body )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "remove_ray_type", { "remove_ray_type", ray_type } )
	end
end
...
function BodyElement:set_body_collision( env, enabled, body )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "body_collision", { "set_collisions_enabled", enabled } )
	end
end
...
function BodyElement:set_mover_collision( env, enabled, body )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "mover_collision", { "set_collides_with_mover", enabled } )
	end
end
...
function BodyElement:set_pushed_by_mover( env, enabled, body )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "pushed_by_mover", { "set_pushed_by_mover", enabled } )
	end
end
...
function BodyElement:interpolate( env, value, body )
	...
	if( value ) then
		...
		if( self.SAVE_STATE ) then
			self:set_cat_state2( body:unit(), body:unit():get_body_index( body:name() ), "interpolate", { "interpolate", value } )
		end
	else
...
function AlertElement:activate_callback( env )
	local new_alert = { "aggression", env.pos, 1200, managers.groupai:state():get_unit_type_filter( "civilians_enemies" ), env.dest_unit }
...
function FunctionElement:activate_callback( env )
	...
	if( target == nil ) then
		target = env.dest_unit	-- Defaults to destination unit.
...
function GraphicGroupElement:activate_callback( env )
	...
	if( activate ) then
		...
		if( self.SAVE_STATE ) then
			self:set_cat_state( env.dest_unit, name, { "activate_graphic_group" } )
		end
	end
	...
	if( visibility ~= nil ) then
		...
		if( self.SAVE_STATE ) then
			self:set_cat_state( env.dest_unit, name, { "set_visibility", visibility } )
		end
	end
end
...
function LightElement:set_enabled( env, enabled, light_obj, name )
	...
	for _,child in ipairs( light_obj:children() ) do
		...
		if( self.SAVE_STATE ) then
			self:set_cat_state( env.dest_unit, child:name(), { "set_visibility", enabled } )
		end
	end
end
...
function LightElement:set_multiplier( env, multiplier, light_obj, name )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state( env.dest_unit, name, { "set_multiplier", multiplier } )
	end
end
...
function LightElement:set_far_range( env, far_range, light_obj, name )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state( env.dest_unit, name, { "set_far_range", far_range } )
	end
end
...
function LightElement:set_spot_angle_start( env, spot_angle_start, light_obj, name )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state( env.dest_unit, name, { "set_spot_angle_start", spot_angle_start } )
	end
end
...
function LightElement:set_spot_angle_end( env, spot_angle_end, light_obj, name )
	...
	if( self.SAVE_STATE ) then
		self:set_cat_state( env.dest_unit, name, { "set_spot_angle_end", spot_angle_end } )
	end
end
...
MaterialElement.MATERIAL_ATTRIBUTE_MAP = MaterialElement.MATERIAL_ATTRIBUTE_MAP or { name = true, diffuse_color = true, diffuse_color_alpha = true }
MaterialElement.FUNC_MAP = MaterialElement.FUNC_MAP or { glossiness = "set_glossiness", render_template = "set_render_template", time = "set_time", state = "set_material_state" }
MaterialElement.TIMER_STATE_MAP = MaterialElement.TIMER_STATE_MAP or { play = true, pause = true, stop = true }
...
function MaterialElement:init( node, unit_element )
	...
	for key, value in pairs( self._parameters ) do
		...
		elseif( not self.BASE_ATTRIBUTE_MAP[ key ] and not self.MATERIAL_ATTRIBUTE_MAP[ key ] ) then	-- Variable
...
function ObjectElement:activate_callback( env )
	...
	if( name ) then
		local object_list = { env.dest_unit:get_object( Idstring( name ) ) }
...
function ObjectElement:set_object( dest_unit, object_list, func_name, value )
	for _,obj in ipairs( object_list ) do
		...
		if( self.SAVE_STATE ) then
		 	-- self:set_cat_state2( dest_unit, obj:name(), func_name, value )
		 	self:set_cat_state2( dest_unit, obj:name(), func_name, { obj:name(), value } )
		end
	end
end
...
function ObjectElement.load( unit, data )
	for name,sub_data in pairs( data ) do
		for func_name,values in pairs( sub_data ) do
			-- print( unit:name():s(), func_name, inspect( values ) ) 
			-- local obj = unit:get_object( name:id() )
			-- obj[ func_name ]( obj, value )
...
function ProjectDecalElement:activate_callback( env )
	...
	elseif( not slotmask ) then
		...
		if( ( #effect > 0 ) and self:run_parsed_func( env, self._play_effect ) ) then
			World:effect_manager():spawn( { effect = effect, position = position, normal = normal or math.UP } )
		end
	end
end
...
function SlotElement:activate_callback( env )
	if( self._frustum_delay ) then
		...
		local data = {}
		local func = function() return self:check_frustum_delay( frustum_close_radius, frustum_extension, frustum_far_clip, visible, env, data ) end
...
function WwiseElement:play( env )
	...
	if switch then
		local switches = string.split( switch, " " ) -- format: { switch_name1, value1, switch_name2 value2, switch_name3, value3... }
		local i = 1
		while i < #switches do
	...
	if( self.SAVE_STATE ) and not skip_save then
		-- self:set_cat_state( env.dest_unit, source, enabled )
		-- self:set_cat_state2( env.dest_unit, source, "post_event", { event } )
		-- self:set_cat_state2( env.dest_unit, event, "post_event", { event } )
...
function WwiseElement:stop( env )
	...
	if( self.SAVE_STATE ) then
		...
		-- self:set_cat_state2( env.dest_unit, source, "stop", {} )
		-- self:set_cat_state2( env.dest_unit, event, "stop", { event } )
		-- self:set_cat_state2( dest_unit, obj:name(), func_name, { obj:name(), value } )
	end
end
...
function WwiseElement.load( unit, data )
	for source,sub_data in pairs( data ) do
		-- for func_name,values in pairs( sub_data ) do
			...
			-- local sound_source = unit:sound_source( source and Idstring( source ) )
			-- sound_source[ func_name ]( sound_source, values[ 1 ] )
			-- local obj = unit:get_object( values[1] )
			-- obj[ func_name ]( obj, value )
		-- end
	end
...
SpawnUnitElement.SPAWN_UNIT_ATTRIBUTE_MAP = SpawnUnitElement.SPAWN_UNIT_ATTRIBUTE_MAP or { name = true, position = true, rotation = true,
																						   transfer_velocity = true, run_sequence = true,
																						   src_link_obj = true, dest_link_obj = true,
																						   from_trigger = true, from_trigger_sequence = true,
																						   to_trigger = true, to_trigger_sequence = true }
		...
		if( transfer_velocity ) then
			if( not unit ) then
				...
				if( getmetatable( transfer_velocity ) == Vector3 ) then
					...
					if( env.damage_type == "collision" ) then
