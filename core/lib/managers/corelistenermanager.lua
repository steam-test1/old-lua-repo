...
function ListenerManager:active_listener_obj()
	local (for generator), (for state), (for control) = pairs(self._listener_map)
	do
		do break end
...
function ListenerManager:callback_on_all_active_listeners(func)
	...
	local (for generator), (for state), (for control) = pairs(self._active_set_stack_map)
	do
		do break end
		local set_id = stack:top()
		if set_id and not done_set_map[set_id] then
			do
				local (for generator), (for state), (for control) = pairs(self._set_map[set_id])
				do
					do break end
...
function ListenerManager:remove_stack(stack_id)
	...
	if active_set_stack then
		...
		do
			local (for generator), (for state), (for control) = pairs(self._stack_activation_map[stack_id])
			do
				do break end
...
function ListenerManager:add_set(set_id, category_id_list)
	if not self._set_map[set_id] then
		...
		if category_id_list then
			local (for generator), (for state), (for control) = ipairs(category_id_list)
			do
				do break end
...
function ListenerManager:remove_category(category_id)
	if self._category_map[category_id] then
		do
			local (for generator), (for state), (for control) = pairs(self._set_map)
			do
				do break end
				if category_map[category_id] then
					self:remove_set_category(set_id, category_id)
...
function ListenerManager:_set_listener_set_active(set_id, active)
	...
	do
		...
		local (for generator), (for state), (for control) = pairs(listener_map)
		do
			do break end
