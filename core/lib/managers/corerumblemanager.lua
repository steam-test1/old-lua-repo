...
4
	if rumble_id then
		if rumble_id == "all" then
			...
			local (for generator), (for state), (for control) = pairs(rumble_id.controllers)
			do
				do break end
...
function RumbleManager:play(name, controller_wrapper, multiplier_data, custom_data)
	...
	if effect then
		...
		do
			local (for generator), (for state), (for control) = pairs(rumble_controllers)
			do
				do break end
...
function RumbleManager:mult_distance_lerp(pos_func_list, params)
	if pos_func_list then
		local closest_pos
		do
			local (for generator), (for state), (for control) = pairs(pos_func_list)
			do
				do break end
