...
function ItemToggle:setup_gui( node, row_item )
	...
	if self:selected_option():parameters().icon then -- If icon based
		row_item.gui_icon = row_item.gui_panel:bitmap( { layer = node.layers.items, x = 0, y = 0, texture_rect = { 0, 0, 24, 24 }, texture = self:selected_option():parameters().icon, blend_mode = node.row_item_blend_mode } )
