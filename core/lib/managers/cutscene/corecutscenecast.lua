...
function CoreCutsceneCast:evaluate_cutscene_at_time(cutscene, time)
	...
	if blob_controller then
		...
		local (for generator), (for state), (for control) = pairs(self:_actor_units_in_cutscene(cutscene))
		do
			do break end
...
function CoreCutsceneCast:_actor_units_in_cutscene(cutscene)
	...
	local result = {}
	do
		local (for generator), (for state), (for control) = pairs(cutscene:controlled_unit_types())
		do
			do break end
