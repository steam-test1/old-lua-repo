...
function ElementAreaTrigger:on_script_activated()
	...
	if self._values.use_shape_element_ids then
		local (for generator), (for state), (for control) = ipairs(self._values.use_shape_element_ids)
		do
			do break end
...
function ElementAreaTrigger:_is_inside(pos)
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._shape_elements)
		do
			do break end
...
function ElementAreaTrigger:set_enabled(enabled)
	if not enabled and Network:is_server() and self._values.trigger_on == "both" then
		local (for generator), (for state), (for control) = ipairs(CoreTable.clone(self._inside))
		do
			do break end
...
function ElementAreaTrigger:instigators()
	if self._values.unit_ids then
		...
		if Network:is_server() then
			local (for generator), (for state), (for control) = ipairs(self._values.unit_ids)
			do
				do break end
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._values.spawn_unit_elements)
		do
			do break end
			local element = self:get_mission_element(id)
			if element then
				local (for generator), (for state), (for control) = ipairs(element:units())
				do
					do break end
					table.insert(instigators, unit)
				end
...
function ElementAreaTrigger:debug_draw()
	...
	do
		do break end
		local (for generator), (for state), (for control) = ipairs(shape_element:get_shapes())
		do
			do break end
			shape:draw(0, 0, self._values.enabled and 0 or 1, self._values.enabled and 1 or 0, 0, 0.2)
		end
...
function ElementAreaTrigger:update_area()
	...
	if self._values.trigger_on == "on_empty" then
		if Network:is_server() then
			self._inside = {}
			do
				local (for generator), (for state), (for control) = ipairs(self:instigators())
				do
					do break end
					if alive(unit) then
		...
		if #instigators == 0 and Network:is_server() then
			...
			local (for generator), (for state), (for control) = ipairs(instigators)
			do
				do break end
				if alive(unit) then
...
function ElementAreaTrigger:_check_instigator_rules(unit)
	do
		local (for generator), (for state), (for control) = ipairs(self._rules_elements)
		do
			do break end
