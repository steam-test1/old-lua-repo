...
function ElementPlaySound:on_executed( instigator )
	...
	if self._values.use_instigator then
		if Network:is_server() then -- This is synced by the unit's sound extension
			if instigator:id() ~= -1 then
		...
		if not self._values.elements or #self._values.elements == 0 then
			self:_play_sound()
		elseif Network:is_server() then -- This is synced by the unit's sound extension
...
function ElementPlaySound:save( data )
	-- Don't really need to save any data
end
