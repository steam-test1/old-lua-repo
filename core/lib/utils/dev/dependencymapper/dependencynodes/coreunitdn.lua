...
function UnitDependencyNode:_walkxml2dependencies(xmlnode, deps)
	...
	if node_name == "depends_on" then
		...
		if depends_on_unit ~= nil then
			local dn = self._get_dn({name = depends_on_unit, type_ = UNIT})
			deps:add(dn)
			if dn == nil then
		...
		if depends_on_mc ~= nil then
			local dn = self._get_dn({name = depends_on_mc, type_ = MATERIAL_CONFIG})
			deps:add(dn)
			if dn == nil then
	...
	elseif node_name == "model" then
		...
		if object_name ~= nil then
			local dn = self._get_dn({name = object_name, type_ = OBJECT})
			deps:add(dn)
			if dn == nil then
	...
	elseif node_name == "extension" and xmlnode:parameter("name") == "cutscene_data" then
		...
		local dn = self._get_dn({name = cutscene_name, type_ = CUTSCENE})
		deps:add(dn)
		if dn == nil then
