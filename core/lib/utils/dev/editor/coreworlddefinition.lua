...
function WorldDefinition:init( params )
	...
	self._cube_lights_path 	= params.cube_lights_path	-- Use when simulating to force loading of cubemaps from world folder instead of simulation folder
	...
	self._ignore_spawn_list			= {} -- When a unit is only_exists_in_editor, the name will be placed here so no other units are spawned
...
-- Returns if a continent is excluded (either by only loaded in editor or from world settings files)
...
function WorldDefinition:_load_world_package()
	...
	-- World package is consistent during the entire level
	...
	-- World init package is unloaded when world has been created
...
-- Loads init continent for a continent. It will be unloaded after world creation
		...
		if not self:_continent_editor_only( data ) then		-- Only create continents that are not editor only
			if not self._excluded_continents[ name ] then	-- Don't create the continent if it is excluded
				...
				if DB:has( 'continent', path ) then
					...
					-- self:_unload_continent_init_package( init_path )
...
-- Should be called after a world has been fully created, even after the mission script initiliazation
...
function WorldDefinition:create( layer, offset )
	...
	if layer == "level_settings" or layer == "all" then
		if self._definition[ "level_settings" ] then
	...
	if layer == "portal" or layer == "all" then
		if self._definition[ "portal" ] then
	...
	if layer == "brush" or layer == "all" then
		if self._definition[ "brush" ] then
...
function WorldDefinition:_create_sounds( data )
	...
	managers.sound_environment:set_default_ambience( values.default_ambience --[[, values.default_ambience_soundbank]] )
...
-- Loads the mass unit file. If we are in editor mode, we also check that the units still exists
-- and offers a replace dialog to the user if not.
...
function WorldDefinition:sky_rotation_modifier( interface )
	return self._environment.sky_rot -- self._definition[ "environment" ].environment_values.sky_rot
end
...
function WorldDefinition:_create_environment( data, offset )
	...
	-- A difficult way to handle the environtment sky rotation
	if not Application:editor() then
	...
	if Application:editor() then -- Only create cubemap gizmos in editor mode
		local units = {}
...
--  self._unit_name, self._generic, offset 
local is_editor = Application:editor()
...
function WorldDefinition:assign_unit_data( unit, data )
	...
	self:_set_only_visible_in_editor( unit, data )		-- Set only visible in editor
...
function WorldDefinition:_setup_unit_id( unit, data )
	...
	self:use_me( unit, Application:editor() ) -- Calls this to see if anyone wants to use it
end

-- Set up editor unit data. Dont set these values if we dont run editor. Saves memory.
...
function WorldDefinition:_setup_lights( unit, data )
	...
	for _,l in ipairs( data.lights ) do
		...
		if light then
			...
			if l.multiplier_nr then	-- Special sync parameter (Skip making lookup when saving and loading at drop in)
				light:set_multiplier( l.multiplier_nr )
			end
			if l.specular_multiplier_nr then -- Special sync parameter (Skip making lookup when saving and loading at drop in)
				light:set_specular_multiplier( l.specular_multiplier_nr )
			end
			if l.multiplier then
				l.multiplier = l.multiplier:id() -- Convert string to idstring if needed
...
-- External call to set up lights, does it from SyncUnitData
...
-- Hides the unit if it should only be visible in edior
...
-- Loads a projection texture to a light
function WorldDefinition:_setup_projection_light( unit, data )
	...
	unit:unit_data().projection_light = data.projection_light 			-- I need to save this to be able to sync it
	...
	if Application:editor() then
		...
		TextureCache:request( texture_name, omni and "CUBEMAP" or "NORMAL", callback( self, self, "_light_loaded_cbk", { light = light, omni = omni, texture_name = texture_name } ) )
	end
end

function WorldDefinition:_light_loaded_cbk( data, texture_idstring )
	-- print( "WorldDefinition:_light_loaded_cbk", inspect( data ), texture_idstring )
	data.light:set_projection_texture( data.texture_name, data.omni )
	if self._requested_light_textures[ data.texture_name ] then
		-- print( " Remove from request list", data.texture_name )
...
-- Flush remaining requested light textures on exec
function WorldDefinition:flush_remaining_lights_textures()
	-- print( "WorldDefinition:flush_remaining_lights_texture()" )
	if self._requested_light_textures then
		for name,texture_name in pairs( self._requested_light_textures ) do
			-- print( " WorldDefinition:flush_remaining_lights_texture", texture_name )
			TextureCache:unretrieve( texture_name )
			-- self._requested_light_textures[ name ] = nil
		end
	end
end

-- External call to set projection light, does it from SyncUnitData
function WorldDefinition:setup_projection_light( ... )
	self:_setup_projection_light( ... )
end

-- Calls a function where the project can assign unit data
...
function WorldDefinition:use_me( unit, is_editor )
	...
	self._all_units[ id ] = self._all_units[ id ] or unit -- If use me is called from extensions (which used by spawn synced unit, add the units to all units here) 
