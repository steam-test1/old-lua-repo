...
effect by construction of the initializer stack.]])
	...
	p = CoreEffectProperty:new("fade_in_start", "value_list", "-1", [[
Distance (in meters) at which an effect starts fading in. -1 means no fading.
Smooth fading only works for effects with a visualizer that respects opacity, other effects will just disappear when they reach the cull distance.]])
	...
	p:add_value("50")
	self:add_property(p)
	p = CoreEffectProperty:new("fade_out_start", "value_list", "-1", [[
Distance (in meters) past which an effect starts fading out. -1 means no fading.
...
Smooth fading only works for effects with a visualizer that respects opacity, other effects will just disappear when they reach the cull distance.]])
	...
	self:add_property(p)
	help = [[
Determines what happens when an atom is spawned at a location which is not visible.
...
The update_render option is required for screen aligned effects to be rendered]]
	...
	self:add_property(p)
	help = [[
Defines the period of time in seconds that the atom must be culled before the cull policy is applied.
...
the effect will be culled while still visible]]
	...
	help = [[
If the effect uses particle-world collision, this radius specifies the region around the grab point within which geometry will be grabbed.
...
far from the expected collision area to be grabbed, while ignoring geometry in the collision area.]]
	...
	help = [[
Internal event listeners - these are listeners capable of handling events spawned by the simulators with a fixed behaviour.
For more advanced event handling, script callbacks can be registered with effect instances at runtime.]]
	...
	local create_event_types = function()
...
function CoreEffectDefinition:init()
	...
	self:add_property(p)
	p = CoreEffectProperty:new("use_random", "list_objects", "", [[
One effect from each of the sets specified here will be chosen at random and included when this effect is spawned.
...
if you specify several sets, this means you will get a random combination of one effect from each set.]])
	...
	self:add_property(p)
	self:add_property(CoreEffectProperty:new("force_synch", "boolean", "false", [[
If set, effect will play on the main thread without frame lag
