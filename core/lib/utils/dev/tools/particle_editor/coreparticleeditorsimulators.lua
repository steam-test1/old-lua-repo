...
function CoreParticleEditorSimulators:create_velocityintegrator()
	...
	local help = [[
Determines which position channel integration is performed on:
...
function CoreParticleEditorSimulators:create_trail()
	...
	local p = CoreEffectProperty:new("mode", "variant", "frequency", [[
Determines mode in which simulator operates:
...
function CoreParticleEditorSimulators:create_windacceleration()
	...
	local help = [[
Radius (cm) that particles are considered to have - this affects how much wind the particle picks up, and thus how fast it will accelerate to wind speed.
The default value is that of a regulation size ping pong ball.]]
	...
	help = [[
Mass (kg) that particles are considered to have - this affects how much wind the particle picks up, and thus how fast it will accelerate to wind speed.
The default value is that of a regulation size ping pong ball.]]
...
Velocities are always offsetted with effect spawn velocity.]]
	...
	simulator:add_property(p)
	help = [[
Box inside which particle velocities will be randomly generated.
...
Accelerates particles with a set acceleration.
...
function CoreParticleEditorSimulators:create_rotationbyvelocity()
	...
	simulator:set_description([[
Rotates particles so that the Z axis is pointing towards the velocity vector
...
keys - use key curve over time]]
	...
	local validate_lifetime = function(p)
		local ret = {
			valid = true,
			message = "",
			node = nil
...
keys - use key curve over time]]
	...
	frequency_keys:add_key({t = 0, v = "1"})
	...
	simulator:add_property(CoreEffectProperty:new("current_position", "boolean", "true", [[
If set, teleported particles will always be moved to current atom position.
...
circle - initializes positions in a circle around (0,0,1) relative to effect location]]
	...
	help = [[
Box inside which particle positions will be randomly generated.
Coordinates are in cm.]]
	...
	q = CoreEffectProperty:new("velocities", "box", "", [[
Box inside which particle normals will be randomly generated.
...
function CoreParticleEditorSimulators:create_particleparticlecollision()
	...
	simulator:set_description([[
Simulates collision between particles.
