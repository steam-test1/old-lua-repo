...
function ColorPickerFields:_field_value(field_name)
	...
	local (for generator), (for state), (for control) = ipairs(field_group)
	do
		do break end
...
function ColorPickerFields:_create_panel(parent_frame, enable_alpha, enable_value)
	...
	if not enable_alpha then
		local (for generator), (for state), (for control) = ipairs(self._field_groups.Alpha)
		do
			do break end
...
function ColorPickerFields:_create_panel_with_fields(parent_frame, fields)
	...
	do
		local (for generator), (for state), (for control) = table.tuple_iterator(fields, 2)
		do
			do break end
			local value = 0
...
function ColorPickerFields:_on_field_edited(edited_field, event)
	do
		local (for generator), (for state), (for control) = ipairs(self:_mirroring_fields(edited_field))
		do
			do break end
...
function ColorPickerFields:_parse_values(field_names)
	local values = {}
	do
		local (for generator), (for state), (for control) = pairs(field_names)
		do
			do break end
...
6
	...
	do
		local (for generator), (for state), (for control) = pairs(field_values)
		do
			do break end
...
function ColorPickerFields:_field_label(field)
	local (for generator), (for state), (for control) = pairs(self._field_groups)
	do
		do break end
